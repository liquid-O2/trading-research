...
CHECK_EXTENSION = ROOT / 'validation/AUCTION_FLOW_SOURCE_CHECK_EXTENSION_V16.json'
...
def execute_source_units_parallel(windows, *, protocol, index, coordinates, outputs, data_root, calendar,
    ...
    try:
        ...
        if packet['configuration']['mode'] == 'throughput' and evidence:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in ('check', 'profile', 'benchmark', 'compute', 'downstream', 'throughput', 'clock-audit'):
        raise SystemExit('usage: run_auction_flow_study.py {check|profile|benchmark|compute|downstream|throughput|clock-audit}')
