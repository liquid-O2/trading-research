    ...
    for metric, spec in HIST_FOR_METRIC.items():
        ...
        if population in ('raw_source', 'unique_source'):
            for row in selected_rows:
                ...
                vv, cc = _histograms_for([row], {day}, hist_name, population, family, cut, right, dte)
                nday = sum(int(count) for count in cc)
                metrics[metric][day] = None if not nday else sum(float(v) * int(c) for v, c in zip(vv, cc)) / nday
