...
def _breach_state(src_known, rcv_known, src_high, src_low, rcv_high, rcv_low) -> str:
    if not src_known and not rcv_known:
        return "unknown"
    if not src_known:
        return "receiver_only" if (rcv_high or rcv_low) else "unknown"
    if not rcv_known:
        return "source_only" if (src_high or src_low) else "unknown"
...
def process_year(*, year: int, units: dict, calendar, writers: dict, groups: dict,
    ...
    for cash in cash_days:
        ...
        for cut_i, cut in enumerate(cut_list):
                ...
                for lag in LATENCY_AFTER_END_S:
                    ...
                    for kind, state_src, state_rcv, lookbacks in (
                        ...
                        for length in lookbacks:
                            ...
                            excursion = None
                            if src_known and src["close"] and sph and src["close"] > 0 and sph > 0:
                                excursion = math.log(src["close"] / sph) if src_h else (
                                    math.log(sph / src["close"]) if src_l else 0.0)
                            writers["smt"].add({
                                "date": day, "year": year, "stage": stage, "session": session,
                                "source_symbol": source.symbol, "receiver_symbol": receiver.symbol,
                                "source_variant": source.variant, "receiver_variant": receiver.variant,
                    ...
                    if src_prep.running and rcv_prep.running:
                        for length in REFERENCE_LOOKBACKS:
                            src_idx = src["index"]
                            rcv_idx = rcv["index"]
                            src_known = src["priced"] and src_idx is not None
                            rcv_known = rcv["priced"] and rcv_idx is not None
            ...
            for key, unit in units.items():
                prep = prepared[key]
                prior_dates = [item for item in intended_day_list if item < cash.day]
                    ...
                    for length in LOOKBACKS:
                        ...
                        if row["index"] is None:
                            ...
                            if rel is None and rel_reason:
                                ...
                                status = "undefined"
                            scale = carry_unit.prior_rth_scale(prep.contract_key[index])
            ...
            _write_mapping_cuts(
                ...
                writers, groups, carry, intended_day_list)
        _emit_source_events(
...
def _write_mapping_cuts(units, prepared, by_obj, views, cash, day, year, stage, session, cut,
    ...
    prior_day = previous_intended_date(intended_days, cash.day)
    for futures_symbol, etf_symbol in MAPPING_CHAINS:
        futs = [unit for unit in units.values() if unit.symbol == futures_symbol]
        etfs = [unit for unit in units.values() if unit.symbol == etf_symbol]
        for fut in futs:
    ...
    for source in units.values():
        ...
        for index in range(lo, hi):
            ...
            if pivot["high_breach"][index]:
                ...
                events.append(("pivot_high", 1, formed, 15, formed - 2 * MINUTE, formed + 2 * MINUTE))
            if pivot["low_breach"][index]:
                formed = int(pivot["low_formed_ns"][index])
                events.append(("pivot_low", -1, formed, 15, formed - 2 * MINUTE, formed + 2 * MINUTE))
                ...
                for receiver in receivers:
                    ...
                    for lag in LATENCY_AFTER_END_S:
                        ...
                        no_ref = (
                            ...
                            or ref_start is None or ref_start >= event_start
                        )
                        ...
                        for horizon in FUTURE_HORIZONS:
                            ...
                            if label_id is None:
                                ...
                                writers["receiver_labels"].add({
                                    ...
                                    "future_status": window["status"],
                                    "future_reasons": "|".join(window.get("reasons") or ()),
                                    "open": window.get("open"), "high": window.get("high"),
                                    "low": window.get("low"), "close": window.get("close"),
                                    "instrument_id": window.get("instrument_id"),
                                    "contract_key": window.get("contract_key"),
