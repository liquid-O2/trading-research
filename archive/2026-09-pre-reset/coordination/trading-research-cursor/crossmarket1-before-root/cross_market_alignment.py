...
def contract_key_array(unit: "UnitBars"):
    ...
    if unit.instrument_kind == INSTRUMENT_CANONICAL and unit.contract_names:
        ...
        return out
    prefix = unit.symbol + ":"
    return np.asarray([prefix + str(int(code)) for code in unit.instrument_id], dtype=object)
...
class PriorCarry:
    ...
    def prior_rth_scale(self, current_contract_key=None):
...
def load_daily_observational(admitted, *, maximum_bytes: int, calendar=None):
    ...
    if calendar is not None:
        intended = [cash.day for cash in intended_cash_dates(
            calendar, date(2010, 1, 1), POPULATION_END_EXCLUSIVE - timedelta(days=1))]
