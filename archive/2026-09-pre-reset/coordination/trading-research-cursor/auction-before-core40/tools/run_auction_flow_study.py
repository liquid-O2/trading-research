...
CHECK_EXTENSION = ROOT / 'validation/AUCTION_FLOW_SOURCE_CHECK_EXTENSION_V31.json'
...
REGISTERED_MODES = ('check', 'profile', 'benchmark', 'compute', 'downstream', 'throughput',
                    'clock-audit', 'source-clock', 'extract', 'extract-check', 'observations')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in REGISTERED_MODES:
        raise SystemExit('usage: run_auction_flow_study.py {check|profile|benchmark|compute|downstream|throughput|clock-audit|source-clock|extract|extract-check|observations}')
