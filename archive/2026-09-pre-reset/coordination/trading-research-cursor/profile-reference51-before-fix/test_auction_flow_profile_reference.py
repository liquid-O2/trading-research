...
from trading_research.research.auction_flow_profile_reference import (
    ...
    require_frozen_variants, variant_definition, write_series_table,
)
...
class ProfileReferenceRunTests(unittest.TestCase):
    ...
    def test_zero_effort_and_unpriced_incomplete_are_null(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            outputs = BoundedOutputs(Path(tmp) / 'fixtures', maximum_total_bytes=64 * 1024 ** 2, maximum_file_bytes=16 * 1024 ** 2)
            unit = _write_unit(
...
class ProfileReferenceCorrectionTests(unittest.TestCase):
            ...
            with self.subTest(variant=variant):
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    outputs = BoundedOutputs(
                        Path(tmp) / 'fixtures', maximum_total_bytes=64 * 1024 ** 2, maximum_file_bytes=16 * 1024 ** 2,
                    )
                    units = [
                        _write_unit(
    ...
    def test_december_january_spanning_week_owned_by_end_date(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(all(row.get('economic_date') == '2021-01-03' for row in weeks_2021))
