...
CHECK_EXTENSION = ROOT / 'validation/AUCTION_FLOW_SOURCE_CHECK_EXTENSION_V56.json'
...
REGISTERED_MODES = ('check', 'profile', 'benchmark', 'compute', 'downstream', 'throughput',
                    'clock-audit', 'source-clock', 'extract', 'extract-check', 'observations', 'core-statistics', 'window-links', 'window-statistics', 'profile-statistics')
...
def limits(protocol, mode='check', *, extension=None):
    ...
    if mode in ('window-statistics','profile-statistics'):
        declared_extension = extension or json.loads(checked(CHECK_EXTENSION)[0])
        operation = load_reference(declared_extension['profile_statistics' if mode=='profile-statistics' else 'window_statistics'])
        allocation = operation['execution_limits']
        if (set(allocation) != {'cpu_seconds','hard_cpu_seconds','wall_seconds','maximum_output_bytes'}
                or any(type(v) is not int or v<=0 for v in allocation.values())
                or allocation['hard_cpu_seconds'] != allocation['cpu_seconds']+10
                or allocation['cpu_seconds'] > resource_spec['cpu_budget_seconds']):
            raise ValueError('bounded statistical allocation differs from family resources')
        cap.update(allocation)
    elif mode == 'window-links':
        declared_extension = extension or json.loads(checked(CHECK_EXTENSION)[0])
        operation = load_reference(declared_extension['window_population'])
        allocation = operation.get('execution_limits', {'cpu_seconds':90000, 'hard_cpu_seconds':90010,
            'wall_seconds':10800, 'maximum_output_bytes':16*1024**3})
        if (set(allocation) != {'cpu_seconds','hard_cpu_seconds','wall_seconds','maximum_output_bytes'}
                or any(type(v) is not int or v <= 0 for v in allocation.values())
                or allocation['hard_cpu_seconds'] != allocation['cpu_seconds'] + 10
                or allocation['cpu_seconds'] > resource_spec['cpu_budget_seconds']):
            raise ValueError('bounded window allocation differs from family resources')
        cap.update(allocation)
    elif mode in ('observations', 'core-statistics'):
        cap.update(cpu_seconds=12000, hard_cpu_seconds=12010, wall_seconds=1800,
                   maximum_output_bytes=16 * 1024**3)
    elif mode in ('profile', 'compute', 'clock-audit'):
...
def effective_family_limits(protocol, extension):
    reference = extension.get('budget_authorization')
    if reference is None:
        return {'max_attempts': protocol['resources']['maximum_attempts'],
                'cpu_budget_seconds': protocol['resources']['cpu_budget_seconds']}
    authorization = load_reference(reference)
    if (authorization.get('family') != protocol['family']
            or authorization.get('kind') != 'family_budget_authorization_v1'):
        raise ValueError('budget authorization does not join the original family')
    return authorization['authorized_limits']


def production_compatibility(authenticated):
    if authenticated['extension'].get('clock_uncertainty_plan'):
        from trading_research.data.clock_uncertainty import configure
        configure(load_reference(authenticated['extension']['clock_uncertainty_plan'])['plans'])
    reference = authenticated['extension'].get('producer_compatibility')
    if reference is None:
        return ()
    from trading_research.research.auction_flow_production import producer_identity
    from trading_research.research.auction_flow_quote_replay import configure_verified_compatibility
    contract = load_reference(reference)
    current = producer_identity(ROOT, manifest=authenticated['packet']['code_manifest'])
    if (contract.get('kind') != 'auction_flow_finite_carry_compatibility_v1'
            or contract['current_producer'] != current):
        raise ValueError('retained producer compatibility does not join registered current code')
    configure_verified_compatibility(previous=contract.get('previous_quote_implementations', contract.get('previous_quote_implementation')),
                                    current=contract['current_quote_implementation'])
    return tuple(contract.get('previous_producers', (contract.get('previous_producer'),)))
...
def authenticate_registered_packet(packet_path):
    ...
    if (attempt['status'] != 'running' or attempt['trial_id'] != packet['trial_id']
            ...
            or state['effective_budgets'][protocol['family']] != effective_family_limits(protocol, load_reference(packet['configuration']['check_extension']))):
    ...
    if (extension['parent_protocol_sha256'] != packet['protocol_sha256']
            ...
            or extension['registered_resources_changed'] is not bool(extension.get('budget_authorization'))):
...
def window_allocation(cap, operational):
    from trading_research.research.auction_flow_window_population import SOURCE_SHARDS
    workers = SOURCE_SHARDS
    child_cpu = operational.get('child_cpu_seconds', [(cap['cpu_seconds'] - 120) // workers] * workers)
    if (not isinstance(child_cpu, list) or len(child_cpu) != workers
            or any(type(value) is not int or value <= 0 for value in child_cpu)
            or sum(child_cpu) + 120 > cap['cpu_seconds']):
        raise ValueError('window child CPU allocations exceed the aggregate reservation')
    allowance = cap['maximum_output_bytes'] - cap['supervisor_output_reserve_bytes']
    return {'workers': workers, 'child_cpu_seconds': child_cpu,
            'child_hard_cpu_seconds': [value + 1 for value in child_cpu],
            'child_output_bytes': (allowance - 512 * 1024**2) // workers,
            'child_address_space_bytes': 4 * 1024**3,
            'aggregate_address_space_bytes': (workers + 1) * 4 * 1024**3}
...
def window_shard(registered_packet_path, child_packet_path):
    ...
    allocation = window_allocation(cap, operational)
    ordinal = child['ordinal']
    ...
    resource.setrlimit(resource.RLIMIT_CPU,
                       (allocation['child_cpu_seconds'][ordinal], allocation['child_hard_cpu_seconds'][ordinal]))
...
def execute_window_shards(*, contract, population, outputs, packet_path, authenticated):
    from trading_research.research.auction_flow_window_population import (
        plan_window_units, run_window_population, source_path_shard)
    packet, cap = authenticated['packet'], authenticated['cap']
    operational = load_reference(authenticated['extension']['window_population'])
    allocation = window_allocation(cap, operational)
...
def profile_statistics_inputs(authenticated):
    ...
    for name,mode,key in (('observations','observations','actual_observation_population'),('windows','window-links','actual_window_population')):
        ...
        if name=='observations':
            valid=summary.get('all_population_materialized') is True
            reference=summary.get('reference')
        else:
            valid=summary.get('phase')=='full' and summary.get('processed_source_windows')==3589
            reference=summary.get('refs',{}).get('population')
        frozen_reference=contract['input_populations']['observations39' if name=='observations' else 'windows46']
        if not valid or reference!=frozen_reference:
            raise ValueError('profile source membership must be the accepted3589windows with exact frozen population')
...
def profile_statistics_selected_keys(operational, contract, population):
    from trading_research.research.auction_flow_profile_statistics import profile_partition_plan
    plan = profile_partition_plan(population, contract)
    keys = [list(k) for k in plan['keys']]
    selected = operational.get('selected_partition_keys')
    if selected is None:
        return keys
...
def profile_statistics_allocation(operational, selected, cap):
    count = len(selected)
    concurrency = min(int(operational.get('concurrency', 1)), count)
    if count < 1 or concurrency < 1 or concurrency > 8:
        raise ValueError('bounded statistical partition allocation required')
    cpu = cap['cpu_seconds']-300
    output = (cap['maximum_output_bytes']-cap['supervisor_output_reserve_bytes']-256*1024**2)//(count+1)
    if cpu < 10 or output < 1024**2:
        raise ValueError('insufficient statistical partition reservation')
    return {'jobs':count,'concurrency':concurrency,'child_cpu_seconds':cpu,'child_hard_cpu_seconds':cpu+1,
...
def profile_statistics_shard(registered_packet_path, child_packet_path):
    bootstrap_process_limits()
    install_source_worker_signal_handlers()
    child = json.loads(checked(child_packet_path)[0])
    registered = json.loads(checked(registered_packet_path,maximum=2*1024**2)[0])
    ...
    authenticated = authenticate_registered_packet(registered_packet_path)
    packet,cap = authenticated['packet'],authenticated['cap']
    operational,contract,population,windows,_ = profile_statistics_inputs(authenticated)
    selected = profile_statistics_selected_keys(operational,contract,population)
    allocation = profile_statistics_allocation(operational,selected,cap)
    ordinal = child['ordinal']
    expected_packet=Path(registered_packet_path).parent/'outputs'/f'profile-statistics-shard-{ordinal:02d}-packet.json'
    directory=expected_packet.parent/f'profile-statistics-shard-{ordinal:02d}'
    if (type(ordinal) is not int or not 0 <= ordinal < len(selected)
            or child['partition_key'] != selected[ordinal] or child['allocation'] != allocation
            or child['attempt_id'] != packet['attempt_id'] or child['trial_id'] != packet['trial_id']
            or child['code_snapshot'] != packet['code_snapshot'] or child['configuration'] != packet['configuration']
            or child['coordinator_pid'] != os.getppid() or packet['configuration']['mode'] != 'profile-statistics'
            or Path(child_packet_path).resolve() != expected_packet.resolve()
            or Path(child['output_directory']).resolve() != directory.resolve()):
        raise ValueError('statistical shard identity or allocation mismatch')
    resource.setrlimit(resource.RLIMIT_CPU,(allocation['child_cpu_seconds'],allocation['child_hard_cpu_seconds']))
    ...
    outputs=BoundedOutputs(directory,maximum_total_bytes=allocation['child_output_bytes'],
                           maximum_file_bytes=min(cap['maximum_output_file_bytes'],allocation['child_output_bytes']))
    actual=run_profile_statistics(population=population,window_population=windows,contract=contract,outputs=outputs,
        load_reference=load_reference,selected_partition_keys=[selected[ordinal]])
    outputs.json('result.json',{'passed':actual['passed'],'attempt_id':packet['attempt_id'],'ordinal':ordinal,
        'partition_key':selected[ordinal],'actual':actual},kind='auction_flow_profile_statistics_shard_v1')
    return 0


def execute_profile_statistics_shards(*, operational, contract, population, windows, outputs, packet_path, authenticated):
    from trading_research.research.auction_flow_profile_statistics import (
        profile_partition_plan,profile_partition_identity,profile_partition_source_refs,neighbor_units,window_unit_refs)
    packet,cap=authenticated['packet'],authenticated['cap']
    selected=profile_statistics_selected_keys(operational,contract,population)
    allocation=profile_statistics_allocation(operational,selected,cap)
    if allocation['aggregate_address_space_bytes']>authenticated['live']['memory_limit_bytes']:
    ...
    jobs=[];directories=[]
    for ordinal,key in enumerate(selected):
        directory=outputs.directory/f'profile-statistics-shard-{ordinal:02d}'
        child={'ordinal':ordinal,'coordinator_pid':os.getpid(),'attempt_id':packet['attempt_id'],
            'trial_id':packet['trial_id'],'code_snapshot':packet['code_snapshot'],'configuration':packet['configuration'],
            'allocation':allocation,'partition_key':key,'output_directory':str(directory)}
        ref=outputs.json(f'profile-statistics-shard-{ordinal:02d}-packet.json',child,kind='auction_flow_profile_statistics_shard_packet_v1')
        jobs.append({'ordinal':ordinal,'argv':[sys.executable,str(Path(__file__).resolve()),'--profile-statistics-shard',str(Path(packet_path).resolve()),ref['path']],
            'cwd':str(ROOT),'env':dict(os.environ)})
        directories.append(directory)
    evidence={}
    try:
        ...
        if outputs.written>outputs.maximum_total:raise ValueError('statistical shards exceeded aggregate output')
        apply_remaining_coordinator_cpu_limit(cap['cpu_seconds'],cap['hard_cpu_seconds'])
        outputs.json('profile-statistics-parallel-execution.json',evidence,kind='auction_flow_profile_statistics_parallel_v1')
    plan=profile_partition_plan(population,contract)
    accepted={load_reference(r,maximum=128*1024**2)['identity'] for r in contract.get('accepted_partitions',[])}
    refs=[];measurements=[]
    for ordinal,directory in enumerate(directories):
        ...
        if (result.get('passed') is not True or result['attempt_id']!=packet['attempt_id'] or result['ordinal']!=ordinal
            ...
            raise ValueError('statistical shard output failed exact partition join')
        ref=result['actual']['refs']['partitions'][0];part=load_reference(ref,maximum=128*1024**2)
        members=plan['members'][tuple(key)]
...
def profile_statistics(authenticated,packet_path):
    import unittest
    from trading_research.operations.artifacts import canonical_json,publish_new
    from trading_research.research.auction_flow_profile_statistics import run_profile_statistics
    from trading_research.research.auction_flow_storage import BoundedOutputs
    packet,protocol,cap=authenticated['packet'],authenticated['protocol'],authenticated['cap']
    operational,contract,population,windows,population_ref=profile_statistics_inputs(authenticated)
    modules=operational['check_modules']
    if not modules or any(m not in authenticated['extension']['additional_check_modules'] for m in modules):
        raise ValueError('statistical checks missing from registered dependency closure')
    names=operational.get('diagnostic_check_names') or modules
    if any(not any(name==module or name.startswith(module+'.') for module in modules) for name in names):
        raise ValueError('diagnostic checks escaped authenticated modules')
    began=time.process_time();result=unittest.TextTestRunner(verbosity=2,failfast=bool(operational.get("failfast",False))).run(unittest.defaultTestLoader.loadTestsFromNames(names))
    tests={'tests':result.testsRun,'errors':len(result.errors),'failures':len(result.failures),'success':result.wasSuccessful(),'cpu_seconds':time.process_time()-began}
    if not tests['success']:raise ValueError('profile-statistics regression failure; no actual reduction')
    if operational.get('diagnostic_check_names'):
        raise ValueError('diagnostic-only fixture checks completed; actual population and full gate remain unaccepted')
    projection=None
    if operational['phase']=='full':
        pilot=load_reference(operational['accepted_pilot_execution']);worker=load_reference(operational['accepted_pilot_worker'])
        if (pilot.get('success') is not True or worker.get('success') is not True or pilot['attempt_id']!=worker['attempt_id']
                or worker.get('mode')!='profile-statistics'):
            raise ValueError('full statistics require successful own-family pilot')
        pilot_operation=load_reference(operational['accepted_pilot_operational'])
    ...
    selected=operational.get('selected_partition_keys')
    if operational.get('concurrency',1)>1:
        refs,evidence,measurements=execute_profile_statistics_shards(operational=operational,contract=contract,population=population,windows=windows,outputs=outputs,packet_path=packet_path,authenticated=authenticated)
        parent_contract={**contract,'accepted_partitions':refs}
        actual=run_profile_statistics(population=population,window_population=windows,contract=parent_contract,outputs=outputs,load_reference=load_reference,selected_partition_keys=selected)
        actual['coordinator_finalization_cpu_seconds']=actual['cpu_seconds']
        actual['partition_measurements']=measurements
    else:
        actual=run_profile_statistics(population=population,window_population=windows,contract=contract,outputs=outputs,load_reference=load_reference,selected_partition_keys=selected)
    report={k:packet[k] for k in ('attempt_id','trial_id','protocol_sha256','code_snapshot','tools_snapshot','configuration')}
    report.update(family=protocol['family'],mode='profile-statistics',tests=tests,success=actual['passed'],
        actual_profile_statistics=actual,input_population=population_ref,source_component_reused=True,
        source_component_verified=False,source_unit_parallel_execution=evidence,runtime_versions=authenticated['versions'],
        measured_resource_projection=projection,derived_output_bytes=outputs.written,
        scope='Actual profile/reference/TPO/VWAP/footprint and forward geometry statistics; no complete auction-family or Context claim.')
    publish_new(Path(packet['worker_report']),canonical_json(report)+b'\n')
    return 0 if report['success'] else 1


def core_parallel_execution(contract):
    parallel = contract.get('parallel_execution')
    return (isinstance(parallel, dict) and parallel.get('worker_count') == 8
            and parallel.get('policy') == 'eight_core_partition_consumers_v1')


def remaining_core_partition_work(plan, contract, load_accepted):
    from trading_research.research.auction_flow_core_statistics import (
        core_partition_identity, core_partition_source_refs)
    accepted = {}
    for ref in contract.get('accepted_partitions', []):
        row = load_accepted(ref)
        accepted[row['identity']] = row
    reused_rows = 0
    remaining_partition_rows = []
    remaining_keys = []
    for key in plan['keys']:
        members = plan['members'][key]
        identity = core_partition_identity(contract, collection=key[0], root=key[1], year=key[2],
                                           source_refs=core_partition_source_refs(members))
        if identity in accepted:
            reused_rows += accepted[identity]['completeness']['observations']
        else:
            remaining_keys.append(key)
            remaining_partition_rows.append(sum(
                unit.get('atomic_rows', unit.get('observation_rows')) for unit in members))
    remaining_rows = plan['intended_rows'] - reused_rows
    if remaining_rows < 0:
        raise ValueError('accepted core partitions claim more rows than the authenticated population')
    return {'accepted': accepted, 'reused_rows': reused_rows, 'remaining_rows': remaining_rows,
            'remaining_keys': remaining_keys, 'remaining_partition_rows': remaining_partition_rows,
            'declared_partition_count': len(plan['keys'])}


def core_full_resource_gate(contract, population, cap, *, load_accepted):
    from trading_research.research.auction_flow_core_statistics import (
        core_partition_plan, project_core_parallel_resources)
    pilot_execution = load_reference(contract['accepted_pilot_execution'])
    pilot_worker = load_reference(contract['accepted_pilot_worker'])
    if (pilot_execution.get('success') is not True or pilot_worker.get('success') is not True
            or pilot_execution['attempt_id'] != pilot_worker['attempt_id']):
        raise ValueError('core full phase lacks accepted registered pilot')
    pilot = pilot_worker['actual_core_statistics']
    annual = [m for m in pilot['partition_measurements'] if m['observations'] >= 100000]
    small = [m for m in pilot['partition_measurements'] if m['observations'] < 100000]
    if not annual:
        raise ValueError('core full projection requires a complete large measurement partition')
    if core_parallel_execution(contract):
        plan = core_partition_plan(population.get('units'),
            collections=contract.get('source_collections', {}), selected_paths=None)
        work = remaining_core_partition_work(plan, contract, load_accepted)
        projection = project_core_parallel_resources(
            annual_measurements=annual, small_measurements=small,
            remaining_rows=work['remaining_rows'],
            remaining_partition_rows=work['remaining_partition_rows'],
            declared_partition_count=work['declared_partition_count'])
        if (projection['cpu_seconds'] > cap['cpu_seconds']
                or projection['parallel_wall_seconds'] > cap['wall_seconds']
                or projection['output_bytes'] > cap['maximum_output_bytes']):
            raise ValueError(
                f'core complete measured projection exceeds registered resources: '
                f'CPU={projection["cpu_seconds"]},wall={projection["parallel_wall_seconds"]},'
                f'bytes={projection["output_bytes"]}')
        return projection
    n = population['observation_rows']
    fixed_cpu = max((m['cpu_seconds'] for m in small), default=10.)
    fixed_wall = max((m['wall_seconds'] for m in small), default=10.)
    cpu = 1.5*(max(m['cpu_seconds']/m['observations'] for m in annual)*n+48*fixed_cpu)
    wall = 1.5*(max(m['wall_seconds']/m['observations'] for m in annual)*n+48*fixed_wall)
    output = 1.5*max(m['output_bytes']/m['observations'] for m in annual)*n+512*1024**2
    if cpu > cap['cpu_seconds'] or wall > cap['wall_seconds']-60 or output > cap['maximum_output_bytes']:
        raise ValueError(f'core complete measured projection exceeds registered resources: CPU={cpu},wall={wall},bytes={output}')
    return {'cpu_seconds': cpu, 'parallel_wall_seconds': wall, 'output_bytes': output}


def core_serial_parity(actual, contract):
    specification = contract.get('parallel_execution', {})
    if contract.get('phase') != 'pilot':
        return {'required': False, 'accepted_pilot': contract.get('accepted_pilot_execution')}
    prior = load_reference(specification['serial_parity_execution'])
    worker = load_reference(specification['serial_parity_worker'])
    if prior.get('success') is not True or worker.get('success') is not True or prior['attempt_id'] != worker['attempt_id']:
        raise ValueError('parallel pilot lacks an authenticated successful serial comparator')
    def digest_rows(result):
        rows={}
        for ref in result['refs']['partitions']:
            part=load_reference(ref,maximum=128*1024**2)
            group=load_reference(part['groups'],maximum=128*1024**2)
            denominator=part['denominators']
            rows[part['identity']]={'groups':group,
                'date_values':[(file['rows'],file['row_group_values']) for file in denominator['files']],
                'rows':denominator['rows'],'completeness':part['completeness'],
                'effect':part['effect'],'source_refs':part['source_refs']}
        return rows
    if digest_rows(actual) != digest_rows(worker['actual_core_statistics']):
        raise ValueError('parallel core measurements diverge from the exact accepted serial population')
    return {'required':True,'passed':True,'partitions':len(actual['refs']['partitions']),
            'observations':actual['processed_observations'],'serial_execution':specification['serial_parity_execution'],
            'compared':'Full group numbers, exclusions, completeness, source identities, matched true-OHLC effects and every Parquet date-column value/validity digest.'}


def core_statistics(authenticated, packet_path):
    import unittest
    from trading_research.operations.artifacts import canonical_json, publish_new
    from trading_research.research.auction_flow_core_statistics import (
        POPULATION_MANIFEST_MAXIMUM_BYTES, run_core_statistics)
    from trading_research.research.auction_flow_storage import BoundedOutputs
    packet, protocol, cap = authenticated['packet'], authenticated['protocol'], authenticated['cap']
    extension = authenticated['extension']
    contract = load_reference(extension['core_statistics'])
    prior = load_reference(extension['core_observation_execution'])
    source_worker = load_reference(extension['core_observation_worker'])
    if (prior.get('success') is not True or source_worker.get('success') is not True
            or prior['attempt_id'] != source_worker['attempt_id']
            or source_worker['actual_observation_population']['all_population_materialized'] is not True):
        raise ValueError('core statistics lack complete accepted observation population')
    population_ref = source_worker['actual_observation_population']['reference']
    population = load_reference(population_ref, maximum=max(128 * 1024**2, POPULATION_MANIFEST_MAXIMUM_BYTES))
    projection = None
    if contract.get('phase') == 'full':
        projection = core_full_resource_gate(
            contract, population, cap,
            load_accepted=lambda ref: load_reference(ref, maximum=128 * 1024**2))
        for name, expected in contract['accepted_consumer_files'].items():
            checked(ROOT / name, expected)
    modules = extension['core_statistic_check_modules']
    if any(name not in extension['additional_check_modules'] for name in modules):
        raise ValueError('core checks are absent from registered dependency closure')
    began = time.process_time()
    result = unittest.TextTestRunner(verbosity=2).run(unittest.defaultTestLoader.loadTestsFromNames(modules))
    tests = dict(tests=result.testsRun, errors=len(result.errors), failures=len(result.failures),
                 success=result.wasSuccessful(), cpu_seconds=time.process_time()-began)
    if not tests['success']:
        raise ValueError('core statistics regressions failed; preserve and reassess before execution')
    outputs = BoundedOutputs(Path(packet_path).parent/'outputs',
        maximum_total_bytes=cap['maximum_output_bytes']-cap['supervisor_output_reserve_bytes'],
        maximum_file_bytes=cap['maximum_output_file_bytes'])
    evidence = None
    if core_parallel_execution(contract):
        child_refs, evidence = execute_core_shards(
            contract=contract, population=population, outputs=outputs,
            packet_path=packet_path, authenticated=authenticated)
        parent_contract = dict(contract)
        parent_contract['accepted_partitions'] = list(contract.get('accepted_partitions', [])) + child_refs
        actual = run_core_statistics(population=population, contract=parent_contract, outputs=outputs,
                                     load_reference=load_reference, finalize=True)
    else:
        actual = run_core_statistics(population=population, contract=contract, outputs=outputs,
                                     load_reference=load_reference)
    if evidence is not None:
        accepted_ids = {load_reference(ref, maximum=128*1024**2)['identity']
                        for ref in contract.get('accepted_partitions', [])}
        parts = [load_reference(ref, maximum=128*1024**2) for ref in child_refs]
        actual['coordinator_finalization_cpu_seconds'] = actual['cpu_seconds']
        actual['cpu_seconds_scope'] = 'coordinator finalization only; registered execution accounts all children'
        actual['partition_measurements'] = [dict(part['measurement'],
            reused=part['identity'] in accepted_ids, identity=part['identity']) for part in parts]
        actual['reused_partitions'] = sum(part['identity'] in accepted_ids for part in parts)
        actual['new_partitions'] = len(parts)-actual['reused_partitions']
        actual['serial_parity'] = core_serial_parity(actual, contract)
        actual['refs']['coordinator_summary'] = actual['refs'].pop('summary')
        actual['refs']['summary'] = outputs.json('core-statistics-parallel-summary.json',actual,
            kind='auction_flow_core_statistics_result_v1')
    report = {key: packet[key] for key in
        ('attempt_id','trial_id','protocol_sha256','code_snapshot','tools_snapshot','configuration')}
    report.update(family=protocol['family'], mode='core-statistics' , tests=tests,
        success=bool(actual['passed']), actual_core_statistics=actual, input_population=population_ref,
        source_component_reused=True, source_component_verified=False,
        source_unit_parallel_execution=None, core_partition_parallel_execution=evidence,
        measured_resource_projection=projection, runtime_versions=authenticated['versions'],
        derived_output_bytes=outputs.written,
        scope='Descriptive core measurement distributions, date uncertainty and source comparisons; no Context or family-completion claim.')
    publish_new(Path(packet['worker_report']), canonical_json(report)+b'\n')
    return 0 if report['success'] else 1


def observation_shard_allocation(contract, cap):
    if contract['worker_count'] != 8 or contract.get('parallel_policy') != 'eight_bounded_retained_consumers_v1':
        raise ValueError('frozen eight-worker observation allocation required')
    allowance = cap['maximum_output_bytes'] - cap['supervisor_output_reserve_bytes']
    return {'workers': 8, 'child_cpu_seconds': (cap['cpu_seconds'] - 120) // 8,
            'child_hard_cpu_seconds': (cap['cpu_seconds'] - 120) // 8 + 1,
            'child_output_bytes': (allowance - 64 * 1024**2) // 8,
            'aggregate_address_space_bytes': 9 * cap['memory_bytes']}


def observation_shard(registered_packet_path, child_packet_path):
    bootstrap_process_limits()
    install_source_worker_signal_handlers()
    child = json.loads(checked(child_packet_path)[0])
    registered = json.loads(checked(registered_packet_path, maximum=2 * 1024**2)[0])
    ...
    authenticated = authenticate_registered_packet(registered_packet_path)
    packet, cap = authenticated['packet'], authenticated['cap']
    contract = load_reference(authenticated['extension']['observation_population'])
    allocation = observation_shard_allocation(contract, cap)
    ordinal = child['ordinal']
    expected_packet = Path(registered_packet_path).parent / 'outputs' / f'observation-shard-{ordinal:02d}-packet.json'
    expected_dir = expected_packet.parent / f'observation-shard-{ordinal:02d}'
    if (type(ordinal) is not int or not 0 <= ordinal < allocation['workers']
            or packet['configuration']['mode'] != 'observations'
            or child['attempt_id'] != packet['attempt_id']
            or child['trial_id'] != packet['trial_id']
            or child['code_snapshot'] != packet['code_snapshot']
            or child['configuration'] != packet['configuration']
            or child['allocation'] != allocation
            or Path(child_packet_path).resolve() != expected_packet.resolve()
            or Path(child['output_directory']).resolve() != expected_dir.resolve()
            or child['coordinator_pid'] != os.getppid()):
        raise ValueError('observation shard identity or allocation mismatch')
    resource.setrlimit(resource.RLIMIT_CPU,
        (allocation['child_cpu_seconds'], allocation['child_hard_cpu_seconds']))
    from trading_research.research.auction_flow_population import validate_catalog, choose_receipts, materialize_receipts
    from trading_research.research.auction_flow_storage import BoundedOutputs
    catalog = load_reference(contract['source_catalog'])
    refs = choose_receipts(validate_catalog(catalog, expected_windows=contract['source_window_count']), contract)
    selected = refs[ordinal::allocation['workers']]
    if child['receipts'] != selected:
        raise ValueError('observation shard is not the frozen catalog partition')
    outputs = BoundedOutputs(expected_dir, maximum_total_bytes=allocation['child_output_bytes'],
                             maximum_file_bytes=min(cap['maximum_output_file_bytes'], allocation['child_output_bytes']))
    units = materialize_receipts(selected, outputs=outputs, load_reference=load_reference)
    outputs.json('result.json', {'passed': True, 'attempt_id': packet['attempt_id'],
        'ordinal': ordinal, 'units': units, 'receipts': selected}, kind='auction_flow_observation_shard_v1')
    return 0


def execute_observation_shards(selected, *, contract, outputs, packet_path, authenticated):
    packet, cap = authenticated['packet'], authenticated['cap']
    allocation = observation_shard_allocation(contract, cap)
    if allocation['aggregate_address_space_bytes'] > authenticated['live']['memory_limit_bytes']:
        raise ValueError('observation workers exceed actual allocated memory')
    jobs, directories = [], []
    for ordinal in range(allocation['workers']):
        directory = outputs.directory / f'observation-shard-{ordinal:02d}'
        child = {'ordinal': ordinal, 'coordinator_pid': os.getpid(), 'attempt_id': packet['attempt_id'],
                 'trial_id': packet['trial_id'], 'code_snapshot': packet['code_snapshot'],
                 'configuration': packet['configuration'], 'allocation': allocation,
                 'receipts': selected[ordinal::allocation['workers']], 'output_directory': str(directory)}
        ref = outputs.json(f'observation-shard-{ordinal:02d}-packet.json', child,
                           kind='auction_flow_observation_shard_packet_v1')
        jobs.append({'ordinal': ordinal, 'argv': [sys.executable, str(Path(__file__).resolve()),
                     '--observation-shard', str(Path(packet_path).resolve()), ref['path']],
                     'cwd': str(ROOT), 'env': dict(os.environ)})
        directories.append(directory)
    evidence = {}
    try:
        supervise_source_children(jobs, concurrency=allocation['workers'],
            cpu_soft_seconds=cap['cpu_seconds'], cpu_hard_seconds=cap['hard_cpu_seconds'],
            wall_deadline_monotonic=time.monotonic() + cap['wall_seconds'] - 120,
            allocated_address_space_bytes=allocation['aggregate_address_space_bytes'], evidence=evidence)
    finally:
        used = sum(p.stat().st_size for directory in directories for p in directory.rglob('*') if p.is_file())
        outputs.written += used
        if outputs.written > outputs.maximum_total:
            raise ValueError('observation shard output exceeded aggregate reservation')
        apply_remaining_coordinator_cpu_limit(cap['cpu_seconds'], cap['hard_cpu_seconds'])
        outputs.json('observation-parallel-execution.json', evidence,
                     kind='auction_flow_observation_parallel_execution_v1')
    joined = {}
    for ordinal, directory in enumerate(directories):
        result = json.loads(checked(directory / 'result.json')[0])
        if (result.get('passed') is not True or result['attempt_id'] != packet['attempt_id']
                or result['ordinal'] != ordinal or result['receipts'] != selected[ordinal::allocation['workers']]
                or [u['receipt'] for u in result['units']] != result['receipts']):
            raise ValueError('observation shard result does not join its complete registered population')
        for unit in result['units']:
            for ref in unit['series']['files']:
                if Path(ref['path']).resolve().parent != directory.resolve():
                    raise ValueError('observation file escaped its registered shard namespace')
                raw, _ = checked(ref['path'], ref['sha256'], maximum=cap['maximum_output_file_bytes'])
                if len(raw) != ref['size_bytes']:
                    raise ValueError('observation shard artifact size mismatch')
            joined[unit['receipt']['sha256']] = unit
    if len(joined) != len(selected):
        raise ValueError('observation shard join lost or duplicated a receipt')
    return [joined[ref['sha256']] for ref in selected], evidence


def observation_population(authenticated, packet_path):
    """Run the declared full-population consumer using accepted source artifacts."""
    import unittest
    from trading_research.operations.artifacts import canonical_json, publish_new
    from trading_research.research.auction_flow_storage import BoundedOutputs
    from trading_research.research.auction_flow_population import run_population, projected_resources

    packet, protocol, cap = authenticated['packet'], authenticated['protocol'], authenticated['cap']
    extension = authenticated['extension']
    contract = load_reference(extension['observation_population'])
    modules = extension['observation_check_modules']
    admitted = protocol['check_modules'] + extension['additional_check_modules']
    if not modules or len(set(modules)) != len(modules) or any(n not in admitted for n in modules):
        raise ValueError('observation checks must belong to the registered family closure')
    if contract['phase'] == 'full':
        prior = load_reference(contract['accepted_pilot_execution'])
        pilot = load_reference(contract['accepted_pilot_worker'])
        if (prior.get('success') is not True or pilot.get('success') is not True
                or prior.get('attempt_id') != pilot.get('attempt_id')):
            raise ValueError('full observation population lacks successful registered pilot evidence')
        accepted = load_reference(pilot['actual_observation_population']['reference'])
        estimate = projected_resources(accepted['inventory'], accepted['units'])
        if (contract['worker_count'] != 8 or accepted.get('serial_reference_parity', {}).get('passed') is not True
                or accepted.get('parallel_execution', {}).get('pending_units') != 0):
            raise ValueError('full population requires accepted complete serial/parallel parity')
        projected_wall = estimate['sequential_wall_seconds_with_margin'] / 8 * 1.25 + 120
        if (estimate['cpu_seconds_with_margin'] > cap['cpu_seconds']
                or projected_wall > cap['wall_seconds'] - 60
                or estimate['output_bytes_with_margin'] > cap['maximum_output_bytes'] - cap['supervisor_output_reserve_bytes']):
            raise ValueError('complete measured observation projection exceeds its registered resources')
        for name, sha in contract['accepted_consumer_files'].items():
            checked(ROOT / name, sha)
    began = time.process_time()
    result = unittest.TextTestRunner(verbosity=2).run(unittest.defaultTestLoader.loadTestsFromNames(modules))
    tests = dict(tests=result.testsRun, failures=len(result.failures), errors=len(result.errors),
                 success=result.wasSuccessful(), cpu_seconds=time.process_time()-began, modules=modules)
    if not result.wasSuccessful():
        raise ValueError('observation regressions failed before population execution')
    outputs = BoundedOutputs(Path(packet_path).parent / 'outputs',
        maximum_total_bytes=cap['maximum_output_bytes'] - cap['supervisor_output_reserve_bytes'],
        maximum_file_bytes=cap['maximum_output_file_bytes'])
    executor = lambda selected: execute_observation_shards(selected, contract=contract, outputs=outputs,
        packet_path=packet_path, authenticated=authenticated)
    population = run_population(contract=contract, outputs=outputs, load_reference=load_reference,
                                parallel_executor=executor)
    report = {key: packet[key] for key in
              ('attempt_id', 'trial_id', 'protocol_sha256', 'code_snapshot', 'tools_snapshot', 'configuration')}
    report.update(family=protocol['family'], mode='observations', tests=tests,
        actual_observation_population=population, success=bool(population['passed']),
        source_component_verified=False, source_component_reused=True,
        source_unit_parallel_execution=None, runtime_versions=authenticated['versions'],
        derived_output_bytes=outputs.written,
        scope='Complete exact atomic measurement table population; empirical family findings remain separate')
    publish_new(Path(packet['worker_report']), canonical_json(report) + b'\n')
    return 0 if report['success'] else 1


def core_shard_allocation(contract, cap):
    if not core_parallel_execution(contract):
        raise ValueError('frozen eight-worker core partition allocation required')
    allowance = cap['maximum_output_bytes'] - cap['supervisor_output_reserve_bytes']
    derived = (cap['cpu_seconds'] - 120) // 8
    child_cpu = derived
    child_as = 4 * 1024 ** 3
    return {'workers': 8, 'child_cpu_seconds': child_cpu,
            'child_hard_cpu_seconds': child_cpu + 1,
            'child_output_bytes': (allowance - 64 * 1024**2) // 8,
            'child_address_space_bytes': child_as,
            'aggregate_address_space_bytes': 9 * child_as}


def core_shard(registered_packet_path, child_packet_path):
    bootstrap_process_limits()
    install_source_worker_signal_handlers()
    child = json.loads(checked(child_packet_path)[0])
    registered = json.loads(checked(registered_packet_path, maximum=2 * 1024**2)[0])
    ...
    authenticated = authenticate_registered_packet(registered_packet_path)
    packet, cap = authenticated['packet'], authenticated['cap']
    contract = load_reference(authenticated['extension']['core_statistics'])
    allocation = core_shard_allocation(contract, cap)
    ordinal = child['ordinal']
    expected_packet = Path(registered_packet_path).parent / 'outputs' / f'core-shard-{ordinal:02d}-packet.json'
    expected_dir = expected_packet.parent / f'core-shard-{ordinal:02d}'
    if (type(ordinal) is not int or not 0 <= ordinal < allocation['workers']
            or packet['configuration']['mode'] != 'core-statistics'
            or child['attempt_id'] != packet['attempt_id']
            or child['trial_id'] != packet['trial_id']
            or child['code_snapshot'] != packet['code_snapshot']
            or child['configuration'] != packet['configuration']
            ...
            or Path(child['output_directory']).resolve() != expected_dir.resolve()
            or child['coordinator_pid'] != os.getppid()):
        raise ValueError('core shard identity or allocation mismatch')
    resource.setrlimit(resource.RLIMIT_CPU,
        (allocation['child_cpu_seconds'], allocation['child_hard_cpu_seconds']))
    from trading_research.research.auction_flow_core_statistics import (
        POPULATION_MANIFEST_MAXIMUM_BYTES, core_partition_plan, run_core_statistics,
        select_core_shard_keys)
    from trading_research.research.auction_flow_storage import BoundedOutputs
    source_worker = load_reference(authenticated['extension']['core_observation_worker'])
    population_ref = source_worker['actual_observation_population']['reference']
    population = load_reference(population_ref, maximum=POPULATION_MANIFEST_MAXIMUM_BYTES)
    phase = contract.get('phase', 'full')
    selected_paths = contract.get('pilot_source_paths') if phase == 'pilot' else None
    plan = core_partition_plan(population.get('units'),
        collections=contract.get('source_collections', {}), selected_paths=selected_paths)
    expected_keys = select_core_shard_keys(plan['keys'], ordinal=ordinal, worker_count=allocation['workers'])
    if child['partition_keys'] != [list(key) for key in expected_keys]:
        raise ValueError('core shard is not the frozen partition-key selection')
    outputs = BoundedOutputs(expected_dir, maximum_total_bytes=allocation['child_output_bytes'],
                             maximum_file_bytes=min(cap['maximum_output_file_bytes'], allocation['child_output_bytes']))
    actual = run_core_statistics(population=population, contract=contract, outputs=outputs,
                                 load_reference=load_reference, partition_keys=expected_keys,
                                 finalize=False)
    outputs.json('result.json', {'passed': True, 'attempt_id': packet['attempt_id'],
        'ordinal': ordinal, 'partition_keys': child['partition_keys'],
        'partition_identities': actual['partition_identities'],
        'refs': actual['refs']}, kind='auction_flow_core_shard_v1')
    return 0


def execute_core_shards(*, contract, population, outputs, packet_path, authenticated):
    from trading_research.research.auction_flow_core_statistics import (
        core_partition_identity, core_partition_plan, core_partition_source_refs,
        select_core_shard_keys)
    packet, cap = authenticated['packet'], authenticated['cap']
    allocation = core_shard_allocation(contract, cap)
    if allocation['aggregate_address_space_bytes'] > authenticated['live']['memory_limit_bytes']:
        raise ValueError('core workers exceed actual allocated memory')
    phase = contract.get('phase', 'full')
    selected_paths = contract.get('pilot_source_paths') if phase == 'pilot' else None
    plan = core_partition_plan(population.get('units'),
        collections=contract.get('source_collections', {}), selected_paths=selected_paths)
    jobs, directories, expected = [], [], []
    for ordinal in range(allocation['workers']):
        directory = outputs.directory / f'core-shard-{ordinal:02d}'
        keys = select_core_shard_keys(plan['keys'], ordinal=ordinal, worker_count=allocation['workers'])
        encoded = [list(key) for key in keys]
        expected.append(keys)
        child = {'ordinal': ordinal, 'coordinator_pid': os.getpid(), 'attempt_id': packet['attempt_id'],
                 'trial_id': packet['trial_id'], 'code_snapshot': packet['code_snapshot'],
                 'configuration': packet['configuration'], 'allocation': allocation,
                 'partition_keys': encoded, 'output_directory': str(directory)}
        ref = outputs.json(f'core-shard-{ordinal:02d}-packet.json', child,
                           kind='auction_flow_core_shard_packet_v1')
        jobs.append({'ordinal': ordinal, 'argv': [sys.executable, str(Path(__file__).resolve()),
                     '--core-shard', str(Path(packet_path).resolve()), ref['path']],
                     'cwd': str(ROOT), 'env': dict(os.environ)})
        directories.append(directory)
    evidence = {}
    try:
        ...
        outputs.written += used
        if outputs.written > outputs.maximum_total:
            raise ValueError('core shard output exceeded aggregate reservation')
        apply_remaining_coordinator_cpu_limit(cap['cpu_seconds'], cap['hard_cpu_seconds'])
        outputs.json('core-parallel-execution.json', evidence,
                     kind='auction_flow_core_parallel_execution_v1')
    accepted_ids = set()
    for ref in contract.get('accepted_partitions', []):
        accepted_ids.add(load_reference(ref, maximum=128 * 1024**2)['identity'])
    joined = []
    seen = set()
    for ordinal, directory in enumerate(directories):
        result = json.loads(checked(directory / 'result.json')[0])
        encoded = [list(key) for key in expected[ordinal]]
        if (result.get('passed') is not True or result['attempt_id'] != packet['attempt_id']
                or result['ordinal'] != ordinal or result['partition_keys'] != encoded
                or len(result['refs']['partitions']) != len(expected[ordinal])
                or len(result['partition_identities']) != len(expected[ordinal])):
            raise ValueError('core shard result does not join its assigned partitions')
        for key, identity, ref in zip(expected[ordinal], result['partition_identities'],
                                      result['refs']['partitions']):
            members = plan['members'][key]
            computed = core_partition_identity(contract, collection=key[0], root=key[1], year=key[2],
                                               source_refs=core_partition_source_refs(members))
            if identity != computed:
                raise ValueError('core shard partition identity does not match the authenticated selection')
            if identity not in accepted_ids:
                if Path(ref['path']).resolve().parent != directory.resolve():
                    raise ValueError('core partition escaped its registered shard namespace')
                raw, _ = checked(ref['path'], ref['sha256'], maximum=cap['maximum_output_file_bytes'])
                if len(raw) != ref['size_bytes']:
                    raise ValueError('core shard artifact size mismatch')
            if identity in seen:
                raise ValueError('core shard join duplicated a partition identity')
            seen.add(identity)
            joined.append(ref)
    expected_ids = {core_partition_identity(contract, collection=key[0], root=key[1], year=key[2],
                    source_refs=core_partition_source_refs(plan['members'][key]))
                    for key in plan['keys']}
    if seen != expected_ids:
        raise ValueError('core shard join lost or duplicated a partition')
...
def audit_reset_context(authenticated, packet_path):
    import math, struct
    import numpy as np
    ...
    from trading_research.operations.artifacts import canonical_json, publish_new
    packet, protocol, cap = authenticated['packet'], authenticated['protocol'], authenticated['cap']
    previous = load_reference(authenticated['extension']['reset_failure_worker'])
    failures = {}
    ...
    if len(failures) != 4:
        raise ValueError('four retained reset failures required')
    outputs = BoundedOutputs(Path(packet_path).parent / 'outputs', maximum_total_bytes=cap['maximum_output_bytes']-cap['supervisor_output_reserve_bytes'], maximum_file_bytes=cap['maximum_output_file_bytes'])
    def exact(value):
        ...
        if type(value) is float and not math.isfinite(value): return {'$float64_bits_le':struct.pack('<d',value).hex()}
        return value
    records=[]
    for failure in failures.values():
        unit=failure['unit']; path=ROOT.parent/'data'/unit['source_path']; stamp=(path.stat().st_size,path.stat().st_mtime_ns)
        parquet=pq.ParquetFile(path); offset=0; offsets=[]
        for group in range(parquet.num_row_groups):
            offsets.append(offset); offset+=parquet.metadata.row_group(group).num_rows
        parts=[]; scanned=0
        for group in unit['source_groups']:
            table=parquet.read_row_group(group,use_threads=False);scanned+=len(table)
            if scanned>unit['physical_scan_rows']:raise ValueError('original scan bound exceeded')
            table=table.append_column('source_row',pa.array(np.arange(offsets[group],offsets[group]+len(table),dtype=np.int64)))
            table=table.filter(pc.and_(pc.greater_equal(table['t'],unit['event_start_ns']),pc.less(table['t'],unit['event_end_ns'])))
            if len(table):parts.append(table)
        table=pa.concat_tables(parts).combine_chunks();t=table['t'].to_numpy();rows=table['source_row'].to_numpy();flags=table['flags'].to_numpy();actions=np.array(pc.cast(table['action'],pa.string()).to_pylist())
        high=np.maximum.accumulate(t);back=np.flatnonzero(t[1:]<high[:-1])+1
        anchor=failure['clock_diagnostic']['first_unresolved_issue']['source_row'];position=int(np.flatnonzero(rows==anchor)[0]);prevmax=int(high[position-1]) if position else int(t[0]);recover=np.flatnonzero(t[position:]>=prevmax);end=position+int(recover[0]) if len(recover) else len(t)
        invalid=((flags.astype(np.int64)&4)!=0)|(actions=='R');prior_invalid=np.flatnonzero(invalid[:position]);latest=None if not len(prior_invalid) else int(prior_invalid[-1])
        selected=sorted(set(range(min(32,len(table))))|set(range(max(0,position-8),min(len(table),end+9))))
        # Bounded original context, never substitute a sorted or corrected source.
        if len(selected)>4096:selected=selected[:2048]+selected[-2048:]
        neighbors=table.take(pa.array(selected,type=pa.int64())).to_pylist()
        records.append({'unit':unit,'source_schema':str(parquet.schema_arrow),'source_stamp':list(stamp),'selected_rows':len(table),'physical_rows_scanned':scanned,'backward_rows':len(back),'first_backward_positions':back[:32].tolist(),'fault_position_in_day':position,'clock_recovery_position':end,'clock_recovery_source_row':None if end==len(t) else int(rows[end]),'clock_highwater_ns':prevmax,'minimum_clock_in_fault_region':int(t[position:max(position+1,end)].min()),'preceding_invalidation_position':latest,'preceding_invalidation':None if latest is None else exact(table.slice(latest,1).to_pylist()[0]),'trades_after_preceding_invalidation_before_fault':int(np.count_nonzero(actions[(latest+1 if latest is not None else 0):position]=='T')),'clock_fault_region_action_counts':{str(a):int(np.count_nonzero(actions[position:end]==a)) for a in np.unique(actions[position:end])},'all_field_context':exact(neighbors)})
        if stamp!=(path.stat().st_size,path.stat().st_mtime_ns):raise ValueError('source changed during audit')
    detail=outputs.json('original-reset-context.json',records,kind='auction_flow_original_reset_context_audit')
    report={key:packet[key] for key in ('attempt_id','trial_id','protocol_sha256','code_snapshot','tools_snapshot','configuration')}
    report.update(family=protocol['family'],mode='clock-audit',success=True,runtime_versions=authenticated['versions'],reset_context=detail,audited_windows=len(records),derived_output_bytes=outputs.written,source_component_verified=False,full_project_under_three_hours_verified=False,scope='Original reset surrounding-event diagnosis; no corrected chronology or measurements claimed.')
    publish_new(Path(packet['worker_report']),canonical_json(report)+b'\n')
    return 0
...
def source_file(registered_packet_path, child_packet_path):
    ...
    try:
        ...
        compatible = production_compatibility(authenticated)
        result = run_file_chain(
            ...
            compatible_producer_identities=compatible)
...
def extract_source(authenticated, packet_path, worker_started):
    ...
    outputs = BoundedOutputs(Path(packet_path).parent / 'outputs',
        ...
        maximum_file_bytes=cap['maximum_output_file_bytes'])
    tests = None
    production_compatibility(authenticated)
...
def worker(packet_path):
    ...
    try:
        ...
        if os.getppid() != packet['supervisor_pid']:
            raise ValueError('worker lacks its actual running same-family attempt, snapshot, limits and supervisor')
        if packet['configuration']['mode'] == 'profile-statistics':
            return profile_statistics(authenticated,packet_path)
        if packet['configuration']['mode'] == 'window-statistics':
            return window_statistics(authenticated,packet_path)
        if packet['configuration']['mode'] == 'window-links':
            return window_population(authenticated, packet_path)
        if packet['configuration']['mode'] == 'core-statistics':
            return core_statistics(authenticated, packet_path)
        if packet['configuration']['mode'] == 'observations':
            return observation_population(authenticated, packet_path)
        ...
        if packet['configuration']['mode'] == 'clock-audit':
            return audit_reset_context(authenticated, packet_path) if extension.get('reset_context_audit') else (audit_nan_failures(authenticated, packet_path) if extension.get('nan_failure_audit') else audit_clock_records(authenticated, packet_path))
...
def parent(mode='check'):
    ...
    extension = json.loads(checked(CHECK_EXTENSION)[0])
    if extension.get('budget_authorization'):
        auth_ref = extension['budget_authorization']
        raw, _ = checked(auth_ref['path'], auth_ref['sha256'])
        registry.amend_budget(authorization=asdict(store.put_bytes(raw, kind='family_budget_authorization_v1')))
    state = registry.state()
    previous_bytes = 0
    ...
    trial = registry.register(name='Acquired auction/flow: ' + (
        ...
        else 'profile/reference/TPO/VWAP/footprint and future geometry statistics' if mode == 'profile-statistics'
        else 'formation/delay/forward-label/event descriptive statistics' if mode == 'window-statistics'
        else 'core measurement distributions and source comparisons' if mode == 'core-statistics'
        else 'complete retained-source observation table population' if mode == 'observations'
        else 'consolidated source and full-window resource check'),
        family=protocol['family'], stage='representation' if mode in ('core-statistics','window-statistics','profile-statistics') else 'engineering', configuration=configuration, code_hash=snapshot.sha256,
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4 and sys.argv[1] == '--observation-shard':
        raise SystemExit(observation_shard(sys.argv[2], sys.argv[3]))
    if len(sys.argv) == 4 and sys.argv[1] == '--window-shard':
        raise SystemExit(window_shard(sys.argv[2], sys.argv[3]))
    if len(sys.argv) == 4 and sys.argv[1] == '--profile-statistics-shard':
        raise SystemExit(profile_statistics_shard(sys.argv[2],sys.argv[3]))
    if len(sys.argv) == 4 and sys.argv[1] == '--window-statistics-shard':
        raise SystemExit(window_statistics_shard(sys.argv[2],sys.argv[3]))
    if len(sys.argv) == 4 and sys.argv[1] == '--core-shard':
        raise SystemExit(core_shard(sys.argv[2], sys.argv[3]))
    if len(sys.argv) != 2 or sys.argv[1] not in REGISTERED_MODES:
        raise SystemExit('usage: run_auction_flow_study.py {check|profile|benchmark|compute|downstream|throughput|clock-audit|source-clock|extract|extract-check|observations|core-statistics}')
