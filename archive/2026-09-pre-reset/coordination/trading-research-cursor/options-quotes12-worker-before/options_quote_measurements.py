...
def _interval_file_relevant(path, chains, date_hi, first_cut, last_cut):
    ...
    if meta is None:
        handle = pq.ParquetFile(path)
        names = {handle.schema_arrow.field(i).name: i for i in range(handle.schema_arrow.n_names)}
        groups = []
        for index in range(handle.num_row_groups):
    ...
    for stats in meta:
        ...
        if end is not None and end[1] is not None and int(end[1]) <= first_cut:
...
class OIJoinAdapter:
    ...
    def _validate_full(self, oi_population, chains, dates):
        if oi_population.get("mode") != "full":
            return False
    ...
    def _load_source_manifest(self):
        for ref in self._refs.get("source_manifest") or []:
            ...
            files = payload.get("files") or payload.get("manifest") or []
    ...
    def _listing_path(self, rel, sha256):
        ...
        if manifest and manifest.get("sha256") and manifest["sha256"] != sha256:
    ...
    def _parse_listing_ids(self, table, chain, declared):
        ...
        invalid = 0
        for i in range(n):
            if osi_null[i] or not exp_ok[i] or not req_ok[i] or right_null[i] or strike_null[i] or not strike_present[i]:
                invalid += 1
                continue
    ...
    def listing_bind(self, chain, request_date):
        ...
        if meta is not None and meta.get("empty_marker"):
            return {
                "listing_known": True, "vix_listing_unknown": False,
                "listing_unavailable": False, "listing_file_present": True,
                "listing_status": "empty_marker", "listing_denominator_known": True,
                "listed_ids": listed if listed is not None else _np().empty(0, dtype=_np().int64),
        ...
        known = bool(listed is not None and present is True and status in {"present", "empty_marker"})
                        ...
                        if len(oi_ids):
                            ...
                            if len(only):
                                ...
                                board.extend(_oi_only_rows(
                                    ...
                                    oi_join, oi_o, support_status, dates,
                                ))
                    if board:
