...
PRESPECIFIED = (
    ...
    "known_unit_fraction", "known_half_fraction",
)
RATIO_SPECS = {
...
HIST_FOR_METRIC = {
    "mean_bid": "bid_raw", "mean_ask": "ask_raw", "mean_mid": "mid_raw",
    "mean_spread": "spread_raw", "mean_sample_age_s": "sample_age_s",
    "mean_payload_age_s": "payload_age_s",
}
...
def _coverage_maps(coverage_rows, *, family="all", cut="all"):
    ...
    for day, raw_items in by_date.items():
        ...
        oi_den_items = []
        for item in items:
            quoted = int(item.get("quoted_count") or 0)
            listed_u = int(item.get("listed_unquoted") or 0) if item.get("listing_known") else 0
            den = quoted + listed_u
            if den:
                oi_den_items.append({"num": item.get("oi_available_count") or 0, "den": den})
        if oi_den_items:
            metrics["oi_covered_fraction"][day] = sum(
                item["num"] / item["den"] for item in oi_den_items
            ) / len(oi_den_items)
            events["oi_covered_fraction"][day] = sum(item["den"] for item in oi_den_items)
        ...
        quoted_any = extras["quoted_contracts"][day]
        if quoted_any:
            metrics["known_unit_fraction"][day] = 1.0
            metrics["known_half_fraction"][day] = 0.5
            events["known_unit_fraction"][day] = quoted_any
            events["known_half_fraction"][day] = quoted_any
    return {"metrics": metrics, "events": events, "raw_means": {
        name: raw_mean(
            sum((extras.get("_skip") or 0) for _ in ()), 0
        ) for name in ()
    }, "sums": sums, "extras": extras}
...
def _merge_maps(*parts):
    ...
    for part in parts:
        ...
        for name, cells in (part.get("metrics") or {}).items():
            metrics[name].update(cells)
        for name, cells in (part.get("events") or {}).items():
            events[name].update(cells)
        ...
        for name, cells in (part.get("sums") or {}).items():
            ...
            for day, raw in cells.items():
                if isinstance(raw, tuple):
                    ...
                    raw_means[name] = raw
            if present:
                raw_means[name] = raw_mean(total_s, total_n)
    return metrics, events, raw_means, extras


def _histograms_for(rows, dates, name):
    values, counts = [], []
    for row in rows:
        if row["request_date"] not in dates:
            continue
        hist = (row.get("histograms") or {}).get(name)
        if not hist:
            continue
...
def _bundle_for(chain_rows, dates, *, family, cut, right, dte, cfg):
    ...
    for row in chain_rows:
        if row["request_date"] not in dates:
            continue
        if row.get("mean_bid") is not None and "mean_bid" not in metrics:
            metrics["mean_bid"][row["request_date"]] = row["mean_bid"]
        if row.get("raw_bid_n"):
            events.setdefault("mean_bid", {})[row["request_date"]] = row["raw_bid_n"]
    bid_s = sum(int(row.get("raw_bid_sum") or 0) for row in chain_rows if row["request_date"] in dates)
    bid_n = sum(int(row.get("raw_bid_n") or 0) for row in chain_rows if row["request_date"] in dates)
    if bid_n and "mean_bid" not in raw_means:
        raw_means["mean_bid"] = raw_mean(bid_s, bid_n)
        for row in chain_rows:
            if row["request_date"] in dates and row.get("mean_bid") is not None:
                metrics.setdefault("mean_bid", {})[row["request_date"]] = row["mean_bid"]
                events.setdefault("mean_bid", {})[row["request_date"]] = row.get("raw_bid_n") or 0
    raw_q = {}
    for metric, hist_name in HIST_FOR_METRIC.items():
        values, counts = _histograms_for(
            [row for row in chain_rows if row["request_date"] in dates], dates, hist_name)
    ...
    for chain in chains:
        ...
        for uname, udates in universes:
            if not udates:
                ...
                continue
            emit = targets if uname == "all_period" else {("all", "all", "all", "all")}
...
def run_statistics(date_aggregates, *, protocol, outputs, intended_all, selected_dates, chains, stages,
    ...
    for chain in chains:
        ...
        chain_group_files[chain] = outputs.json(
            f"groups-{chain.lower()}.json", chain_groups, kind="options_quote_chain_groups_v1")
    stats_payload = {
        "kind": VERSION,
        "configuration": cfg,
        ...
        "note": "Complete metric tables are in groups and groups-*.json; not a 10-metric summary.",
    }
...
def finish_quote_outputs(
    ...
    coverage_table = schema.empty_table() if not coverage_rows else pa.table(
        {field.name: [row.get(field.name) for field in schema] for field in schema},
        schema=schema,
    )
...
__all__ = [
    ...
    "exact_quantiles_from_values", "finish_quote_outputs", "raw_mean",
