...
Local unmatched quote contract_ids are deterministic in
[LOCAL_ID_BASE, LOCAL_ID_BASE+2**30). Integer 0 is reserved for unresolved
invalid identity and is not a tradable contract. Invalid OSI/right/strike never
shares a sort key with a valid identity, even when OSI text matches.
"""
...
from trading_research.research.options_oi_columnar import (
    ...
    _is_compact_report, _millistrike_column, _osi_unique_parse, _unique_string_map,
)
...
# Deterministic unmatched-quote namespace. Never 0; does not collide with
# typical OI contract_id sequences that start at 1.
LOCAL_ID_BASE = 1_073_741_824
LOCAL_ID_MASK = 0x3FFFFFFF
UNRESOLVED_ID = 0
...
def alias_conflict_schema():
    ...
    return pa.schema([
        ...
        ("example_contract_id", pa.int32()), ("example_ts_event_ns", pa.int64()),
...
def _empty_frame(n=0):
    ...
    frame = {
        ...
        "contract_id": np.empty(n, dtype=np.int32),
        "sort_id": np.empty(n, dtype=np.int32),
...
class QuoteCodebook:
    ...
    def local_id(chain, osi, exp, milli, right):
        ...
        digest32 = int.from_bytes(hashlib.sha256(payload).digest()[:4], "little")
        return int(LOCAL_ID_BASE + (digest32 & LOCAL_ID_MASK))
    ...
    def intern_valid(self, chain, osi_codes, osi_book, exp_days, milli, right_code, valid, dates):
        ...
        sort_id = np.zeros(n, dtype=np.int32)
        if n == 0:
        ...
        if np.any(valid):
            ...
            resolved = np.empty(len(uniq), dtype=np.int32)
            ...
            for u, key in enumerate(uniq):
                ...
                if oi_id is None:
                    local = self.local.get(lookup)
                    if local is None:
                        local = int(self.local_id(chain, osi, exp, int(key[2]), right))
        ...
        if np.any(invalid):
            ...
            uids = np.empty(len(uinv), dtype=np.int32)
            for u, key in enumerate(uinv):
                packed = (int(key[0]), int(key[1]), int(key[2]), int(key[3]))
...
def parse_quote_typed(table, *, record, dates, clocks, osi_book, codebook, session_open_ns,
    ...
    req_days = req_days_raw.astype(np.int32, copy=False)
    req_days[~req_ok] = np.int32(-1)
    exp_days = exp_days_raw.astype(np.int32, copy=False)
    ...
    frame["row_index"] = np.arange(n, dtype=np.int64)
...
def _read_filtered(refs, columns, filters=None):
    import pyarrow.dataset as ds
    tables = []
    for ref in refs:
        dataset = ds.dataset(ref["path"], format="parquet")
        available = set(dataset.schema.names)
        use = [name for name in columns if name in available]
        extra = []
        if "last_eq_first" in available:
            extra.append("last_eq_first")
        table = dataset.to_table(columns=use + [name for name in extra if name not in use],
                                 filter=filters)
        tables.append(table)
    if not tables:
        return None
...
class OIJoinAdapter:
    ...
    def __init__(self, oi_population, *, chains, dates):
    ...
    def _validate_full(self, oi_population, chains, dates):
        if not self._refs["identity_map"] or not self._refs["reports"] or not self._refs["asof_intervals"]:
            return False
        if not self._refs["coverage"]:
            return False
        if oi_population.get("mode") == "pilot":
            return False
        for chain in chains:
    ...
    def prepare_day(self, chain, request_date, prev_date):
        """Load reports and intersecting intervals for one chain-date; previous day released by caller."""
        np = _np()
        import pyarrow.dataset as ds
        horizon = {
        ...
        if not self.available:
            ...
            return horizon
        dates = [request_date] + ([prev_date] if prev_date else [])
        if self._refs["reports"]:
            filt = (ds.field("chain") == chain) & ds.field("request_date").isin(dates)
            table = _read_filtered(
                self._refs["reports"],
                ["contract_id", "chain", "request_date", "listed", "first_oi", "last_oi", "last_eq_first"],
                filt,
            )
            if table is not None and len(table):
                if _is_compact_report(table) and "last_eq_first" in table.schema.names:
                    # last-null = first; listing does not need last endpoint.
                    pass
                ...
                today = np.array([label == request_date for label in req], dtype=bool)
                if listed is None:
                    listed_today = cid[today]
                else:
                    listed_today = cid[today & listed.astype(bool)]
                horizon["listed_ids"] = np.sort(listed_today.astype(np.int32, copy=False))
                horizon["reports_present"] = True
                del table
        cache = self._chain_intervals(chain)
        if cache is not None and cache["cid"].size:
            keep = cache["req_iso"] <= request_date
            if np.any(keep):
    ...
    def _chain_intervals(self, chain):
        np = _np()
        if self._interval_chain == chain and self._interval_cache is not None:
            return self._interval_cache
        if not self._refs.get("asof_intervals"):
            self._interval_chain = chain
            self._interval_cache = {"cid": np.empty(0, dtype=np.int32)}
            return self._interval_cache
        import pyarrow.dataset as ds
        table = _read_filtered(
        ...
        req = table.column("request_date").to_pylist()
        req_iso = np.array([
            "" if label is None else (label if type(label) is str else str(label))
            for label in req
        ], dtype=object)
        exp = table.column("expiration").to_pylist()
        ...
        self._interval_cache = {
            ...
            "req_iso": req_iso,
            "req_days": np.array([
                -1 if label == "" else codebook.days(label) for label in req_iso
            ], dtype=np.int32),
            "exp_days": np.array([
                -1 if item is None else codebook.days(
                    item if type(item) is str else item.isoformat()
                )
                for item in exp
            ], dtype=np.int32),
    ...
    def listing_bind(self, chain, request_date):
        ...
        denom = bool(cov.get("listing_denominator_known"))
        status = cov.get("listing_status") or "unknown"
        present = cov.get("listing_file_present")
        known = bool(denom and present is True and status == "present")
        listed = None
        if known and self._horizon is not None:
            listed = self._horizon["listed_ids"]
        return {
            "listing_known": known, "vix_listing_unknown": False,
            "listing_unavailable": False, "listing_file_present": present,
            "listing_status": status, "listing_denominator_known": denom,
            "listed_ids": listed, "oi_unavailable": False,
        }
...
def _hist_add(store, name, values, weights=None):
    np = _np()
    ...
    w = np.ones(len(values), dtype=np.int64) if weights is None else np.asarray(weights, dtype=np.int64)[finite]
    uniq, inv = np.unique(values, return_inverse=True)
    counts = np.zeros(len(uniq), dtype=np.int64)
    np.add.at(counts, inv, w)
    bag = store.setdefault(name, {"v": np.empty(0, dtype=np.float64), "c": np.empty(0, dtype=np.int64)})
    if bag["v"].size == 0:
        bag["v"], bag["c"] = uniq, counts
        return
    merged_v = np.concatenate([bag["v"], uniq])
    merged_c = np.concatenate([bag["c"], counts])
    u, inv = np.unique(merged_v, return_inverse=True)
    c = np.zeros(len(u), dtype=np.int64)
    np.add.at(c, inv, merged_c)
    bag["v"], bag["c"] = u, c
...
def _build_board_columns(chain, declared, cut_label, cut_ns, family, frame, chosen, osi_book,
    ...
    hide_price = conflict & (raw > BASE_INVALID_NUMERIC)
...
def _coverage_row(*, chain, declared, cut_label, family, intended, session, missing_file,
    ...
    universe = [row for row in board_rows
                if row["listed"] is True and row["oi_available"] and row["oi"] is not None and row["oi"] >= 0]
    ...
    return {
        ...
        "oi_available_count": sum(1 for row in board_rows if row["oi_available"]),
...
def _date_aggregate(chain, declared, intended, session, family_quality, coverage_rows, board_rows,
                    support_rows, paired, hists, unique_by_family=None):
...
def run(*, protocol, admitted, oi_population, store, outputs, selected_dates=None,
    ...
    oi_join = OIJoinAdapter(oi_population, chains=work_chains, dates=work_dates)
    ...
    for chain in work_chains:
        for day in selected_days:
                ...
                for record in records:
                    ...
                    for table in tables:
                        parsed, exc = parse_quote_typed(
                            ...
                            session_close_ns=session.close_at,
                        )
                        if exc:
            ...
            if listed_ids is not None:
                listed_ids = np.sort(np.asarray(listed_ids, dtype=np.int32))
            ...
            for cut_label, cut_at, early_na, at_close in _cut_plan(session, day):
                for family in families_for_cuts:
                    ...
                    board = []
                    if parsed is not None and not early_na:
                        ...
                        if len(chosen):
                            ...
                            quoted_ids = np.sort(cids[resolved]) if np.any(resolved) else np.empty(0, dtype=np.int32)
                            if listing["listing_known"] and listed_ids is not None and not early_na:
            ...
            for row in day_boards:
                if not row["quoted"] or not row["usable"]:
                    continue
                weight = int(row.get("alias_multiplicity") or 1)
                if row.get("bid") is not None:
                    _hist_add(hists, "bid_raw", [row["bid"]], [weight])
                    _hist_add(hists, "bid_unique", [row["bid"]], [1])
                if row.get("ask") is not None:
                    _hist_add(hists, "ask_raw", [row["ask"]], [weight])
                if row.get("mid") is not None:
                    _hist_add(hists, "mid_raw", [row["mid"]], [weight])
                if row.get("spread") is not None:
                    _hist_add(hists, "spread_raw", [row["spread"]], [weight])
                if row.get("sample_age_ns") is not None:
                    _hist_add(hists, "sample_age_s", [row["sample_age_ns"] / NS], [1])
                if row.get("unchanged_payload_age_ns") is not None:
                    _hist_add(hists, "payload_age_s", [row["unchanged_payload_age_ns"] / NS], [1])
            hist_out = {name: {"v": bag["v"].tolist(), "c": bag["c"].tolist()} for name, bag in hists.items()}
            if day_coverage:
    ...
    reconstruction = {
        ...
        "local_id_namespace": {
            ...
            "formula": "LOCAL_ID_BASE + (sha256(chain|osi|exp|milli|right)[:4] & mask)",
...
def _concat_parquet_refs(refs, outputs, name, kind):
    import pyarrow.parquet as pq
    tables = []
    for ref in refs:
        ...
        for item in items:
            if item is None:
                continue
            tables.append(pq.read_table(item["path"]))
    if not tables:
        return None
    table = _pa().concat_tables(tables, promote_options="default")
...
def combine_partition_results(*, protocol, results, outputs, load_reference):
    ...
    for result in results:
        ...
        if refs.get("date_aggregates"):
            aggregate_rows.extend(_load_result_json(refs["date_aggregates"], load_reference))
    ...
    coverage_ref = _concat_parquet_refs(coverage_refs, outputs, "coverage.parquet", OUTPUT_SCHEMAS["coverage"])
    board_ref = _concat_parquet_refs(board_refs, outputs, "cut-board.parquet", OUTPUT_SCHEMAS["cut_board"])
    quality_ref = _concat_parquet_refs(quality_refs, outputs, "source-quality.parquet", OUTPUT_SCHEMAS["source_quality"])
    support_ref = _concat_parquet_refs(support_refs, outputs, "underlier-support.parquet", OUTPUT_SCHEMAS["underlier_support"])
    except_ref = _concat_parquet_refs(except_refs, outputs, "exceptions.parquet", OUTPUT_SCHEMAS["exceptions"])
    alias_ref = _concat_parquet_refs(alias_refs, outputs, "alias-conflict.parquet", OUTPUT_SCHEMAS["alias_conflict"])
    ident_ref = _concat_parquet_refs(ident_refs, outputs, "identity-match.parquet", OUTPUT_SCHEMAS["identity_match"])
    admit_ref = _concat_parquet_refs(admit_refs, outputs, "admission.parquet", OUTPUT_SCHEMAS["admission"])
    fred_ref = _concat_parquet_refs(fred_refs, outputs, "fred-support.parquet", OUTPUT_SCHEMAS["fred_support"])
    action_ref = _concat_parquet_refs(action_refs, outputs, "action-support.parquet", OUTPUT_SCHEMAS["action_support"])
    manifests = []
    for ref in manifest_refs:
        item = ref[0] if isinstance(ref, list) else ref
        payload = _load_result_json(item, load_reference)
        manifests.extend(payload.get("files") or payload.get("manifest") or [])
    ...
    aggregates_ref = outputs.json("date-aggregates.json", aggregate_rows,
    ...
    return {
        ...
        "joined_coverage_complete": all(result.get("joined_coverage_complete") for result in results),
        "family": FAMILY,
        "version": VERSION,
        "protocol_scientific_hash": sci,
        "implementation_hashes": impl,
...
__all__ = [
    ...
    "VERSION", "admitted_identity", "combine_partition_results", "contract",
    "coverage_schema", "implementation_hashes", "load_calendar", "quote_dte_bucket",
    "run", "scientific_hash", "utc_cut_ns",
]
