...
def measured_unit(unit, *, protocol, index, coordinates, outputs, data_root, storage_encoding='structural',
                  anchor_calendar=None, cohort_contract=None, structure_memory_contract=None,
                  book_contract=None):
    ...
    try:
        ...
        storage = {kind: series.finish() for kind, series in events.items()}
    ...
    structure_memory_check = None
    if structure_memory_contract is not None:
        from trading_research.research.auction_flow_structure_memory_preflight import check_structure_memory_unit
        structure_memory_check = check_structure_memory_unit(
            measured, unit, measurement_reference=measurement_ref,
            trade_storage=storage['trades'], quote_storage=storage['quotes'],
            excluded_storage=storage['excluded'], outputs=outputs, contract=structure_memory_contract)
    book_check = None
    if book_contract is not None:
        from trading_research.research.auction_flow_book_preflight import check_book_unit
        book_check = check_book_unit(measured, unit, measurement_reference=measurement_ref,
            quote_storage=storage['quotes'], outputs=outputs, contract=book_contract)
    result = {'unit': unit, 'measurement': measurement_ref, 'event_storage': storage,
        ...
        'actual_structure_memory_validation': structure_memory_check,
        'actual_book_reinitialization_validation': book_check,
...
def partition_parity(full, *, protocol, index, coordinates, outputs, data_root):
    unit = full['unit']
    ...
    for number, (a, b) in enumerate(((start, cut), (cut, end))):
        ...
        try:
            ...
            stores = {kind: series.finish() for kind, series in events.items()}
        except BaseException:
            for series in events.values():
                if not series.closed:
                    series.abort()
...
def run_measured_units(windows, *, protocol, index, coordinates, outputs, data_root, calendar,
                       unit_executor=None, cohort_contract=None, structure_memory_contract=None,
                       book_contract=None):
    ...
    if unit_executor is None:
        ...
        for unit in windows:
            result, reference = measured_unit(unit, protocol=protocol, index=index,
                ...
                structure_memory_contract=structure_memory_contract, book_contract=book_contract)
...
def run_preflight(*, protocol, root, outputs, load_reference, check_extension, unit_executor=None):
    ...
    results, refs = run_measured_units(windows, protocol=protocol, index=index, coordinates=coordinates,
        ...
        structure_memory_contract=check_extension.get('structure_memory_validation'),
        book_contract=check_extension.get('book_reinitialization_validation'))
    ...
    structure_memory = [result.get('actual_structure_memory_validation') for result in results]
    books = [result.get('actual_book_reinitialization_validation') for result in results]
    for declared, rows, label in (
            (check_extension.get('structure_memory_validation'), structure_memory, 'structure/memory'),
            (check_extension.get('book_reinitialization_validation'), books, 'reported BBO')):
        if declared is not None and (not rows or any(not row or row.get('passed') is not True for row in rows)):
            raise IntegrityError(f'declared full-unit {label} validation was skipped or failed')
    ...
    return {'version': VERSION, 'passed': True, 'resource_units': refs,
        ...
        'actual_structure_memory_validation_units': structure_memory,
        'actual_book_reinitialization_validation_units': books,
