...
from trading_research.research.auction_flow_measurements import (
    ...
    _add_validated_trade_arrays, _validated_trade_arrays,
)
...
class PreparedTradeBatch:
    ...
    def _prepare(self, table):
        ...
        values = {name: _owned_integer(table[name]) for name in _NUMERIC}
...
class TapeWindow:
    ...
    def add(self, table):
        ...
        try:
            # The source reader uses bounded batches; retain the larger
            # public input domain by preparing contiguous bounded slices.
            if not len(table):
                with prepare_trade_batch(table) as prepared:
                    self._ingest_prepared(prepared, 0, 0)
            else:
                for offset in range(0, len(table), _BATCH_ROWS):
                    with prepare_trade_batch(table.slice(offset, _BATCH_ROWS)) as prepared:
    ...
    def _ingest_prepared(self, prepared, left, right):
        ...
        validated = _validated_trade_arrays(size=size, side=side, event_ns=t, source_order=order)
        for flow in self.flows.values():
            _add_validated_trade_arrays(flow, validated)
