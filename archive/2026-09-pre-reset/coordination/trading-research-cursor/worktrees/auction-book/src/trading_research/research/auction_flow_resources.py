...
def source_cost_projection(units, protocol, schedule, *, excluded_storage_reserve_bytes_per_source_year=512 * 1024**2):
    ...
    event_storage_cpu_per_byte = max(u['parquet_and_roundtrip_cpu_seconds'] / max(1,
        sum(v['serialized_bytes'] for v in u['event_storage'].values())
        + sum(v['serialized_bytes'] for v in u['native_storage'])) for u in units)
    partitions = []
    ...
    return {'kind': 'measured_disjoint_source_workload_projection_v3', 'partitions': partitions,
        ...
        'failure_output_cpu_reserve_basis': 'maximum observed exact Parquet write/read/hash CPU per serialized byte; separate fixed failure-output allowance',
