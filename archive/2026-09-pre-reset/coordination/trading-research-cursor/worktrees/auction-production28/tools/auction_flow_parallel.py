...
def require_file_output_reservations(maximums, available_bytes, *, coordinator_reserve_bytes=0):
    """Per-file reservations plus coordinator reserve; do not use 512MiB times every day."""
    if type(coordinator_reserve_bytes) is not int or coordinator_reserve_bytes < 0:
        raise ValueError('coordinator output reserve is invalid')
    if not isinstance(maximums, (list, tuple)) or not maximums:
        raise ValueError('per-file output reservations required')
    if any(type(value) is not int or value <= 0 for value in maximums):
        raise ValueError('per-file output reservation is invalid')
    if type(available_bytes) is not int or available_bytes < 0:
        raise ValueError('available derived output is invalid')
    reserved = sum(maximums)
    if reserved + coordinator_reserve_bytes > available_bytes:
        raise ValueError('file reservations plus coordinator reserve exceed remaining derived-output allowance')
    return reserved
...
def _child_reservations(reservation_per_child, count):
    if type(reservation_per_child) is int:
        if reservation_per_child <= 0:
            raise ValueError('source child output bound is invalid')
        return [reservation_per_child] * count
    if not isinstance(reservation_per_child, (list, tuple)) or len(reservation_per_child) != count:
        raise ValueError('per-file output reservations must match child directories')
    if any(type(value) is not int or value <= 0 for value in reservation_per_child):
        raise ValueError('per-file output reservation is invalid')
    return list(reservation_per_child)
...
def charge_child_output_bytes(outputs, directories, *, reservation_per_child, evidence):
    ...
    reservations = _child_reservations(reservation_per_child, len(directories))
    child_bytes = join_directory_bytes(directories)
    overruns = []
    for directory, reserved in zip(directories, reservations):
...
def source_file_namespace(attempt_id, ordinal, *, runs_root):
    if type(attempt_id) is not str or not attempt_id or '/' in attempt_id or attempt_id in ('.', '..'):
        raise ValueError('source file attempt identity is invalid')
    if type(ordinal) is not int or ordinal < 0:
        raise ValueError('source file ordinal is invalid')
    directory = Path(runs_root) / attempt_id / 'outputs' / f'source-file-{ordinal:03d}'
    return directory.resolve(), (directory / 'packet.json').resolve()


def require_source_file_namespace(*, attempt_id, ordinal, child_packet_path, output_directory, runs_root):
    expected_dir, expected_packet = source_file_namespace(attempt_id, ordinal, runs_root=runs_root)
    if Path(child_packet_path).resolve() != expected_packet:
        raise ValueError('source file packet is outside the registered attempt namespace')
    if Path(output_directory).resolve() != expected_dir:
        raise ValueError('source file output directory is outside the registered attempt namespace')
...
def supervise_source_children(jobs, *, concurrency, cpu_soft_seconds, cpu_hard_seconds,
                              ...
                              evidence=None, allowed_failure_exit_codes=()):
    ...
    try:
        while pending or live:
            ...
            if any(not row['success'] and row['exit_code'] not in allowed_failure_exit_codes for row in finished):
                stop_live()
                reap_live()
                publish()
    ...
    if any(not completed[index]['success'] and completed[index]['exit_code'] not in allowed_failure_exit_codes for index in range(len(jobs))):
        publish()
