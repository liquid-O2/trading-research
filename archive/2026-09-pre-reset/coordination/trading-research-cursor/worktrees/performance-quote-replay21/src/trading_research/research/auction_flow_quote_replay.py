...
def _column_digests(table):
    return {field.name: value_digest(table.select([field.name])) for field in table.schema}
...
def _iter_quote_replay(descriptor, *, check_implementation=True):
        ...
        for original in batch.quotes:
            ...
            while offset < len(table):
                ...
                if (value_digest(part) != expected.get('sha256')
                        or _column_digests(part) != expected.get('column_sha256')
                        or set(expected.get('column_sha256', {})) != set(part.schema.names)):
...
def _verify_quote_replay_descriptor(descriptor):
...
class QuoteReplaySeries:
    ...
    def append(self, table, source=None):
        ...
        try:
            ...
            while offset < len(table):
                ...
                self.chunks.append({'rows': amount, 'sha256': digest_sha, 'column_sha256': _column_digests(part)})
                self.rows += amount
                self.groups += 1
                offset += amount
    ...
    def finish(self, source_manifest):
        ...
        try:
            ...
            compared = _verify_quote_replay_descriptor(stored)
