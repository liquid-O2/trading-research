...
from trading_research.data.compact import CompactProjector, arrow_bytes
...
class AuctionFlowStream:
    ...
    def _enrich(self, table, raw, source_order_base, source_key):
        ...
        table = table.append_column("source_key", pa.array([source_key] * len(table)))
    ...
    def _project(self, original, *, source, physical_rows):
        ...
        raw = raw.append_column("source_order", pa.array(range(before, before + len(raw)), type=pa.int64()))
        ...
        for q in quotes:
            ...
            eligible = pa.array([True] * len(q)) if invalid_from is None else pc.less(q["source_order"], invalid_from)
        ...
        excluded = excluded.append_column("source_key", pa.array([key] * len(excluded)))
                ...
                with pq.ParquetFile(path) as pf:
                    ...
                    while True:
                        ...
                        while remaining:
                            ...
                            address_parts.append(pa.array(range(first, first + count), type=pa.int64()))
    ...
    def manifest(self):
        ...
        return {"version": VERSION, "dataset": self.dataset, "start_ns": self.start,
