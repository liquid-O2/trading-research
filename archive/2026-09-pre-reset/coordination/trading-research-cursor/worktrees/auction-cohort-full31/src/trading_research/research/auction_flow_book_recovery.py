...
def _bound_address_shape(address):
    if type(address) is not dict or set(address) != set(_ADDRESS_FIELDS):
        raise IntegrityError(_CARRY)
    key, action = address["source_key"], address["raw_action"]
    if (type(key) is not str or not 0 < len(key) <= MAX_SOURCE_KEY
            or (action is not None and (type(action) is not str or len(action) > MAX_ACTION))):
        raise IntegrityError(_CARRY)
    for name in ("source_row", "source_order", "t", "known_at", "raw_flags"):
        value = address[name]
        maximum = 255 if name == "raw_flags" else 2**64 - 1 if name.startswith("source_") else 2**63 - 1
...
def _bound_carry(carry, *, maximum_episodes):
    ...
    if (type(episodes) not in (list, tuple) or len(episodes) > min(maximum_episodes, MAX_EPISODES)
        ...
        raise IntegrityError(_CARRY)
    # Validate every primitive before encoding caller-supplied state. Shape
    # checks after json conversion would already have traversed an arbitrary
    # nested object in, for example, a count or retained address.
    for name in ("version", "policy", "policy_version"):
        if type(carry[name]) is not str or len(carry[name]) > 256:
            raise IntegrityError(_CARRY)
    for name in ("completed", "blocked", "observed_history_complete", "cumulative_coverage_complete"):
        if type(carry[name]) is not bool:
            raise IntegrityError(_CARRY)
    for name in (*_COUNT_NAMES, "instrument_id", "latency_ns", "event_start_ns", "event_end_ns",
                 "next_start_ns", "prefix_start_ns", "maximum_rows", "maximum_episodes"):
        if type(carry[name]) is not int or not 0 <= carry[name] <= 2**63 - 1:
            raise IntegrityError(_CARRY)
    for name in ("last_t", "last_source_order", "last_source_row"):
        value = carry[name]
        if value is not None and (type(value) is not int or not 0 <= value <= 2**64 - 1):
            raise IntegrityError(_CARRY)
    for name in ("last_source_address", "first_unresolved_invalidation"):
        if carry[name] is not None:
            _bound_address_shape(carry[name])
    weight = 0
    for item in episodes:
        if (type(item) is not dict or set(item) != set(_EPISODE_FIELDS)
                or type(item["episode_index"]) is not int
                or not 1 <= item["episode_index"] <= maximum_episodes
                or type(item["unresolved"]) is not bool
                or type(item["provider_flagged_records"]) is not int
                or not 0 <= item["provider_flagged_records"] <= MAX_ROWS):
            raise IntegrityError(_CARRY)
        for name in ("invalidation", "recovery"):
            if item[name] is not None:
                _bound_address_shape(item[name])
        weight += len(canonical_json(item))
...
def _state_changes(inv_idx, rec_idx, start_blocked):
    ...
    while True:
        if blocked:
            r = int(np.searchsorted(rec_idx, last, side="right"))
            if r >= rec_idx.size:
                break
            last = int(rec_idx[r])
            ...
            i = int(np.searchsorted(inv_idx, last, side="right"))
            if i >= inv_idx.size:
                break
            last = int(inv_idx[i])
...
class ReportedBBOReinitializer:
    ...
    def _restore(self, carry):
        ...
        if (any(type(counts[name]) is not int or counts[name] < 0 for name in _COUNT_NAMES)
            ...
            raise IntegrityError(_CARRY)
        # Hamming distance between the original and candidate validity masks
        # must agree with their two one-counts.
        removed_twice = (counts["original_valid_rows"] + counts["changed_book_valid_rows"]
                         - counts["candidate_valid_rows"])
        if (removed_twice < 0 or removed_twice % 2
                or removed_twice // 2 > counts["original_valid_rows"]
                or counts["changed_book_valid_rows"] - removed_twice // 2
                   > counts["rows"] - counts["original_valid_rows"]
                or counts["clear_rows"] + counts["unknown_action_rows"] > counts["rows"]):
            raise IntegrityError(_CARRY)
        ...
        if any(item["unresolved"] for item in parsed[:-1]):
            raise IntegrityError(_CARRY)
        if (len(parsed) + sum(not item["unresolved"] for item in parsed) > counts["rows"]
                or sum(item["invalidation"]["raw_action"] == "R" for item in parsed) > counts["clear_rows"]
                or sum(item["invalidation"]["raw_action"] not in _KNOWN for item in parsed)
                   > counts["unknown_action_rows"]):
            raise IntegrityError(_CARRY)
    ...
    def _report(self, coverage_complete):
        ...
        return {
            ...
            "empty_observed_window": self._interval_rows == 0,
            "current_interval_rows": self._interval_rows,
            "counter_scope": "cumulative from prefix_start_ns except current_interval_rows",
