...
def side_geometry_rows(cells, *, grid, history_complete, pseudo_mass=Fraction(0)):
    ...
    return {"rows": tuple(rows), "cdf_buy": cdf(pb), "cdf_sell": cdf(ps),
        ...
        "maximum_rows": tuple(r.row for r in cells if r.delta == max(deltas)),
        "minimum_rows": tuple(r.row for r in cells if r.delta == min(deltas)),
        "absolute_peak_rows": tuple(r.row for r in cells if abs(r.delta) == max(map(abs, deltas))),
