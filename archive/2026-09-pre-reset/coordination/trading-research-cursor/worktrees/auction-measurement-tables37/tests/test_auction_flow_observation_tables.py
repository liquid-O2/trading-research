...
def volume6_atom(*, instrument_id=7, number=0, start=0, end=100, latency=250, coverage_complete=True,
    ...
    vwap = Fraction(1230433, 95) if fractions == 'objects' else {'$fraction': [1230433, 95]}
    variance = Fraction(7836, 9025) if fractions == 'objects' else {'$fraction': [7836, 9025]}
    q = Fraction(10) if fractions == 'objects' else {'$fraction': [10, 1]}
    ...
    weighted = {
        'priced_volume': 6, 'sum_price_volume': 60, 'sum_price_squared_volume': 604,
    ...
    trade = tape(
        ...
        size_quantiles={'1/4': q, '1/2': q, '3/4': q, '19/20': q},
...
class AuctionFlowObservationTableTests(unittest.TestCase):
    ...
    def test_true_ohlc_interior_reversal_is_not_close(self):
        all_flow = flow('all', buy=5, sell=4, unknown=0, prints=3, excluded_prints=0, excluded_volume=0,
        ...
        trade = tape(instrument_id=7, start=0, end=100, known=350, flows=flows, prints=3,
                     ...
                     weighted={'priced_volume': 9, 'sum_price_volume': 3591, 'sum_price_squared_volume': 1432809,
                               'vwap_ticks': Fraction(399), 'variance_ticks_squared': Fraction(0),
                               'weighted_quantile_ticks': {'1/20': 398, '1/4': 399, '1/2': 399, '3/4': 400, '19/20': 400}},
                     profile={'coverage_complete': True, 'rows': [[398, 0, 4, 0], [400, 5, 0, 0]],
                              'unpriced_buy_sell_unknown': [0, 0, 0], 'total_volume': 9},
                     count_intensity=Fraction(3 * 10**9, 100), volume_intensity=Fraction(9 * 10**9, 100))
    ...
    def test_priced_moments_and_multiple_poc_ties(self):
        ...
        self.assertEqual(row['print_size_quantile25'], 10.0)
    ...
    def test_fraction_tags_and_native_fractions_are_accepted(self):
        ...
        receipt, measured, reference = receipt_and_measured(
            instruments=[one_instrument(atoms=[item])], start=0, end=100, width=100)
        row = observation_rows(receipt, measured, receipt_reference=reference)[0]
        self.assertEqual(row['vwap_ticks'], float(Fraction(1230433, 95)))
        self.assertEqual(row['variance_ticks_squared'], float(Fraction(7836, 9025)))
