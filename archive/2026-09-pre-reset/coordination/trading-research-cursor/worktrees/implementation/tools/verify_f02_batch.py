...
def supervise():
    ...
    registration = json.loads((ROOT / 'reports/f02-kernel-registration.json').read_text())
    ...
    for relative, ref in (('validation/F02_KERNEL_PROTOCOL.md', protocol_ref),
                          ('tests/golden/f02-kernels.json', golden_ref)):
    ...
    review = registry.artifacts.put_bytes((ROOT / 'validation/F02_KERNEL_BATCH_REVIEW.md').read_bytes(), kind='implementation_review')
    ...
    configuration = {'protocol': asdict(protocol_ref), 'golden': asdict(golden_ref),
                     'review': asdict(review), 'supervisor': asdict(harness), 'suite': 'all',
                     'cpu_reservation_seconds': 180, 'cpu_hard_seconds': 190,
                     'address_space_bytes': 4 * 1024 ** 3, 'wall_limit_seconds': 240}
    trial = registry.register(name='F02 fixed kernels and existing combined regression suite',
        family='F02-registry-payoff-reference-v1', stage='engineering', configuration=configuration,
        code_hash=digest(manifest), data_hashes={'independent_golden': golden_ref.sha256},
        fold_version='no-market-fit', target_version='fixed-equations-and-causal-identity-v1')
    ...
    result_refs = []
    try:
        with tempfile.TemporaryDirectory(prefix='f02-verification-') as temporary:
    ...
    report = {'success': status == 'succeeded', 'trial_id': trial, 'attempt_id': attempt,
              ...
              'limitations': ['Fixed synthetic assertions do not admit a dated historical contract cohort.',
                              'No option fair-value, profitability, consumer P5 or future P7 claim.']}
    ...
    summary = {'success': report['success'], 'trial_id': trial, 'attempt_id': attempt, 'artifact': asdict(ref),
               ...
               'cpu_seconds': cpu, 'wall_seconds': wall, 'peak_rss_bytes': peak_rss}
    (ROOT / 'reports' / f'f02-kernel-verification-{attempt[:12]}.json').write_text(json.dumps(summary, indent=2) + '\n')
