...
from verify_foundation_extensions import ROOT, SPECS, atomic_json, validate_registrations

LIST_FIELDS = ('reference_and_code_artifacts', 'verification_artifacts', 'review_artifacts', 'assertion_ids')
REASON = 'Attach exact F05–F08 partial engineering evidence from one physical supervised execution.'
...
def main():
    ...
    for path, ref in ((Path(__file__), preflight['recorder']),
                      (ROOT / 'tools/verify_foundation_extensions.py', configuration['supervisor'])):
    ...
    for prefix, family, case_key, approved_modules in SPECS:
        registration = by_family[family]
        source_ref = artifact_ref(registration['source_cases'])
        original_cases = ledger.artifacts.read_json(source_ref)['preparation'][case_key]
        ...
        if (len(original_by_id) != len(original_cases) or len(case_ids) != len(set(case_ids))
                or set(case_ids) != set(original_by_id) or sorted(coverage['test_modules']) != sorted(approved_modules)
                or mapping['approved_modules'] != list(approved_modules)):
        ...
        for case in coverage['cases']:
            ...
            for method in case['test_methods']:
                matches = [a for a in family_assertions if a.rsplit('.', 1)[-1] == method]
                if len(matches) != 1:
                    raise ValueError('mapped assertion was ambiguous or did not execute')
                assertions.append(matches[0])
            cases.append({'case_id': case['case_id'], 'source_case_as_prepared': original_by_id[case['case_id']],
                          'partial_assertion_ids': sorted(set(assertions)), 'remaining': case['remaining'],
    ...
    attachment_key = 'foundation-extensions:' + supervised['shared_execution_id']
