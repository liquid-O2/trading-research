...
def produce(root, index, start, end, continuation=None, source_paths=None, source_clock_policy=None):
    kwargs = {}
    if continuation is not None:
        kwargs['continuation'] = continuation
    if source_paths is not None:
        kwargs['source_paths'] = source_paths
    if source_clock_policy is not None:
        kwargs['source_clock_policy'] = source_clock_policy
...
class AuctionFlowQuoteReplayTests(unittest.TestCase):
    def test_compatibility_pins_both_implementations_and_preserves_rejection(self):
        import trading_research.research.auction_flow_quote_replay as replay
        saved = replay._verified_implementation_compatibility
        current = replay.implementation_identity(); previous = copy.deepcopy(current)
        previous['files']['auction_flow_data.py']['sha256'] = 'a'*64
        try:
            replay.configure_verified_compatibility(previous=previous, current=current)
            replay._require_implementation(previous)
            changed = copy.deepcopy(previous); changed['files']['auction_flow_data.py']['sha256'] = 'b'*64
            with self.assertRaises(IntegrityError):
                replay._require_implementation(changed)
            target = copy.deepcopy(current); target['python'] = 'wrong'
            with self.assertRaises(IntegrityError):
                replay.configure_verified_compatibility(previous=previous, current=target)
        finally:
            replay._verified_implementation_compatibility = saved
    ...
    def test_tamper_and_raw_trade_size_fail_before_publication_or_at_exhausted_read(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            tamper_number = 0
            def reject(mutator):
                nonlocal tamper_number
                tamper_number += 1
                changed = copy.deepcopy(descriptor)
                mutator(changed)
                reference = outputs.json(f'tampered-{tamper_number}.json', changed,
                                         kind='auction_flow_quote_replay_descriptor')
                series_copy = {**published, 'replay_descriptor': reference, 'serialized_bytes': reference['size_bytes']}
                with self.assertRaises(IntegrityError):
                    list(read_series_tables(series_copy))

            for key, value in [('schema', 'tampered'), ('series', 'other'), ('version', 'other'), ('serialized_bytes', 0), ('compared_values', 0)]:
                with self.subTest(outer_field=key), self.assertRaises(IntegrityError):
                    list(read_series_tables({**published, key: value}))
            ...
            yielded = []
            with self.assertRaises(IntegrityError):
                for item in read_series_tables({**published, 'replay_descriptor': raw_ref, 'serialized_bytes': raw_ref['size_bytes']}):
                    yielded.append(item)
    ...
    def test_resource_projection_keeps_physical_rates_and_forecasts_replay_by_window(self):
        def unit(*, quote_encoding='structural', quote_bytes=8000, quote_rows=40, replay_cpu=0.0,
            ...
            if quote_encoding == ENCODING:
                ...
                quotes['quote_replay_metadata_cpu_seconds'] = 0.2 if reader_batches else replay_cpu
            return {
        ...
        self.assertEqual(replay['cpu_coefficients']['quote_replay_validation']['cpu_seconds_per_work_unit'], 0.45)
        self.assertEqual(replay['partitions'][0]['disjoint_source_cpu_components']['quote_replay_validation'], 3.6)
        self.assertEqual(replay['cpu_coefficients']['quote_replay_metadata_and_dispatch']['cpu_seconds_per_work_unit'], 0.2)
        ...
        self.assertEqual(empty['cpu_coefficients']['quote_replay_metadata_and_dispatch']['cpu_seconds_per_work_unit'], 0.25)
        self.assertEqual(empty['partitions'][0]['disjoint_source_cpu_components']['quote_replay_metadata_and_dispatch'], 0.5)
    ...
    def test_quote_descriptor_retains_source_clock_policy_and_rejects_mismatch(self):
        rows = [raw(100), raw(90, action='A', side='N', flags=168), raw(110)]
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            index = AuctionFlowDataTests().write(root, rows)
            path = index['datasets'][DATASET][0]['path']
            quotes, manifest, _ = produce(
                root, index, 0, 200, source_paths=(path,),
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            self.assertEqual([r['t'] for table in quotes for r in table.to_pylist()], [100, 110])
            outputs = BoundedOutputs(root / 'out', maximum_total_bytes=4 * 1024**2, maximum_file_bytes=2 * 1024**2)
            index_ref = outputs.json('source-index.json', index, kind='four_dataset_full_footer_index')
            series = QuoteReplaySeries(
                outputs, 'quotes', data_root=root, source_index_reference=index_ref, root='NQ',
                start_ns=0, end_ns=200, source_paths=(path,), maximum_scan_rows=100, latency_ns=250,
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            for table in quotes:
                series.append(table)
            published = series.finish(manifest)
            descriptor = read_json_artifact(published['replay_descriptor'])
            self.assertEqual(descriptor['source_clock_policy'], SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            self.assertEqual(_verify_quote_replay_descriptor(descriptor), published['compared_values'])
            changed = copy.deepcopy(descriptor)
            del changed['source_clock_policy']
            reference = outputs.json('strict-reinterpret.json', changed, kind='auction_flow_quote_replay_descriptor')
            with self.assertRaises(IntegrityError):
                list(read_series_tables({**published, 'replay_descriptor': reference,
                                         'serialized_bytes': reference['size_bytes']}))

    def test_strict_default_quote_descriptor_omits_source_clock_policy(self):
        rows = [raw(2), raw(4), raw(6)]
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            index = AuctionFlowDataTests().write(root, rows)
            ...
            published = series.finish(manifest)
            descriptor = read_json_artifact(published['replay_descriptor'])
            self.assertNotIn('source_clock_policy', descriptor)
            self.assertEqual(_verify_quote_replay_descriptor(descriptor), published['compared_values'])
