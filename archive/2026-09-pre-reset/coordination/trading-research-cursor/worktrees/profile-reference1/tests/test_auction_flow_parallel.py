...
class AuctionFlowParallelTests(unittest.TestCase):
    def test_utilization_allocation_admits_seventeen_with_real_quota_and_ram(self):
        allocation = hardware_allocation()
        allocation['kind'] = 'auction_flow_user_hardware_execution_v2'
        allocation['execution']['maximum_workers'] = 17
        allocation['execution']['aggregate_memory_bytes'] = 72 * 1024 ** 3
        live = {'cpu_quota_us': 1785000, 'cpu_period_us': 100000,
                'memory_limit_bytes': 82999996416}
        planned = PARALLEL.plan_source_unit_execution(
            protocol_fixture(), allocation, live_limits=live, protocol_sha256=PROTOCOL_SHA)
        self.assertEqual(planned['maximum_concurrency'], 17)
        self.assertEqual(planned['allocated_address_space_bytes'], 18 * 4 * 1024 ** 3)
        self.assertLess(planned['allocated_address_space_bytes'], live['memory_limit_bytes'])
        actual = PARALLEL.actual_source_unit_allocation(planned, 20)
        self.assertEqual(actual['actual_concurrency'], 17)
        tighter = dict(live, memory_limit_bytes=64 * 1024 ** 3)
        reduced = PARALLEL.plan_source_unit_execution(
            protocol_fixture(), allocation, live_limits=tighter, protocol_sha256=PROTOCOL_SHA)
        self.assertEqual(reduced['maximum_concurrency'], 15)
    ...
    def test_remaining_coordinator_cpu_enforcement_in_subprocess(self):
        script = (
            ...
            'soft, hard = apply_remaining_coordinator_cpu_limit(300, 310, children_cpu_seconds=100)\n'
            'assert (soft, hard) == (200, 210), (soft, hard)\n'
            'assert resource.getrlimit(resource.RLIMIT_CPU) == (200, 210)\n'
            'soft, hard = apply_remaining_coordinator_cpu_limit(300, 310, children_cpu_seconds=50)\n'
            'assert (soft, hard) == (200, 210), (soft, hard)\n'
            'try:\n'
            '    apply_remaining_coordinator_cpu_limit(300, 310, children_cpu_seconds=300)\n'
            'except ValueError:\n'
            '    assert resource.getrlimit(resource.RLIMIT_CPU) == (200, 210)\n'
            'else:\n'
            '    raise SystemExit(2)\n'
...
"""
    ...
    def test_per_file_reservations_and_partial_failure_accounting(self):
        reserved = PARALLEL.require_file_output_reservations(
            (10 * 1024 ** 2, 20 * 1024 ** 2), 64 * 1024 ** 2, coordinator_reserve_bytes=1024 ** 2)
        self.assertEqual(reserved, 30 * 1024 ** 2)
        with self.assertRaises(ValueError):
            PARALLEL.require_file_output_reservations(
                (10 * 1024 ** 2, 20 * 1024 ** 2), 20 * 1024 ** 2, coordinator_reserve_bytes=1024 ** 2)
        with self.assertRaises(ValueError):
            PARALLEL.require_output_reservation(3589, 536870912, 16 * 1024 ** 3)
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            small = root / 'source-file-000'
            large = root / 'source-file-001'
            small.mkdir()
            large.mkdir()
            (small / 'a.bin').write_bytes(b'12345')
            (large / 'b.bin').write_bytes(b'abcdef')
            outputs = BoundedOutputs(root / 'out', maximum_total_bytes=64, maximum_file_bytes=32)
            evidence = {}
            joined = PARALLEL.charge_child_output_bytes(
                outputs, (small, large), reservation_per_child=(8, 16), evidence=evidence)
            self.assertEqual(joined, 11)
            self.assertEqual(evidence['joined_output_bytes'], 11)
            self.assertEqual(evidence['child_directory_overruns'], [])
            over = BoundedOutputs(root / 'out-over', maximum_total_bytes=64, maximum_file_bytes=32)
            overrun = {}
            with self.assertRaises(ValueError):
                PARALLEL.charge_child_output_bytes(
                    over, (small, large), reservation_per_child=(4, 4), evidence=overrun)
            self.assertEqual(overrun['joined_output_bytes'], 11)
            self.assertTrue(over.failed)
            jobs = [fixture_job(0, root / 'ok', delay=0.0),
                    fixture_job(1, root / 'fail', delay=0.0, code=2)]
            continued = {}
            report = PARALLEL.supervise_source_children(
                jobs, concurrency=2, cpu_soft_seconds=1200, cpu_hard_seconds=1210,
                wall_deadline_monotonic=time.monotonic() + 10,
                allocated_address_space_bytes=16 * 1024 ** 3, poll_seconds=0.05,
                evidence=continued, allowed_failure_exit_codes=(2,))
            self.assertEqual([row['ordinal'] for row in report['children']], [0, 1])
            self.assertTrue(report['children'][0]['success'])
            self.assertFalse(report['children'][1]['success'])
            self.assertEqual(report['children'][1]['exit_code'], 2)
            expected_dir, expected_packet = PARALLEL.source_file_namespace(
                'attempt-a', 0, runs_root=root / 'runs')
            self.assertTrue(str(expected_dir).endswith('source-file-000'))
            expected_dir.mkdir(parents=True)
            PARALLEL.require_source_file_namespace(
                attempt_id='attempt-a', ordinal=0,
                child_packet_path=expected_packet, output_directory=expected_dir,
                runs_root=root / 'runs')
            other = root / 'runs' / 'attempt-b' / 'outputs' / 'source-file-000'
            other.mkdir(parents=True)
            with self.assertRaises(ValueError):
                PARALLEL.require_source_file_namespace(
                    attempt_id='attempt-a', ordinal=0,
                    child_packet_path=other / 'packet.json', output_directory=other,
                    runs_root=root / 'runs')

    def test_bootstrap_keeps_inherited_hard_cpu_and_small_preauth_soft(self):
        tight = (
            'import resource, sys\n'
            f'sys.path.insert(0, {str(ROOT / "tools")!r})\n'
            'import run_auction_flow_study as runner\n'
            'resource.setrlimit(resource.RLIMIT_CPU, (300, 310))\n'
            'runner.bootstrap_process_limits()\n'
            'assert resource.getrlimit(resource.RLIMIT_CPU) == (300, 310)\n'
        )
        opened = (
            'import resource, sys\n'
            f'sys.path.insert(0, {str(ROOT / "tools")!r})\n'
            ...
            'runner.bootstrap_process_limits()\n'
            'soft, hard = resource.getrlimit(resource.RLIMIT_CPU)\n'
            'assert soft == runner.BOOTSTRAP_SOFT_CPU_SECONDS\n'
            'assert hard == runner.FAMILY_HARD_CPU_SECONDS\n'
            'assert soft < hard\n'
        )
        before = resource.getrlimit(resource.RLIMIT_CPU)
        for script in (tight, opened):
            result = subprocess.run([sys.executable, '-c', script], cwd=str(ROOT / 'tools'),
                                    capture_output=True, text=True)
            self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(resource.getrlimit(resource.RLIMIT_CPU), before)


class SourceFileFailurePublicationTests(unittest.TestCase):
    def test_aborted_writer_keeps_partial_bytes_and_original_error_receipt(self):
        spec = importlib.util.spec_from_file_location('source_failure_runner', ROOT / 'tools/run_auction_flow_study.py')
        runner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runner)
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / 'packet.json').write_bytes(b'original packet')
            outputs = BoundedOutputs(root / 'artifacts', maximum_total_bytes=65536, maximum_file_bytes=32768)
            try:
                with outputs.create('partial.bin') as stream:
                    stream.write(b'exact failed output')
                    raise ValueError('original source-clock failure')
            except ValueError:
                pass
            from trading_research.research.auction_flow_storage import ParquetSeries
            ParquetSeries(outputs, 'aborted-series').abort()
            self.assertTrue(outputs.failed)
            result = {'success': False, 'completed_windows': 2,
                      'pending_units': [{'source_path': 'a.parquet', 'event_start_ns': 30}],
                      'failures': [{'error': 'IntegrityError: original source-clock failure'}]}
            child = {'output_directory': folder, 'maximum_output_bytes': 65536,
                     'ordinal': 0, 'source_path': 'a.parquet'}
            packet = {'attempt_id': 'a', 'limits': {'maximum_output_file_bytes': 32768}}
            reference = runner.publish_source_file_result(outputs=outputs, packet=packet, child=child, result=result)
            self.assertEqual((root / 'artifacts/partial.bin').read_bytes(), b'exact failed output')
            self.assertTrue(outputs.failed)
            self.assertEqual(json.loads(Path(reference['path']).read_text()), result)
            self.assertEqual(reference['sha256'], hashlib.sha256(Path(reference['path']).read_bytes()).hexdigest())
            self.assertLessEqual(sum(p.stat().st_size for p in root.rglob('*') if p.is_file()), 65536)

    def test_failure_receipt_cannot_exceed_original_remaining_reservation(self):
        spec = importlib.util.spec_from_file_location('source_failure_runner_limits', ROOT / 'tools/run_auction_flow_study.py')
        runner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runner)
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / 'packet.json').write_bytes(b'x' * 100)
            outputs = BoundedOutputs(root / 'artifacts', maximum_total_bytes=100, maximum_file_bytes=100)
            outputs.failed = True
            child = {'output_directory': folder, 'maximum_output_bytes': 101,
                     'ordinal': 0, 'source_path': 'a.parquet'}
            packet = {'attempt_id': 'a', 'limits': {'maximum_output_file_bytes': 32768}}
            with self.assertRaisesRegex(ValueError, 'remaining original file reservation'):
                runner.publish_source_file_result(outputs=outputs, packet=packet, child=child,
                                                 result={'success': False, 'failures': [{'error': 'original'}]})
            self.assertFalse((root / 'failure').exists())


class OriginalClockFailureDiagnosticTests(unittest.TestCase):
    def test_diagnostic_skips_admitted_snapshot_and_retains_first_unresolved_original_pair(self):
        import pyarrow as pa
        import pyarrow.parquet as pq
        spec = importlib.util.spec_from_file_location('source_diagnostic_runner', ROOT / 'tools/run_auction_flow_study.py')
        runner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runner)
        base = {'action': 'C', 'side': 'N', 'price': 100.0, 'size': 0,
            'bid_px': 100.0, 'ask_px': 100.25, 'bid_sz': 3, 'ask_sz': 4, 'instrument_id': 7, 'flags': 128}
        rows = [{**base, 't': 100}, {**base, 't': 90, 'action': 'A', 'flags': 168},
                {**base, 't': 80, 'action': 'A', 'flags': 168}, {**base, 't': 101}]
        with tempfile.TemporaryDirectory() as folder:
            pq.write_table(pa.Table.from_pylist(rows), Path(folder) / 'raw.parquet')
            unit = {'source_path': 'raw.parquet', 'source_groups': [0],
                'event_start_ns': 0, 'event_end_ns': 200, 'physical_scan_rows': 4}
            result = runner.diagnose_unresolved_source_clock(unit, data_root=folder)
            issue = result['first_unresolved_issue']
            self.assertEqual(result['earlier_admitted_redundant_snapshots'], 1)
            self.assertEqual((issue['source_row'], issue['previous_source_row']), (2, 1))
            self.assertEqual(issue['record'], rows[2])
            self.assertEqual(issue['physical_predecessor'], rows[1])
            self.assertFalse(result['source_values_modified'])
            self.assertFalse(result['source_policy_changed'])


class SourceFileDispatchTests(unittest.TestCase):
    def test_declared_heavy_first_preserves_original_jobs_and_ties(self):
        spec = importlib.util.spec_from_file_location('source_dispatch_runner', ROOT / 'tools/run_auction_flow_study.py')
        runner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runner)
        chains = [{'root': 'NQ', 'source_path': name, 'source_metadata_sha256': name * 64}
                  for name in ('a', 'b', 'c')]
        jobs = [{'ordinal': i, 'output_directory': f'original-{i}', 'argv': [str(i)]} for i in range(3)]
        plan = {'dispatch_strategy': 'largest_declared_file_first_v1',
                'files': [{**chain, 'dispatch_work_weight': weight}
                          for chain, weight in zip(chains, (1, 1, 10), strict=True)]}
        ordered = runner.order_source_file_jobs(jobs, chains, plan)
        self.assertEqual([j['ordinal'] for j in ordered], [2, 0, 1])
        for j in ordered:
            self.assertIs(j, jobs[j['ordinal']])
            self.assertEqual(j['output_directory'], f"original-{j['ordinal']}")
        self.assertEqual([j['ordinal'] for j in jobs], [0, 1, 2])
        legacy = runner.order_source_file_jobs(jobs, chains, {})
        self.assertEqual(legacy, jobs)
        for bad in (None, float('nan'), -1, True):
            plan['files'][0]['dispatch_work_weight'] = bad
            with self.assertRaisesRegex(ValueError, 'finite nonnegative'):
                runner.order_source_file_jobs(jobs, chains, plan)
