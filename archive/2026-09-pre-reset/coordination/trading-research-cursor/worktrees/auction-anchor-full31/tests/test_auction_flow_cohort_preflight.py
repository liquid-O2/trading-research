...
from trading_research.research import auction_flow_cohort_preflight as cohort_preflight
from trading_research.research.auction_flow_cohort_preflight import (
    ...
    VERSION, check_cohort_unit, _IntervalIndex, _add_slices,
)
...
def _literal_slices(batches, start, end):
    found = []
    for batch in batches:
        ...
        left = 0
        n = len(batch)
        while left < n and int(stamps[left]) < start:
            left += 1
        right = left
        while right < n and int(stamps[right]) < end:
            right += 1
        if left < right:
...
def _slice_field(slices, name):
    values = []
    for batch, left, right in slices:
        values.extend(int(item) for item in batch.values[name][left:right])
    return values
...
class _RecordWindow:
    ...
    def add_prepared(self, prepared, left, right):
        self.added.append((prepared, left, right))


class AuctionFlowCohortSliceIndexTests(unittest.TestCase):
    def _batches(self, stamp_groups, *, instrument_id=NQ_ID, source_key='src', start_order=0):
        batches = []
        order = start_order
        for stamps in stamp_groups:
            rows = [row(at, source_order=order + offset, instrument_id=instrument_id,
                        source_key=source_key) for offset, at in enumerate(stamps)]
            order += len(stamps)
            batch = prepare_trade_batch(make_trade_table(rows))
    ...
    def _watch_bounds(self):
        calls = []
        original = cohort_preflight._bounds

        def wrapped(prepared, start_ns, end_ns):
            calls.append((prepared, start_ns, end_ns))
            return original(prepared, start_ns, end_ns)

        cohort_preflight._bounds = wrapped
        self.addCleanup(lambda: setattr(cohort_preflight, '_bounds', original))
        return calls

    def test_literal_half_open_split_ties_boundaries_gaps_and_empty(self):
        ...
        for start, end in cases:
            got = index.slices(start, end)
            expected = _literal_slices(batches, start, end)
            self.assertEqual(got, expected)
            times = _slice_field(got, 't')
            self.assertEqual(times, [stamp for stamp in times if start <= stamp < end])
            self.assertEqual(times, sorted(times))
            orders = _slice_field(got, 'source_order')
            self.assertEqual(orders, sorted(orders))
            self.assertEqual(len(orders), len(set(orders)))
        self.assertEqual(_slice_field(index.slices(200, 201), 't'), [200, 200, 200, 200])
        self.assertEqual(index.slices(200, 200), ())
        self.assertEqual(index.slices(201, 200), ())
        self.assertEqual(_slice_field(index.slices(100, 200), 't'), [100])
        self.assertEqual(_slice_field(index.slices(200, 300), 't'), [200, 200, 200, 200])
        self.assertEqual(_slice_field(index.slices(200, 301), 't'), [200, 200, 200, 200, 300])
        self.assertEqual(index.slices(301, 500), ())
        empty = _IntervalIndex([])
        self.assertEqual(empty.slices(0, 1000), ())
        self.assertEqual(empty.slices(10, 10), ())
        self.assertEqual(empty.slices(20, 5), ())

    def test_clipped_minute_whole_then_minutes_then_earlier_prefix(self):
        available = START + MINUTE + DELAY
        batches = self._batches((
            (START + 1, START + 2, START + MINUTE - 5),
            (START + MINUTE + 10, available + 5),
            (START + 2 * MINUTE + 3,),
        ))
        index = _IntervalIndex(batches)
        whole = (START, END)
        ...
        clipped = (available, START + 2 * MINUTE)
        prefix = (START, START + 2)
        ordered = (whole, *minutes, clipped, prefix)
        for start, end in ordered:
            self.assertEqual(index.slices(start, end), _literal_slices(batches, start, end))
        self.assertEqual(_slice_field(index.slices(*clipped), 't'), [available + 5])
        self.assertEqual(_slice_field(index.slices(*prefix), 't'), [START + 1])
        self.assertEqual(_slice_field(index.slices(*whole), 'source_order'), [0, 1, 2, 3, 4, 5])
        self.assertEqual(_slice_field(index.slices(*minutes[0]), 'source_order'), [0, 1, 2])
        self.assertEqual(_slice_field(index.slices(*minutes[1]), 'source_order'), [3, 4])
        self.assertEqual(_slice_field(index.slices(*minutes[2]), 'source_order'), [5])
        self.assertIs(index.slices(*whole), index.slices(*whole))
        self.assertIs(index.slices(*prefix), index.slices(*prefix))

    def test_isolated_instruments_keep_source_ownership(self):
        nq = self._batches(((10, 20, 30), (40, 50)), instrument_id=NQ_ID, source_key='nq')
        es = self._batches(((15, 25, 35), (45, 55)), instrument_id=ES_ID, source_key='es',
                           start_order=100)
        nq_index = _IntervalIndex(nq)
        es_index = _IntervalIndex(es)
        start, end = 12, 46
        nq_slices = nq_index.slices(start, end)
        es_slices = es_index.slices(start, end)
        self.assertEqual(nq_slices, _literal_slices(nq, start, end))
        self.assertEqual(es_slices, _literal_slices(es, start, end))
        self.assertEqual(_slice_field(nq_slices, 't'), [20, 30, 40])
        self.assertEqual(_slice_field(es_slices, 't'), [15, 25, 35, 45])
        self.assertTrue(all(batch is nq[0] or batch is nq[1] for batch, _, _ in nq_slices))
        self.assertTrue(all(batch is es[0] or batch is es[1] for batch, _, _ in es_slices))
        self.assertEqual({int(batch.values['instrument_id'][left]) for batch, left, _ in nq_slices},
                         {NQ_ID})
        self.assertEqual({int(batch.values['instrument_id'][left]) for batch, left, _ in es_slices},
                         {ES_ID})
        self.assertEqual(nq[0].source_key(0), 'nq')
        self.assertEqual(es[0].source_key(0), 'es')
        self.assertNotEqual(_slice_field(nq_slices, 'source_order'),
                            _slice_field(es_slices, 'source_order'))

    def test_cached_intervals_reuse_bounds_and_repeat_contributions(self):
        batches = self._batches(tuple((10 * index,) for index in range(8)))
        index = _IntervalIndex(batches)
        calls = self._watch_bounds()
        first = (25, 55)
        gap = (51, 59)
        empty = (80, 80)
        backward = (40, 20)
        first_slices = index.slices(*first)
        self.assertEqual(first_slices, _literal_slices(batches, *first))
        self.assertEqual(_slice_field(first_slices, 't'), [30, 40, 50])
        self.assertEqual(len(calls), 3)
        self.assertTrue(all(start == 25 and end == 55 for _, start, end in calls))
        cached = index.slices(*first)
        self.assertIs(cached, first_slices)
        self.assertEqual(len(calls), 3)
        self.assertEqual(index.slices(*gap), ())
        self.assertEqual(len(calls), 3)
        self.assertEqual(index.slices(*empty), ())
        self.assertEqual(index.slices(*backward), ())
        self.assertEqual(len(calls), 3)
        later = index.slices(5, 25)
        self.assertEqual(_slice_field(later, 't'), [10, 20])
        self.assertEqual(len(calls), 5)
        self.assertEqual(index.slices(5, 25), later)
        self.assertEqual(len(calls), 5)
        counts = {'sliced_prints': 0, 'full_path_print_contributions': 0,
                  'scalar_reference_sliced_prints': 0}
        window = _RecordWindow()
        added = _add_slices(window, index, *first, counts)
        self.assertEqual(added, 3)
        self.assertEqual(window.added, list(first_slices))
        self.assertEqual(counts['sliced_prints'], 3)
        self.assertEqual(counts['full_path_print_contributions'], 3)
        self.assertEqual(len(calls), 5)
        added = _add_slices(window, index, *first, counts)
        self.assertEqual(added, 3)
        self.assertEqual(window.added, list(first_slices) + list(first_slices))
        self.assertEqual(counts['sliced_prints'], 6)
        self.assertEqual(counts['full_path_print_contributions'], 6)
        self.assertEqual(len(calls), 5)
        scalar = _RecordWindow()
        added = _add_slices(scalar, index, 5, 25, counts, contribution=False,
                            slice_key='scalar_reference_sliced_prints')
        self.assertEqual(added, 2)
        self.assertEqual(scalar.added, list(later))
        self.assertEqual(counts['scalar_reference_sliced_prints'], 2)
        self.assertEqual(counts['full_path_print_contributions'], 6)
        self.assertEqual(len(calls), 5)
