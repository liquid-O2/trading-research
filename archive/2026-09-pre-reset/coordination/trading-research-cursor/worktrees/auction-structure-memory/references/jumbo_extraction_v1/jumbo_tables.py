...
from references.jumbo_extraction_v1.ohlc_ranges import (
...
def features_at(window, *, origin, cash, prior, prior_widths, anchors, recipe, prefix=None):
    ...
    features.update(width_ticks=int(g.width), formation_minutes=window.expected_minutes,
                    log_volume=math.log1p(sum(s.volume[window.left:window.right])))
    if g.usable:
...
def _special(window, cash, recipe, *, evaluation_cut):
    from references.jumbo_extraction_v1.ohlc_mechanisms import (
...
def extract_dates(series: MinuteBars, dates, *, root, recipes, plan, evaluation_cut):
    ...
    for cash in dates:
        ...
        or15 = windows["OR15"]
        activity_volumes.append(sum(series.volume[or15.left:or15.right]) if or15.complete else None)
