...
class CompactProjector:
    ...
    def _project(self,batch,*,source_part:str):
        ...
        table=table.append_column('source_order',pa.array(range(self.rows,self.rows+len(table)),type=pa.int64()))
        ...
        for instrument in pc.unique(table['instrument_id']).to_pylist():
            if type(instrument) is not int or instrument<=0:raise IntegrityError("unresolved source instrument")
            part=table.filter(pc.equal(table['instrument_id'],instrument));flags=part['flags'];actions=pc.cast(part['action'],pa.string());side=pc.cast(part['side'],pa.string())
            ...
            if instrument in self.blocked:eligible=pa.array([False]*len(part))
            elif invalid_index>=0:
                eligible=pc.less(pa.array(range(len(part)),type=pa.int64()),invalid_index);self.blocked[instrument]=int(part['source_order'][invalid_index].as_py())
            else:eligible=pa.array([True]*len(part))
