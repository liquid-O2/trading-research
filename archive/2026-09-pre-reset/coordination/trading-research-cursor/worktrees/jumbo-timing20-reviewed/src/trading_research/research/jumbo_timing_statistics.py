...
def run_timing_statistics(*, store, development, confirmation, analysis_plan, outputs, roots=None):
    ...
    if roots is not None:
        roots = tuple(roots)
        if not roots or len(set(roots)) != len(roots) or any(root not in ('NQ', 'ES') for root in roots):
            raise ContractError('root selection must be an explicit unique NQ/ES partition')
        variants = tuple((name, [shard for shard in shards if shard['root'] in roots])
                         for name, shards in variants
                         if any(shard['root'] in roots for shard in shards))
    ...
    for variant_name, shards in variants:
        ...
        product_a, a_rows = _product_a(common, analysis_plan, settings, tertiles,
                                       variant=variant_name, resources=resources, formations=formations)
    ...
    tables = _sanitize({
        ...
        "units": {"prices_and_widths": "native integer ticks", "normalized_targets": "prior actual RTH widths",
                  "clocks": "UTC nanoseconds", "availability_delay": "minutes relative to common 10:01 cut",
                  "geometry": "dimensionless ratios", "transition_rates": "eligible-date proportions"},
...
def _unique_shards(shards):
    ...
    for shard in shards:
        ...
        if key in seen:
            raise IntegrityError("duplicate retained root/year shard")
...
def _materialize_variant(store, shards, analysis_plan, *, resources, variant):
    ...
    for index, shard in enumerate(ordered):
        shard_started = time.process_time()
        selected_clocks = tuple(dict.fromkeys((*TRANSITION_CLOCKS, *analysis_plan['matched_clock_comparison']['clocks'])))
        records = _read_transition_formations(store, shard, selected_clocks)
        ...
        if resources["pilot"] is None:
            resources["pilot"] = {
                ...
                "common_rows": int(common.size), "cpu_seconds": time.process_time()-shard_started,
        ...
        print(f'Jumbo {variant} {shard["root"]}/{shard["year"]}: {common.size} common-clock rows', flush=True)
...
def _read_transition_formations(store, shard, selected_clocks=TRANSITION_CLOCKS):
    table = _read_columns(store, shard["tables"]["formations"], list(FORMATION_COLUMNS))
    import pyarrow as pa
    import pyarrow.compute as pc
    kept = table.filter(pc.is_in(table["clock"], value_set=pa.array(list(selected_clocks))))
    records = {}
    if kept.num_rows == 0:
        return records
...
def _product_a(common, plan, settings, tertiles, *, variant, resources, formations=None):
    ...
    rows = []
    formation_delay = np.full(common.size, np.nan, dtype=np.float64)
    for index in range(common.size):
        clock = _clock_name(common, clocks[index])
        root = common.categories["root"][int(roots[index])]
        year = int(years[index])
        formation = (formations or {}).get((root, str(iso[index]), clock), {})
        available_at = formation.get('available_at_ns')
        if available_at is not None and own[index]:
            formation_delay[index] = (int(available_at)-int(origin[index]))/60_000_000_000
        rows.append({
            ...
            "contract_key": formation.get('contract_key'),
            "source_version": formation.get('source_version'),
            "formation_id": formation.get('formation_id'),
            "formation_start_ns": formation.get('formation_start_ns'),
            "formation_end_ns": formation.get('formation_end_ns'),
            "formation_available_at_ns": available_at,
            "formation_available_minus_common_cut_minutes": _optional_float(formation_delay[index]),
            "feature_known_at_ns": int(origin[index]),
            "target_maturity_ns": int(maturity[index]) if maturity[index] > 0 else None,
        ...
        for root, code in _root_codes(common):
            ...
            for clock_code, clock in enumerate(common.categories["clock"]):
                ...
                if not np.any(mask):
                    continue
                clock_iso = iso[mask]
                clock_targets = {name: values[mask] for name, values in targets.items()}
                clock_target_ok = target_ok[mask]
                ...
                for target in COMMON_TARGETS:
                    values = {day: _optional_float(clock_targets[target][at])
                              if clock_target_ok[at] else None
                ...
                for group_name, labels, allowed in groups:
                    for label in allowed:
                        ...
                        local_member = member[mask]
                        record = _group_record(clock_iso, local_member, clock_target_ok, clock_targets,
                        ...
                        for target in COMMON_TARGETS:
                            metrics[f"{clock}|{group_name}|{label}|{TARGET_LABELS[target]}"] = {
                                day: (_optional_float(clock_targets[target][at])
                                      if local_member[at] and clock_target_ok[at] else None)
                                for at, day in enumerate(clock_iso)
                            }
            ...
            pair_tables, pair_metrics, pair_ok = _paired_tables(
                ...
                formation_delay, width_bins, vol_bins, overlap_lookup, pairs, settings, resources)
            ...
            slices[slice_name]["roots"][root] = {
                ...
                "control_limit": "Separate width×prior-vol and overlap stratifications are descriptive. This does not claim conditional predictive increment or causal identification.",
...
def _group_record(clock_iso, member, target_ok, targets, settings, group_name, label):
    eligible = np.asarray(member, dtype=bool) & target_ok
    distributions = {TARGET_LABELS[target]: _descriptive(values[eligible & np.isfinite(values)],
                        settings['descriptive_quantiles']) for target, values in targets.items()}
    dates = int(np.count_nonzero(member))
    events = distributions['common_terminal']['count']
    sparse = min(dates, events) < settings['minimum_dates'] or events < settings['minimum_events']
    return {'group': group_name, 'label': label, 'dates': dates, 'events': events,
            'sparse': sparse, 'interval_claimable': not sparse,
            'quantiles': distributions['common_terminal'], 'target_distributions': distributions}
...
def _paired_tables(common, root_mask, iso, intended, own, target_ok, targets, width,
    ...
    for source, neighbor, kind in pairs:
        ...
        if source_iso.tolist() != neighbor_iso.tolist():
            raise IntegrityError('paired clock rows must preserve the same ordered date keys')
        ...
        paired_available = source_own & neighbor_own
        ...
        for field, array, present, locator, mask in (
            ("source_width", width, paired_available, source_at, source_mask),
            ("neighbor_width", width, paired_available, neighbor_at, neighbor_mask),
            ("source_open_position", open_position, paired_available, source_at, source_mask),
            ("neighbor_open_position", open_position, paired_available, neighbor_at, neighbor_mask),
            ("source_availability_minus_common_cut_minutes", delay, paired_available, source_at, source_mask),
            ("neighbor_availability_minus_common_cut_minutes", delay, paired_available, neighbor_at, neighbor_mask),
        ):
            if array is None:
                continue
            values = array[mask]
        ...
        for output_name, source_name, neighbor_name in (
            ('width_difference', 'source_width', 'neighbor_width'),
            ('open_position_difference', 'source_open_position', 'neighbor_open_position'),
            ('availability_difference_minutes', 'source_availability_minus_common_cut_minutes', 'neighbor_availability_minus_common_cut_minutes')):
            left = metrics.get(f'{source}|vs|{neighbor}|{source_name}')
            right = metrics.get(f'{source}|vs|{neighbor}|{neighbor_name}')
            if left is not None and right is not None:
                metrics[f'{source}|vs|{neighbor}|{output_name}'] = {
                    day: _paired_optional(left[day], right[day]) for day in intended}
        if overlap_values is not None:
            ...
            for label in OVERLAP_LABELS:
                member = paired_available & (neighbor_bins == label)
                resources["conditional_groups"] += 1
    ...
    for record in pair_tables:
        ...
        record["metrics"] = {
            ...
            for name in ("both_available", "source_width", "neighbor_width",
                         ...
                         "source_availability_minus_common_cut_minutes", "neighbor_availability_minus_common_cut_minutes",
                         "width_difference", "open_position_difference", "availability_difference_minutes")
...
def _transition_row(variant, root, day, year, jtr, turn, prior, turn_name):
    row = {
        ...
        "feature_known_at_ns": None, "target_maturity_ns": None,
    ...
    if jtr is None or turn is None:
        ...
        return row
    row['contract_key'] = jtr.get('contract_key')
    row['source_version'] = jtr.get('source_version')
    row['formation_id'] = jtr.get('formation_id')
    row['target_formation_id'] = turn.get('formation_id')
    row['formation_start_ns'] = jtr.get('formation_start_ns')
    row['formation_end_ns'] = jtr.get('formation_end_ns')
    row['target_start_ns'] = turn.get('formation_start_ns')
    row['target_end_ns'] = turn.get('formation_end_ns')
    row["feature_known_at_ns"] = jtr.get("available_at_ns")
    row['target_maturity_ns'] = turn.get('available_at_ns')
    ...
    if turn.get("available_at_ns") is None or jtr.get("available_at_ns") is None:
        ...
        return row
    if not _complete_ohlc(jtr) or not _complete_ohlc(turn) or jtr.get("zero_width") is True:
        row["identification"] = "censored_incomplete_or_zero_width"
        row["zero_width"] = bool(jtr.get("zero_width"))
        return row
...
def _complete_ohlc(record):
    return record is not None and record.get("status") == "complete" and all(record.get(name) is not None for name in
...
def _transition_root_tables(rows, intended, settings, resources):
    ...
    for turn in TURN_WINDOWS:
        ...
        metrics[f"{turn}|definite_print"] = {day: None if by_date[day].get("target_missing", True) else float(by_date[day].get("identification") == "definite_print")
                                             for day in intended}
        metrics[f"{turn}|compatible"] = {
            day: None if by_date[day].get("target_missing", True) else float(any(by_date[day].get(f"{level}_compatible") for level in LEVEL_NAMES))
            for day in intended
        }
        metrics[f"{turn}|no_event"] = {day: None if by_date[day].get("target_missing", True) else float(by_date[day].get("identification") == "no_event")
                                       for day in intended}
    ...
    for left, right in (("turn_source", "turn_earlier"), ("turn_source", "turn_later")):
        metrics[f"{left}|paired_print_minus|{right}"] = {
            day: _paired_optional(metrics[f"{left}|definite_print"][day], metrics[f"{right}|definite_print"][day])
            for day in intended
        }
    ...
    for turn, table in tables.items():
        table["rates"] = {name: _rate_interval_record(intervals[key], metrics[key], settings)
                          for name, key in (
    ...
    tables["paired"] = {
        "source_minus_earlier_print": _paired_interval_record(intervals.get("turn_source|paired_print_minus|turn_earlier"), metrics["turn_source|paired_print_minus|turn_earlier"], settings),
        "source_minus_later_print": _paired_interval_record(intervals.get("turn_source|paired_print_minus|turn_later"), metrics["turn_source|paired_print_minus|turn_later"], settings),
        "source_minus_earlier_inside": _paired_interval_record(intervals.get("turn_source|paired_inside_minus|turn_earlier"), metrics["turn_source|paired_inside_minus|turn_earlier"], settings),
        "source_minus_later_inside": _paired_interval_record(intervals.get("turn_source|paired_inside_minus|turn_later"), metrics["turn_source|paired_inside_minus|turn_later"], settings),
        "same_dates": len(intended),
        "zero_event_dates_retained": True,
...
def _rate_interval_record(metric, values, settings):
    record = _interval_record(metric, settings)
    valid = [value for value in values.values() if value is not None]
    positives = sum(value == 1 for value in valid)
    record.update(positive_events=positives, negative_events=len(valid)-positives)
    if min(positives, len(valid)-positives) < settings['minimum_events']:
        record = _strip_ci(record)
        record['no_ci_claim'] = 'Fewer than 20 positive or negative events; retained descriptive rate only.'
    return record


def _paired_interval_record(metric, values, settings):
    record = _interval_record(metric, settings)
    nonzero = sum(value is not None and value != 0 for value in values.values())
    record['discordant_dates'] = nonzero
    if nonzero < settings['minimum_events']:
        record = _strip_ci(record)
        record['no_ci_claim'] = 'Fewer than 20 discordant paired dates; retained descriptive difference only.'
    return record
...
def _date_schema():
    ...
    fields = [
        ...
        ("formation_id", pa.string()), ("target_formation_id", pa.string()),
        ("formation_start_ns", pa.int64()), ("formation_end_ns", pa.int64()),
        ("formation_available_at_ns", pa.int64()),
        ("target_start_ns", pa.int64()), ("target_end_ns", pa.int64()),
        ("formation_available_minus_common_cut_minutes", pa.float64()),
    ...
    for variant, payload in (tables.get("variants") or {}).items():
        ...
        for slice_name, slice_payload in (payload.get("product_a", {}).get("slices") or {}).items():
            ...
            for root, root_payload in (slice_payload.get("roots") or {}).items():
                ...
                lines.extend(("", "| Clock / width-to-prior-range bin | Eligible dates | Mean up | Mean down | Mean terminal | Terminal 95% interval |", "|---|---:|---:|---:|---:|---|"))
                for clock, clock_payload in (root_payload.get('clocks') or {}).items():
                    for cell in clock_payload.get('conditionals', []):
                        if cell['group'] != 'width_ratio':
                            continue
                        estimates = cell.get('intervals', {})
                        def estimate(name):
                            value = (estimates.get(name) or {}).get('estimate')
                            return 'unavailable' if value is None else f'{value:.5g}'
                        terminal = estimates.get('common_terminal') or {}
                        ci = terminal.get('bootstrap')
                        interval = 'not claimed' if ci is None else f"[{ci['lower']:.5g}, {ci['upper']:.5g}]"
                        lines.append(f"| {clock} / {cell['label']} | {cell['events']} | {estimate('common_up')} | {estimate('common_down')} | {estimate('common_terminal')} | {interval} |")
                lines.extend(("", "| Paired clocks | Common eligible dates | Width difference (ticks) | Availability difference (minutes) |", "|---|---:|---:|---:|"))
                for pair in root_payload.get('pairs', []):
                    if pair.get('group'):
                        continue
                    metrics = pair.get('metrics', {})
                    width = metrics.get('width_difference') or {}
                    delay = metrics.get('availability_difference_minutes') or {}
                    lines.append(f"| {pair['source']} minus {pair['neighbor']} | {width.get('valid_dates', 0)} | {width.get('estimate')} | {delay.get('estimate')} |")
            lines.append("")
