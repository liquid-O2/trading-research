...
def load_frozen_trade_modules():
    ...
    def load(name, filename):
        ...
        if hashlib.sha256(path.read_bytes()).hexdigest() != files[filename]["sha256"]:
            raise IntegrityError("frozen independent trade reference changed")
...
class AuctionFlowPreparedTradeTests(unittest.TestCase):
    ...
    def test_large_public_table_preserves_frozen_domain_with_bounded_preparation(self):
        count = 65_537
        table = make_trade_table([trade_row(i, source_order=i) for i in range(count)])
        expected = finish_ordinary([table], Window=frozen_tape_window(), end=count + 1,
                                   maximum_trades=count)
        observed = finish_ordinary([table], end=count + 1, maximum_trades=count)
        self.assertEqual(observed, expected)
        with self.assertRaises(ContractError):
            prepare_trade_batch(table)

    def test_unsigned_address_and_mutable_arrow_buffer_keep_original_values(self):
        table = make_trade_table([trade_row(10, source_order=0), trade_row(20, source_order=1)])
        table = table.set_column(table.schema.get_field_index("source_row"), "source_row",
                                 pa.array([2**63 + 3, 2**64 - 1], type=pa.uint64()))
        self.three_way([table])
        sizes = np.array([7, 11], dtype=np.int64)
        table = table.set_column(table.schema.get_field_index("size"), "size", pa.array(sizes))
        expected = finish_ordinary([table], Window=frozen_tape_window())
        with prepare_trade_batch(table) as prepared:
            sizes[:] = 99
            window = tape_window(TapeWindow)
            window.add_prepared(prepared, 0, 2)
            self.assertEqual(publish(window), expected)
            self.assertEqual(prepared.values["size"].tolist(), [7, 11])
            self.assertEqual(prepared.values["source_row"].tolist(), [2**63 + 3, 2**64 - 1])
