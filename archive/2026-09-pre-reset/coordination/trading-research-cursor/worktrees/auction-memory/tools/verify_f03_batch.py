...
def supervise():
    ...
    registration = json.loads((ROOT / 'reports/f03-interval-registration.json').read_text())
    ...
    for relative, ref in (('validation/F03_INTERVAL_PROTOCOL.md', protocol_ref),
                          ('tests/golden/f03-intervals.json', golden_ref)):
    ...
    review = registry.artifacts.put_bytes((ROOT / 'validation/F03_INTERVAL_BATCH_REVIEW.md').read_bytes(), kind='implementation_review')
    ...
    trial = registry.register(name='F03 intervals, durable timers and existing combined regression suite',
        family='F03-interval-timer-reference-v1', stage='engineering', configuration=configuration,
        code_hash=digest(manifest), data_hashes={'independent_golden': golden_ref.sha256},
        fold_version='no-market-fit', target_version='fixed-intervals-and-calendar-action-identity-v1')
    ...
    result_refs = []
    try:
        with tempfile.TemporaryDirectory(prefix='f03-verification-') as temporary:
    ...
    summary = {'success': report['success'], 'trial_id': trial, 'attempt_id': attempt, 'artifact': asdict(ref),
               ...
               'cpu_seconds': cpu, 'wall_seconds': wall, 'peak_rss_bytes': peak_rss}
    (ROOT / 'reports' / f'f03-interval-verification-{attempt[:12]}.json').write_text(json.dumps(summary, indent=2) + '\n')
