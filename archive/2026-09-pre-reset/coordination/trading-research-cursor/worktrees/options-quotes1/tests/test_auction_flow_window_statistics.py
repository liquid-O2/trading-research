...
class WindowStatisticsLiteralTests(unittest.TestCase):
    ...
    def test_integrated_event_pairs_units_and_authenticated_reuse(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            reuse_out = BoundedOutputs(Path(folder) / 'reuse', maximum_total_bytes=16 * 1024 ** 2,
                                       maximum_file_bytes=8 * 1024 ** 2)
            replay = run_window_statistics(
                population={'kind':'auction_flow_window_population_v1', 'population_source_windows':3,
                            'units':[unit_a,unit_b,unit_c]},
                contract={**contract,'accepted_partitions':result['refs']['partitions']},
                outputs=reuse_out,load_reference=loader,
                selected_partition_keys=[['monthly_acquisitions','NQ',2020],['monthly_acquisitions','NQ',2021]])
            self.assertEqual(replay['counts']['events_without_receiver'], result['counts']['events_without_receiver'])
            self.assertEqual(replay['counts']['events_without_window'], result['counts']['events_without_window'])
            self.assertEqual(replay['counts']['reused_partitions'],2)
