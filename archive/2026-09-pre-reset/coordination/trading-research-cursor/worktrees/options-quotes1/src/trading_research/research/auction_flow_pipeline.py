...
from trading_research.data.compact import SOURCE_CLOCK_STRICT, named_source_clock_policy
...
def _replay_mask(batch):
    import numpy as np

    orders = getattr(batch, 'replay_source_orders', ())
    if not orders or batch.raw is None or not len(batch.raw):
        return None
    return np.isin(batch.raw['source_order'].to_numpy(zero_copy_only=False),
...
class InstrumentWindows:
    ...
    def trade_rows(self, table):
        ...
        with prepare_trade_batch(table) as prepared:
            self.whole_tape.add_prepared(prepared, 0, len(prepared))
            self.native.add_trades(table)
            self.cpu['whole_trade_and_native_consumers'] += time.process_time() - started
            started = time.process_time()
            for number, left, right in prepared.atomic_slices(start=self.start, width=self.width):
                ...
                if number not in self.tape:
                    ...
                    self.tape[number] = TapeWindow(instrument_id=self.instrument_id, start_ns=a, end_ns=b,
                        latency_ns=self.latency, maximum_trades=self.maximum_trades, maximum_profile_cells=self.maximum_cells)
                self.tape[number].add_prepared(prepared, left, right)
        ...
        if len(table):
            with prepare_quote_batch(table) as prepared:
                for number, left, right in prepared.atomic_slices(start=self.start, width=self.width):
                    ...
                    self.quotes[number].add_prepared(prepared, left, right)
    ...
    def finish(self, *, source_intervals, source_owners, coordinates, native_ownership, retain_native_table):
        ...
        for number in range(count):
            ...
            clock_intervals = tuple((a,b) for a,b in getattr(self, 'clock_uncertain_intervals', ()) if a < end and b > start)
            if clock_intervals:
                quality = Counter(quality)
                quality['clock_uncertain_intervals'] = len(clock_intervals)
            ...
            source_complete = (_covered(source_intervals, start, end) and presence and owner_stable
                and not any(quality.get(k, 0) for k in ("gap_rows", "unknown_action_rows", "invalid_trade_size_rows", "clock_uncertain_intervals")))
            ...
            if clock_intervals:
                quotes['source_clock_uncertain_intervals'] = clock_intervals
                quotes['duration_basis'] = 'exact duration excludes these uncertain clock intervals; state follows original source order'
        ...
        source_eligible = native_ownership.eligible(self.instrument_id, source_intervals)
        clock_uncertainty = getattr(self, 'clock_uncertain_intervals', ())
        if clock_uncertainty:
            starts = self.start + np.arange(len(source_eligible))*self.native_width
            for lo, hi in clock_uncertainty:
                source_eligible &= ~((starts < hi) & (starts+self.native_width > lo))
        native = self.native.table(source_eligible=source_eligible,
...
def measure_raw_window(*, data_root: Path, index, coordinates, root, start_ns, end_ns,
                       ...
                       source_clock_policy=SOURCE_CLOCK_STRICT, retain_replay_batch=None):
    ...
    if (root not in ("NQ", "ES") or coordinates.root != root
            ...
            or any(callback is not None and not callable(callback) for callback in
                   (retain_trade_batch, retain_native_table, retain_quote_batch, retain_excluded_batch,
                    inspect_raw_batch, retain_replay_batch))
    ...
    source_clock_policy = named_source_clock_policy(source_clock_policy)
    parameters = {'root': root, 'latency_ns': latency_ns, 'atomic_width_ns': atomic_width_ns,
                  'native_width_ns': native_width_ns}
    if source_clock_policy != SOURCE_CLOCK_STRICT:
        parameters['source_clock_policy'] = source_clock_policy
    ...
    native_ownership.clock_uncertain_intervals = stream.clock_uncertain_intervals
    ...
    def instrument_windows(instrument):
        ...
        if instrument not in by_instrument:
            ...
            value = by_instrument[instrument]
            value.clock_uncertain_intervals = stream.clock_uncertain_intervals
            value.native.clock_uncertain_intervals = stream.clock_uncertain_intervals
            value.time_at_price.clock_uncertain_intervals = stream.clock_uncertain_intervals
            value.quotes[0].clock_uncertain_intervals = tuple((a,b) for a,b in stream.clock_uncertain_intervals if a < value.quotes[0].end)
    ...
    while True:
        ...
        work['raw_batches'] += 1
        at = time.process_time()
        native_ownership.add(batch.raw, replay_mask=_replay_mask(batch))
        ...
        if retain_replay_batch is not None and batch.replay_records is not None and len(batch.replay_records):
            at = time.process_time()
            retain_replay_batch(batch.replay_records, batch.source)
        ...
        temporal = batch.raw if batch.temporal_raw is None else batch.temporal_raw
        for instrument in pc.unique(batch.raw["instrument_id"]).to_pylist():
            ...
            at = time.process_time()
            temporal_part = part if temporal is batch.raw else temporal.filter(pc.equal(temporal["instrument_id"], instrument))
            by_instrument[instrument].time_at_price.add(temporal_part, trade_parts.get(instrument), source_key=batch.source['source_key'])
