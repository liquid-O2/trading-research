...
def measured_unit(unit, *, protocol, index, coordinates, outputs, data_root, storage_encoding='structural',
    ...
    clock_options = {} if source_clock_policy is None else {'source_clock_policy': source_clock_policy}
    ...
    if quote_storage == 'replay':
        events['quotes'] = QuoteReplaySeries(
            ...
            continuation=None, **clock_options)
    if source_clock_policy is not None:
        events['replayed_snapshots'] = ParquetSeries(outputs, f'{name}-replayed-snapshots', encoding=storage_encoding)
    ...
    try:
        ...
        measured = measure_raw_window(data_root=data_root, index=index, coordinates=coordinates, root=unit['root'],
            ...
            retain_replay_batch=events['replayed_snapshots'].append if source_clock_policy is not None else None,
            **clock_options)
    ...
    storage_cpu = sum(r['cpu_seconds'] for r in [*storage.values(), *native_storage]
                      if r.get('encoding') != 'raw-source-quote-replay-v1')
    anchor_check = None
