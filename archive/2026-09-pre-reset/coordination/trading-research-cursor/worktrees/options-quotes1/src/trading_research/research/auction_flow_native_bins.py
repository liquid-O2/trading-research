...
class NativeEventBins:
    ...
    def indices(self, at):
        ...
        if (np.any(at < self.start) or np.any(at >= self.end)
                or not clock_order_valid(at, None, getattr(self, 'clock_uncertain_intervals', ()))):
    ...
    def quote_updates(self, *, at, order, fresh, measured, ofi, same):
        ...
        if (np.any(order < 0) or np.any(order[1:] <= order[:-1])
                or self._last_quote_order is not None and (int(order[0]) <= self._last_quote_order
                                                           or not clock_order_valid(at, self._last_quote_at, getattr(self, 'clock_uncertain_intervals', ())))):
            raise IntegrityError("native quote projection repeated or changed source order")
        self._last_quote_order = int(order[-1])
        self._last_quote_at = max(int(at.max()), self._last_quote_at if self._last_quote_at is not None else 0)
    ...
    def _fill_quote_updates(self, bins, at, order, fresh, measured, ofi, same):
        ...
        if getattr(self, 'clock_uncertain_intervals', ()):
            np.add.at(self.columns['quote_rows'], bins, 1)
            np.add.at(self.columns['fresh_quotes'], bins, fresh.astype(np.int64))
        else:
...
class NativeSourceOwnership:
    ...
    @_open
    def add(self, raw, replay_mask=None):
        import numpy as np
        import pyarrow as pa
        ...
        if replay_mask is not None:
            replay_mask = np.asarray(replay_mask)
            if replay_mask.dtype != bool or replay_mask.shape != (len(at),):
        ...
        if (np.any(at < self.start) or np.any(at >= self.end)
                ...
                or self._last_order is not None and int(order[0]) <= self._last_order):
            raise IntegrityError('native source ownership lost original ordered raw identity or clock')
        ...
        if uncertainty:
            from trading_research.data.clock_uncertainty import clock_order_valid
            economic = at if replay_mask is None else at[~replay_mask]
            if not clock_order_valid(economic, self._last_at, uncertainty):
                raise IntegrityError('unadmitted native source clock reversal')
            if len(economic):
                self._last_at = max(int(economic.max()), self._last_at if self._last_at is not None else 0)
            if replay_mask is not None:
                if np.any(replay_mask & ((flags != 168) | ~pc.fill_null(pc.equal(pc.cast(raw['action'],pa.string()),'A'),False).to_numpy())):
                    raise IntegrityError('native replay mask changed')
                self.replay_raw_rows += int(np.count_nonzero(replay_mask))
        elif replay_mask is None:
            if (np.any(at[1:] < at[:-1])
                    or self._last_at is not None and int(at[0]) < self._last_at):
                raise IntegrityError('native source ownership lost original ordered raw identity or clock')
            self._last_at = int(at[-1])
        else:
            previous_max = np.maximum.accumulate(np.r_[self._last_at if self._last_at is not None else int(at[0]), at])[:-1]
            actions = pc.cast(raw['action'], pa.string())
            marked_action = pc.fill_null(pc.equal(actions, 'A'), False).to_numpy(zero_copy_only=False)
            if (np.any(replay_mask & ((flags != 168) | ~marked_action | (at >= previous_max)))
                    or ('side' in raw.column_names and np.any(replay_mask & ~pc.fill_null(
                        pc.equal(pc.cast(raw['side'], pa.string()), 'N'), False).to_numpy(zero_copy_only=False)))):
                raise IntegrityError('native replay mask does not identify backward snapshot records')
            economic = at[~replay_mask]
            if ((len(economic) and np.any(economic[1:] < economic[:-1]))
                    or (self._last_at is not None and len(economic) and int(economic[0]) < self._last_at)):
                raise IntegrityError('native source ownership lost original ordered raw identity or clock')
            if len(economic):
                self._last_at = int(economic[-1])
            self.replay_raw_rows += int(np.count_nonzero(replay_mask))
        self._last_order = int(order[-1])
        self._raw_count += len(at)
        bins = (at - self.start) // self.width
        if replay_mask is None and not uncertainty:
            _add_sum(self.raw_count, bins, np.ones(len(bins), dtype=np.int64))
        else:
            np.add.at(self.raw_count, bins, 1)
        actions = pc.cast(raw['action'], pa.string())
        ...
        if replay_mask is None and not uncertainty:
            ...
            np.logical_or.at(self.invalid, bins, bad)
        if np.any(ordinary):
            b, ids = bins[ordinary], instruments[ordinary]
            starts, ends, unique = _groups(b)
            if uncertainty:
                np.add.at(self.ordinary_count, b, 1)
                np.minimum.at(self.low, b, ids)
                np.maximum.at(self.high, b, ids)
                self.last[b] = ids
            else:
