...
def _finite_exact_positive_quarter_point(value):
    if type(value) is bool or type(value) not in (int, float):
        return False
    scaled = value * 4
    return math.isfinite(scaled) and scaled == int(scaled) and 0 < scaled < 2**53


def _positive_finite_size(value):
    if type(value) is bool or type(value) not in (int, float):
        return False
    if type(value) is float and (not math.isfinite(value) or value != int(value)):
        return False
...
def _exact_int(value):
    ...
    if type(value) is int:
        return value
    if type(value) is float and math.isfinite(value) and value == int(value):
        return int(value)
    return None
...
def authenticate_redundant_backward_snapshot(*, event, predecessor, same_physical_source, physically_adjacent):
    """Admit a BBO-identical snapshot; predecessor gap flags remain invalidating."""
    ...
    if predecessor.get('action') == 'T' or predecessor.get('action') not in _ORDINARY_QUOTE_ACTIONS:
        return None
    if _exact_int(predecessor.get('flags')) not in _ORDINARY_QUOTE_FLAGS:
        return None
...
def classify_redundant_backward_snapshots(events, *, source_key=None):
    ...
    for item in events:
        ...
        if type(key) is not str or not key:
            raise IntegrityError('original physical source identity required')
...
class CompactProjector:
    ...
    def _apply_replay_clock(self, table, *, physical_source_key):
        ...
        evidence = []
        if np.any(backward):
            if type(physical_source_key) is not str or not physical_source_key:
                raise IntegrityError('original physical source identity required')
            for index in np.flatnonzero(backward).tolist():
                ...
                if reason is None:
                    from trading_research.data.clock_uncertainty import original_record
                    plan = getattr(self, 'clock_uncertainty_events', {}).get(event['source_row'])
                    if (plan is None or original_record(event) != plan['event']
                            or predecessor is None or original_record(predecessor) != plan['predecessor']):
                        raise IntegrityError("source event order/null clock requires separate resolution")
                    self.clock_uncertainty_seen.add(event['source_row'])
                    continue
                replay[index] = True
                evidence.append({'index': index, 'reason': reason, 'event': event, 'predecessor': dict(predecessor)})
        economic = t[~replay]
        if len(economic):
            self.prior_time = max(int(economic.max()), self.prior_time if self.prior_time is not None else 0)
