...
scientific identity, then concatenates each partition's finalized groups.
"""
...
from trading_research.research.auction_flow_profile_reference import (
    ...
    empty_completeness, footprint_parameters, frozen_variant_ids,
...
METRIC_UNITS = {
    ...
    'future_val': 'grid_rows',
    'future_vah': 'grid_rows',
    ...
    'transport_wasserstein': 'quarter_point_ticks',
}
...
GEOMETRY_METRIC_NAMES = (
    ...
    'side_overlap', 'side_total_variation', 'side_wasserstein', 'transport_wasserstein',
...
JOIN_METRIC_NAMES = (
    ...
    'future_input_known', 'future_coverage_complete',
)
PAIRED_METRIC_NAMES = (
...
def _geometry_metrics(row):
    return {
        ...
        'overshoot_mass': _ratio(row.get('overshoot_num'), row.get('overshoot_den')),
        'priced_volume': row.get('priced_volume'),
        'vwap_ticks': _ratio(row.get('vwap_ticks_num'), row.get('vwap_ticks_den')),
        'variance_ticks_squared': _ratio(
            row.get('variance_ticks_squared_num'), row.get('variance_ticks_squared_den')),
        'sd_ticks_approx': row.get('sd_ticks_approx'),
        'source_flat_bar_lost_mass': row.get('source_flat_bar_lost_mass'),
        'concentration': _ratio(row.get('concentration_num'), row.get('concentration_den')),
        'dominance_margin': _ratio(row.get('dominance_margin_num'), row.get('dominance_margin_den')),
        'signed_delta': row.get('signed_delta'),
        'absolute_delta_mass': _ratio(row.get('absolute_delta_mass_num'), row.get('absolute_delta_mass_den')),
        'side_overlap': _ratio(row.get('side_overlap_num'), row.get('side_overlap_den')),
        'side_total_variation': _ratio(
            row.get('side_total_variation_num'), row.get('side_total_variation_den')),
        'side_wasserstein': _ratio(row.get('side_wasserstein_num'), row.get('side_wasserstein_den')),
        ...
        'pin069_vwap': _ratio(row.get('pin069_vwap_num'), row.get('pin069_vwap_den')),
        'pin069_bar_volume': row.get('pin069_bar_volume'),
        'unpriced_only_atoms': row.get('unpriced_only_atoms'),
        'input_cells': row.get('input_cells'),
        'transport_wasserstein': _ratio(row.get('side_wasserstein_num'), row.get('side_wasserstein_den')),
        ...
        'percent_band_0_low': _ratio(row.get('percent_band_0_low_num'), row.get('percent_band_0_low_den')),
        'percent_band_0_high': _ratio(row.get('percent_band_0_high_num'), row.get('percent_band_0_high_den')),
        'percent_band_1_low': _ratio(row.get('percent_band_1_low_num'), row.get('percent_band_1_low_den')),
        'percent_band_1_high': _ratio(row.get('percent_band_1_high_num'), row.get('percent_band_1_high_den')),
        'percent_band_2_low': _ratio(row.get('percent_band_2_low_num'), row.get('percent_band_2_low_den')),
        'percent_band_2_high': _ratio(row.get('percent_band_2_high_num'), row.get('percent_band_2_high_den')),
        'q_1_20': _ratio(row.get('q_1_20_num'), row.get('q_1_20_den')),
        'q_1_4': _ratio(row.get('q_1_4_num'), row.get('q_1_4_den')),
        'q_1_2': _ratio(row.get('q_1_2_num'), row.get('q_1_2_den')),
        'q_3_4': _ratio(row.get('q_3_4_num'), row.get('q_3_4_den')),
        'q_19_20': _ratio(row.get('q_19_20_num'), row.get('q_19_20_den')),
        'weighted_median': _ratio(row.get('weighted_median_num'), row.get('weighted_median_den')),
        'mad': _ratio(row.get('mad_num'), row.get('mad_den')),
        'robust_scale': _ratio(row.get('robust_scale_num'), row.get('robust_scale_den')),
        'unpriced_buy': row.get('unpriced_buy'),
        'unpriced_sell': row.get('unpriced_sell'),
        'unpriced_unknown': row.get('unpriced_unknown'),
        'low_overflow_buy': row.get('low_overflow_buy'),
        'low_overflow_sell': row.get('low_overflow_sell'),
        'low_overflow_unknown': row.get('low_overflow_unknown'),
        'high_overflow_buy': row.get('high_overflow_buy'),
        'high_overflow_sell': row.get('high_overflow_sell'),
        'high_overflow_unknown': row.get('high_overflow_unknown'),
    }
...
def accumulate_geometry_row(acc, row, *, collection, year):
    ...
    for name, value in metrics.items():
        ...
        if complete or name in ('priced_volume', 'source_flat_bar_lost_mass'):
            store[name].add(day, value)
    sha = row.get('canonical_raw_values_sha256')
    if sha and day:
        acc['alias_dates'][sha].add(day)
    if not complete or row.get('selected_partition_member') is False:
        return
    ...
    if row.get('proxy_variant') is None:
        ...
        _pair_exact_proxy(acc, collection, year, match, row.get('proxy_variant'), day, metrics, row)
...
def _release_geometry_group(acc, collection, year, match):
    variants = acc['geometry_index'].get(match) or {}
    if len(variants) < len(FROZEN_GEOMETRY_VARIANTS):
        return
    _pair_variants(acc, collection, year, match)
    proxies = [key for key in acc['proxy_index'] if key[0] == match]
    if len(proxies) < len(BAR_PROXY_VARIANTS):
        return
    for key in proxies:
...
def authenticate_reused_partition(part, load_reference):
    ...
    return {'part': part, 'completeness': completeness, 'groups': groups, 'paired': paired}
...
def _markdown_report(*, groups, completeness, population, cpu, output_bytes, reused,
    ...
    lines.extend(['', '## Groups', ''])
    lines.append(
        'Full group metric payloads remain in the bound `groups` and `paired` JSON '
        'references. This table is the complete readable index; reconstruct values from those refs.'
    )
    lines.append('')
    lines.append(
        '| collection | root | year | stage | session | anchor | variant | proxy | '
        'status | contrast | formation | latency | horizon | metrics | defined | dates | events |'
    )
    lines.append('|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|')
    for group in groups:
        metrics = group.get('metrics') or {}
        defined = sum(1 for payload in metrics.values() if not (payload or {}).get('undefined'))
        dates = max(
            (int((payload or {}).get('eligible_independent_dates') or 0) for payload in metrics.values()),
            default=0,
        )
        events = max(
            (int((payload or {}).get('eligible_observations') or 0) for payload in metrics.values()),
            default=0,
        )
        lines.append(
            ...
            f'{group.get("horizon_kind")}/{group.get("horizon_minutes")} | {len(metrics)} | '
            f'{defined} | {dates} | {events} |'
        )
...
def run_profile_statistics(*, population, window_population, contract, outputs, load_reference,
    ...
    completeness_refs, group_parts, paired_parts = [], [], []
    ...
    for ordinal, key in enumerate(selected_keys):
        ...
        if identity in accepted and accepted[identity][1].get('passed') is True:
            ...
            group_parts.extend(reused_payload['groups'].get('groups') or [])
            paired_parts.extend(reused_payload['paired'].get('groups') or [])
            ...
            group_parts.extend(groups_payload.get('groups') or [])
            paired_parts.extend(paired_payload.get('groups') or [])
    ...
    if np is None:
        np, pa = _require_numpy(), _require_pyarrow()
    finalized = group_parts
    paired_out = paired_parts
    group_ref = outputs.json('profile-groups.json', {
        'kind': 'auction_flow_profile_group_statistics_v1',
        'groups': finalized, 'family_complete': False,
        'variant_ids': list(frozen_variant_ids()),
        'bar_proxy_variants': list(BAR_PROXY_VARIANTS),
        'concatenated_partition_groups': True,
    }, kind='auction_flow_profile_group_statistics_v1')
...
__all__ = [
    ...
    'neighbor_units', 'new_partition_accumulators', 'profile_partition_identity',
