...
from trading_research.research.auction_flow_profile_reference import (
    ...
    decode_exact_int, delayed_selection_blocks_origin, encode_exact_int,
    extract_sparse_profile, extract_unit_cells, future_mass_on_frozen_grid,
    geometry_from_mass, geometry_schema, integer_profile_geometry, labelled_sd,
    match_atom_identity, null_geometry, pin069_reanchor, publication_known,
...
def _write_window_unit(outputs, name, *, path, root, start, end, feature_rows, label_rows, sha=SHA):
    features = _write_table(outputs, f'{name}-features', feature_rows, schema=feature_schema())
...
class ProfileReferenceCorrectionTests(unittest.TestCase):
    ...
    def test_join_rows_are_four_formations_by_three_latencies_by_four_horizons(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(len(rows) == 48 for rows in by_geometry.values()))
            ...
            with self.subTest(variant=variant):
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    outputs = BoundedOutputs(
                        tmp, maximum_total_bytes=64 * 1024 ** 2, maximum_file_bytes=16 * 1024 ** 2,
                    )
                    units = [
                        _write_unit(
    ...
    def test_later_data_does_not_change_earlier_numeric_references(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            early, _ = _run(Path(tmp) / 'early', units=[first_unit], selected=[['weekly_acquisitions', 'NQ', 2020]])
    ...
    def test_large_volume_numerator_roundtrip_and_side_wasserstein(self):
        volume = 2 ** 32
        stored, text = encode_exact_int(volume * volume)
        self.assertEqual(decode_exact_int(stored, text), volume * volume)
        self.assertGreater(volume * volume, 2 ** 63 - 1)
        mass = IntegerMass()
        mass.add({
            'rows': ((100, volume, 0, 0),),
            'unpriced': (0, 0, 0),
            'prints': 1, 'unpriced_prints': 0,
        ...
        self.assertEqual(compact['poc_count'], 1)
        self.assertEqual(decode_exact_int(compact['achieved_mass_num'], compact.get('achieved_mass_num_text')), volume)
        sides = IntegerMass()
        sides.add({
            'rows': ((100, 10, 0, 0), (102, 0, 10, 0)),
            'unpriced': (0, 0, 0),
        ...
        self.assertIsNotNone(side['side_wasserstein_num'])
        self.assertNotEqual(side['side_wasserstein_num'], 0)
    ...
    def test_intended_link_no_event_rate_is_one_half(self):
        dates = (date(2020, 6, 15), date(2020, 6, 16))
        units = []
        windows = []
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for group in intended:
                metric = (group.get('metrics') or {}).get('no_event') or {}
                mean = metric.get('event_mean')
                events = metric.get('eligible_observations')
                if events == 2 and mean == 0.5:
                    found = True
                    break
            self.assertTrue(found)
