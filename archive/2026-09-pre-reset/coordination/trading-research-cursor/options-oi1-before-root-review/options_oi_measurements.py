...
INTERVAL_RECONSTRUCTION = (
    ...
    "Late observations stay pending until a later cut. "
    ...
    def apply_through(self, cut_ns, cut_date):
        ...
        for event in self.pending:
            if event[1] <= cut_ns and event[3] <= cut_date:
                ...
                still.append(event)
        self.pending = still
        applied.sort(key=lambda e: (e[1], e[3], e[0], e[4], e[5]))
        closed = []
        i = 0
        while i < len(applied):
            j = i + 1
            while j < len(applied) and applied[j][0] == applied[i][0] and applied[j][1] == applied[i][1]:
                j += 1
            group = applied[i:j]
            cid, ts = group[0][0], group[0][1]
            pick = min(group, key=lambda item: (item[4], item[5]))
            ambiguous = len({item[2] for item in group}) != 1
            ...
            state = {
                ...
                "ambiguous": ambiguous, "valid_from_ns": ts, "valid_to_ns": None,
...
__all__ = [
    ...
    "eastern_date_from_ns", "intended_cash_dates", "load_calendar", "map_unique_eastern",
    "millistrike", "parse_osi", "previous_intended", "reconstruct_asof",
