...
def run_measurement(packet,protocol,store,outputs):
    ...
    if packet['mode']=='full':
        ...
        gate=quote_full_gate(protocol,admitted,prior,oi_population)
...
def quote_partition_plan(protocol,admitted):
    ...
    cap=protocol['resources'];reserve=1024**3;floor=32*1024**2
...
def quote_full_gate(protocol,admitted,pilot,oi_population):
    all_dates=quote_dates(protocol);selected=set(all_dates[:8]);parts=quote_partition_plan(protocol,admitted)
...
def quote_shard(registered_path,child_path):
    ...
    jobs=quote_partition_plan(protocol,admitted);ordinal=child['ordinal']
...
def run_quote_shards(packet,protocol,admitted,store,outputs):
    helper=quote_parallel_tools(packet);live=helper.live_hardware_limits();cap=protocol['resources'];concurrency=6
    ...
    parts=quote_partition_plan(protocol,admitted);jobs=[];directories=[]
...
def parent(mode,predecessor=None,pilot=None,oi=None):
    ...
    config={'implementation_paths':[] if mode=='admit' else implementation_paths(),'mode':mode,'protocol_sha256':sha,'predecessor':prior,'pilot':pilot_reference,'runner_sha256':runner_sha,
