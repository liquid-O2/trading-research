...
def source_path_shard(source_path, *, shards=SOURCE_SHARDS):
    if type(shards) is not int or shards != SOURCE_SHARDS:
        raise IntegrityError('source-path shards are frozen to eight workers')
    path = _text(source_path, what='source_path')
            ...
            if prior is not None:
                ...
                if planned['unavailable']:
                    ...
                    validation = {
                        ...
                        'instrument_ids': [], 'cut_count': (cut_end - cut_start) // CUT_CADENCE_NS,
                ...
                summary = {
                    ...
                    'cut_end_ns': cut_end,
                    'cut_count': int(validation.get('cut_count', (cut_end - cut_start) // CUT_CADENCE_NS)),
