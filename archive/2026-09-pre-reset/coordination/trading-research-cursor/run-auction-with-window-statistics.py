...
CHECK_EXTENSION = ROOT / 'validation/AUCTION_FLOW_SOURCE_CHECK_EXTENSION_V38.json'
...
REGISTERED_MODES = ('check', 'profile', 'benchmark', 'compute', 'downstream', 'throughput',
                    'clock-audit', 'source-clock', 'extract', 'extract-check', 'observations', 'core-statistics', 'window-links', 'window-statistics')
...
def limits(protocol, mode='check', *, extension=None):
    ...
    if mode == 'window-statistics':
        declared_extension = extension or json.loads(checked(CHECK_EXTENSION)[0])
        operation = load_reference(declared_extension['window_statistics'])
...
def parent(mode='check'):
    ...
    trial = registry.register(name='Acquired auction/flow: ' + (
        ...
        family=protocol['family'], stage='representation' if mode in ('core-statistics','window-statistics') else 'engineering', configuration=configuration, code_hash=snapshot.sha256,
