...
**Source-object implementation snapshot (2026-09-12).** [O161 contract](/workspace/planning/phase-1-live/FORMULAS.md#o161) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Retain source strength, horizon and scale separately from an explicitly labeled regression slope. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/process_observations.py).

**Source-specific evidence limits.** The strength formula is unpublished. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
