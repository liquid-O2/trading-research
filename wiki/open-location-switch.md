...
**Source-object implementation snapshot (2026-09-12).** [O013 contract](/workspace/planning/phase-1-live/FORMULAS.md#o013) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Classify the cash open relative to prior value, prior range, and optional current range while excluding participation context unavailable at use time. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** Participation/RVOL context arriving after use is excluded; open location alone does not supply it. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
