...
**Source-object implementation snapshot (2026-09-12).** [O009 contract](/workspace/planning/phase-1-live/FORMULAS.md#o009) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Measure range width in points, ticks, percent, and prior-width ratio using separately identified denominator parents. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** Source width class remains unavailable unless separately supplied; each optional denominator must have its own parent. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
