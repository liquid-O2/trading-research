...
**Source-object implementation snapshot (2026-09-12).** [O058 contract](/workspace/planning/phase-1-live/FORMULAS.md#o058) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Measure body and volume ratios from one current complete candle plus exactly fourteen causal same-timeframe bars under explicit averaging/reset/equality policies. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** Only the declared 14-period policies are evaluated; insufficient or mismatched history stays unknown. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
