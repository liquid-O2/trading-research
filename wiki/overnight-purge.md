...
**Source-object implementation snapshot (2026-09-12).** [O012 contract](/workspace/planning/phase-1-live/FORMULAS.md#o012) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Retire a selected liquidity reference only after an ordered qualifying native visit in the declared consumption scope. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** The visit rule and consumption scope are source policy; absent policy keeps retirement unknown. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
