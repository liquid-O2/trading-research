...
**Source-object implementation snapshot (2026-09-12).** [O081 contract](/workspace/planning/phase-1-live/FORMULAS.md#o081) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Evaluate poor-high/poor-low structure only under the instrument-specific adjacent-row criterion and compatible source grid. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/auction_geometry.py).

**Source-specific evidence limits.** The one-row NQ criterion is instrument-specific; incompatible grids leave the label unknown. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
