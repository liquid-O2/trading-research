...
**Source-object implementation snapshot (2026-09-12).** [O093 contract](/workspace/planning/phase-1-live/FORMULAS.md#o093) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Keep established and older profiles distinct, then validate the Sires break, older-POC tag, rejection, and source target sequence. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/auction_geometry.py).

**Source-specific evidence limits.** The narrower Sires route cannot alias Saint's return-to-value route or reuse the same profile as both balances. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
