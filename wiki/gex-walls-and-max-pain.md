...
**Source-object implementation snapshot (2026-09-12).** [O036 contract](/workspace/planning/phase-1-live/FORMULAS.md#o036) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Preserve every wall identity, dated price/band, units and selected wall; compute same-unit spot relations and prevent max-pain identity aliasing. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/context_observations.py).

**Source-specific evidence limits.** Wall generation requires the unpublished source engine or actual attributed readouts. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
