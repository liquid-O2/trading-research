...
**Source-object implementation snapshot (2026-09-12).** [O159 contract](/workspace/planning/phase-1-live/FORMULAS.md#o159) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Retain an attributed custom C-score, source unit and context and reject substitution of a z-score. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/process_observations.py).

**Source-specific evidence limits.** The custom score formula is undisclosed. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
