...
**Source-object implementation snapshot (2026-09-12).** [O089 contract](/workspace/planning/phase-1-live/FORMULAS.md#o089) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Measure entry-stop-target distances for the cited Asia-range case while preserving the source range reference and leaving automatic target construction unavailable. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/auction_geometry.py).

**Source-specific evidence limits.** The implementation records cited distances only; it does not derive a general Asia-range target rule. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
