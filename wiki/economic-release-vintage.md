...
**Source-object implementation snapshot (2026-09-12).** [O162 contract](/workspace/planning/phase-1-live/FORMULAS.md#o162) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Resolve the chosen series/reference-period initial or latest available vintage at the cutoff; preserve release history and reject impossible release ordering or ambiguous ties. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/process_observations.py).

**Source-specific evidence limits.** Missing observed availability stays unknown; publication-as-availability remains an explicit assumption. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
