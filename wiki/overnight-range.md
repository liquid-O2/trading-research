...
**Source-object implementation snapshot (2026-09-12).** [O011 contract](/workspace/planning/phase-1-live/FORMULAS.md#o011) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Measure overnight high, low, width, exact window identity, member ownership, and availability; enforce Sires 18:00-09:30 ET when selected. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** Other authors' overnight clocks are accepted only as explicitly identified windows; incomplete tape remains a hole. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
