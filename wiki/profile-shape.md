...
**Source-object implementation snapshot (2026-09-12).** [O085 contract](/workspace/planning/phase-1-live/FORMULAS.md#o085) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Preserve a source profile-shape label and permission, sub-balance/LVN identities, and completed break-retest evidence without inferring direction from shape. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/auction_geometry.py).

**Source-specific evidence limits.** P/b shape direction and trade permission are not inferred automatically. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
