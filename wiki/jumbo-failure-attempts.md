...
**Source-object implementation snapshot (2026-09-12).** [O024 contract](/workspace/planning/phase-1-live/FORMULAS.md#o024) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Count distinct failed attempts at the same level and branch by the decision time and apply the source invalidation/allocation policy separately. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** Three attempts are an invalidation signature only under the admitted source policy and same level/branch identity. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
