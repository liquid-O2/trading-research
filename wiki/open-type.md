...
**Source-object implementation snapshot (2026-09-12).** [O083 contract](/workspace/planning/phase-1-live/FORMULAS.md#o083) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Measure the cash-open path through as-of and preserve a source open-type label as provisional unless elapsed coverage and label timing support it. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/auction_geometry.py).

**Source-specific evidence limits.** Source open type is not automatically classified and remains provisional while the session develops. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
