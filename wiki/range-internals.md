...
**Source-object implementation snapshot (2026-09-12).** [O007 contract](/workspace/planning/phase-1-live/FORMULAS.md#o007) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Compute EQ and 25/75 percent quadrants from one selected positive-width parent range, rejecting caller replacement geometry. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** Pure arithmetic; validity depends entirely on the selected parent range coverage and identity. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
