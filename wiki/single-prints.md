...
**Source-object implementation snapshot (2026-09-12).** [O079 contract](/workspace/planning/phase-1-live/FORMULAS.md#o079) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Find source-selected interior single-print rows and evaluate later repair without treating outer tails as single prints. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/auction_geometry.py).

**Source-specific evidence limits.** Interior band and repair convention are source selections; outer tails are excluded by contract. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
