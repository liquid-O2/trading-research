...
**Source-object implementation snapshot (2026-09-12).** [O084 contract](/workspace/planning/phase-1-live/FORMULAS.md#o084) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Preserve provisional and final auction day-type observations with taxonomy, author, evidence cutoff, and permission reference. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/auction_geometry.py).

**Source-specific evidence limits.** Provisional and final labels never substitute for each other or backdate their evidence. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
