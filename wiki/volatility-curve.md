...
**Source-object implementation snapshot (2026-09-12).** [O044 contract](/workspace/planning/phase-1-live/FORMULAS.md#o044) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Retain the exact selected tenor pair, comparable units and chosen difference/ratio; include an event change only after its actual publication. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/context_observations.py).

**Source-specific evidence limits.** Curve shape and event interpretation require the source observation; no tenor substitution occurs. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
