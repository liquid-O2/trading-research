...
**Source-object implementation snapshot (2026-09-12).** [O014 contract](/workspace/planning/phase-1-live/FORMULAS.md#o014) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Project the configured exhaustion ladder from one frozen range and measure observed sweep depth independently of the source-selected reversal band. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** The ladder is formulaic; author-selected reversal band and coordinate convention are not auto-discovered. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
