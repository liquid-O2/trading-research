...
**Source-object implementation snapshot (2026-09-12).** [O037 contract](/workspace/planning/phase-1-live/FORMULAS.md#o037) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Retain max-pain identity separately from same-price walls and calculate only the stated difference. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/context_observations.py).

**Source-specific evidence limits.** The payout engine and terminal prediction are not inferred from a supplied max-pain label. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
