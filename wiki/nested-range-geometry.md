# Nested source range geometry

Object in [JJumboFX — SDRange / Time-Based Ranges](method-jumbo-tbr.md).

Later charts layer inner profile/range spans inside the outer 6–9 framework and project the selected span independently. These are refinements of the same range loop. [JR] pp.11, 14 and the May 2026 charts; posts 2055344660986364371 / 2026059018750378427.

**Not a standalone trade.** An inner gray box or blue line is not automatically outer-range EQ or EV. Layering is not evidence of another operating method.

**Record before use.** Outer and inner range IDs, literal source labels, independent bounds/widths, projection parent, version, drawing/known_at and unresolved construction.

**Phase 1 observation.** A figure-annotated span can support a fixture observation. Automatic generation remains unknown without its source construction; never silently substitute the outer width.

**Source-object implementation snapshot (2026-09-12).** [O016 contract](/workspace/planning/phase-1-live/FORMULAS.md#o016) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Preserve distinct outer and inner range identities, compute both widths/midpoints, and bind later projection to the selected span. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/range_geometry.py).

**Source-specific evidence limits.** Nested geometry does not infer an inner span, labels, or projection parent. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.

**Related objects.** [Jumbo's 06:00–09:00 range](tbr-6-9-range.md) · [The 1.33–1.66 extension area](extensions-1-33-1-66.md) · [Jumbo EVRange](ev-range-expected-move.md) · [Volume profile](value-and-profiles.md)

[Index](index.md) · [Observation contract](source-sequence-fidelity.md) · [Source map](source-catalog.md)

[JR]: </workspace/sources/x-raw-2026-09-11/JJumboFX_Raw_X_Archive_v2.pdf>
[FORMULAS]: </workspace/planning/phase-1-live/FORMULAS.md>
