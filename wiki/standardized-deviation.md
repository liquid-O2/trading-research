...
**Source-object implementation snapshot (2026-09-12).** [O160 contract](/workspace/planning/phase-1-live/FORMULAS.md#o160) is complete, with executable checks passing in the [object review](/workspace/implementation/validation/phase1-completion/obligation-matrix.json). Compute selected prior-record mean, sample/population scale, raw deviation and standardized deviation with distinct series IDs, causal vintages and a zero-scale case. [Implementation](/workspace/implementation/src/trading_research/research/method_pack/objects/process_observations.py).

**Source-specific evidence limits.** The source baseline and variance convention must be observed; no fitted baseline search is performed. See [current status](current-status.md) for the separate historical comparison scope; implementation completion does not establish a source trade or its performance.
