"""Register bounded admission and measurement of the acquired daily volatility sources."""
...
PROTOCOL=ROOT/'validation/DAILY_VOLATILITY_COMPLEX_MEASUREMENT_V1.json'
RUNS=ROOT/'reports/daily-volatility-runs'
...
def admit_sources(protocol,store,progress_path):
    ...
    for source in protocol['source_files']:
        ...
        ref=asdict(store.put_bytes(raw,kind='daily_volatility_original_source'))
    ...
    return {'kind':'daily_volatility_admitted_sources_v1','sources':records,'source_files':len(records),
...
def worker(packet_path):
    ...
    if (attempt['status']!='running' or attempt['trial_id']!=packet['trial_id'] or attempt['family']!=protocol['family']
        ...
        raise ValueError('daily volatility worker does not identify its registered supervisor and attempt')
    ...
    if packet['mode']=='admit':
        ...
        from trading_research.research.daily_volatility_complex import run
        actual=run(packet=packet,protocol=protocol,root=ROOT,store=store)
        actual_ref=asdict(store.put_json(actual,kind='daily_volatility_measurement_results_v1'))
...
def parent(mode,predecessor=None):
    ...
    for attempt in state['attempts'].values():
        ...
        for ref in attempt.get('result_artifacts',[]):
            if ref['kind']=='daily_volatility_execution_v1':used+=store.read_json(artifact_ref(ref))['output_bytes']
    ...
    if predecessor:
        ...
        if (d.get('success') is not True or d.get('mode')!='admit' or attempt is None or attempt['status']!='succeeded'
                or not any(r['sha256']==digest(d) for r in attempt['result_artifacts'])):raise ValueError('daily-volatility measurement needs exactaccepted admission')
        prior={'path':str(path),'sha256':hashlib.sha256(prior_raw).hexdigest(),'size_bytes':len(prior_raw)}
    if mode=='measure' and prior is None:raise ValueError('daily-volatility measurement needsadmission')
    snapshot=code_snapshot(ROOT,store);runner_sha=hashlib.sha256(checked(__file__)).hexdigest()
    config={'mode':mode,'protocol_sha256':sha,'predecessor':prior,'runner_sha256':runner_sha,
        'thread_limits':{'OMP_NUM_THREADS':'1','OPENBLAS_NUM_THREADS':'1','MKL_NUM_THREADS':'1','ARROW_NUM_THREADS':'1'}}
    trial=registry.register(name='daily-volatility:'+mode,family=protocol['family'],stage='original' if mode=='admit' else 'representation',
        configuration=config,code_hash=snapshot.sha256,data_hashes={'protocol':sha},fold_version='2020plus-daily-volatility-v1',target_version='daily-volatility-curve-observation-v1')
    ...
    try:
        ...
        worker_ref=None if worker_value is None else asdict(store.put_json(worker_value,kind='daily_volatility_worker_v1'))
        ...
        report={'kind':'daily_volatility_execution_v1','family':protocol['family'],'mode':mode,'attempt_id':attempt,'trial_id':trial,
        ...
        ref=store.put_json(report,kind=report['kind']);registry.finish(attempt,status='succeeded' if success else 'failed',cpu_seconds=cpu,
            wall_seconds=wall,peak_rss_bytes=usage.ru_maxrss*1024,reason='bounded daily volatility study result; all failures retained',result_artifacts=(asdict(ref),))
        ...
        if not finished:
            ...
            registry.finish(attempt,status='interrupted',cpu_seconds=0 if process is None else None if usage is None else usage.ru_utime+usage.ru_stime,
                wall_seconds=time.monotonic()-started,peak_rss_bytes=0 if usage is None else usage.ru_maxrss*1024,reason='daily volatility supervisor failure; reservation retained if usageunknown')


if __name__=='__main__':
    if len(sys.argv)==3 and sys.argv[1]=='--worker':raise SystemExit(worker(sys.argv[2]))
    if len(sys.argv) not in (2,3) or sys.argv[1] not in ('admit','measure'):raise SystemExit('usage: run_daily_volatility_study.py admit|measure [admissionexecution]')
