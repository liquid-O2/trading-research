...
BASE_SHA = 'ce02a5122e5277a7667196e573cb4e8c30414cd8fd3dfed2b7700c07aacbe0ad'
...
def diagnose(packet, protocol, store, outputs):
    ...
    try:
        ...
        run(protocol=protocol, admitted=admitted, oi_population=oi, store=store, outputs=outputs,
            selected_dates=base.quote_dates(protocol)[:8], selected_chains=['SPXW'])
        completed = True
    ...
    ref = outputs.json('profile.json', {
        ...
        'bound_cpu_seconds':60, 'selected_chains':['SPXW'], 'base_runner_sha256':BASE_SHA,
        'functions':sorted(rows,key=lambda row:-row['cumulative_seconds']),
        'completed_selected_population':completed,
        'note':'Unchanged implementation, actual source measurements only. Tests already passed46 inpilot12; this skips tests and is diagnostic only, never a pilot/full gate.',
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        raise SystemExit('usage: run_options_quote_source_profile13.py admission-execution oi-execution')
