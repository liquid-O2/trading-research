...
MODES = ("check", "pilot", "admit", "develop", "confirm", "timing-pilot", "timing-full")
TIMING_CONTRACT = ROOT / "validation/JUMBO_DELIVERABLE1_TIMING_V1.json"
...
def worker(packet_path):
    ...
    try:
        ...
        if (attempt["status"] != "running" or attempt["trial_id"] != packet["trial_id"]
                ...
                or trial["configuration"].get("timing_contract") != packet.get("timing_contract")
        ...
        if packet['mode'].startswith('timing-'):
            checked(TIMING_CONTRACT, packet['timing_contract']['sha256'])
            from trading_research.research.jumbo_timing_entry import run
            report = run(packet, protocol, ROOT)
            success = report.get('success') is True
        elif packet["mode"] == "check":
            import unittest
            ...
            if packet.get('descriptive_confirmation') is True:
                from trading_research.research.jumbo_descriptive_confirmation import run
            else:
        ...
        report["timing_contract"] = packet.get("timing_contract")
...
def parent(mode, parent_report=None, phase=None, resource_authorization=None):
    ...
    timing_mode = mode.startswith("timing-")
    phase = phase or {"check": "code_check", "pilot": "admission_pilot", "admit": "full_admission",
                      "develop": "extract", "confirm": "confirmation", "timing-pilot": "extract", "timing-full": "confirmation"}[mode]
    if phase not in {"check": ("code_check",), "pilot": ("admission_pilot",), "admit": ("full_admission",),
                     "develop": ("extract", "fit"), "confirm": ("confirmation",), "timing-pilot": ("extract",), "timing-full": ("confirmation",)}[mode]:
    ...
    timing_ref = None
    if timing_mode:
        timing_raw, _ = checked(TIMING_CONTRACT)
        timing_ref = asdict(store.put_bytes(timing_raw, kind='jumbo_deliverable1_timing_contract_v1'))
        contracts['timing']['scientific_contract'] = timing_ref
        contracts['timing']['id'] = digest({k:v for k,v in contracts['timing'].items() if k != 'id'})
    predecessor = None
    state = registry.state()
    if parent_report:
    ...
    required = ("develop" if mode == "develop" and phase == "fit" else
                {"pilot": "check", "admit": "pilot", "develop": "admit", "confirm": "develop", "timing-pilot": "confirm", "timing-full": "timing-pilot"}.get(mode))
    ...
    if mode == 'timing-full':
        if (predecessor.get('verification_contracts', {}).get('timing') != contracts['timing']
                or predecessor.get('tools_snapshot') != asdict(tool_snapshot)):
            raise ValueError('timing full requires exact successful numerical/test/scientific/tool identity of NQ pilot')
    if mode != "check" and not timing_mode:
    ...
    configuration = {"mode": mode, "protocol_sha256": protocol_sha, "timing_contract": timing_ref,
    ...
    trial = registry.register(name=f"{protocol['study_id']}:{mode}:{phase}", family=protocol["family"],
                              stage="representation" if timing_mode else "engineering" if mode == "check" else "model" if mode in ("develop", "confirm") else "original",
    ...
    packet = {"mode": mode, "trial_id": trial, "attempt_id": attempt, "timing_contract": timing_ref,
              "resource_amendment": amendment_ref,
    ...
    try:
        ...
        joined = worker_value is not None and all(worker_value.get(k) == packet[k] for k in
                  ("trial_id", "attempt_id", "protocol_sha256", "code_snapshot", "tools_snapshot", "mode", "phase", "analysis_plan", "execution_plan", "model_plan", "verification_contracts", "resource_amendment", "hardware_execution", "descriptive_confirmation", "timing_contract"))
        ...
        report = {"family": protocol["family"], "mode": mode, "trial_id": trial, "attempt_id": attempt, "timing_contract": timing_ref,
