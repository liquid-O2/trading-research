...
from trading_research.research.options_oi_measurements import (
    ...
    dte_days, eastern_date_from_ns, intended_cash_dates, iter_logical_lifecycle,
    iter_logical_reports, millistrike, parse_osi, previous_intended, read_logical_parts,
    reconstruct_asof_intervals, resolve_contract_report, run,
...
from trading_research.research.options_oi_columnar import (
    COMPACT_LIFECYCLE_KIND, compact_lifecycle_schema, compact_report_schema,
)
...
def _read_parts(refs):
    return read_logical_parts(refs)
...
class ColumnarRoundtripTests(unittest.TestCase):
    def test_compact_report_last_eq_first_restores_last_endpoint(self):
        import pyarrow as pa
        import pyarrow.parquet as pq
        schema = compact_report_schema()
        table = pa.table({
            "report_id": [1], "contract_id": [0], "chain": ["NDX"], "request_date": ["2020-01-02"],
            "listed": [True], "first_oi": [100], "first_ts_event_ns": [10],
            "first_file_id": [1], "first_row_index": [0],
            "last_oi": [None], "last_ts_event_ns": [None], "last_file_id": [None],
            "last_row_index": [None], "alias_multiplicity": [1], "same_time_conflict": [False],
            "conflict_disposition": ["none"], "update_candidate": [False],
            "observed_update_difference": [None], "certified_revision": [False],
            "usable_for_statistics": [True], "n_source_rows": [1], "n_distinct_times": [1],
            "support_observations": [1], "dte": [15], "dte_bucket": ["8-30"], "right": ["CALL"],
            "causal_feature_eligible": [False], "last_eq_first": [True],
        }, schema=schema)
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / "reports-0000.parquet"
            pq.write_table(table, path)
            rows = list(iter_logical_reports([{"path": str(path), "kind": "options_oi_resolved_reports_v3"}]))
        self.assertEqual(rows[0]["first_oi"], 100)
        self.assertEqual(rows[0]["last_oi"], 100)
        self.assertEqual(rows[0]["last_ts_event_ns"], 10)
        self.assertEqual(rows[0]["last_file_id"], 1)
        self.assertEqual(rows[0]["last_row_index"], 0)

    def test_compact_lifecycle_reconstructs_endpoints_from_report_ids(self):
        import pyarrow as pa
        import pyarrow.parquet as pq
        reports = pa.table({
            "report_id": [10, 11], "contract_id": [0, 0], "chain": ["NDX", "NDX"],
            "request_date": ["2020-01-02", "2020-01-03"], "listed": [True, True],
            "first_oi": [100, 120], "first_ts_event_ns": [10, 30],
            "first_file_id": [1, 2], "first_row_index": [0, 0],
            "last_oi": [None, None], "last_ts_event_ns": [None, None],
            "last_file_id": [None, None], "last_row_index": [None, None],
            "alias_multiplicity": [1, 1], "same_time_conflict": [False, False],
            "conflict_disposition": ["none", "none"], "update_candidate": [False, False],
            "observed_update_difference": [None, None], "certified_revision": [False, False],
            "usable_for_statistics": [True, True], "n_source_rows": [1, 1],
            "n_distinct_times": [1, 1], "support_observations": [1, 1],
            "dte": [15, 14], "dte_bucket": ["8-30", "8-30"], "right": ["CALL", "CALL"],
            "causal_feature_eligible": [False, False], "last_eq_first": [True, True],
        }, schema=compact_report_schema())
        life = pa.table({
            "contract_id": [0], "chain": ["NDX"], "prior_date": ["2020-01-02"],
            "next_date": ["2020-01-03"], "prior_report_id": [10], "next_report_id": [11],
            "first_delta": [20], "first_abs_delta": [20], "first_censor": [None],
            "first_usable": [True], "first_within_stage": [True], "first_within_year": [True],
            "last_delta": [20], "last_abs_delta": [20], "last_censor": [None],
            "last_usable": [True], "last_within_stage": [True], "last_within_year": [True],
            "appearance": ["continuing"], "listed_on_prior": [True],
            "certified_listing_birth": [False], "intraday_terminal_holdings_identified": [False],
            "cross_gap_compressed": [False], "is_boundary": [False],
            "prior_stage": ["training"], "next_stage": ["training"],
            "causal_feature_eligible": [False],
        }, schema=compact_lifecycle_schema())
        self.assertNotIn("first_prior_oi", life.schema.names)
        with tempfile.TemporaryDirectory() as folder:
            rpath = Path(folder) / "reports-0000.parquet"
            lpath = Path(folder) / "lifecycle-0000.parquet"
            pq.write_table(reports, rpath)
            pq.write_table(life, lpath)
            rows = list(iter_logical_lifecycle(
                [{"path": str(lpath), "kind": COMPACT_LIFECYCLE_KIND}],
                [{"path": str(rpath), "kind": "options_oi_resolved_reports_v3"}],
            ))
        self.assertEqual(rows[0]["first_prior_oi"], 100)
        self.assertEqual(rows[0]["first_next_oi"], 120)
        self.assertEqual(rows[0]["last_prior_oi"], 100)
        self.assertEqual(rows[0]["last_next_oi"], 120)
        self.assertEqual(rows[0]["first_delta"], 20)
        self.assertEqual(rows[0]["first_prior_file_id"], 1)
        self.assertEqual(rows[0]["first_next_file_id"], 2)

    def test_run_writes_compact_lifecycle_and_logical_roundtrip(self):
        import pyarrow.parquet as pq
        day_a, day_b = date(2020, 1, 2), date(2020, 1, 3)
        exp = date(2020, 1, 17)
        ts_a = local_timestamp(day_a, time(6, 30), ZONE)
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            sources = [
                _admit(1, ndx_c_a, "NDX", "contracts", "2020-01-02",
                       "thetadata-opra/opra__ndx-options__contracts", "contracts",
                       _write_parquet(root / ndx_c_a, _contracts([("NDX", exp, 3400.0, "CALL", day_a, NDX_OSI)]))),
                _admit(2, ndx_o_a, "NDX", "open_interest", "2020-01-02",
                       "thetadata-opra/opra__ndx-options__open-interest", "oi",
                       _write_parquet(root / ndx_o_a, _oi([("NDX", exp, 3400.0, "CALL", 100, day_a, ts_a, NDX_OSI)]))),
                _admit(3, ndx_c_b, "NDX", "contracts", "2020-01-03",
                       "thetadata-opra/opra__ndx-options__contracts", "contracts",
                       _write_parquet(root / ndx_c_b, _contracts([("NDX", exp, 3400.0, "CALL", day_b, NDX_OSI)]))),
                _admit(4, ndx_o_b, "NDX", "open_interest", "2020-01-03",
                       "thetadata-opra/opra__ndx-options__open-interest", "oi",
                       _write_parquet(root / ndx_o_b, _oi([("NDX", exp, 3400.0, "CALL", 120, day_b, ts_b, NDX_OSI)]))),
            ]
            admitted = {
                "kind": "options_oi_admitted_sources_v1", "source_files": 4, "sources": sources,
                "schemas": {"contracts": CONTRACT_SCHEMA, "oi": OI_SCHEMA},
                "source_bytes": sum(item["source"]["size_bytes"] for item in sources),
                "rows_by_role": {"contracts": None, "open_interest": None},
            }
            protocol = dict(PROTOCOL, data_root=str(root))
            outputs = BoundedOutputs(Path(folder) / "out", maximum_total_bytes=32 * 1024 * 1024,
                                     maximum_file_bytes=16 * 1024 * 1024)
            result = run(protocol=protocol, admitted=admitted, outputs=outputs,
                         selected_dates=["2020-01-02", "2020-01-03"])
            life_refs = result["refs"]["lifecycle"]
            physical = pq.read_table(life_refs[0]["path"])
            self.assertIn("prior_report_id", physical.schema.names)
            self.assertNotIn("first_prior_oi", physical.schema.names)
            self.assertEqual(life_refs[0]["kind"], COMPACT_LIFECYCLE_KIND)
            reconstructed = _read_parts(life_refs)
            ndx = [row for row in reconstructed if row["chain"] == "NDX" and row["first_delta"] == 20]
            self.assertTrue(ndx)
            self.assertEqual(ndx[0]["first_prior_oi"], 100)
            self.assertEqual(ndx[0]["first_next_oi"], 120)
            reports = _read_parts(result["refs"]["reports"])
            self.assertTrue(any(row["first_oi"] == 100 and row["last_oi"] == 100 for row in reports))
            self.assertTrue(any(row["first_oi"] == 120 and row["last_oi"] == 120 for row in reports))
...
class RootBoundedSmallTablesTests(unittest.TestCase):
    def test_timestamp_maps_are_request_date_bounded_with_exact_clocks(self):
        from datetime import timedelta
        import numpy as np
        from trading_research.research.options_oi_columnar import ClockMaps
        clocks = ClockMaps()
        clocks.osi['retained-contract'] = ('identity',)
        midnight_cache = {}
        for offset in range(512):
            day = date(2020, 1, 2) + timedelta(days=offset)
            label = day.isoformat()
            clocks.begin_request_date(label)
            midnight = cut_ns_on(label, '00:00')
            stamps = midnight + np.arange(1, 257, dtype=np.int64)
            labels = clocks.eastern_labels(stamps)
            seconds, valid = clocks.seconds_after(stamps, labels, midnight_cache)
            self.assertEqual(labels.tolist(), [label] * 256)
            self.assertTrue(valid.all())
            np.testing.assert_array_equal(seconds, np.arange(1, 257) / 1_000_000_000)
            self.assertEqual(len(clocks.east), 256)
            self.assertEqual(len(clocks.seconds), 256)
        self.assertEqual(clocks.osi, {'retained-contract': ('identity',)})
        clocks.begin_request_date('2020-03-08')
        stamps = np.array([1583649000000000000, 1583652600000000000], dtype=np.int64)
        labels = clocks.eastern_labels(stamps)
        seconds, valid = clocks.seconds_after(stamps, labels, midnight_cache)
        self.assertEqual(labels.tolist(), ['2020-03-08', '2020-03-08'])
        self.assertEqual(seconds.tolist(), [5400.0, 9000.0])
        self.assertTrue(valid.all())
