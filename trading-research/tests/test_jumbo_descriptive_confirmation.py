...
from trading_research.research import jumbo_descriptive_confirmation as study
from trading_research.research import jumbo_study as original_study
...
class DescriptiveConfirmationDispatchTests(unittest.TestCase):
    ...
    def test_fitted_predecessor_retains_model_confirmation_route(self):
        ...
        saved = sys.modules.get("trading_research.research.jumbo_pipeline")
        try:
            with patch.object(study, "_run_extraction") as extract, patch.object(original_study, "ArtifactStore", return_value=self.store), patch.dict(sys.modules, {"trading_research.research.jumbo_pipeline": fake}):
                result = original_study.run(self.packet(self.fit_identity, descriptive=False), {"study_id": "literal"}, self.root)
        finally:
            if saved is not None:
                sys.modules["trading_research.research.jumbo_pipeline"] = saved
            else:
...
class DescriptiveConfirmationRegistrationTests(unittest.TestCase):
    def test_fit_predecessor_does_not_label_a_develop_fit_run_as_description(self):
        predecessor = {'success': True, 'mode': 'develop', 'phase': 'extract'}
        self.assertFalse(verification.descriptive_confirmation_from_predecessor(predecessor, mode='develop'))
        self.assertFalse(verification.authenticate_descriptive_confirmation(False, predecessor, mode='develop'))
        with self.assertRaises(ValueError):
            verification.authenticate_descriptive_confirmation(True, predecessor, mode='develop')
