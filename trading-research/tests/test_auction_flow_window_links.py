...
class PartialSourceCutBoundaryRegression(unittest.TestCase):
    def test_exact_terminal_source_endpoint_keeps_all_grid_cuts_before_it(self):
        from trading_research.research.auction_flow_window_links import _validate_cuts
        start = 1788393600000000000
        end = 1788415799901114378
        acquired = source_identity(start=start, end=end)
        actual_start, actual_end, cuts = _validate_cuts(start, end, acquired)
        self.assertEqual((actual_start, actual_end), (start, end))
        self.assertEqual(len(cuts), 74)
        self.assertEqual(int(cuts[0]), 1788393600000000000)
        self.assertEqual(int(cuts[-1]), 1788415500000000000)
        self.assertLess(int(cuts[-1]), end)
        self.assertTrue(all(int(value) % 300000000000 == 0 for value in cuts))

    def test_partial_envelope_is_half_open_and_does_not_shift_grid(self):
        from trading_research.research.auction_flow_window_links import _validate_cuts
        from trading_research.errors import ContractError
        start = 1788393600000000000
        acquired = source_identity(start=start, end=start + 86400000000000)
        self.assertEqual(_validate_cuts(start, start + 300000000000, acquired)[2].tolist(), [start])
        self.assertEqual(_validate_cuts(start, start + 300000000001, acquired)[2].tolist(), [start, start + 300000000000])
        with self.assertRaises(ContractError):
            _validate_cuts(start + 1, start + 300000000001, acquired)
