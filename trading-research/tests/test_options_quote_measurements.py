...
class QuoteFixtureCase(unittest.TestCase):
    ...
    def test_nanoseconds_over_2_53_roundtrip_and_wrong_unit_rejects(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            sources = [_admit(1, rel, "NDX", "near", "2020-01-02",
                              ...
                              "quote_1m", _write_parquet(root / rel, table))]
            result = self._run(folder, root, sources, ["2020-01-02"], ["NDX"])
            rows = _read_parts(result["refs"]["cut_board"])
            self.assertFalse(_near_quoted(rows, cut="10:00"))
            board = _near_quoted(rows, cut="15:00")
    ...
    def test_unequal_quote_counts_give_independent_date_and_raw_means(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            aggregates = load_date_aggregates(result['refs']['date_aggregates'])
    ...
    def test_weighted_histogram_matches_literal_order_statistics_without_expansion(self):
        from trading_research.research.options_quote_statistics import exact_quantiles_from_histogram, _histogram_total
        q = exact_quantiles_from_histogram([10, 2, 0, 5, 2], [2, 1, 2, 1, 2], [0, .25, .5, .75, 1])
        self.assertEqual(q, {'0': 0.0, '0.25': 1.5, '0.5': 2.0, '0.75': 6.25, '1': 10.0})
        self.assertEqual(_histogram_total([10, 2, 0, 5, 2], [2, 1, 2, 1, 2]), (31.0, 8))
        self.assertEqual(exact_quantiles_from_histogram([0, 10], [2_000_000_000, 2_000_000_000], [.5]),
                         {'0.5': 5.0})
        self.assertEqual(exact_quantiles_from_histogram([None, float('nan'), 4, 9], [1, 1, 2, 0], [.5]),
                         {'0.5': 4.0})

    def test_bounded_histograms_keep_daily_canonical_family_and_exact_ranks(self):
        from trading_research.research.options_quote_statistics import (
            HistogramUniverse, _bundle_for, _cfg, _summarize,
            write_date_aggregates, load_date_aggregates, _merge_exact_histograms,
        )
        import numpy as np
        def hist(pop, family, value, count=1):
            return {f'bid|{pop}|{family}|-|call|0': {'v': [value], 'c': [count]}}
        first = {'chain': 'NDX', 'request_date': '2020-01-02', 'histograms': {
            **hist('raw_source', 'near', 10), **hist('raw_source', 'broad', 100),
            **hist('raw_source', 'union', 999), **hist('unique_source', 'near', 10),
            **hist('unique_source', 'broad', 100), **hist('unique_source', 'union', 5)}}
        second = {'chain': 'NDX', 'request_date': '2020-01-03', 'histograms': {
            **hist('raw_source', 'near', 2, 3), **hist('unique_source', 'near', 2, 3)}}
        dates = ['2020-01-02', '2020-01-03', '2020-01-06']
        cfg = _cfg(PROTOCOL)
        with tempfile.TemporaryDirectory() as folder:
            ref = write_date_aggregates(self._outputs(folder), [first, second])
            lazy = load_date_aggregates(ref, materialize_histograms=False)
            self.assertTrue(all('histograms' not in row and 'histograms_ref' in row for row in lazy))
            eager = load_date_aggregates(ref)
            self.assertEqual([row['histograms'] for row in eager], [first['histograms'], second['histograms']])
            pool = HistogramUniverse(lazy, set(dates), cfg['quantiles'])
            self.assertEqual(pool.loaded_days, 2)
            self.assertEqual(pool.daily_totals('bid', 'raw_source', 'all', 'all', 'all'),
                             {'2020-01-02': (110.0, 2), '2020-01-03': (6.0, 3)})
            self.assertEqual(pool.daily_totals('bid', 'unique_source', 'all', 'all', 'all'),
                             {'2020-01-02': (5.0, 1), '2020-01-03': (6.0, 3)})
            metrics, events, means, quantiles = _bundle_for(lazy, set(dates), family='all',
                cut='all', right='all', dte='all', cfg=cfg, histograms=pool)
            stats = _summarize(metrics, dates, cfg, event_counts=events,
                raw_means=means, raw_quantiles=quantiles)['metrics']
            self.assertEqual(stats['mean_bid_raw']['estimate'], 28.5)
            self.assertEqual(stats['mean_bid_raw']['raw_mean'], 23.2)
            self.assertEqual(stats['mean_bid_raw']['raw_quantiles']['0.5'], 2.0)
            self.assertEqual(stats['mean_bid_raw']['actual_valid_event_count'], 5)
            self.assertEqual(stats['mean_bid_raw']['missing_date_count'], 1)
            self.assertEqual(stats['mean_bid_unique']['estimate'], 3.5)
            self.assertEqual(stats['mean_bid_unique']['raw_mean'], 2.75)
            self.assertEqual(stats['mean_bid_unique']['raw_quantiles']['0.75'], 2.75)
            self.assertEqual(pool.summary('bid', 'raw_source', 'all', '10:00', 'all', 'all'),
                             pool.summary('bid', 'raw_source', 'all', 'cash_close', 'all', 'all'))
            # Authenticating metadata alone cannot certify a mutated nested payload.
            Path(lazy[0]['histograms_ref']['path']).write_bytes(b'corrupt')
            with self.assertRaises(IntegrityError):
                HistogramUniverse(lazy, set(dates), cfg['quantiles'])
        values, counts = _merge_exact_histograms(
            (np.array([0.1]), np.array([2_000_000_000], dtype=np.int64)),
            (np.array([0.1, 0.10000000000000002]), np.array([2_000_000_000, 1], dtype=np.int64)))
        self.assertEqual(values.tolist(), [0.1, 0.10000000000000002])
        self.assertEqual(counts.tolist(), [4_000_000_000, 1])

    def test_source_integer_grouping_preserves_joint_keys_and_inverse(self):
        import numpy as np
        from trading_research.errors import ContractError
        from trading_research.research.options_quote_measurements import _lex_unique_integer_rows, _right_dte_codes
        raw = np.array([[4, -1, 2**53 + 1, 0, 3, 6], [4, -1, 2**53 + 1, 0, 3, 5],
                        [-3, 7, -1, -1, 2, -9], [4, -1, 2**53 + 1, 0, 3, 6]], dtype=np.int64)
        unique, inverse = _lex_unique_integer_rows(tuple(raw[:, i] for i in range(raw.shape[1])))
        self.assertEqual(unique.tolist(), [[-3, 7, -1, -1, 2, -9],
            [4, -1, 2**53 + 1, 0, 3, 5], [4, -1, 2**53 + 1, 0, 3, 6]])
        self.assertEqual(inverse.tolist(), [2, 1, 0, 2])
        np.testing.assert_array_equal(unique[inverse], raw)
        empty, inverse = _lex_unique_integer_rows((np.empty(0, dtype=np.int64),) * 2)
        self.assertEqual(empty.shape, (0, 2))
        self.assertEqual(inverse.size, 0)
        with self.assertRaises(ContractError):
            _right_dte_codes(np.array([65536]), np.array([1]))

    def test_source_quality_counts_physical_rows_and_kept_conflicts_per_joint_group(self):
        from trading_research.research.options_quote_measurements import _empty_frame, _source_quality_rows
        frame = _empty_frame(5)
        for value in frame.values():
            if hasattr(value, 'fill'):
                value.fill(0)
        frame['right_code'][:] = [0, 0, 0, 1, -1]
        frame['dte_code'][:] = [1, 1, 1, 4, -1]
        frame['raw_base'][:] = [8, 8, 2, 8, 4]
        frame['keep'][:] = [True, False, True, True, True]
        frame['unique_conflict'][:] = [True, False, True, False, False]
        frame['condition_zero'][:] = [False, True, True, False, False]
        frame['bid_null'][:] = [False, False, True, False, False]
        frame['ts_ok'][:] = [True, True, False, True, True]
        frame['multiplicity'][:] = [100, 100, 1, 1, 1]
        rows = _source_quality_rows(frame, 'NDX', '2020-01-02', 'near')
        indexed = {(r['right'], r['dte_bucket']): r for r in rows}
        self.assertEqual(set(indexed), {('CALL', '0'), ('PUT', '8-14'), ('unknown', 'unknown')})
        call = indexed[('CALL', '0')]
        for key, expected in {'raw_rows': 3, 'unique_events': 2, 'raw_two_sided': 2,
            'raw_invalid_numeric': 1, 'unique_conflict': 1, 'unique_invalid_numeric': 1,
            'unique_two_sided': 0, 'raw_condition_zero': 2, 'unique_condition_zero': 1,
            'missing_bid': 1, 'missing_ts': 1}.items():
            self.assertEqual(call[key], expected, key)
        self.assertEqual(indexed[('PUT', '8-14')]['unique_two_sided'], 1)
        self.assertEqual(indexed[('unknown', 'unknown')]['unique_all_zero'], 1)

    def test_typed_source_histograms_preserve_masked_float_values_and_large_counts(self):
        import numpy as np
        from trading_research.research.options_quote_measurements import _hist_batch, _hist_merge, _freeze_hists
        store = {}
        _hist_batch(store, 'bid', 'raw_source', 'near', '-',
            np.array([0, 0, 0, 1, -1, 1]), np.array([1, 1, 1, 4, -1, 4]),
            np.array([0.1, 0.10000000000000002, 0.1, 7, float('nan'), 9]),
            np.array([True, True, True, True, True, False]))
        hists = _freeze_hists(store)
        self.assertEqual(set(hists), {'bid|raw_source|near|-|CALL|0', 'bid|raw_source|near|-|PUT|8-14'})
        call = hists['bid|raw_source|near|-|CALL|0']
        self.assertEqual(call['v'].tolist(), [0.1, 0.10000000000000002])
        self.assertEqual(call['c'].tolist(), [2, 1])
        self.assertEqual(hists['bid|raw_source|near|-|PUT|8-14']['v'].tolist(), [7.0])
        _hist_merge(store, 'large', [1.0], [2_000_000_000])
        _hist_merge(store, 'large', [1.0], [2_000_000_000])
        self.assertEqual(store['large'][1].tolist(), [4_000_000_000])

    def test_pooled_shards_match_daily_histograms_and_reject_partial_date_substitution(self):
        from trading_research.research.options_quote_statistics import HistogramUniverse
        from trading_research.research.auction_flow_storage import read_json_artifact
        dates = {'2020-01-02', '2020-01-03'}
        rows = [
            {'chain': 'NDX', 'request_date': '2020-01-02', 'histograms': {
                'bid|raw_source|near|-|CALL|0': {'v': [1., 2.], 'c': [2, 1]},
                'bid|unique_source|union|-|CALL|0': {'v': [1., 2.], 'c': [1, 1]}}},
            {'chain': 'NDX', 'request_date': '2020-01-03', 'histograms': {
                'bid|raw_source|near|-|CALL|0': {'v': [2., 3.], 'c': [1, 3]},
                'bid|unique_source|near|-|CALL|0': {'v': [2., 3.], 'c': [1, 2]}}},
        ]
        for row in rows:
            row['histograms']['ask|raw_source|near|-|CALL|0'] = dict(row['histograms']['bid|raw_source|near|-|CALL|0'])
        baseline = HistogramUniverse(rows, dates)
        with tempfile.TemporaryDirectory() as folder:
            outputs = self._outputs(folder)
            reference = baseline.write_pool(outputs, chain='NDX', name='combined', maximum_part_bins=1)
            self.assertLess(reference['stored_bins'],reference['pooled_bins'])
            compact = [{k: v for k, v in row.items() if k != 'histograms'} |
                       {'pooled_histograms_ref': reference} for row in rows]
            loaded = HistogramUniverse(compact, dates)
            independent = []
            for i, row in enumerate(rows):
                pool = HistogramUniverse([row], {row['request_date']})
                ref = pool.write_pool(outputs, chain='NDX', name=f'one-day-{i}')
                independent.append({'chain': row['chain'], 'request_date': row['request_date'],
                                    'pooled_histograms_ref': ref})
            combined = HistogramUniverse(independent, dates)
            self.assertEqual(loaded.loaded_days, 2)
            self.assertEqual(loaded.daily, baseline.daily)
            self.assertEqual(combined.daily, baseline.daily)
            for pop in ('raw_source', 'unique_source'):
                for family in ('all', 'near', 'union'):
                    args = ('bid', pop, family, 'all', 'all', 'all')
                    self.assertEqual(loaded.summary(*args), baseline.summary(*args))
                    self.assertEqual(combined.summary(*args), baseline.summary(*args))
            q, (total, count) = loaded.summary('bid', 'raw_source', 'all', 'all', 'all', 'all')
            self.assertEqual((total, count), (15.0, 7))
            self.assertEqual(q['0.5'], 2.0)
            self.assertEqual(loaded.summary('ask','raw_source','all','all','all','all')[1],(15.0,7))
            with self.assertRaises(IntegrityError):
                HistogramUniverse(compact, {'2020-01-02'})
            manifest = read_json_artifact(reference)
            Path(manifest['parts'][0]['path']).write_bytes(b'corrupt')
            with self.assertRaises(IntegrityError):
                HistogramUniverse(compact, dates)
    ...
    def test_combine_two_chains_matches_fresh_and_rejects_duplicate_ownership(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            r_ndx = run(protocol=self._protocol(root), admitted=admitted_both,
                        ...
                        selected_dates=["2020-01-02"], selected_chains=["NDX"],calculate_statistics=False)
            r_qqq = run(protocol=self._protocol(root), admitted=admitted_both,
                        ...
                        selected_dates=["2020-01-02"], selected_chains=["QQQ"],calculate_statistics=False)
            ...
            self.assertFalse(r_ndx['statistics_performed'])
            self.assertIsNone(r_ndx['refs']['statistics'])
    ...
    def test_future_original_request_date_is_not_substituted(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            quality = _read_parts(result["refs"]["source_quality"])
            self.assertEqual(sum(row["raw_rows"] for row in quality if row["source_family"] != "union"), 2)
            self.assertEqual(sum(row["raw_rows"] for row in quality if row["source_family"] == "union"), 2)
    ...
    def test_unknown_right_group_reconciles_to_all_input_rows(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            result = self._run(folder, root, sources, ["2020-01-02"], ["NDX"])
            quality = _read_parts(result["refs"]["source_quality"])
            self.assertEqual(sum(row["raw_rows"] for row in quality if row["source_family"] != "union"), 3)
            self.assertEqual(sum(row["raw_rows"] for row in quality if row["source_family"] == "union"), 3)
    ...
    def test_listing_only_oi_only_and_empty_listing(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertIsNone(coverage[0]["listed_count"])
            self.assertFalse(coverage[0]['listing_denominator_known'])
    ...
    def test_two_listed_one_usable_is_half_and_full_quoted_coverage(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            result = self._run(folder, root, sources, ["2020-01-02"], ["NDX"], oi=oi)
            groups = load_group_statistics(result)
            all_cuts = groups["NDX|all_period|all|near|all|all"]["statistics"]["metrics"]
            self.assertAlmostEqual(all_cuts["quoted_fraction"]["estimate"], 0.8)
            metrics = groups["NDX|all_period|10:00|near|all|all"]["statistics"]["metrics"]
    ...
    def test_cuts_rights_dtes_families_and_raw_unique_differ(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            sources = [
                ...
                _admit(2, broad, "NDX", "broad", "2020-01-02",
                       ...
                       "quote_1m", _write_parquet(root / broad, _quotes([
                           ...
                           _q("NDX", exp_near, 3400.0, "CALL", 10, 10, 1, 1, 50, 50, 6.0, 6.2, day, ts_10, NDX_OSI),
                           _q("NDX", exp_near, 3400.0, "CALL", 10, 10, 1, 1, 50, 50, 9.0, 9.2, day, ts_15, NDX_OSI),
                       ]))),
            ]
            result = self._run(folder, root, sources, ["2020-01-02"], ["NDX"])
            groups = load_group_statistics(result)
            ...
            self.assertEqual(uniq, 7.5)
            self.assertEqual(groups["NDX|all_period|all|broad|all|all"]["statistics"]["metrics"]["mean_bid_raw"]["estimate"], 7.0)
            self.assertGreater(
...
class ColumnarBoardLiteralTests(unittest.TestCase):
    def test_arrow_boards_preserve_nulls_exact_clocks_and_quote_denominators(self):
        import numpy as np
        from trading_research.research.options_quote_measurements import (
            _board_table, _BoardColumns, _board_slices, _cut_board_hists)
        clock = 2**53 + 3
        table = _board_table({'chain':'NDX','source_family':'near','cut_label':'09:30',
            'right':'CALL','dte_bucket':'0','contract_id':np.array([1,2,3],dtype=np.int64),
            'ts_event_ns':[clock,None,clock+7], 'sample_age_ns':[0,None,7],
            'unchanged_payload_age_ns':[0,None,7], 'quoted':[True,True,False],
            'usable':[True,False,False], 'bid':[1.,3.,100.], 'ask':[2.,4.,101.],
            'mid':[1.5,None,None], 'spread':[1.,None,None], 'base_class':['two_sided','conflict',None]},3)
        view = _BoardColumns(table)
        self.assertEqual(table['ts_event_ns'].to_pylist(),[clock,None,clock+7])
        self.assertEqual(view.values('ts_event_ns').tolist(),[clock,0,clock+7])
        self.assertEqual(view.present('ts_event_ns').tolist(),[True,False,True])
        slices = _board_slices(table)
        self.assertEqual(set(slices),{'near|09:30|CALL|0'})
        row=slices['near|09:30|CALL|0']
        self.assertEqual((row['quoted'],row['usable'],row['conflict'],row['bid_n'],row['bid_sum']),(2,1,1,2,4.))
        hist={};_cut_board_hists(hist,table)
        values,counts=hist['bid|cut|near|09:30|CALL|0']
        self.assertEqual(values.tolist(),[1.]);self.assertEqual(counts.tolist(),[1])

    def test_shared_histogram_sort_does_not_duplicate_prior_mass(self):
        from trading_research.research.options_quote_measurements import _empty_frame,_source_frame_hists
        frame=_empty_frame(2)
        for value in frame.values():
            if hasattr(value,'fill'):value.fill(0)
        frame['bid'].fill(1);frame['ask'].fill(2)
        frame['keep'].fill(True);frame['usable_raw'].fill(True)
        hist={}
        _source_frame_hists(hist,frame,'near');_source_frame_hists(hist,frame,'near')
        for population in ('raw_source','unique_source'):
            values,counts=hist[f'bid|{population}|near|-|CALL|expired']
            self.assertEqual(values.tolist(),[1.]);self.assertEqual(counts.tolist(),[4])
        only={};_source_frame_hists(only,frame,'union',include_raw=False)
        self.assertTrue(only)
        self.assertTrue(all('|unique_source|' in key for key in only))
