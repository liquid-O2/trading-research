...
def parse_raw_minute_arrays(
    *,
    ...
    aliases = ()
    if n > 1:
        if np.any(t_ms[1:] < t_ms[:-1]):
            raise IntegrityError(f'raw minute timestamps decrease: {symbol}/{year}, file_id={file_id}')
        repeated = np.flatnonzero(t_ms[1:] == t_ms[:-1]) + 1
        if len(repeated):
            identical = instrument_id[repeated] == instrument_id[repeated - 1]
            for values in (open_, high, low, close, volume):
                bits = values.view(np.uint64)
                identical &= bits[repeated] == bits[repeated - 1]
            if not bool(np.all(identical)):
                bad = repeated[~identical]
                raise IntegrityError(f'conflicting same-time minute observations: {symbol}/{year}, '
                                     f'file_id={file_id}, rows={bad[:20].tolist()}')
            keep = np.r_[True, t_ms[1:] != t_ms[:-1]]
            first = np.maximum.accumulate(np.where(keep, np.arange(n), 0))
            aliases = tuple((int(index), int(first[index])) for index in repeated)
            t_ms, instrument_id = t_ms[keep], instrument_id[keep]
            open_, high, low, close, volume = (values[keep] for values in (open_, high, low, close, volume))
            n = int(t_ms.size)
    ...
    return UnitBars(
        ...
        identical_source_aliases=aliases,
        symbol=symbol,
        variant=variant,
        year=year,
        file_id=int(file_id),
...
class UnitBars:
    ...
    identical_source_aliases: tuple = ()
...
def unit_quality_row(unit: UnitBars) -> dict:
    ...
    return {
        ...
        "rows": len(unit),
        "source_rows": len(unit) if unit.source_row_count is None else unit.source_row_count,
        "identical_source_alias_rows": len(unit.identical_source_aliases),
        "identical_source_aliases": [{"row_index": row, "first_row_index": first}
                                     for row, first in unit.identical_source_aliases],
