...
from trading_research.research.auction_flow_storage import (
    BoundedOutputs, ParquetSeries, read_json_artifact, read_series_tables as _read_scalar_series_tables,
)
...
EXACT_INT_FIELDS = (
    'requested_mass_num', 'requested_mass_den',
    'achieved_mass_num', 'achieved_mass_den',
    'overshoot_num', 'overshoot_den',
    'mean_row_num', 'mean_row_den',
    'dominance_margin_num', 'dominance_margin_den',
    'concentration_num', 'concentration_den',
    'priced_volume', 'sum_price_volume', 'sum_price_squared_volume',
    'vwap_ticks_num', 'vwap_ticks_den',
    'variance_ticks_squared_num', 'variance_ticks_squared_den',
    'percent_band_0_low_num', 'percent_band_0_low_den',
    'percent_band_0_high_num', 'percent_band_0_high_den',
    'percent_band_1_low_num', 'percent_band_1_low_den',
    'percent_band_1_high_num', 'percent_band_1_high_den',
    'percent_band_2_low_num', 'percent_band_2_low_den',
    'percent_band_2_high_num', 'percent_band_2_high_den',
    'q_1_20_num', 'q_1_20_den', 'q_1_4_num', 'q_1_4_den',
    'q_1_2_num', 'q_1_2_den', 'q_3_4_num', 'q_3_4_den',
    'q_19_20_num', 'q_19_20_den',
    'weighted_median_num', 'weighted_median_den',
    'mad_num', 'mad_den', 'robust_scale_num', 'robust_scale_den',
    'side_overlap_num', 'side_overlap_den',
    'side_total_variation_num', 'side_total_variation_den',
    'side_wasserstein_num', 'side_wasserstein_den',
    'absolute_delta_mass_num', 'absolute_delta_mass_den',
    'signed_delta',
    'footprint_ratio_num', 'footprint_ratio_den',
    'pin069_vwap_num', 'pin069_vwap_den',
    'pin069_close_volume', 'pin069_sum_close_volume', 'pin069_bar_volume',
    'source_flat_bar_lost_mass',
    ...
    'future_transport_num', 'future_transport_den',
)


def normalize_exact_integers(row):
    """Encode declared exact integer fields as int64 plus decimal-string escape."""
    if not row:
        return row
    for name in EXACT_INT_FIELDS:
        if name not in row and f'{name}_text' not in row:
            continue
        value = decode_exact_int(row.get(name), row.get(f'{name}_text'))
        stored, text = encode_exact_int(value)
        row[name] = stored
...
def integer_profile_geometry(mass, spec):
    ...
    compact = {
        ...
        'requested_mass_num': requested.numerator,
        'requested_mass_den': requested.denominator,
        'achieved_mass_num': achieved,
        'achieved_mass_den': 1,
        'overshoot_num': overshoot.numerator,
    ...
    return normalize_exact_integers(compact)
...
def compact_geometry(result, *, view=None, width=1, origin=0):
    ...
    return normalize_exact_integers(out)
...
def vwap_bands(moments):
    ...
    return normalize_exact_integers(bands)
...
def bind_owned_atom_evidence(atom):
    """Bind a validated, owned atom before the immutable TPO projections reuse it."""
    if not atom.get('evidence_id'):
        atom['evidence_id'] = digest({
...
def future_mass_on_frozen_grid(mass, following, *, coordinate_identity, known_at_ns,
    ...
    overflow_unidentified = (sum(low) + sum(high) + sum(future.unpriced)) > 0
    prices = list(range(int(lower), int(upper)))
    current_h = [
        mass.buy.get(price, 0) + mass.sell.get(price, 0) + mass.unknown.get(price, 0)
        for price in prices
    ]
    future_h = [
        future.buy.get(price, 0) + future.sell.get(price, 0) + future.unknown.get(price, 0)
        for price in prices
    ]
    current_tot = sum(current_h)
    future_tot = sum(future_h)
    transport = None
    if current_tot and future_tot and prices:
        transport = _same_grid_wasserstein(prices, current_h, future_h, current_tot, future_tot)
    pair = _fraction_pair(transport)
    stored_num, text_num = encode_exact_int(pair[0])
    stored_den, text_den = encode_exact_int(pair[1])
    row['future_transport_num'] = stored_num
    row['future_transport_num_text'] = text_num
    row['future_transport_den'] = stored_den
    row['future_transport_den_text'] = text_den
    row['future_transport_overflow_unidentified'] = overflow_unidentified
...
def measurement_reference(unit):
    """Resolve the accepted receipt layout without changing observation artifacts."""
    original = unit.get('original_refs') or {}
    direct = unit.get('measurement') or original.get('measurement') or (unit.get('artifacts') or {}).get('measurement')
    if isinstance(direct, dict) and direct.get('path'):
        return direct
    receipt_ref = unit.get('receipt') or original.get('receipt_reference')
    if not isinstance(receipt_ref, dict):
        return None
    receipt = read_json_artifact(receipt_ref)
    if receipt.get('kind') != 'auction_flow_source_window_receipt_v1' or receipt.get('success') is not True:
        raise IntegrityError('profile measurement requires successful source-window receipt')
    identity = receipt.get('unit') or {}
    expected = {'root': unit.get('root'), 'source_path': unit.get('source_path'),
                'event_start_ns': unit.get('source_window_start_ns'),
                'event_end_ns': unit.get('source_window_end_ns')}
    if any(identity.get(key) != value for key, value in expected.items()):
        raise IntegrityError('profile receipt root/source/window identity differs')
    if original.get('receipt_sha256') not in (None, receipt_ref.get('sha256')):
        raise IntegrityError('profile original receipt hash differs')
    ref = (receipt.get('artifacts') or {}).get('measurement')
    if not isinstance(ref, dict) or not ref.get('path'):
        raise IntegrityError('profile receipt lost its measurement reference')
    expected_sha = original.get('measurement_sha256')
    if expected_sha is None or expected_sha != ref.get('uncompressed_sha256', ref.get('sha256')):
        raise IntegrityError('profile original measurement hash differs from receipt')
    return ref


def load_measurement_cells(unit):
    """Read one measurement JSON, extract atoms, and drop the payload."""
    ref = measurement_reference(unit)
    if ref is None:
...
def attach_observation_identity(atoms, observations, *, unit):
    ...
    out = []
    for atom in atoms:
        key = (atom['instrument_id'], atom.get('contract_key'), atom['event_start_ns'], atom['event_end_ns'])
        obs = by_key.get(key)
...
def geometry_schema(pa):
    ...
    exact = set(EXACT_INT_FIELDS)
    fields = []
    for field in raw:
        fields.append(field)
        if field.name in exact:
            fields.append(text(f'{field.name}_text'))
...
def join_schema(pa):
    ...
    return pa.schema([
        ...
        text('future_unpriced_overflow_reason'),
        i64('future_unpriced_buy'), i64('future_unpriced_sell'), i64('future_unpriced_unknown'),
        i64('future_transport_num'), text('future_transport_num_text'),
        i64('future_transport_den'), text('future_transport_den_text'),
        flag('future_transport_overflow_unidentified'),
...
def write_series_table(outputs, name, table):
    ...
    if not len(table):
        series.append(_encode_profile_lists(table))
    else:
        offset = 0
        while offset < len(table):
            amount = min(BATCH_HINT, len(table) - offset)
            series.append(_encode_profile_lists(table.slice(offset, amount)))
            offset += amount
    return series.finish()


_PROFILE_LIST_SCHEMA = b'profile-exact-list-schema-v1'


def _encode_profile_lists(table):
    """Keep profile list values lossless in the existing scalar-only store."""
    pa = _require_pyarrow()
    columns = [i for i, field in enumerate(table.schema) if pa.types.is_list(field.type)]
    if not columns:
        return table
    metadata = dict(table.schema.metadata or {})
    if _PROFILE_LIST_SCHEMA in metadata:
        raise IntegrityError('profile list table is already encoded')
    metadata[_PROFILE_LIST_SCHEMA] = table.schema.serialize().to_pybytes()
    for i in columns:
        field = table.schema.field(i)
        values = [None if value is None else json.dumps(value, separators=(',', ':'), allow_nan=False)
                  for value in table.column(i).to_pylist()]
        table = table.set_column(i, pa.field(field.name, pa.string(), nullable=field.nullable),
                                 pa.array(values, type=pa.string()))
    return table.replace_schema_metadata(metadata)


def read_series_tables(series):
    """Authenticate scalar storage, then restore exact profile list columns."""
    pa = _require_pyarrow()
    for table in _read_scalar_series_tables(series):
        raw_schema = (table.schema.metadata or {}).get(_PROFILE_LIST_SCHEMA)
        if raw_schema is None:
            yield table
            continue
        if not 0 < len(raw_schema) <= 1024 ** 2:
            raise IntegrityError('profile logical schema exceeds its bound')
        schema = pa.ipc.read_schema(pa.BufferReader(raw_schema))
        if schema.names != table.column_names:
            raise IntegrityError('profile logical column identity changed')
        for i, field in enumerate(schema):
            if pa.types.is_list(field.type):
                values = [None if value is None else json.loads(value)
                          for value in table.column(i).to_pylist()]
                table = table.set_column(i, field, pa.array(values, type=field.type))
        table = table.replace_schema_metadata(schema.metadata)
        if not table.schema.equals(schema, check_metadata=True):
            raise IntegrityError('profile logical schema failed restoration')
        yield table
...
class ChunkWriter:
    ...
    def flush(self):
        ...
        self.series.append(_encode_profile_lists(table))
    ...
    def finish(self):
        ...
        if self.rows == 0 and not self._empty_written:
            self.series.append(_encode_profile_lists(_table_from_rows(self.schema, [], self.pa)))
...
def emit_geometry_payload(*, mass, spec, coordinate_identity, known_at_ns,
    ...
    normalize_exact_integers(compact)
    return compact, view
...
def _feature_for_arm(features, formation, latency):
    for feature in features:
        ...
        if feat_lat is None or feat_lat != int(latency):
            continue
        return feature
    return None
...
def _scenario_publication(geometry_row, latency):
    ...
    if source_pub is None:
        return None
    arm = int(latency)
    scenario = int(source_pub) - SOURCE_LATENCY_NS + arm
    incoming = geometry_row.get('input_known_ns')
    if incoming is not None:
        scenario = max(scenario, int(incoming) - SOURCE_LATENCY_NS + arm)
            ...
            for horizon_kind in HORIZON_KINDS:
                ...
                for horizon in horizons:
                    ...
                    if horizon_kind == 'fixed_minutes' and horizon is not None:
                        ...
                        row['future_low_overflow_unpriced'] = None
                        row['future_high_overflow_unpriced'] = None
                        row['future_unpriced_overflow_reason'] = 'unpriced_side_of_grid_unidentified'
                        row['future_transport_num'] = future_compact.get('future_transport_num')
                        row['future_transport_num_text'] = future_compact.get('future_transport_num_text')
                        row['future_transport_den'] = future_compact.get('future_transport_den')
                        row['future_transport_den_text'] = future_compact.get('future_transport_den_text')
                        row['future_transport_overflow_unidentified'] = future_compact.get(
                            'future_transport_overflow_unidentified')
                    writer.append(row)
...
class IdentityEngine:
    ...
    @staticmethod
    def _civil_day(ns):
        return datetime.fromtimestamp(int(ns) // NS, timezone.utc).astimezone(ZoneInfo(ZONE)).date()

    def _civil_owned(self, anchor):
        return self._civil_day(int(anchor.end_ns - 1)).year == int(self.year)

    def _civil_output_identity(self, anchor):
        economic = economic_date(int(anchor.end_ns - 1))
        return date.fromisoformat(economic), economic, stage_name(self.root, economic, self.policy)

    def _record_prior_completed(self, day, named, same, pin_snap):
        for variant in ('cash_rth', 'observed_futures_18_17'):
            anchor = next((item for item in named.get('anchors') or () if item.variant == variant), None)
            if anchor is None:
                continue
            members = _atoms_in_spans(same, anchor.spans)
            if not members:
                continue
            hist = IntegerMass()
            for atom in members:
            ...
            if variant == 'cash_rth' and hist.last_priced_ticks is not None:
                self.close_history.append({
                    'ticks': hist.last_priced_ticks,
                    'known': hist.last_priced_known_at_ns,
                })
            if hist.priced_volume <= 0:
                continue
            coverage = coverage_from_atoms(anchor.spans, members)
            input_known = _input_known(members, anchor.end_ns)
            published = publication_known(
                input_known_ns=input_known, selection_known_ns=anchor.selection_known_at_ns,
                cut_ns=anchor.end_ns, latency_ns=SOURCE_LATENCY_NS,
            )
            attached = pin_snap
            if attached is None:
                attached = self._pin_at(published)
            elif (
                attached.get('pin069_published_ns') is None
                or int(attached['pin069_published_ns']) > int(published)
            ):
                attached = None
            compact, _view = emit_geometry_payload(
                mass=hist, spec=self.baseline,
                coordinate_identity=f'{self.root}:{self.contract_key}:{self.lineage}',
                known_at_ns=published, coverage_complete=coverage['coverage_complete'],
                include_side=False, include_vwap=True, include_footprint=False,
                footprint_params=self.footprint_params, verify_kernel=False,
                anchor_kind=anchor.kind, pin069=attached,
            )
            cells_hash = digest(sorted(hist.source_hashes) + [anchor.variant, anchor.end_ns])
            self.published_completed.append({
                'variant': variant,
                'contract_key': self.contract_key,
                'instrument_id': self.instrument_id,
                'published_ns': published,
                'val': compact.get('physical_val_ticks', compact.get('val_row')),
                'vah': compact.get('physical_vah_ticks', compact.get('vah_row')),
                'poc': compact.get('physical_poc_ticks', compact.get('scalar_poc')),
                'geometry_id': geometry_id_for(
                    anchor=anchor, variant_id=BASELINE_VARIANT_ID,
                    cut_ns=anchor.end_ns, cells_hash=cells_hash,
                ),
            })
        _ = day
    ...
    def finish(self):
        ...
        for variant in CIVIL_VARIANTS:
            ...
            if key is None or key in self.civil_emitted:
                continue
            if variant not in self.civil_anchor:
                continue
            anchor = self.civil_anchor[variant]
            if not self._civil_owned(anchor):
                continue
            day_out, economic_out, stage_out = self._civil_output_identity(anchor)
            prev = [atom for atom in same if _wholly_inside(atom, anchor.spans)]
            self._emit_completed(
                day_out, economic_out, stage_out, prev[-1] if prev else sample, anchor, same, None,
                last_year_day=day_out, force_civil=True,
            )
    ...
    def flush_ready(self, horizon_ns):
        ...
        civil_days = sorted({self._civil_day(atom['event_start_ns']) for atom in self.pending})
        for civil_day in civil_days:
            if civil_day in self.civil_flushed:
                continue
            need = local_timestamp(civil_day + timedelta(days=1), time(0), ZONE) + 60 * MINUTE_NS
            if horizon_ns is not None and int(horizon_ns) < need:
                continue
            self._flush_civil_day(civil_day)
            self.civil_flushed.add(civil_day)
        if self.flushed:
    ...
    def _cross_civil_periods(self, day, same, sample, stage, economic):
        ...
        for variant in CIVIL_VARIANTS:
            ...
            if old_key == new_key:
                continue
            if old_key not in self.civil_emitted and variant in self.civil_anchor:
                anchor = self.civil_anchor[variant]
                if self._civil_owned(anchor):
                    day_out, economic_out, stage_out = self._civil_output_identity(anchor)
                    prev = [atom for atom in same if _wholly_inside(atom, anchor.spans)]
                    self._emit_completed(
                        day_out, economic_out, stage_out, prev[-1] if prev else sample,
                        anchor, same, None, last_year_day=day_out, force_civil=True,
                    )
            self._reset_civil(variant)
            self.civil_period[variant] = new_key

    def _flush_civil_day(self, day):
        same = self._same()
        atoms = [atom for atom in same
                 if self._civil_day(atom['event_start_ns']) == day]
        if not atoms:
            return
            ...
            for civ in named.get('anchors') or ():
                ...
                for minutes in BRACKET_MINUTES:
                    for representation in ('whole_trade_visits', 'ohlc_range_proxy'):
                        ...
                        try:
                            if atomic is None:
                                atomic = to_atomic_trades(atom, civ)
                            engine.add(atomic)
                        except (ContractError, IntegrityError) as exc:
        ...
        for anchor in named.get('anchors') or ():
            if anchor.variant in CIVIL_VARIANTS and _civil_period_end(day, anchor.variant):
                self._emit_completed(day, economic, stage, sample, anchor, same, None,
                                     last_year_day=date(self.year, 12, 31), force_civil=True)
    ...
    def flush_day(self, day):
        ...
        if not emit_selected:
            self._record_prior_completed(day, named, same, pin_snap)
            return
        ...
        anchors = [a for a in named.get('anchors') or () if a.variant not in CIVIL_VARIANTS]
    ...
    def _emit_completed(self, day, economic, stage, sample, anchor, same, pin_snap, *, last_year_day,
        ...
        if anchor.variant in CIVIL_VARIANTS:
            ...
            if not ends and day != last_year_day and not force_civil:
                return
            day, economic, stage = self._civil_output_identity(anchor)
        ...
        completed_id = geometry_id_for(
            anchor=anchor, variant_id=BASELINE_VARIANT_ID, cut_ns=cut_ns, cells_hash=cells_hash,
        )
...
def compute_profile_partition(*, key, members, neighbors, identity, source_refs, contract,
    ...
    for index, (selected, unit) in enumerate(queued):
        ...
        if extracted.get('status') == 'unavailable_source_window':
            ...
            continue
        measurement_ref = measurement_reference(unit)
        ...
        measured_keys = {
            (atom['instrument_id'], atom.get('contract_key'), atom['event_start_ns'], atom['event_end_ns'])
            for atom in extracted['atoms']
        }
        for row in observations:
            key = (row.get('instrument_id'), row.get('contract_key'), row.get('event_start_ns'), row.get('event_end_ns'))
        ...
        for atom in merged:
            ...
            bind_owned_atom_evidence(accepted)
    ...
    completeness_out = {'kind': 'auction_flow_profile_partition_completeness_v1', **completeness, 'conflicts': list(alias.conflicts), 'family_complete': False}
...
__all__ = [
    ...
    'decode_exact_int', 'encode_exact_int', 'EXACT_INT_FIELDS', 'FORMATION_MINUTES',
    'LATENCY_NS', 'normalize_exact_integers',
    'compute_profile_partition', 'coverage_from_atoms', 'dedup_atoms',
    'delayed_selection_blocks_origin', 'empty_completeness', 'emit_geometry_payload',
    'extract_sparse_profile',
