...
def _float64_or_none(value, *, what):
    ...
    if type(value) is float:
        if not math.isfinite(value):
            raise IntegrityError(f'{what} must be finite when observed')
        return value
...
def _archive_contains(intervals, start, end):
    ...
    for item in intervals:
        ...
        if a >= b:
            raise IntegrityError('source archive interval must have positive extent')
        spans.append((a, b))
    covered = start
...
def _profile_summary(profile, *, coverage_complete):
    ...
    for index, row in enumerate(rows):
        ...
        sides = [_int(row[j], what=f'sparse_profile.rows[{index}].side', minimum=0) for j in (1, 2, 3)]
        if previous_row is not None and price <= previous_row:
...
def _validate_unit(receipt, measured, receipt_reference):
    ...
    if (rec.get('status') not in ('measured', 'unavailable_source_window')
            or rec.get('status') != payload.get('status')):
...
def observation_rows(receipt, measured, *, receipt_reference):
    ...
    if not instruments:
        if (measured['status'] != 'unavailable_source_window'
                or any(unit['counts'].get(k) != 0 for k in ('raw_rows', 'trades', 'volume', 'quote_rows'))):
            raise IntegrityError('unavailable source window cannot contain unassigned observed records')
        return []
    if measured['status'] != 'measured':
        raise IntegrityError('unavailable source window unexpectedly has instrument measurements')
    ...
    for instrument in instruments:
        ...
        for atom in atoms:
            ...
            for side in ('buy', 'sell', 'unknown'):
                if profile[f'priced_{side}'] + profile[f'unpriced_{side}'] != checked['all'][side]:
                    raise IntegrityError('profile side mass does not reconcile with observed flow')
...
def write_observation_unit(receipt, measured, *, receipt_reference, outputs):
    ...
    if not len(table):
        series.append(table)  # Retain the explicit empty schema without inventing an instrument.
    offset = 0
    ...
    summary = {
        ...
        'root': receipt['unit']['root'],
        'source_path': receipt['unit']['source_path'],
        'source_window_start_ns': receipt['unit']['event_start_ns'],
        'source_window_end_ns': receipt['unit']['event_end_ns'],
        'source_window_status': measured['status'],
        'unavailable_reason': 'no observed instrument or incoming source owner in this window' if not rows else None,
        ...
        'original_refs': {
            ...
            'measurement_sha256': _measurement_sha256(receipt),
            'canonical_raw_values_sha256': _canonical_raw_values_sha256(receipt, measured),
            'receipt_reference': {
