...
class AuctionFlowStream:
    ...
    def __init__(self, *, data_root: Path, index: dict, dataset: str,
        ...
        from trading_research.data.clock_uncertainty import plans_for
        self.clock_uncertainty_plans = plans_for(self.lineage_records, self.start, self.end)
        self.clock_uncertain_intervals = tuple(tuple(p['interval']) for p in self.clock_uncertainty_plans)
        self.projector.clock_uncertainty_events = {p['event']['source_row']: p for p in self.clock_uncertainty_plans}
        self.projector.clock_uncertainty_seen = set()
    ...
    def __iter__(self):
        ...
        try:
            ...
            if set(self.projector.clock_uncertainty_events) != self.projector.clock_uncertainty_seen:
                raise IntegrityError('declared original reset fault was not encountered exactly')
    ...
    def manifest(self):
        ...
        if self.clock_uncertain_intervals:
            payload['source_clock_uncertainty'] = {'kind': 'source_order_state_uncertain_duration_v1',
                'intervals': self.clock_uncertain_intervals, 'original_events': self.clock_uncertainty_plans,
                'state_order': 'original physical source order',
                'timing': 'no exact dwell/standing duration inside these half-open intervals; original event clocks unchanged'}
        return payload
