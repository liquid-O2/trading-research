...
from trading_research.research.auction_flow_core_statistics import (
    ...
    STAGE_ORDER, labeled_date_and_event_means, stage_name,
)
from trading_research.research.auction_flow_profile_reference import (
    BAR_PROXY_VARIANTS, BASELINE_VARIANT_ID, FROZEN_CONTRACT_KIND, CIVIL_VARIANTS, _civil_period_end,
    ...
    require_frozen_variants, write_series_table, read_series_tables, measurement_reference,
)
...
def profile_partition_plan(population, contract):
    ...
    for unit in units:
        ...
        from trading_research.research.auction_flow_profile_reference import IdentityEngine
        years.update((IdentityEngine._civil_day(start).year, IdentityEngine._civil_day(end - 1).year))
...
def profile_partition_source_refs(members, *, role='selected'):
    ...
    for unit in members:
        ...
        measurement = measurement_reference(unit) or {}
        refs.append({
            'role': role,
...
def civil_completion_dates(year, stage, root, anchor, policy):
    """Predetermined opportunities owned by civil year, labeled by futures date.

    Midnight after a civil period ends is in the following futures economic
    date. Civil ownership and economic output year can therefore differ.
    """
    if anchor not in CIVIL_VARIANTS:
        raise ContractError('civil completion universe requires a civil anchor')
    day, end = date(int(year), 1, 1), date(int(year) + 1, 1, 1)
    result = []
    while day < end:
        if _civil_period_end(day, anchor):
            label = (day + timedelta(days=1)).isoformat()
            if stage_name(root, label, policy) == stage:
                result.append(label)
        day += timedelta(days=1)
    return result


def _iter_finalized_groups(groups, *, intended_by_stage, stats, np):
    ...
    for key in groups:
        meta = _gmeta(key)
        by_universe[(meta['root'], meta['year'], meta['stage'], meta.get('anchor'))].append(key)
    for (root, year, stage, anchor), keys in by_universe.items():
        intended = list(intended_by_stage.get((root, year, stage, anchor),
                       intended_by_stage.get((root, year, stage), ())))
        if not intended:
            for key in keys:
                stores = groups[key]
                meta = _gmeta(key)
                ...
                yield {**_gmeta(key), 'metrics': metrics, 'intended_dates': intended,
...
def write_partition_groups(outputs, prefix, acc, *, stats, policy, collection, root, year, contract):
    ...
    for anchor in CIVIL_VARIANTS:
        for stage in (*STAGE_ORDER, 'unassigned'):
            intended_by_stage[(root, year, stage, anchor)] = civil_completion_dates(
                year, stage, root, anchor, policy)
    def write_groups(groups, suffix, kind, extra):
        parts, batch = [], []
        group_count = 0
        def flush():
            if not batch:
                return
            parts.append(outputs.json_compressed(
                f'{prefix}-{suffix}-{len(parts):04d}.json.zst',
                {'kind': kind, 'groups': batch}, kind=kind,
                maximum_uncompressed_bytes=32 * 1024 ** 2, measurement_fast_path=True))
            batch.clear()
        for group in _iter_finalized_groups(groups, intended_by_stage=intended_by_stage, stats=stats, np=np):
            batch.append(group)
            group_count += 1
            if len(batch) >= 32:
                flush()
        flush()
        return outputs.json(prefix + '-' + suffix + '.json', {
            'kind': kind, 'parts': parts, 'group_count': group_count,
            'family_complete': False, 'source_collection': collection,
            'root': root, 'economic_year': year, **extra,
        }, kind=kind)
    groups_ref = write_groups(acc['groups'], 'groups', 'auction_flow_profile_group_statistics_v1', {
        'variant_ids': list(frozen_variant_ids()), 'bar_proxy_variants': list(BAR_PROXY_VARIANTS)})
    paired_ref = write_groups(acc['paired'], 'paired', 'auction_flow_profile_paired_contrasts_v1', {
...
def _authenticate_json_ref(reference, load_reference, *, kinds):
    ...
    if reference.get('encoding') == 'canonical-json-zstd-v1':
        ...
        payload = read_json_artifact(reference, maximum_uncompressed_bytes=JSON_LOAD_MAX)
    else:
...
def authenticate_reused_partition(part, load_reference):
    ...
    for payload, role in ((groups, 'groups'), (paired, 'paired')):
        for _ in iter_authenticated_profile_groups(payload, load_reference, role=role):
            pass
    del groups
    del paired
...
def iter_authenticated_profile_groups(manifest, load_reference, *, role='groups'):
    """Authenticate and release bounded group parts, retaining logical group order."""
    payload_kind = ('auction_flow_profile_group_statistics_v1' if role == 'groups'
                    else 'auction_flow_profile_paired_contrasts_v1')
    if not isinstance(manifest, dict):
        raise IntegrityError('profile group manifest required')
        ...
        if 'parts' in manifest:
            for ref in manifest['parts']:
                ...
                for group in payload.get('groups') or ():
                    ...
                    yield group
                del payload
            if count != manifest['group_count']:
                raise IntegrityError('profile group manifest lost or repeated groups')
        else:
            yield from manifest.get('groups') or ()
        return
    refs = manifest.get('partition_groups' if role == 'groups' else 'partition_paired') or ()
    for ref in refs:
        payload = _authenticate_json_ref(ref, load_reference, kinds={payload_kind})
        yield from iter_authenticated_profile_groups(payload, load_reference, role=role)
        del payload
...
def _append_report_metric_rows(lines, group):
    ...
    def quantiles(payload, key):
        values = payload.get(key) or {}
        return ', '.join(number(values.get(str(level))) for level in QUANTILE_LEVELS)
    metrics = group.get('metrics') or {}
    identity = '; '.join(f'{key}={group.get(key)}' for key in _GFIELDS if group.get(key) is not None)
    lines.extend(['', '### ' + identity.replace('|', '/'), ''])
    bootstrap = next((p.get('bootstrap') for p in metrics.values() if p and p.get('bootstrap')), None)
    if bootstrap:
        lines.append(f"Bootstrap: confidence={bootstrap.get('confidence')}; replicates={bootstrap.get('replicates')}; "
                     f"date block={bootstrap.get('block_length')}; seed={bootstrap.get('seed')}; "
                     f"method={bootstrap.get('method')}. No end wrapping.")
        lines.append('')
    lines.extend(['| metric | unit | date mean | event mean | dates | observations | interval | valid replicates | date quantiles | raw quantiles | support |',
                  '|---|---|---|---|---|---|---|---|---|---|---|'])
    undefined = []
    for name, payload in metrics.items():
        payload = payload or {}
        if payload.get('undefined') and not payload.get('eligible_observations'):
            undefined.append(name)
            continue
        support = payload.get('support') or {}
        boot = payload.get('bootstrap') or {}
        interval = f"[{number(boot.get('lower'))}, {number(boot.get('upper'))}]"
        lines.append(f"| {name} | {payload.get('unit')} | {number(_readable_mean(payload.get('date_mean')))} | "
                     f"{number(payload.get('event_mean'))} | {payload.get('eligible_independent_dates')} | "
                     f"{payload.get('eligible_observations')} | {interval} | {boot.get('valid_replicates')} | "
                     f"{quantiles(payload, 'sample_quantiles')} | {quantiles(payload, 'raw_quantiles')} | "
                     f"{', '.join(support.get('reasons') or ()) or 'adequate'} |")
    if undefined:
        lines.extend(['', 'Undefined (no valid observations; full unit/status fields retained in the group artifact): ' + ', '.join(undefined) + '.'])
...
def _markdown_report(*, completeness, population, cpu, output_bytes, reused,
    ...
    lines.append('')
    lines.append('Quantiles in each row use probabilities: ' + ', '.join(str(level) for level in QUANTILE_LEVELS) + '. Display precision is eight significant digits; group artifacts retain full precision.')
    stream.write(('\n'.join(lines) + '\n').encode())
    lines.clear()
