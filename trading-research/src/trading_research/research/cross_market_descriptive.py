...
def summarize_groups(groups: dict, intended_by_key: dict, *, batch_size: int = STAT_METRIC_BATCH) -> dict:
    ...
    for key, payload in groups.items():
        ...
        if not intended:
            out[key] = {**_undefined_group(), "reason": "no_intended_dates"}
            continue
...
def _results_md_group_lines(statistics) -> list[str]:
    ...
    for class_name in class_order:
        ...
        for key, payload in rows:
            ...
            if payload.get("status") == "undefined":
                undefined += 1
                lines.append(f"- `{key}` undefined: {payload.get('reason')}; events=0 dates=0")
                continue
            defined += 1
...
def write_results_md(counts, statistics, resources, refs, quality, support) -> str:
    lines = [
        ...
        f"- Identical repeated source rows: {counts.get('identical_source_alias_rows', 0)}; one completed bar per timestamp, aliases retained in source quality.",
...
def run(*, protocol: dict, admitted: dict, store, outputs: BoundedOutputs,
    ...
    phase_totals = {}
    valid_bars = invalid_bars = raw_source_rows = identical_alias_rows = 0
    event_seq = [1]
    label_ids = {}
    class_tallies = {}
    ...
    try:
        ...
        for year in years:
            ...
            if units:
                for unit in units.values():
                    ...
                    raw_source_rows += len(unit) if unit.source_row_count is None else unit.source_row_count
                    identical_alias_rows += len(unit.identical_source_aliases)
                process_year(
        ...
        counts = {
            ...
            "identical_source_alias_rows": identical_alias_rows,
