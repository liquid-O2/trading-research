...
def _lex_unique_integer_rows(columns):
    """Exact lexicographic tuples with an inverse map, without record sorting."""
    np = _np()
    columns = tuple(np.asarray(column) for column in columns)
    if not columns or any(column.ndim != 1 or column.dtype.kind != 'i' for column in columns):
        raise ContractError('signed integer tuple columns required')
    n = len(columns[0])
    if any(len(column) != n for column in columns):
        raise ContractError('integer tuple columns differ in length')
    if not n:
        return np.empty((0, len(columns)), dtype=np.int64), np.empty(0, dtype=np.int64)
    ...
    rows = order[first]
    return np.stack([column[rows] for column in columns], axis=1), inverse


def _right_dte_codes(rights, dtes):
    np = _np()
    rights = np.asarray(rights)
    dtes = np.asarray(dtes)
    if (rights.shape != dtes.shape or np.any((rights < -1) | (rights > 1))
            or np.any((dtes < -1) | (dtes >= len(DTE_NAMES)))):
        raise ContractError('right/DTE categorical codes outside the declared domain')
    return (rights.astype(np.int16) + 1) * (len(DTE_NAMES) + 1) + (dtes.astype(np.int16) + 1)
...
class QuoteCodebook:
    ...
    def intern_valid(self, chain, osi_codes, osi_book, exp_days, milli, right_code, valid, dates,
        ...
        if np.any(valid):
            uniq, inverse = _lex_unique_integer_rows((
        ...
        invalid = ~valid
        if np.any(invalid):
            uinv, inverse = _lex_unique_integer_rows((
...
def annotate_runs(frame):
    ...
    if n == 0:
        return frame
    if not _frame_is_ordered(frame):
        order = np.lexsort((frame["row_index"], frame["file_id"], frame["ts_event_ns"], frame["sort_id"]))
        for name in FRAME_ARRAYS:
            frame[name] = frame[name][order]
    sid, ts = frame["sort_id"], frame["ts_event_ns"]
...
def parse_quote_typed(table, *, record, dates, clocks, osi_book, codebook, session_open_ns,
    ...
    if np.any(ts_ok):
        east_days = _eastern_day_codes(ts_arr, dates, clocks)
        east_days[~ts_ok] = -1
...
def _source_quality_rows(frame, chain, declared, family):
    ...
    width = 3 * (len(DTE_NAMES) + 1)
    classes = len(BASE_CLASSES)
    keep = frame['keep']
    def counts(mask=None):
        selected = codes if mask is None else codes[mask]
        return np.bincount(selected, minlength=width)
    raw_sizes = counts()
    unique_sizes = counts(keep)
    raw_base = frame['raw_base'].astype(np.int64)
    raw_classes = np.bincount(codes.astype(np.int64) * classes + raw_base,
                             minlength=width * classes).reshape(width, classes)
    unique_base = np.where(raw_base <= BASE_INVALID_NUMERIC, raw_base,
                           np.where(frame['unique_conflict'], BASE_CONFLICT, raw_base))
    unique_classes = np.bincount(codes[keep].astype(np.int64) * classes + unique_base[keep],
                                minlength=width * classes).reshape(width, classes)
    raw_flags = {name: counts(frame[name]) for name in QUALITY_FLAGS}
    unique_flags = {name: counts(keep & frame[name]) for name in QUALITY_FLAGS}
    missing = {name: counts(frame[column]) for name, column in (
        ('missing_bid', 'bid_null'), ('missing_ask', 'ask_null'),
        ('missing_bid_size', 'bid_size_null'), ('missing_ask_size', 'ask_size_null'))}
    missing['missing_ts'] = counts(~frame['ts_ok'])
    rows = []
    for code in np.flatnonzero(raw_sizes):
        right, dte = divmod(int(code), len(DTE_NAMES) + 1)
        right, dte = right - 1, dte - 1
        row = {'chain': chain, 'request_date': declared, 'source_family': family,
               'right': RIGHT_NAMES.get(right, 'unknown'),
               'dte_bucket': DTE_NAMES[dte] if 0 <= dte < len(DTE_NAMES) else 'unknown',
               'raw_rows': int(raw_sizes[code]), 'unique_events': int(unique_sizes[code])}
        row.update({name: int(values[code]) for name, values in missing.items()})
        for index, name in enumerate(BASE_CLASSES):
            row[f'raw_{name}'] = int(raw_classes[code, index])
            row[f'unique_{name}'] = int(unique_classes[code, index])
        for name in QUALITY_FLAGS:
            row[f'raw_{name}'] = int(raw_flags[name][code])
            row[f'unique_{name}'] = int(unique_flags[name][code])
        rows.append(row)
    return rows
...
def _hist_merge(store, key, values, counts):
    from trading_research.research.options_quote_statistics import _typed_histogram, _merge_exact_histograms
    hist = _typed_histogram(values, counts)
    store[key] = _merge_exact_histograms(store.get(key), hist)
...
def _hist_batch(store, metric, population, family, cut, right_codes, dte_codes, values, mask,
    ...
    if not np.any(valid):
        return
    codes = (_right_dte_codes(right_codes, dte_codes) if category_codes is None else category_codes)[valid]
    vals = vals[valid]
    order = np.lexsort((vals, codes))
    vals, codes = vals[order], codes[order]
    starts = np.r_[0, np.flatnonzero((codes[1:] != codes[:-1]) | (vals[1:] != vals[:-1])) + 1]
    counts = np.diff(np.r_[starts, len(vals)]).astype(np.int64, copy=False)
    vals, codes = vals[starts], codes[starts]
    group_starts = np.r_[0, np.flatnonzero(codes[1:] != codes[:-1]) + 1, len(codes)]
    for begin, end in zip(group_starts[:-1], group_starts[1:]):
        right, dte = divmod(int(codes[begin]), len(DTE_NAMES) + 1)
        right, dte = right - 1, dte - 1
        right_label = RIGHT_NAMES.get(right, 'unknown')
        dte_label = DTE_NAMES[dte] if 0 <= dte < len(DTE_NAMES) else 'unknown'
        _hist_merge(store, f'{metric}|{population}|{family}|{cut}|{right_label}|{dte_label}',
                    vals[begin:end], counts[begin:end])
...
def _source_frame_hists(store, frame, family, *, include_raw=True):
    np = _np()
    ...
    for metric, values, present in (
        ...
        _source_hist_populations(store,metric,family,values,present,keep,codes,include_raw)
...
def _freeze_hists(store):
    # HistogramUniverse consumes these typed bins before final shard serialization.
    return {key: {'v': values, 'c': counts} for key, (values, counts) in store.items()}
...
def _listed_unquoted_rows(chain, declared, cut_label, cut_ns, family, unquoted, listing, oi_join,
    ...
    np, pa = _np(), _pa()
    n = len(unquoted)
    identity_table = _unquoted_identity_columns(oi_join, declared, unquoted)
    cols = {
        ...
        **{name:identity_table[name] for name in identity_table.schema.names},
...
def _coverage_row(*, chain, declared, cut_label, family, intended, session, missing_file,
    ...
    return {
        ...
        "cut_status": _cut_status(early_na=early_na, missing_file=missing_file,
                                  empty_marker=empty_marker, quoted=bool(nquoted)),
        ...
        "quoted_count": nquoted,
        "quoted_listed": view.count(quoted & listed & listed_known),
        "listed_unquoted": view.count(~quoted & listed & listed_known),
        "quoted_unlisted": view.count(quoted & ~listed & listed_known),
        "usable_count": nusable,
        ...
        "sample_stale_60_count": view.count(ages & view.values('sample_stale_60')),
        "sample_stale_300_count": view.count(ages & view.values('sample_stale_300')),
        "sample_stale_900_count": view.count(ages & view.values('sample_stale_900')),
        "payload_stale_60_count": view.count(ages & view.values('payload_stale_60')),
        "payload_stale_300_count": view.count(ages & view.values('payload_stale_300')),
        "payload_stale_900_count": view.count(ages & view.values('payload_stale_900')),
        "quoted_with_age": view.count(ages),
        "bid_sum": view.sum('bid', usable),
        "bid_n": view.count(usable & view.present('bid')),
        "ask_sum": view.sum('ask', usable),
        "ask_n": view.count(usable & view.present('ask')),
        "mid_sum": view.sum('mid', usable),
        "mid_n": view.count(usable & view.present('mid')),
        "spread_sum": view.sum('spread', usable),
        "spread_n": view.count(usable & view.present('spread')),
        "rel_spread_sum": view.sum('relative_spread', usable),
        "rel_spread_n": view.count(usable & view.present('relative_spread')),
        "sample_age_sum": float((view.values('sample_age_ns')[ages] / NS).sum()),
        "payload_age_sum": float((view.values('unchanged_payload_age_ns')[ages] / NS).sum()),
        "oi_available_count": view.count(oi_universe),
        "oi_quoted_available_count": view.count(oi_universe & quoted),
        "oi_missing_count": view.count(view.values('oi_missing')),
        "oi_ambiguous_count": view.count(view.values('oi_ambiguous')),
        "oi_zero_count": view.count(view.values('oi_zero')),
        "oi_amount_unknown": view.count(~view.present('oi') & ~oi_universe),
        ...
        "gt60_dte_listed": view.count((view.values('dte_bucket') == '61+') & listed & listed_known),
        "no_sample": 0 if nquoted else 1,
...
def _board_slices(board_rows):
    ...
    if not len(view):
        return {}
    def encode(name, choices):
        column = _pc().index_in(view.table[name],value_set=_pa().array(choices))
        return _pc().fill_null(column,len(choices)).to_numpy(zero_copy_only=False).astype(np.int64)
    fam = encode('source_family',list(FAMILY_NAMES))
    cut = encode('cut_label',list(ALL_CUTS))
    right = encode('right',['CALL','PUT'])
    dte = encode('dte_bucket',list(DTE_NAMES))
    if np.any(fam >= len(FAMILY_NAMES)) or np.any(cut >= len(ALL_CUTS)):
        raise IntegrityError('board lost its declared family or cut')
    width_dte, width_right = len(DTE_NAMES)+1, 3
    codes = ((fam*len(ALL_CUTS)+cut)*width_right+right)*width_dte+dte
    size = len(FAMILY_NAMES)*len(ALL_CUTS)*width_right*width_dte
    quoted = view.values('quoted')
    def counts(mask):
        return np.bincount(codes[mask],minlength=size)
    metrics = {'quoted':counts(quoted),'usable':counts(quoted & view.values('usable'))}
    cls = view.values('base_class')
    metrics['invalid'] = counts(quoted & np.isin(cls,('invalid_identity','invalid_clock','invalid_numeric')))
    for name in ('conflict','all_zero','one_sided','crossed','locked','two_sided'):
        metrics[name] = counts(quoted & (cls == name))
    for name in ('bid','ask','mid','spread'):
        mask = quoted & view.present(name)
        metrics[name+'_sum'] = np.bincount(codes[mask],weights=view.values(name)[mask],minlength=size)
        metrics[name+'_n'] = counts(mask)
    ages = quoted & view.present('sample_age_ns')
    metrics['sample_age_sum'] = np.bincount(codes[ages],weights=view.values('sample_age_ns')[ages]/NS,minlength=size)
    metrics['payload_age_sum'] = np.bincount(codes[ages],weights=view.values('unchanged_payload_age_ns')[ages]/NS,minlength=size)
    metrics['age_n'] = counts(ages)
    for name in ('sample_stale_60','sample_stale_300','sample_stale_900',
                 'payload_stale_60','payload_stale_300','payload_stale_900'):
        metrics[name] = counts(ages & view.values(name))
    slices = {}
    for code in np.flatnonzero(metrics['quoted']):
        q,d = divmod(int(code),width_dte);q,r = divmod(q,width_right);f,c = divmod(q,len(ALL_CUTS))
        key = '|'.join((FAMILY_NAMES[f],ALL_CUTS[c],('CALL','PUT','unknown')[r],
                        DTE_NAMES[d] if d < len(DTE_NAMES) else 'unknown'))
        slices[key] = {name:value[code].item() for name,value in metrics.items()}
    return slices
...
def run(*, protocol, admitted, oi_population, store, outputs, selected_dates=None,
        selected_chains=None, calculate_statistics=True, execution_resources=None):
    ...
    phase_cpu = {name: 0.0 for name in ('source_rows', 'cut_boards', 'histogram_pool', 'pool_serialization')}
    np = _np()
    ...
    for chain in work_chains:
        histogram_pool = HistogramUniverse()
        pool_row_start = len(date_aggregates)
        for day_index, day in enumerate(selected_days):
            day_source_start = pytime.process_time()
            label = day.isoformat()
            ...
            if owned.get("union") is not None:
                _source_frame_hists(day_hists, owned["union"], "union", include_raw=False)
                ...
                for key, bag in list(day_hists.items()):
                    ...
                    if bits[1] != 'raw_source' or bits[2] == 'union':
                        continue
                    bits[2] = 'union'
                    _hist_merge(day_hists, '|'.join(bits), bag[0], bag[1])
            del family_frames
            ...
            phase_cpu['source_rows'] += pytime.process_time() - day_source_start
            cut_started = pytime.process_time()
                ...
                for family in families_for_cuts:
                    ...
                    if not early_na:
                        ...
                        if len(chosen):
                            ...
                            board.append(_build_board_columns(
                        ...
                        if listing["listing_known"] and listed_ids is not None:
                            ...
                            if len(unquoted):
                                ...
                                board.append(_listed_unquoted_rows(
            ...
            oi_join.release_day()
            day_boards = _concat_boards(day_boards)
            ...
            if "near" in boards_by_family or "broad" in boards_by_family:
                ...
                for cut_label in ALL_CUTS:
                    paired_by_cut[cut_label] = _paired_stats(
                        near.filter(_pc().equal(near['cut_label'],cut_label)),
            ...
            phase_cpu['cut_boards'] += pytime.process_time() - cut_started
            pool_started = pytime.process_time()
            histogram_pool.add_row(aggregate)
            phase_cpu['histogram_pool'] += pytime.process_time() - pool_started
            date_aggregates.append({key: value for key, value in aggregate.items() if key != 'histograms'})
            del aggregate, hist_out, day_hists, owned
            if day_index + 1 == len(selected_days) or selected_days[day_index + 1].year != day.year:
                pool_started = pytime.process_time()
                pool_ref = histogram_pool.write_pool(outputs, chain=chain,
                    name=f'histogram-pool-{chain.lower()}-{day.year}')
                phase_cpu['pool_serialization'] += pytime.process_time() - pool_started
                for row in date_aggregates[pool_row_start:]:
                    row['pooled_histograms_ref'] = pool_ref
                histogram_pool = HistogramUniverse()
                pool_row_start = len(date_aggregates)
    ...
    return finish_quote_outputs(
        ...
        parse_cpu=parse_cpu, oi_support_cpu=oi_support_cpu, phase_cpu=phase_cpu,
        listing_unknown_chains=sorted(CHAINS_WITHOUT_LISTING),
        joined_coverage_complete=bool(oi_join.full_population),
        max_worker_bytes=int((execution_resources or spec.get("resources", {})).get("memory_bytes",MAX_WORKER_BYTES)),
