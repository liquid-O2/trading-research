...
class TimeAtPrice:
    ...
    def _add(self, raw, trades, *, source_key):
        ...
        clock_ok = clock_order_valid(t, None if self.last_raw is None else self.last_raw[0], getattr(self, 'clock_uncertain_intervals', ()))
        if (any(v.dtype.kind not in "iu" for v in (t, order, flags)) or np.any(t < self.start) or np.any(t >= self.end)
                or not clock_ok or np.any(order < 0) or np.any(order[1:] <= order[:-1]) or np.any(flags > 255)
                ...
                or self.last_raw is not None and int(order[0]) <= self.last_raw[1]):
    ...
    def _durations(self, starts, ends, prices, valid):
        ...
        for cap, cells in self.dwell.items():
            ...
            if uncertainty:
                ...
                eligible, selected_prices = valid[owners], prices[owners]
            else:
                eligible, selected_prices = valid, prices
            keep = eligible & (b > a)
            aa, bb = a[keep], b[keep]
            rows = (selected_prices[keep] - self.origin) // self.width
            if not len(rows):
                continue
