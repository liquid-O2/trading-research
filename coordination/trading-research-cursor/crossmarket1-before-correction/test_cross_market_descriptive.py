...
from trading_research.research.cross_market_alignment import (
    ...
    parse_raw_minute_arrays, running_breaches, running_references,
    scenario_known_at, session_label, volume_unit_for,
)
from trading_research.research.cross_market_descriptive import (
    ...
    independent_date_support, pair_timing_rows, run,
)
...
class CalendarMappingSmtFixtures(unittest.TestCase):
    def test_dst_and_early_close_calendar(self):
        ...
        self.assertEqual(summer_open - winter_open, -3600 * NS)
...
class PairingAndStatisticsFixtures(unittest.TestCase):
    ...
    def test_shared_one_vs_one_hundred_same_date_events_balanced(self):
        ...
        self.assertEqual(len(cells["2020-01-06"]), 1)
        self.assertEqual(len(cells["2020-01-07"]), 100)
...
class IntegratedTinyTables(unittest.TestCase):
    def test_integrated_tiny_nearby_source_event_all_horizons_typed_tables(self):
        ...
        events = result["refs"]["events"]
        self.assertGreater(events["rows"], 0)
        import pyarrow as pa
        tables = list(read_series_tables(events))
        self.assertTrue(tables)
        combined = pa.concat_tables(tables)
        kinds = set(combined["event_kind"].to_pylist())
        self.assertTrue(any(kind.startswith("running") for kind in kinds))
        horizons = set(combined["horizon_minutes"].to_pylist())
        self.assertEqual(horizons, {1, 5, 15, 30, 60})
        self.assertEqual(str(combined.schema.field("source_start_ns").type), "int64")
        self.assertEqual(str(combined.schema.field("label_start_ns").type), "int64")
        self.assertEqual(str(combined.schema.field("horizon_minutes").type), "int64")
        self.assertEqual(str(combined.schema.field("terminal_log").type), "double")
        self.assertTrue(all(value is None for value in combined["local_node_presence"].to_pylist()))
        self.assertIn(1, combined["direction"].to_pylist())
        self.assertGreaterEqual(combined["no_receiver"].to_pylist().count(False), 5)
