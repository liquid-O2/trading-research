...
OPERATIONAL_LIMITS=ROOT/'validation/CROSS_MARKET_OPERATIONAL_LIMITS_V1.json'
...
def effective_resources(protocol, reference):
    ...
    if (resources['memory_bytes']!=protocol['resources']['memory_bytes']
...
def run_measurement(packet,protocol,store,outputs):
    ...
    if packet['mode']=='full':
        ...
        if (pilot['runner_sha256']!=packet['runner_sha256'] or pilot['configuration_implementation_paths']!=paths
            ...
            raise ValueError('full cross-market requires unchanged tested runner and transitive implementation')
...
def worker(packet_path):
    resource.setrlimit(resource.RLIMIT_CPU,(9010,9010));resource.setrlimit(resource.RLIMIT_AS,(4294967296,4294967296))
