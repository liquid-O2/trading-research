...
def integer_profile_geometry(mass, spec):
    ...
    values = np.zeros(end - start + 1, dtype=np.int64)
    for row in rows:
        values[row - start] = (
            int(buy.get(row, 0)) + int(sell.get(row, 0)) + int(unknown.get(row, 0))
        )
    mass_total = int(values.sum())
    maximum = int(values.max()) if values.size else 0
    modes = tuple(start + int(i) for i in np.flatnonzero(values == maximum)) if maximum else ()
    ...
    while index < n_values:
        ...
        if a > start and b < end and height < left and height < right:
            valleys.append((a, b))
        index = last + 1
    requested = Fraction(mass_total * int(numer), int(denom))
    overshoot = max(Fraction(0), Fraction(achieved) - requested)
    unique_heights = np.unique(values)
    second = int(unique_heights[-2]) if unique_heights.size > 1 else 0
    dominance = Fraction(maximum - second)
    sq = int(np.dot(values.astype(np.int64, copy=False), values.astype(np.int64, copy=False)))
    concentration = Fraction(sq, mass_total * mass_total) if mass_total else None
    weights = np.arange(start, end + 1, dtype=np.int64)
    mean_row = Fraction(int(np.dot(weights, values)), mass_total) if mass_total else None
    ...
    compact = {
        ...
        'achieved_mass_num': int(achieved),
        'achieved_mass_den': 1,
...
def compact_side_geometry(mass):
    ...
    return {
        ...
        'side_wasserstein_num': None,
        'side_wasserstein_den': None,
...
class AliasIndex:
    """O(1) exact-alias collapse. Conflicting overlap is unavailable, no winner."""
    ...
    def add(self, atom):
        ...
        if prior is not None:
            if content_alias_key(prior) == content:
                return None
            self.conflicts.append({'left': atom_identity_key(atom), 'right': atom_identity_key(prior)})
            self.poisoned.add(slot)
            self.retracted.append(prior)
            self.by_slot.pop(slot, None)
            self.by_content.pop(content_alias_key(prior), None)
            return None
        other = self.by_content.get(content)
        if other is not None:
            return None
        self.by_slot[slot] = atom
        self.by_content[content] = atom
        return atom
...
def join_schema(pa):
    ...
    return pa.schema([
        ...
        flag('future_input_known'), text('future_mass_id'),
...
def emit_join_rows(*, writer, completeness, windows, geometry_row, feature_lookup,
    ...
    features = [] if match is None else list(match.get('features') or ([match['feature']] if match.get('feature') else ()))
    labels = [] if match is None else match['labels']
    if match is None:
            ...
            for horizon_kind in HORIZON_KINDS:
                ...
                for horizon in horizons:
                    ...
                    for candidate in labels:
                        ...
                        break
                    row = _empty_join_row()
                    formation = None if feature is None else feature.get('formation_minutes')
                    scenario = None if cut_ns is None else int(cut_ns) + int(latency)
                    row.update({
                        ...
                        'actual_known_at_ns': None if feature is None else feature.get('known_at_ns'),
                        'scenario_known_at_ns': scenario,
                    ...
                    own_ready = prefix_ok and (
                        coverage_ok or geometry_row.get('prefix_complete') is True
                    )
                    if geometry_row.get('prefix_complete') is not True:
                        own_ready = False
                        row['censor_reason'] = row.get('censor_reason') or 'own_geometry_incomplete'
                    if not own_ready or scientific.get('eligible') is not True:
                        row['join_status'] = (
                            'stage_leak' if scientific.get('scientific_reason') == 'label_maturity_not_same_named_stage'
                            else 'formation_incomplete' if not own_ready
                            else 'rejected'
                        )
                    ...
                    if horizon_kind == 'fixed_minutes' and horizon is not None:
                        cache_key = (geometry_row.get('geometry_id'), cut_ns, horizon)
                        if cache_key not in future_cache:
                            ...
                            future_cache[cache_key] = (future_compact, future_known)
                        future_compact, future_known = future_cache[cache_key]
...
class IdentityEngine:
    """Bounded per-contract state: rolling window, civil masses, one unflushed day."""
    ...
    def __init__(self, *, instrument_id, contract_key, collection, root, year,
        ...
        self.prior_close = {'ticks': None, 'known': None}
        self.clock_cache = {}
    ...
    def finish(self):
        ...
        named = self._named(last)
        same = [atom for atom in self.pending if atom.get('source_collection') == self.collection]
        day_atoms = self._day_atoms(last)
        if not same:
            return
        sample = day_atoms[0] if day_atoms else same[-1]
        economic = last.isoformat()
        stage = stage_name(self.root, economic, self.policy)
        for anchor in named.get('anchors') or ():
            if anchor.variant not in CIVIL_VARIANTS:
                continue
            if (anchor.variant, anchor.start_ns) in self.civil_emitted:
                continue
            self._emit_completed(
                last, economic, stage, sample, anchor, same, None,
    ...
    def flush_ready(self, horizon_ns):
        ...
        for day in days:
            if int(day.isoformat()[:4]) != int(self.year) or day in self.flushed:
                continue
        ...
        if self.flushed:
            last = max(self.flushed)
            keep_after = _economic_session_bounds(last)[1] - 240 * MINUTE_NS
    ...
    def flush_day(self, day):
        ...
        for atom in day_atoms:
            ...
            for civ in named.get('anchors') or ():
                if civ.variant not in CIVIL_VARIANTS or not _wholly_inside(atom, civ.spans):
                    continue
                seen = (civ.variant, atom['event_start_ns'], atom['event_end_ns'])
                if seen in self.civil_seen:
                    continue
                self.civil_seen.add(seen)
        ...
        same = [atom for atom in self.pending if atom.get('source_collection') == collection]
    ...
    def _emit_developing(self, day, economic, stage, sample, cash_anchor, futures_anchor, same,
        ...
        for kind, base in (
            ...
            for cut_ns in cuts:
                ...
                compact['observed_prior_close_ticks'] = self.prior_close['ticks']
                compact['observed_prior_close_known_at_ns'] = self.prior_close['known']
                gid = geometry_id_for(
                ...
                prior = next(
                    (
                        item for item in self.published_completed
                        if item['variant'] == base.variant
                        and item['contract_key'] == self.contract_key
                        and item['published_ns'] <= cut_ns
                        and item.get('instrument_id') == self.instrument_id
                    ),
                    None,
                )
    ...
    def _emit_completed(self, day, economic, stage, sample, anchor, same, pin_snap, *, last_year_day,
        ...
        if anchor.variant in CIVIL_VARIANTS:
            civ_key = (anchor.variant, anchor.start_ns)
        ...
        for spec in FROZEN_GEOMETRY_VARIANTS:
            ...
            compact, _view = emit_geometry_payload(
                ...
                pin069=pin_snap if include_aux else None,
            )
            compact['observed_prior_close_ticks'] = self.prior_close['ticks']
            compact['observed_prior_close_known_at_ns'] = self.prior_close['known']
            gid = geometry_id_for(
        ...
        if complete:
            ...
            if anchor.variant == 'cash_rth' and hist.last_priced_ticks is not None:
                self.prior_close['ticks'] = hist.last_priced_ticks
                self.prior_close['known'] = hist.last_priced_known_at_ns
        for proxy in BAR_PROXY_VARIANTS:
            ...
            _fill_geometry_identity(
                row, collection=collection, root=root, year=year, stage=stage,
                session=session, atom=sample, anchor=anchor,
        ...
        if anchor.variant in CIVIL_VARIANTS:
            for minutes in BRACKET_MINUTES:
                for representation in ('whole_trade_visits', 'ohlc_range_proxy'):
                    ...
                    tpo_row.update({
                        ...
                        'instrument_id': self.instrument_id, 'source_path': sample.get('source_path'),
                        'economic_date': economic,
                    })
                    if engine is None:
            ...
            _write_tpo(
                ...
                path=sample.get('source_path'), economic=economic,
...
def compute_profile_partition(*, key, members, neighbors, identity, source_refs, contract,
    ...
    _DEAD_LEGACY_LOOP = r'''
        atoms = by_identity[ident]
        atoms.sort(key=lambda item: (item['event_start_ns'], item['event_end_ns'], item.get('source_path') or ''))
        instrument_id, contract_key = ident
        if contract_key is None or not str(contract_key).startswith(str(root) + ':'):
            completeness['missing_contract_identity'] += 1
            continue
        lineage = source_lineage_id(collection, root, contract_key)
        same = [
        ...
        civil_mass = {name: IntegerMass() for name in CIVIL_VARIANTS}
        civil_atoms = {name: [] for name in CIVIL_VARIANTS}
        prior_close = {'ticks': None, 'known': None}
        pin069_state = None
        clock_cache = {}
        civil_emitted = set()
        year_days = [item for item in days if int(item.isoformat()[:4]) == year]
        last_year_day = year_days[-1] if year_days else None
        rolling_state = {
            minutes: {'hist': IntegerMass(), 'window': deque(), 'cursor': 0}
            for minutes in ROLLING_MINUTES
        }
        pending = list(same)
        for day in days:
            ...
            cut_for_clocks = local_timestamp(day, time(17, 0), ZONE)
            cache_key = (day, instrument_id, contract_key)
            if cache_key not in clock_cache:
                clock_cache[cache_key] = _clock_anchors_for_day(
                    day=day, cut_ns=cut_for_clocks, calendar=calendar, root=root,
                    instrument_id=instrument_id, contract_key=contract_key,
                    source_lineage=lineage,
                )
            named = clock_cache[cache_key]
            for missing in named.get('unavailable') or ():
            ...
            session = session_of(cut_for_clocks, calendar=calendar, known_at_ns=cut_for_clocks)['session']
            day_atoms = [
                atom for atom in same
                if date.fromisoformat(economic_date(int(atom['event_start_ns']))) == day
                or date.fromisoformat(economic_date(int(atom['event_end_ns'] - 1))) == day
            ]
            sample = day_atoms[0] if day_atoms else same[0]
            cash_anchor = next((a for a in named.get('anchors') or () if a.variant == 'cash_rth'), None)
            futures_anchor = next(
                (a for a in named.get('anchors') or () if a.variant == 'observed_futures_18_17'), None)
            utc4 = next(
                (a for a in named.get('anchors') or () if a.variant == 'source_06_09_fixed_utc_minus4'), None)
            if utc4 is not None and pin069_state is None:
                pin069_state = IncrementalPIN069(utc4)
            for atom in day_atoms:
                if pin069_state is not None and _wholly_inside(atom, utc4.spans if utc4 else ()):
                    pin069_state.add(atom)
                for civ in named.get('anchors') or ():
                    if civ.variant in CIVIL_VARIANTS and _wholly_inside(atom, civ.spans):
                        civil_mass[civ.variant].add(atom)
                        civil_atoms[civ.variant].append(atom)
            ...
            for kind, base in developing:
                if base is None:
                    ...
                    continue
                start, end = base.start_ns, min(base.end_ns, same[-1]['event_end_ns'])
                cuts = aligned_cuts(start, end)
                hist = IntegerMass()
                members_dev = [atom for atom in same if _wholly_inside(atom, ((base.start_ns, base.end_ns),))]
                members_dev.sort(key=lambda item: item['event_end_ns'])
                ...
                for cut_ns in cuts:
                    ...
                    pin_snap = pin069_state.snapshot() if pin069_state is not None else None
                    compact, _view = emit_geometry_payload(
                        mass=hist, spec=baseline,
                        coordinate_identity=f'{root}:{contract_key}:{lineage}',
                        known_at_ns=published, coverage_complete=prefix_complete,
                        include_side=True, include_vwap=True, include_footprint=True,
                        footprint_params=footprint_params, verify_kernel=False,
                        anchor_kind=base.kind, pin069=pin_snap,
                    )
                    ...
                    row['prefix_complete'] = prefix_complete
                    row['formation_complete'] = prefix_complete
                    row['acquired_event_start_ns'] = sample.get('acquired_event_start_ns')
                    row['acquired_event_end_ns'] = sample.get('acquired_event_end_ns')
                    prior = next(
                        (
                            item for item in published_completed
                            if item['variant'] == base.variant
                            and item['contract_key'] == contract_key
                            and item['published_ns'] <= cut_ns
                    ...
                    prev_price = last_price
                    emit_geo(row, compact, complete=False)
                    completeness['baseline_emitted'] += 1
                    emit_join_rows(
                        writer=join_w, completeness=completeness, windows=windows,
                        geometry_row=row, feature_lookup=None, cut_ns=cut_ns,
                        collection=collection, root=root, year=year, stage=stage,
                        session=row['session'], economic=economic,
                        identity_atoms=same, mass=hist, motion=motion,
                        later_link=False, future_cache=future_cache,
                    )

            for minutes in ROLLING_MINUTES:
                width = minutes * MINUTE_NS
                state = rolling_state[minutes]
                hist, window, cursor = state['hist'], state['window'], state['cursor']
                cuts = aligned_cuts(
                    min(atom['event_start_ns'] for atom in same),
                    max(atom['event_end_ns'] for atom in same),
                )
                for cut_ns in cuts:
                    eco = economic_date(int(cut_ns - 1))
                    if int(eco[:4]) != int(year):
                        continue
                    if date.fromisoformat(eco) != day:
                        continue
                    ...
                    while window and window[0]['event_start_ns'] < cut_ns - width:
                        hist.remove(window.popleft())
                    members_roll = list(window)
                    if any(atom.get('source_collection') not in (None, collection) for atom in members_roll):
                        raise IntegrityError('rolling window mixed a foreign source collection')
                    rolling = rolling_anchor(
                        minutes=minutes, event_end_ns=cut_ns,
                        selection_known_at_ns=cut_ns, source_versions=(lineage,),
                    ...
                    row['acquired_event_start_ns'] = sample.get('acquired_event_start_ns')
                    row['acquired_event_end_ns'] = sample.get('acquired_event_end_ns')
                    emit_geo(row, compact, complete=coverage['coverage_complete'] and hist.priced_volume > 0)
                    completeness['baseline_emitted'] += 1
                    emit_join_rows(
                        writer=join_w, completeness=completeness, windows=windows,
                        geometry_row=row, feature_lookup=None, cut_ns=cut_ns,
                ...
                rolling_state[minutes]['cursor'] = cursor

            for anchor in named.get('anchors') or ():
                if anchor.variant in CIVIL_VARIANTS:
                    civ_key = (anchor.variant, anchor.start_ns)
                    ends = _civil_period_end(day, anchor.variant)
                    if civ_key in civil_emitted:
                        continue
                    if not ends and day != last_year_day:
                        continue
                    civil_emitted.add(civ_key)
                members_fixed = _atoms_in_spans(same, anchor.spans)
                coverage = coverage_from_atoms(anchor.spans, members_fixed)
                cut_ns = anchor.end_ns
                input_known = _input_known(members_fixed, cut_ns)
                ...
                if anchor.variant in CIVIL_VARIANTS:
                    hist = civil_mass[anchor.variant]
                    members_fixed = civil_atoms[anchor.variant]
                    coverage = coverage_from_atoms(anchor.spans, members_fixed)
                else:
                    hist = IntegerMass()
                    for atom in members_fixed:
                    ...
                    hist.copy_flags_from(members_fixed)
                complete = coverage['coverage_complete'] and hist.priced_volume > 0
                variants = FROZEN_GEOMETRY_VARIANTS if complete else (baseline,)
                pin_snap = pin069_state.snapshot() if pin069_state is not None else None
                completed_id = None
                baseline_row = None
                cells_hash = digest(sorted(hist.source_hashes) + [anchor.variant, cut_ns])
                for spec in variants:
                    ...
                    compact, _view = emit_geometry_payload(
                        ...
                        verify_kernel=complete, anchor_kind=anchor.kind, pin069=pin_snap if include_aux else None,
                    )
                    compact['observed_prior_close_ticks'] = prior_close['ticks']
                    compact['observed_prior_close_known_at_ns'] = prior_close['known']
                    gid = geometry_id_for(
                    ...
                    row['acquired_event_end_ns'] = sample.get('acquired_event_end_ns')
                    row['provenance_paths'] = ','.join(sorted(hist.source_paths))
                    emit_geo(row, compact, complete=complete)
                    if spec['id'] == BASELINE_VARIANT_ID:
                        baseline_row = row
                if complete:
                    published_completed.append({
                        'variant': anchor.variant,
                        'contract_key': contract_key,
                        'instrument_id': instrument_id,
                        'published_ns': published,
                    ...
                    if anchor.variant == 'cash_rth' and hist.last_priced_ticks is not None:
                        prior_close['ticks'] = hist.last_priced_ticks
                        prior_close['known'] = hist.last_priced_known_at_ns
                    for proxy in BAR_PROXY_VARIANTS:
                        proxy_row, _ = bar_proxy_from_mass(
                            hist, proxy,
                            coordinate_identity=f'{root}:{contract_key}:{lineage}',
                        ...
                        row['acquired_event_start_ns'] = sample.get('acquired_event_start_ns')
                        row['acquired_event_end_ns'] = sample.get('acquired_event_end_ns')
                        emit_geo(row, proxy_row, complete=complete)
                        completeness['proxy_emitted'] += 1
                    _write_tpo(
                        writer=tpo_w, completeness=completeness, collection=collection,
                        root=root, year=year, stage=stage, session=session, anchor=anchor,
                        cut_ns=cut_ns, contract_key=contract_key, instrument_id=instrument_id,
                        path=sample.get('source_path'), economic=economic,
                        members=members_fixed, published=published, geometry_id=completed_id,
                    )
                if baseline_row is not None:
                    ...
                    later_cuts = aligned_cuts(cut_ns, min(cut_ns + 6 * 60 * MINUTE_NS, same[-1]['event_end_ns']))
                    for later in later_cuts:
                        if later <= cut_ns or published > later:
                            continue
                        emit_join_rows(
                            writer=join_w, completeness=completeness, windows=windows,
                            geometry_row=baseline_row, feature_lookup=None, cut_ns=later,
                            collection=collection, root=root, year=year, stage=stage,
                            session=session_of(later, calendar=calendar, known_at_ns=published)['session'],
                            economic=economic, identity_atoms=same, mass=hist, motion={
                                'boundary_moving': False, 'price_crossing_frozen_va': False,
                                'prior_val_row': None, 'prior_vah_row': None,
                                'current_val_row': baseline_row.get('val_row'),
                                'current_vah_row': baseline_row.get('vah_row'),
                            },                             later_link=True, future_cache=future_cache,
                        )
    '''
