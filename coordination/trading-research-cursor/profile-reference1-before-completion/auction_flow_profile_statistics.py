...
METRIC_UNITS = {
    ...
    'peak_count': 'count',
    'valley_count': 'count',
    'dominance_margin': 'dimensionless',
...
JOIN_METRIC_NAMES = (
    ...
    'future_poc', 'future_low_overflow_buy', 'future_high_overflow_buy',
    'future_input_known',
)
PAIRED_METRIC_NAMES = (
...
def accumulate_join_row(acc, row, *, collection, year):
    ...
    for name in ('future_poc', 'future_low_overflow_buy', 'future_high_overflow_buy'):
        if name not in store:
            store[name] = _DateStore()
...
def _markdown_report(*, groups, completeness, population, cpu, output_bytes, reused,
    ...
    lines.append(
        ...
        'status | contrast | formation | latency | horizon | metric | unit | date_mean | '
        'event_mean | dates | events | undefined |'
    )
    lines.append('|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|')
    for group in groups:
        ...
        for name, payload in metrics.items():
            ...
            lines.append(
                ...
                f'{group.get("horizon_kind")}/{group.get("horizon_minutes")} | {name} | '
                f'{payload.get("unit")} | {date_mean} | {payload.get("event_mean")} | '
                f'{payload.get("eligible_independent_dates")} | '
...
def run_profile_statistics(*, population, window_population, contract, outputs, load_reference,
    ...
    for ordinal, key in enumerate(selected_keys):
        ...
        if identity in accepted and accepted[identity][1].get('passed') is True:
            ...
            for name in (
                'units_scanned', 'units_unavailable', 'units_neighbor', 'atoms_matched',
                'geometry_emitted', 'geometry_complete', 'geometry_partial', 'geometry_null',
                'tpo_emitted', 'tpo_failed', 'joins_emitted', 'joins_missing', 'joins_censored',
            ):
