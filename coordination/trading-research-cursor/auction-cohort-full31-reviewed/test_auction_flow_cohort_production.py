...
def build_measured(rows, *, start, end, delay, atoms, instrument_id=INSTRUMENT,
    ...
    return {
        ...
        'source_manifest': {'physical_scan_rows': len(rows), 'expected_physical_scan_rows': len(rows),
            'source_continuation': {'completed': True}, 'projection': {'counts': {'trades': len(rows)}}},
        'instruments': [{
...
class AuctionFlowCohortProductionTests(unittest.TestCase):
    ...
    def test_full_training_thresholds_cuts_paths_and_cases(self):
        ...
        with TemporaryDirectory() as folder:
            ...
            self.assertTrue(all(item['requires_date_block_aggregation'] for item in whole_rows))
            self.assertTrue(all(item['instrument_id'] == INSTRUMENT for item in whole_rows))
    ...
    def test_missing_tampered_and_duplicate_expected_members_reject(self):
        ...
        with TemporaryDirectory() as folder:
            ...
            incomplete, _, _ = write_receipt(folder, 'incomplete', second, late, source_complete=False,
                                             coordinate_complete=False, mutate_measurement=lambda m: m['source_manifest']['source_continuation'].update(completed=False))
            with self.assertRaises(IntegrityError):
                accumulate_training_collection(
                    root='NQ', collection='monthly', expected_members=(first, second),
...
class StreamingTrainingReceiptTests(unittest.TestCase):
    def test_actual_receipt_kind_loads_once_per_window_without_reloading_at_completion(self):
        from unittest.mock import patch
        import trading_research.research.auction_flow_cohort_production as production
        first = member('NQ', MONTHLY_A, DAY1, DAY1 + MINUTE, '2020-01-01')
        second = member('NQ', MONTHLY_A, DAY2, DAY2 + MINUTE, '2020-01-02')
        with TemporaryDirectory() as folder:
            a, _, out_a = write_receipt(folder, 'stream-a', first, [row(DAY1 + 1, source_order=0, size=2)])
            b, _, out_b = write_receipt(folder, 'stream-b', second, [row(DAY2 + 1, source_order=1, size=7)])
            ra = out_a.json('source-window-receipt.json', a, kind=WINDOW_RECEIPT_KIND)
            rb = out_b.json('source-window-receipt.json', b, kind=WINDOW_RECEIPT_KIND)
            with patch.object(production, '_load_measurement', wraps=production._load_measurement) as loaded:
                acc = TrainingCollectionAccumulator(root='NQ', collection='monthly',
                    expected_members=(first, second), contract=full_population_cohort_contract())
                acc.add_receipt(ra)
                acc.add_receipt(rb)
                report = acc.complete()
                self.assertEqual(loaded.call_count, 2)
                self.assertEqual(report['prints'], 2)
                self.assertEqual(report['contracts'], 9)
                self.assertEqual(report['histogram_report']['histogram'], ((2, 1), (7, 1)))
                acc.close()

class AcquiredCoverageTests(unittest.TestCase):
    def test_complete_acquired_projection_retains_temporal_and_coordinate_gaps(self):
        item = member('NQ', MONTHLY_A, DAY1, DAY1 + MINUTE, '2020-01-01')
        contract = full_population_cohort_contract()
        with TemporaryDirectory() as folder:
            receipt, _, _ = write_receipt(folder, 'gaps', item,
                [row(DAY1 + 1, source_order=0, size=9)], source_complete=False, coordinate_complete=False)
            result = accumulate_training_collection(root='NQ', collection='monthly',
                expected_members=(item,), receipts=(receipt,), contract=contract)
            self.assertEqual(result['histogram_report']['histogram'], ((9, 1),))
            self.assertFalse(result['members'][0]['source_complete'])
            self.assertFalse(result['members'][0]['coordinate_complete'])
            self.assertFalse(result['full_temporal_market_history_complete'])
            self.assertEqual(result['members'][0]['member_completeness_basis'], 'complete_selected_acquired_physical_scan')

    def test_incomplete_selected_scan_cannot_enter_training(self):
        item = member('NQ', MONTHLY_A, DAY1, DAY1 + MINUTE, '2020-01-01')
        with TemporaryDirectory() as folder:
            receipt, _, _ = write_receipt(folder, 'partial', item, [row(DAY1 + 1, source_order=0)],
                mutate_measurement=lambda m: m['source_manifest'].update(expected_physical_scan_rows=2))
            with self.assertRaisesRegex(IntegrityError, 'complete selected acquired physical scan'):
                accumulate_training_collection(root='NQ', collection='monthly', expected_members=(item,),
                    receipts=(receipt,), contract=full_population_cohort_contract())
