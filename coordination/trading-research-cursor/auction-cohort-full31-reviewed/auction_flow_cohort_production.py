...
_ROW_FIELDS = (
    ...
    'acquisition_overlap_key', 'row_role', 'instrument_id', 'contract_key', 'requires_date_block_aggregation')
_CPU_NAMES = (
...
def full_population_cohort_contract():
    ...
    return {
        ...
        'member_completeness_basis': 'complete_selected_acquired_physical_scan',
    }
...
def validate_cohort_production_contract(contract):
    ...
    if (contract.get('kind') != CONTRACT_KIND
            ...
            or contract.get('member_completeness_basis') != 'complete_selected_acquired_physical_scan'):
...
def _window_complete(measured):
    ...
    if not instruments:
        # No raw instrument is evidence of neither temporal nor coordinate coverage.
        return False, False
...
def _acquired_projection_complete(measured, trades):
    """Completed supplied tape projection, never unobserved temporal market history."""
    manifest = measured.get('source_manifest') or {}
    scanned = manifest.get('physical_scan_rows')
    expected = manifest.get('expected_physical_scan_rows')
    continuation = manifest.get('source_continuation') or {}
    counts = (manifest.get('projection') or {}).get('counts') or {}
    if (type(scanned) is not int or type(expected) is not int or scanned < 0
            or scanned != expected or continuation.get('completed') is not True):
        raise IntegrityError('training/evaluation receipt lacks a complete selected acquired physical scan')
    if (type(counts.get('trades')) is not int or counts['trades'] != trades.get('rows')
            or counts['trades'] != sum(item['whole_window']['prints'] for item in measured['instruments'])):
        raise IntegrityError('acquired projection trade count does not join retained trades and measurements')
    return True
...
def load_cohort_receipt(receipt_or_ref):
    ...
    if (type(receipt_or_ref) is dict and {'path', 'sha256', 'size_bytes'} <= set(receipt_or_ref)
            and 'unit' not in receipt_or_ref):
...
def authenticate_cohort_receipt(receipt, *, expected_member, contract):
    ...
    if receipt.get('unit_identity') != identity:
    ...
    return {
        ...
        'window_acquired_complete': _acquired_projection_complete(measured, artifacts['trades']),
        'member_completeness_basis': 'complete_selected_acquired_physical_scan',
            ...
            with prepare_trade_batch(part) as prepared:
                if len(prepared):
                    ...
                    if (int(t[0]) < expected['event_start_ns'] or int(t[-1]) >= expected['event_end_ns']
                            or np.any(t < expected['event_start_ns']) or np.any(t >= expected['event_end_ns'])):
...
def accumulate_training_collection(*, root, collection, expected_members, receipts, contract):
    """Stream complete windows into one bounded histogram, retaining small metadata."""
    accumulator = TrainingCollectionAccumulator(root=root, collection=collection,
        expected_members=expected_members, contract=contract)
    try:
        for receipt in receipts:
            accumulator.add_receipt(receipt)
...
class TrainingCollectionAccumulator:
    """One current decompressed window, one histogram, explicit member metadata."""
    ...
    def __init__(self, *, root, collection, expected_members, contract):
        ...
        self.root, self.collection, self.contract = root, collection, contract
        self.bounds = training_bounds(root)
        self._expected = [schedule_member_key(item) for item in expected_members]
        self._keys = [_join_key(item) for item in self._expected]
        if not self._keys or len(set(self._keys)) != len(self._keys):
        ...
        if any(not self.bounds['start_date'] <= item['utc_date'] <= self.bounds['end_date']
               for item in self._expected):
            raise ContractError('testing, calibration or confirmation rows cannot enter training')
        self._pending = dict(zip(self._keys, self._expected, strict=True))
        self._members = {}
        self._failed = self._closed = False
        self.cpu, self.counts = _cpu(), _counts()
        manifests = {'kind': 'full_acquired_eligible_projection', 'root': root, 'collection': collection,
            'histogram_scope': HISTOGRAM_SCOPE, 'full_temporal_market_history_complete': False,
            'training_scope': TRAINING_SCOPE, 'contract_identity': contract_identity(contract)}
        self.histogram = TradeSizeHistogram(source_manifest=manifests, fold_manifest=dict(manifests),
                                           aggregation_unit=contract['aggregation_unit'])
        self.helper_cpu = 0.0

    def _live(self):
        if self._failed or self._closed:
            raise IntegrityError('failed or closed training accumulator cannot publish or continue')
    ...
    def add_receipt(self, receipt):
        ...
        try:
            ...
            item = authenticate_cohort_receipt(receipt, expected_member=self._pending[key], contract=self.contract)
            if item['split'] != 'train' or not item['window_acquired_complete']:
                raise IntegrityError('a missing or incomplete expected training window cannot be treated as empty or completed')
            at = _acc(self.cpu, 'receipt_authentication_and_membership', at)
            member_id, source_key, last_obs, published = _ingest_training_trades(item, self.histogram, self.counts)
            _acc(self.cpu, 'histogram_accumulation', at)
            # Do not retain item: it contains the full decompressed measurement.
            member = {'id': member_id, 'expected': item['expected'], 'unit_identity': item['unit_identity'],
                'source_key': source_key, 'end': item['expected']['event_end_ns'],
                'published_at': item['published_at'], 'last_observation_ns': last_obs,
                'last_observation_published_at': published, 'history_complete': item['window_acquired_complete'],
                'member_completeness_basis': item['member_completeness_basis'],
                'source_complete': item['source_complete'], 'coordinate_complete': item['coordinate_complete'],
                'whole_window_inside_acquired_file': item['whole_window_inside_acquired_file'],
                'measurement_reference': item['measurement_reference'], 'split': item['split']}
            ...
            self.helper_cpu += time.process_time() - began
    ...
    def complete(self):
        ...
        report = self.histogram.record(coverage_complete=True)
        self._closed = True
        members = tuple(self._members[key] for key in self._keys)
        event_period_cut = self.bounds['event_period_cut_ns']
        last_window_published = max(item['published_at'] for item in members)
        observations = [item['last_observation_published_at'] for item in members
                        if item['last_observation_published_at'] is not None]
        last_observation_published = max(observations) if observations else None
        knowledge_cut = max(event_period_cut, last_window_published,
                            last_observation_published or event_period_cut)
        helper = self.helper_cpu + time.process_time() - began
        cpu = dict(self.cpu)
        cpu['orchestration_and_report_assembly'] = max(0.0, helper - sum(cpu.values()))
        return {'version': VERSION, 'kind': HISTOGRAM_KIND, 'publication_status': 'unpublished',
            'output_kind': _UNPUBLISHED, 'admitted_for_serving': False,
            'root': self.root, 'collection': self.collection, 'histogram_report': report, 'members': members,
            'coverage': {'expected_members': len(members), 'joined_members': len(members), 'missing_members': (),
                'partial_files': tuple(m['expected']['utc_date'] for m in members
                                       if m['whole_window_inside_acquired_file'] is False),
                'acquired_projection_complete': True, 'full_temporal_market_history_complete': False,
                'histogram_scope': HISTOGRAM_SCOPE},
            'event_period_cut_ns': event_period_cut, 'last_window_published_at': last_window_published,
            'last_observation_published_at': last_observation_published, 'knowledge_cut_ns': knowledge_cut,
            'full_temporal_market_history_complete': False, 'histogram_scope': HISTOGRAM_SCOPE,
            'contract_identity': contract_identity(self.contract), 'prints': report['prints'],
            'contracts': report['contracts'], 'histogram_identity': report['identity'],
            'cpu_components_disjoint': cpu, 'helper_cpu_seconds': helper, 'workload_counts': dict(self.counts)}
...
def fit_training_collection(payload, *, contract, evaluation_members=None):
    ...
    if not report.get('prints'):
        return {
            ...
            'available_at': payload['knowledge_cut_ns'],
...
def measure_source_window_cohorts(receipt, *, training, contract, outputs, expected_member=None):
    ...
    try:
        ...
        for instrument in instruments:
            ...
            for row in rowset[rowset_start:]:
                row['instrument_id'] = raw_id
                row['contract_key'] = instrument['coordinate'].get('contract_key')
                row['independent_date_unit'] = False
                row['requires_date_block_aggregation'] = True
            reports.append(instrument_report)
    finally:
        _release(prepared)
    if not instruments:
        # Retain the requested empty raw-instrument case without inventing an instrument or quiet minutes.
        for item in plan:
            row = _unavailable_row(expected['root'], expected['collection'], expected['source_path'], date,
                split, 'whole_window', None, start_ns, end_ns, False, False, item['name'],
                'no_observed_raw_instrument', independent=False)
            row.update(instrument_id=None, contract_key=None, requires_date_block_aggregation=True)
            rowset.append(row)
    ...
    result = {
        ...
        'date_aggregation': {
            ...
            'independent_date_unit': False,
            'requires_date_block_aggregation': True,
