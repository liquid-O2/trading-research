...
OPERATIONAL_LIMITS=ROOT/'validation/OPTIONS_QUOTE_OPERATIONAL_LIMITS_V2.json'
...
def quote_partition_plan(protocol,admitted,*,resources=None):
    ...
    cap=resources or protocol['resources'];reserve=6*1024**3;floor=256*1024**2
...
def quote_full_gate(protocol,admitted,pilot,oi_population,*,resources=None):
    ...
    if (projected_cpu>cap['mode_cpu_seconds']['full'] or projected_output>cap['per_attempt_output_bytes']-32*1024**2
            ...
            or statistics_output>6*1024**3-512*1024**2 or wall>cap['wall_seconds']):raise ValueError('measured quote full projection exceeds registered bounds: '+str(gate))
    return gate
...
def quote_shard(registered_path,child_path):
    resource.setrlimit(resource.RLIMIT_CPU,(90010,90010));resource.setrlimit(resource.RLIMIT_AS,(17179869184,17179869184))
