...
class HistogramUniverse:
    ...
    def add_pool(self, reference, *, allowed_dates, chain):
        ...
        for part in payload['parts']:
            ...
            for item in piece['histograms']:
                keys = [tuple(item['key']), *(tuple(key) for key in item.get('aliases',[]))]
    ...
    def write_pool(self, outputs, *, chain, name, maximum_part_bins=250_000):
        ...
        for item in equal_histograms.values():
            ...
            for begin in range(0, len(values), maximum_part_bins):
                ...
                if bins and bins + end - begin > maximum_part_bins:
                    flush()
                records.append({'key':list(keys[0]),'aliases':[list(key) for key in keys[1:]],
                    'v':values[begin:end].tolist(),'c':counts[begin:end].tolist()})
                bins += end - begin
                total_bins += (end - begin)*len(keys)
        ...
        ref = outputs.json_compressed(f'{name}.json.zst', {
            ...
            'input_bins':self.input_bins,'pooled_bins':total_bins,'stored_bins':stored_bins,
        ...
        return {**ref, 'input_bins':self.input_bins,'pooled_bins':total_bins,'stored_bins':stored_bins,
