...
class QuoteWindow:
    ...
    def continue_window(self, *, end_ns, native_sink=_KEEP_NATIVE):
        ...
        result.clock_uncertain_intervals = tuple((a,b) for a,b in getattr(self, 'clock_uncertain_intervals', ()) if a < result.end and b > result.start)
        return result
    ...
    def _exposure(self, *, start, end, bid, ask, bid_size, ask_size, valid, economic_at):
        ...
        uncertainty = getattr(self, 'clock_uncertain_intervals', ())
        if uncertainty:
            from trading_research.data.clock_uncertainty import certain_segments
            a, b, owners = certain_segments(a, b, uncertainty)
            bid, ask, bid_size, ask_size, valid, economic_at = (v[owners] for v in (bid, ask, bid_size, ask_size, valid, economic_at))
    ...
    def add_prepared(self, prepared, start=0, stop=None):
        ...
        from trading_research.data.clock_uncertainty import clock_order_valid
        clock_ok = clock_order_valid(t, None if self.previous is None else self.previous['t'], getattr(self, 'clock_uncertain_intervals', ()))
        if (any(v.dtype.kind not in "iu" or v.ndim != 1 for v in values.values())
                ...
                or not clock_ok or np.any(order[1:] <= order[:-1])
                or self.previous is not None and order[0] <= self.previous["source_order"]):
        ...
        if self._try_add_prepared_compiled(
                prepared, start, stop, values, t, order, valid, snapshot, flags,
                bid, ask, qb, qa, updates, clears, previous):
            return
    ...
    def _exact_native_bins(self):
        from trading_research.research.auction_flow_native_bins import NativeEventBins

        return type(self.native_sink) is NativeEventBins

    def _try_add_prepared_compiled(self, prepared, start, stop, values, t, order, valid, snapshot,
                                   flags, bid, ask, qb, qa, updates, clears, previous):
        import numpy as np
        from trading_research.data import compact_native
        from trading_research.data.compact_native import fuse_quote_slice

        if getattr(self, 'clock_uncertain_intervals', ()):
            return False
        if compact_native._SLICE is None:
            return False
        if self.native_sink is not None and not self._exact_native_bins():
            return False
        # Widening unusual prepared dtypes can change NumPy overflow and
        ...
        if (any(a.dtype != np.int64 or not a.flags.c_contiguous or not a.flags.aligned
            ...
            return False
        n = int(len(t))
        exp_starts = np.empty(n, dtype=np.int64)
        exp_ends = np.empty(n, dtype=np.int64)
        exp_bid = np.empty(n, dtype=np.int64)
        exp_ask = np.empty(n, dtype=np.int64)
        exp_qb = np.empty(n, dtype=np.int64)
        exp_qa = np.empty(n, dtype=np.int64)
        exp_duration = np.empty(n, dtype=np.int64)
        exp_mid = np.empty(n, dtype=np.int64)
        exp_dur_imbalance = np.empty(n, dtype=np.longdouble)
        exp_dur_spread = np.empty(n, dtype=np.longdouble)
        meas_ofi = np.empty(n, dtype=np.int64)
        meas_same = np.empty(n, dtype=np.int64)
        meas_spread = np.empty(n, dtype=np.int64)
        meas_depth = np.empty(n, dtype=np.longdouble)
        meas_imbalance = np.empty(n, dtype=np.longdouble)
        meas_micro = np.empty(n, dtype=np.longdouble)
        fresh_out = np.empty(n, dtype=np.uint8)
        measured_out = np.empty(n, dtype=np.uint8)
        stats = np.empty(22, dtype=np.int64)
        if not fuse_quote_slice(
                t=t, order=order, bid=bid, ask=ask, bid_size=qb, ask_size=qa, flags=flags,
                valid=valid, snapshot=snapshot, updates=updates, clears=clears, previous=previous,
                window_start=self.start, window_end=self.end, maximum_age=self.maximum_age,
                origin_ticks=self.origin_ticks, row_ticks=self.row_ticks,
                exp_starts=exp_starts, exp_ends=exp_ends, exp_bid=exp_bid, exp_ask=exp_ask,
                exp_qb=exp_qb, exp_qa=exp_qa, exp_duration=exp_duration,
                exp_dur_imbalance=exp_dur_imbalance, exp_dur_spread=exp_dur_spread, exp_mid=exp_mid,
                meas_ofi=meas_ofi, meas_same=meas_same, meas_depth=meas_depth,
                meas_imbalance=meas_imbalance, meas_micro=meas_micro, meas_spread=meas_spread,
                fresh_out=fresh_out, measured_out=measured_out, stats=stats):
            return False
        n_keep, n_meas = int(stats[0]), int(stats[1])
        starts, ends = exp_starts[:n_keep], exp_ends[:n_keep]
        keep_bid, keep_ask = exp_bid[:n_keep], exp_ask[:n_keep]
        keep_qb, keep_qa = exp_qb[:n_keep], exp_qa[:n_keep]
        duration = exp_duration[:n_keep]
        mid = exp_mid[:n_keep]
        ofi, same = meas_ofi[:n_meas], meas_same[:n_meas]
        fresh = fresh_out.astype(bool, copy=False)
        measured = measured_out.astype(bool, copy=False)
        mid_keys = mid_amounts = None
        if n_keep:
            ranked = np.argsort(mid, kind="stable")
            rows, mass = mid[ranked], duration[ranked]
            edges = np.r_[0, np.flatnonzero(rows[1:] != rows[:-1]) + 1]
            mid_keys, mid_amounts = rows[edges], np.add.reduceat(mass, edges)
            new_keys = 0
            for row in mid_keys:
                if int(row) not in self.midpoint_durations:
                    new_keys += 1
            if new_keys and len(self.midpoint_durations) + new_keys > 250000:
                raise ContractError("displayed occupancy exceeds its finite retained row capacity")
        counts = np.bincount(prepared.pair_codes[start:stop], minlength=len(prepared.pair_keys))
        ...
        if self.native_sink is not None:
            self.native_sink.apply_compiled_quote_channels(
                at=t, order=order, fresh=fresh, measured=measured, ofi=ofi, same=same,
                starts=starts, ends=ends, bid=keep_bid, ask=keep_ask,
                bid_size=keep_qb, ask_size=keep_qa)
        if n_keep:
            self.duration += int(stats[12])
            self.positive_duration += int(stats[13])
            for row, amount in zip(mid_keys, mid_amounts, strict=True):
                key = int(row)
                self.midpoint_durations[key] = self.midpoint_durations.get(key, 0) + int(amount)
            self._sum("duration_imbalance_ns", exp_dur_imbalance[:n_keep])
            self._sum("duration_spread_ticks_ns", exp_dur_spread[:n_keep])
        if n_meas:
            if self.ofi + int(stats[14]) > self.ofi_high:
                self.ofi_high, self.ofi_high_at, self.ofi_high_order = (
                    self.ofi + int(stats[14]), int(stats[15]), int(stats[16]))
            if self.ofi + int(stats[17]) < self.ofi_low:
                self.ofi_low, self.ofi_low_at, self.ofi_low_order = (
                    self.ofi + int(stats[17]), int(stats[18]), int(stats[19]))
            self.ofi += int(stats[8])
            self.same_price_ofi += int(stats[9])
            self.price_change_ofi += int(stats[10])
            self.positive_updates += int(stats[11])
            self._sum("depth_normalized_ofi", meas_depth[:n_meas])
            self._sum("update_imbalance", meas_imbalance[:n_meas])
            self._sum("microprice_minus_midpoint_ticks", meas_micro[:n_meas])
            self._sum("spread_ticks", meas_spread[:n_meas])
        self.events += n
        self.fresh += int(stats[2])
        self.pressure_updates += n_meas
        self.invalid += int(stats[3])
        self.snapshots += int(stats[4])
        self.gaps += int(stats[5])
        self.clears += int(stats[6])
        self.equal_times += int(stats[7])
        for key, count in action_counts:
            self.action_sides[key] = self.action_sides.get(key, 0) + count
        self.previous = terminal_values
        self.previous.update(economic_at=int(stats[20]), source_key=source_key,
                             raw_action=raw_action, raw_side=raw_side)
        return True
