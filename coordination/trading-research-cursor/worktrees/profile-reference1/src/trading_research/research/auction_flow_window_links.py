...
def _validate_cuts(cut_start_ns, cut_end_ns, acquired):
    ...
    if start % CUT_CADENCE_NS:
        raise ContractError('cut interval start must fall on an exact UTC five-minute boundary')
    ...
    import numpy as np
    # The acquisition envelope may end between grid cuts. Keep every grid
    # instant in [start,end); neither round source time nor invent a final cut.
    n_cuts = (end - start + CUT_CADENCE_NS - 1) // CUT_CADENCE_NS
