...
def _exact_sum(values):
    ...
    if values is None or len(values) == 0:
        return 0
    # Only validated quantity slices enter here: at most 65,536 values,
    # each below 2**32-1, so the exact sum is strictly below 2**48.
    return int(np.sum(values, dtype=np.uint64))
...
class AuctionFlowMemoryWindow:
    ...
    def _retain_selected(self, prepared, left, i, t, known, price, size, side, order, rows, flags):
        ...
        # Slice-wide integer, clock, ordering and domain checks already passed.
        # Python int preserves every uint64 source address without narrowing.
        quantity = int(size[i])
        self._parent.append(index)
        self._sel_t.append(int(t[i]))
        self._sel_known.append(int(known[i]))
        self._sel_price.append(int(price[i]))
        self._sel_size.append(quantity)
        self._sel_side.append(int(side[i]))
        self._sel_order.append(int(order[i]))
        self._sel_row.append(int(rows[i]))
        self._sel_flags.append(int(flags[i]))
    ...
    def _build_cluster(self, members, preceding_bytes):
        quantity = 0
        priced = 0
        low = high = None
        first_at = last_at = None
        records = []
        member_bytes = preceding_bytes
        for index in members:
            ...
            self.cluster_member_records += 1
            # This is a lower bound on the final report. Stop before retaining
            # an oversized connected component, even when it is the first one.
            member_bytes = self._accumulate_bytes(member_bytes, record)
    ...
    def _finish(self):
        ...
        for members in components.values():
            cluster = self._build_cluster(members, acc)
        ...
        report = {
            ...
            "work": (
                ...
                ("cluster_member_records", self.cluster_member_records),
            ),
        }
