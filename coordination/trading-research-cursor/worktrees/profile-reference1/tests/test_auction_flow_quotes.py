...
def _quote_backend(compiled=None):
...
def _restore_quote_backend(previous):
    from trading_research.data import compact_native
...
class RecordingQuoteSink:
    ...
    def quote_updates(self, **kwargs):
        self.updates.append(kwargs)

    def quote_exposure(self, **kwargs):
        self.exposures.append(kwargs)


class AuctionFlowCompiledQuoteSliceTests(unittest.TestCase):
    BRANCH_ROWS = (
        quote_row(10, source_order=0, raw_action="A", snapshot=1, bid_size=7, ask_size=9),
        quote_row(20, source_order=1, bid_size=11, ask_size=5),
        quote_row(25, source_order=2, book_valid=0, raw_action="T", raw_flags=0,
                  bid=0, ask=0, bid_size=0, ask_size=0),
        quote_row(30, source_order=3, bid=401, ask=402, bid_size=6, ask_size=4),
        quote_row(35, source_order=4, snapshot=1, bid=401, ask=402, bid_size=9, ask_size=1),
        quote_row(40, source_order=5, bid=401, ask=402, bid_size=8, ask_size=2),
        quote_row(40, source_order=6, bid=401, ask=402, bid_size=7, ask_size=3),
        quote_row(50, source_order=7, book_valid=0, raw_flags=132, raw_action="M",
                  bid=0, ask=0, bid_size=0, ask_size=0),
        quote_row(55, source_order=8, book_valid=0, raw_action="R", raw_side=None,
                  bid=0, ask=0, bid_size=0, ask_size=0),
        quote_row(60, source_order=9, book_valid=0, raw_action=None, raw_side="B",
                  bid=0, ask=0, bid_size=0, ask_size=0),
        quote_row(70, source_order=10, snapshot=1, bid_size=4, ask_size=4),
        quote_row(90, source_order=11, bid_size=8, ask_size=2),
    )

    def _tables(self):
        return [make_quote_table(self.BRANCH_ROWS)]

    def _finish(self, tables, *, compiled=None, **kwargs):
        previous = _quote_backend(compiled)
        try:
            return finish_prepared(tables, **kwargs)
        finally:
            _restore_quote_backend(previous)

    def test_compiled_and_reference_canonical_json_match_frozen_on_the_same_slices(self):
        from trading_research.data import compact_native

        tables = self._tables()
        frozen = finish_ordinary(tables, Window=frozen_quote_window())
        fallback = self._finish(tables, compiled=False)
        current = self._finish(tables)
        self.assertEqual(fallback, frozen)
        self.assertEqual(current, frozen)
        together = make_quote_table(self.BRANCH_ROWS)
        sliced = [together.slice(offset, 1) for offset in range(len(together))]
        sliced_frozen = finish_ordinary(sliced, Window=frozen_quote_window())
        self.assertEqual(self._finish(sliced, compiled=False), sliced_frozen)
        self.assertEqual(self._finish(sliced), sliced_frozen)
        prepared_frozen = replay_atomic(
            tables, Window=frozen_quote_window(), start=0, end=100, width=20, delay=250, prepared=False)
        previous = _quote_backend(False)
        try:
            prepared_fallback = replay_atomic(
                tables, Window=QuoteWindow, start=0, end=100, width=20, delay=250, prepared=True)
        finally:
            _restore_quote_backend(previous)
        prepared_current = replay_atomic(
            tables, Window=QuoteWindow, start=0, end=100, width=20, delay=250, prepared=True)
        self.assertEqual(prepared_fallback, prepared_frozen)
        self.assertEqual(prepared_current, prepared_frozen)
        ...
        self.assertGreater(after['compiled_quote_slice_rows'], before['compiled_quote_slice_rows'])

    def test_negative_origin_age_ties_and_custom_sink_keep_reference_semantics(self):
        from trading_research.data import compact_native

        rows = [
            quote_row(10, source_order=0, bid=400, ask=403, bid_size=8, ask_size=2),
            quote_row(20, source_order=1, bid=400, ask=403, bid_size=9, ask_size=1),
            quote_row(20, source_order=2, bid=396, ask=397, bid_size=1, ask_size=4),
            quote_row(45, source_order=3, bid=396, ask=397, bid_size=2, ask_size=2),
        ]
        tables = [make_quote_table(rows)]
        kwargs = dict(start=0, end=80, delay=250, maximum_age=18)
        frozen = finish_ordinary(tables, Window=frozen_quote_window(), **kwargs)
        window_kwargs = dict(instrument_id=1, start_ns=0, end_ns=80, latency_ns=250,
                             maximum_events=100, maximum_age_ns=18, origin_ticks=-7, row_ticks=2)

        def finish_origin(compiled=None):
            previous = _quote_backend(compiled)
            try:
                window = QuoteWindow(**window_kwargs)
                for table in tables:
                    with prepare_quote_batch(table) as prepared:
                        window.add_prepared(prepared)
                return window.finish(coverage_complete=True)
            finally:
                _restore_quote_backend(previous)

        fallback = finish_origin(False)
        current = finish_origin()
        self.assertEqual(current, fallback)
        self.assertEqual(current['ofi_path']['high_source_order'], fallback['ofi_path']['high_source_order'])
        self.assertEqual(current['displayed_midpoint_occupancy'], fallback['displayed_midpoint_occupancy'])
        mid = (400 + 403 - 2 * (-7)) // 4
        self.assertIn(mid, dict(current['displayed_midpoint_occupancy']['duration_ns_by_row']))
        sink_fallback, sink_current = RecordingQuoteSink(end=80), RecordingQuoteSink(end=80)
        previous = _quote_backend(False)
        try:
            finish_prepared(tables, start=0, end=80, delay=250, maximum_age=18, native_sink=sink_fallback)
        finally:
            _restore_quote_backend(previous)
        finish_prepared(tables, start=0, end=80, delay=250, maximum_age=18, native_sink=sink_current)
        self.assertEqual(len(sink_current.updates), len(sink_fallback.updates))
        self.assertEqual(len(sink_current.exposures), len(sink_fallback.exposures))
        self.assertGreaterEqual(len(sink_current.updates), 1)
        self.assertEqual(self._finish(tables, compiled=False, **kwargs), frozen)
        self.assertEqual(self._finish(tables, **kwargs), frozen)
        if compact_native._SLICE is not None:
            self.assertGreaterEqual(compact_native.execution_counts()['compiled_quote_slice_calls'], 1)

    def test_acquired_int16_flags_execute_compiled_with_exact_values(self):
        import numpy as np
        from trading_research.data import compact_native
        results = []
        ...
        for enabled in (False, None):
            ...
            try:
                window = QuoteWindow(instrument_id=1, start_ns=0, end_ns=100, latency_ns=250)
                with prepare_quote_batch(make_quote_table(self.BRANCH_ROWS)) as prepared:
                    prepared.values['raw_flags'] = prepared.values['raw_flags'].astype(np.int16)
                    window.add_prepared(prepared)
                results.append(window.finish(coverage_complete=True))
            finally:
                _restore_quote_backend(previous)
        self.assertEqual(results[0], results[1])
        if compact_native._SLICE is not None:
            self.assertGreater(compact_native.execution_counts()['compiled_quote_slice_calls'], before)

    def test_prepared_integer_dtype_variants_keep_reference_arithmetic(self):
        import numpy as np
        from trading_research.data import compact_native
    ...
    def test_explicit_noncompiled_fallback_and_empty_slice_behavior_stay_unchanged(self):
        from trading_research.data import compact_native

        empty = make_quote_table(self.BRANCH_ROWS).slice(0, 0)
        previous = _quote_backend(False)
        try:
            with prepare_quote_batch(empty) as batch:
                QuoteWindow(instrument_id=1, start_ns=0, end_ns=100, latency_ns=250).add_prepared(batch)
        ...
        with prepare_quote_batch(empty) as batch:
            QuoteWindow(instrument_id=1, start_ns=0, end_ns=100, latency_ns=250).add_prepared(batch)
        tables = self._tables()
        self.assertEqual(self._finish(tables, compiled=False), finish_ordinary(tables, Window=frozen_quote_window()))
        if compact_native._SLICE is None:
            self.assertFalse(compact_native.fuse_quote_slice(
                t=None, order=None, bid=None, ask=None, bid_size=None, ask_size=None, flags=None,
                valid=None, snapshot=None, updates=None, clears=None, previous=None,
                window_start=0, window_end=100, maximum_age=None, origin_ticks=0, row_ticks=1,
                exp_starts=None, exp_ends=None, exp_bid=None, exp_ask=None, exp_qb=None, exp_qa=None,
                exp_duration=None, exp_dur_imbalance=None, exp_dur_spread=None, exp_mid=None,
                meas_ofi=None, meas_same=None, meas_depth=None, meas_imbalance=None, meas_micro=None,
                meas_spread=None, fresh_out=None, measured_out=None, stats=None))
