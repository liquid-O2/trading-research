...
class AuctionFlowPipelineTests(unittest.TestCase):
    ...
    def test_redundant_backward_snapshot_preserves_exposure_dwell_and_raw_snapshot_counts(self):
        from trading_research.data.compact import SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1
        known = [raw(100, action='A'), raw(110, action='T', flags=0, size=3)]
        replayed = [raw(100, action='A'), raw(90, action='A', side='N', flags=168),
                    raw(110, action='T', flags=0, size=3)]
        twins = replayed + [raw(110, action='T', flags=0, size=3)]
        inspected = []

        def inspect(table, source):
            inspected.extend(table.to_pylist())
        ...
        def run(rows, **kwargs):
            with tempfile.TemporaryDirectory() as folder:
                ...
                footer = AuctionFlowDataTests().write(root, rows)
                return measure_raw_window(data_root=root, index=footer, coordinates=self.coordinates(), root='NQ',
                    start_ns=0, end_ns=200, maximum_scan_rows=100, latency_ns=250, atomic_width_ns=20,
                    native_width_ns=20, inspect_raw_batch=kwargs.get('inspect'),
                    source_clock_policy=kwargs.get('source_clock_policy', SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1),
                    continuation=kwargs.get('continuation'))

        economic = run(known)
        with_replay = run(replayed, inspect=inspect)
        self.assertEqual([r['t'] for r in inspected], [100, 90, 110])
        self.assertEqual(with_replay['source_manifest']['projection']['counts']['raw_rows'], 3)
        self.assertEqual(with_replay['source_manifest']['projection']['counts']['quote_rows'], 1)
        self.assertEqual(with_replay['source_manifest']['projection']['counts']['trades'], 1)
        self.assertEqual(with_replay['source_manifest']['projection']['counts']['disposed_snapshot_rows'], 1)
        quote_e = economic['instruments'][0]['atomic_windows']
        quote_r = with_replay['instruments'][0]['atomic_windows']
        for left, right in zip(quote_e, quote_r, strict=True):
            self.assertEqual(left['quote']['ofi_contracts'], right['quote']['ofi_contracts'])
            self.assertEqual(left['quote']['fresh_quote_updates'], right['quote']['fresh_quote_updates'])
            self.assertEqual(left['quote']['observed_trusted_standing_duration_ns'],
                             right['quote']['observed_trusted_standing_duration_ns'])
            self.assertEqual(left['trade']['prints'], right['trade']['prints'])
        self.assertEqual(economic['instruments'][0]['time_at_price']['dwell'],
                         with_replay['instruments'][0]['time_at_price']['dwell'])
        quality = with_replay['instruments'][0]['atomic_windows'][90 // 20]['source_quality']
        self.assertEqual(quality['snapshot_rows'], 1)
        self.assertEqual(quality['raw_rows'], 1)
        native = with_replay['instruments'][0]['native_event_measurements']
        self.assertEqual(native['quality_counts']['source_eligible'],
                         economic['instruments'][0]['native_event_measurements']['quality_counts']['source_eligible'])
        self.assertEqual(with_replay['measurement_continuation']['parameters']['source_clock_policy'],
                         SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            footer = AuctionFlowDataTests().write(root, replayed)
            first = measure_raw_window(data_root=root, index=footer, coordinates=self.coordinates(), root='NQ',
                start_ns=0, end_ns=105, maximum_scan_rows=100, latency_ns=250, atomic_width_ns=20,
                native_width_ns=20, source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            second = measure_raw_window(data_root=root, index=footer, coordinates=self.coordinates(), root='NQ',
                start_ns=105, end_ns=200, maximum_scan_rows=100, latency_ns=250, atomic_width_ns=20,
                native_width_ns=20, source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1,
                continuation=first['measurement_continuation'])
            self.assertEqual(second['source_manifest']['projection']['counts']['raw_rows'], 1)
            self.assertEqual(second['source_manifest']['projection']['counts']['trades'], 1)
        failed = [raw(100, action='A'), raw(90, action='A', side='N', flags=168), raw(95, action='A')]
        with self.assertRaises(IntegrityError):
            run(failed)
        equal = run(twins)
        self.assertEqual(equal['source_manifest']['projection']['counts']['trades'], 2)
        self.assertEqual(equal['source_manifest']['projection']['counts']['volume'], 6)
        self.assertEqual(equal['instruments'][0]['whole_window']['prints'], 2)
