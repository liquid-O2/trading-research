...
class AuctionFlowDataTests(unittest.TestCase):
    def test_nonfinite_terminal_bbo_survives_json_and_adjacent_continuation_exactly(self):
        import json
        import struct
        bits = ('000000000000f87f', '000000000000f8ff')
        terminal = raw(8, action='C', side='N')
        terminal.update(bid_px=struct.unpack('<d', bytes.fromhex(bits[0]))[0],
                        ask_px=struct.unpack('<d', bytes.fromhex(bits[1]))[0], bid_sz=0, ask_sz=0)
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder); index = self.write(root, [raw(0), terminal, raw(12)])
            first = self.stream(root, index, start=0, end=10,
                                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            list(first)
            carry = json.loads(json.dumps(first.carry(), allow_nan=False))
            for name, expected in zip(('bid_px', 'ask_px'), bits):
                self.assertEqual(carry['last_original_record'][name], {'$float64_bits_le': expected})
            second = self.stream(root, index, start=10, end=20, continuation=carry,
                                 source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            for name, expected in zip(('bid_px', 'ask_px'), bits):
                self.assertEqual(struct.pack('<d', second._last_original_record[name]).hex(), expected)
            batches = list(second)
            self.assertEqual([r['source_order'] for b in batches for r in b.raw.to_pylist()], [2])
            self.assertEqual(second.carry()['next_source_order'], 3)

    def test_finite_carry_keeps_legacy_values_and_invalid_float_encodings_are_rejected(self):
        import copy
        from trading_research.operations.artifacts import digest
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder); index = self.write(root, [raw(0), raw(8), raw(12)])
            first = self.stream(root, index, start=0, end=10,
                                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            list(first); carry = first.carry()
            self.assertEqual({k: carry['last_original_record'][k] for k in RAW_FIELDS}, raw(8))
            for encoded in ({'$float64_bits_le': 'xyz'}, {'$float64_bits_le': '000000000000f03f'},
                            {'$float64_bits_le': '000000000000f87f', 'extra': 1}):
                bad = copy.deepcopy(carry); bad['last_original_record']['bid_px'] = encoded
                bad['sha256'] = digest({k: v for k, v in bad.items() if k != 'sha256'})
                with self.assertRaises(IntegrityError):
                    self.stream(root, index, start=10, end=20, continuation=bad,
                                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
    ...
    def test_redundant_backward_snapshot_keeps_raw_and_exposes_temporal_view(self):
        from trading_research.operations.artifacts import digest
        rows = [raw(100), raw(90, action='A', side='N', flags=168), raw(110, action='T', flags=0)]
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            index = self.write(root, rows)
            together = AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=0, end_ns=200,
                maximum_scan_rows=100, latency_ns=250, batch_rows=8,
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            batches = list(together)
            raw_rows = [r for b in batches for r in b.raw.to_pylist()]
            temporal = [r for b in batches for r in b.temporal_raw.to_pylist()]
            replays = [r for b in batches for r in b.replay_records.to_pylist()]
            quotes = [r for b in batches for q in b.quotes for r in q.to_pylist()]
            trades = [r for b in batches for t in b.trades for r in t.to_pylist()]
            self.assertEqual([r['t'] for r in raw_rows], [100, 90, 110])
            self.assertEqual([r['source_order'] for r in raw_rows], [0, 1, 2])
            self.assertEqual([r['t'] for r in temporal], [100, 110])
            self.assertEqual(len(replays), 1)
            self.assertEqual(replays[0]['source_row'], 1)
            self.assertEqual(replays[0]['predecessor_source_row'], 0)
            self.assertEqual(replays[0]['flags'], 168)
            self.assertEqual([q['t'] for q in quotes], [100])
            self.assertEqual([t['t'] for t in trades], [110])
            report = together.manifest()
            self.assertEqual(report['projection']['counts']['raw_rows'], 3)
            self.assertEqual(report['projection']['counts']['quote_rows'], 1)
            self.assertEqual(report['projection']['counts']['disposed_snapshot_rows'], 1)
            self.assertEqual(report['source_clock_policy'], SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            carry = together.carry()
            self.assertEqual(carry['clock_highwater_ns'], 110)
            self.assertEqual(carry['last_original_record']['t'], 110)
            split = AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=0, end_ns=200,
                maximum_scan_rows=100, latency_ns=250, batch_rows=1,
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            split_batches = list(split)
            self.assertEqual([r['t'] for b in split_batches for r in b.raw.to_pylist()], [100, 90, 110])
            self.assertEqual(split.manifest()['projection']['counts'], report['projection']['counts'])
            first = AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=0, end_ns=105,
                maximum_scan_rows=100, latency_ns=250, batch_rows=8,
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            list(first)
            second = AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=105, end_ns=200,
                maximum_scan_rows=100, latency_ns=250, batch_rows=8,
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1, continuation=first.carry())
            list(second)
            self.assertEqual(second.manifest()['projection']['counts']['raw_rows'], 1)
            self.assertEqual(second.carry()['clock_highwater_ns'], 110)
            with self.assertRaises(IntegrityError):
                AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=105, end_ns=200,
                    maximum_scan_rows=100, latency_ns=250, continuation=first.carry())
            missing = {k: v for k, v in first.carry().items() if k != 'last_original_record'}
            missing['sha256'] = digest({k: v for k, v in missing.items() if k != 'sha256'})
            with self.assertRaises(IntegrityError):
                AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=105, end_ns=200,
                    maximum_scan_rows=100, latency_ns=250,
                    source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1, continuation=missing)
            unsigned = dict(first.carry())
            unsigned['clock_highwater_ns'] = 1
            with self.assertRaises(IntegrityError):
                AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=105, end_ns=200,
                    maximum_scan_rows=100, latency_ns=250,
                    source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1, continuation=unsigned)
            tampered = dict(first.carry())
            record = dict(tampered['last_original_record'])
            record['source_row'] = record['source_row'] + 17
            tampered['last_original_record'] = record
            tampered['sha256'] = digest({k: v for k, v in tampered.items() if k != 'sha256'})
            with self.assertRaises(IntegrityError):
                AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=105, end_ns=200,
                    maximum_scan_rows=100, latency_ns=250,
                    source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1, continuation=tampered)

    def test_replay_inside_continued_window_uses_global_source_order(self):
        rows = [raw(10), raw(20), raw(120), raw(110, side='N', flags=168), raw(130)]
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            index = self.write(root, rows)
            options = dict(data_root=root, index=index, dataset=DATASET, maximum_scan_rows=100,
                           latency_ns=250, batch_rows=1,
                           source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            first = AuctionFlowStream(**options, start_ns=0, end_ns=100)
            list(first)
            second = AuctionFlowStream(**options, start_ns=100, end_ns=200, continuation=first.carry())
            batches = list(second)
            self.assertEqual([r['source_order'] for b in batches for r in b.raw.to_pylist()], [2, 3, 4])
            self.assertEqual([r['source_order'] for b in batches for r in b.temporal_raw.to_pylist()], [2, 4])
            records = [r for b in batches for r in b.replay_records.to_pylist()]
            self.assertEqual([(r['source_row'], r['source_order'], r['predecessor_source_order'])
                              for r in records], [(3, 3, 2)])
            self.assertEqual([r['source_order'] for b in batches for q in b.quotes for r in q.to_pylist()], [2, 4])
            self.assertEqual(second.carry()['next_source_order'], 5)

    def test_gap_predecessor_replay_keeps_raw_gap_and_blocked_carry(self):
        rows = [raw(100, flags=132), raw(90, side='N', flags=168), raw(110)]
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder); index = self.write(root, rows)
            stream = AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=0, end_ns=200,
                maximum_scan_rows=100, latency_ns=250, batch_rows=1,
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            batches = list(stream)
            self.assertEqual([r['t'] for b in batches for r in b.raw.to_pylist()], [100, 90, 110])
            self.assertEqual([r['t'] for b in batches for r in b.temporal_raw.to_pylist()], [100, 110])
            self.assertEqual(stream.carry()['blocked']['1']['source_row'], 0)
            self.assertFalse(stream.carry()['prefix_flow_complete']['1'])
            self.assertEqual([r['book_valid'] for b in batches for q in b.quotes for r in q.to_pylist()], [0, 0])

    def test_nonadjacent_filtered_row_cannot_authenticate_a_backward_snapshot(self):
        rows = [raw(100), raw(50), raw(90, action='A', side='N', flags=168)]
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            index = self.write(root, rows)
            stream = AuctionFlowStream(data_root=root, index=index, dataset=DATASET, start_ns=80, end_ns=200,
                maximum_scan_rows=100, latency_ns=250, batch_rows=8,
                source_clock_policy=SOURCE_CLOCK_REDUNDANT_BACKWARD_SNAPSHOT_V1)
            with self.assertRaises(IntegrityError):
                list(stream)
