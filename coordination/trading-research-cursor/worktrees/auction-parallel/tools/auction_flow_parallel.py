...
MAXIMUM_WORKERS_CEILING = 16
AGGREGATE_MEMORY_CEILING = 64 * 1024 ** 3
...
def plan_source_unit_execution(protocol, allocation, *, live_limits, protocol_sha256):
    ...
    if (allocation.get('kind') != KIND or allocation.get('family') != protocol.get('family')
    ...
    if (declared.get('vcpus') != SUPPLIED_VCPUS or declared.get('memory_bytes') != SUPPLIED_MEMORY_BYTES
            ...
            or type(workers) is not int or workers != MAXIMUM_WORKERS_CEILING
            or type(aggregate) is not int or aggregate != AGGREGATE_MEMORY_CEILING
    ...
    return {
        ...
        'maximum_workers_ceiling': MAXIMUM_WORKERS_CEILING,
        'aggregate_memory_bytes': memory_ceiling,
        'aggregate_memory_ceiling_bytes': AGGREGATE_MEMORY_CEILING,
