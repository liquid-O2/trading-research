...
class AuctionFlowStorageTests(unittest.TestCase):
    ...
    def test_structural_cross_column_references_keep_exact_domain_and_older_descriptors(self):
        ...
        by_col = {op['column']: op for op in descriptor['operations']}
        self.assertNotIn('ask', by_col)
        self.assertEqual(encoded['ask'].to_pylist(), [107, -33, 2**62 + 6])
        ...
        earlier_ask = quotes.set_column(quotes.schema.get_field_index('ask'), 'ask',
                                        pa.array([7, 7, 6], type=pa.int64()))
        earlier_descriptor = {'version': CODEC_VERSION, 'operations': [
            {'column': 'ask', 'reference': 'bid', 'null_restores_sentinel': None}]}
        self.assertEqual(value_digest(decode_table(earlier_ask, earlier_descriptor)), value_digest(quotes))
    ...
    def test_structural_cross_column_storage_roundtrip_files_tampering_and_quote_ask(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(quote_physical['ask'][0].as_py(), 100)
