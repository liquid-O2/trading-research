...
MAX_INSTRUMENTS = 8
MAX_CUT_SPAN_NS = 3 * 24 * 60 * MINUTE
MAX_LOOKBACK_NS = 240 * MINUTE
MAX_LOOKAHEAD_NS = 60 * MINUTE
_SHA_LEN = 64
_INT64_MIN = -(2 ** 63)
_INT64_MAX = 2 ** 63 - 1
_TRADE_REQUIRED = (
...
def contract_definitions(contract=None):
    ...
    return {
        ...
        'identity': payload.get('identity', 'root+physicalsourcepath+instrument+rawcontract+formation/end'),
    }
...
def _label_fields(pa):
    fields = [
        ...
        _flag_field(pa, 'left_censored'), _flag_field(pa, 'right_censored'),
        _text_field(pa, 'censor_reason', nullable=True),
        _flag_field(pa, 'complete'), _flag_field(pa, 'no_new_trade'), _flag_field(pa, 'no_priced_trade'),
...
def _validate_cuts(cut_start_ns, cut_end_ns, acquired):
    ...
    if end - start > MAX_CUT_SPAN_NS:
        raise ContractError('cut interval cannot exceed three UTC days')
    if start < acquired['acquired_event_start_ns'] or end > acquired['acquired_event_end_ns']:
        raise IntegrityError('cut interval is outside the acquired physical file event bounds')
    import numpy as np
...
def _observation_arrays(observations, identity, cut_start, cut_end):
    ...
    required = [
        ...
        'instrument_id', 'contract_key', 'event_start_ns', 'event_end_ns',
        'coordinate_complete', 'source_instrument_presence', 'source_coverage_complete',
    ...
    if np.any(event_start < cut_start - MAX_LOOKBACK_NS) or np.any(event_end > cut_end + MAX_LOOKAHEAD_NS):
        raise IntegrityError('observation support exceeds the declared lookback/lookahead bound')
    if np.any(event_start < identity['acquired_event_start_ns']) or np.any(event_end > identity['acquired_event_end_ns']):
        raise IntegrityError('observation atoms fall outside the acquired physical file bounds')
...
def _trade_arrays(trades, identity, cut_start, cut_end):
    ...
    if np.any(event_ns < cut_start - MAX_LOOKBACK_NS) or np.any(event_ns >= cut_end + MAX_LOOKAHEAD_NS):
        raise IntegrityError('trade support exceeds the declared lookback/lookahead bound')
    ...
    delay_ok = bool(np.all(delays == SOURCE_LATENCY_NS))
    unique = tuple(int(v) for v in np.unique(instrument_id))
    if len(unique) > MAX_INSTRUMENTS:
        raise ContractError('window-link input cannot exceed eight instruments')
    grouped = {}
    ...
    return {
        ...
        'delay_ok': delay_ok,
        'delays': tuple(int(v) for v in np.unique(delays)),
        'n': int(len(table)),
    }
...
def _concat_offset_path(open_a, high_a, low_a, close_a, high_at, low_at, high_ord, low_ord,
    ...
    if n == 0:
        return None
    delta = close_a.astype(np.int64, copy=False) - open_a.astype(np.int64, copy=False)
    offset = np.zeros(n, dtype=np.int64)
    if n > 1:
        walked = np.cumsum(delta[:-1], dtype=np.int64)
        offset[1:] = walked
        exact = _add_checked(*delta[:-1].tolist(), what='path_offset')
        if int(walked[-1]) != exact:
            raise IntegrityError('path_offset exceeds the exact int64 domain')
    rel_high = offset + (high_a.astype(np.int64, copy=False) - open_a.astype(np.int64, copy=False))
    rel_low = offset + (low_a.astype(np.int64, copy=False) - open_a.astype(np.int64, copy=False))
    close = int(offset[-1] + delta[-1])
    if not _INT64_MIN <= close <= _INT64_MAX:
        raise IntegrityError('offset CVD close exceeds the exact int64 domain')
    hi = int(np.argmax(rel_high))
    lo = int(np.argmin(rel_low))
...
def _formation_id(identity, instrument_id, contract_key, minutes, cut_ns):
    ...
    return f"{identity['root']}|{identity['source_path']}|{instrument_id}|{key}|{minutes}|{cut_ns}"
...
def _feature_row(identity, instrument_id, minutes, cut_ns, arrays, idx, stages):
    ...
    left_censor = left >= right or int(starts[left]) > start
    right_censor = left >= right or int(ends[right - 1]) < cut_ns
    tiled = False
    if left < right:
    ...
    atoms_complete = tiled and not left_censor and not right_censor
    reasons = []
    if left_censor:
        reasons.append('missing_left_atom')
    if right_censor:
        reasons.append('missing_right_atom')
    if left < right and not tiled and not (left_censor and right_censor and left >= right):
    ...
    row = {
        ...
        'censor_reason': None if atoms_complete else ','.join(reasons) if reasons else 'incomplete_atom_coverage',
    ...
    if view is None or not atoms_complete:
        return row
    row['coordinate_complete'] = bool(np.all(view['coordinate_complete']))
    row['source_instrument_presence'] = bool(np.all(view['presence']))
    row['source_coverage_complete'] = bool(np.all(view['source_coverage_complete']))
    row['flow_history_complete'] = bool(np.all(view['flow_history_complete']))
    row['price_history_complete'] = bool(np.all(view['price_history_complete']))
    row['empty_observed_window'] = bool(np.all(view['empty_observed_window']))
    row['quote_coverage_complete'] = bool(np.all(view['quote_coverage_complete']))
    row['full_standing_window_eligible'] = bool(np.all(view['standing_eligible']))
    row['full_pressure_transition_window_eligible'] = bool(np.all(view['pressure_eligible']))
    ...
    if priced_volume:
        vwap = sum_pv / priced_volume
        row['vwap_ticks'] = float(vwap)
        row['variance_ticks_squared'] = float(sum_p2v / priced_volume - vwap * vwap)
    ...
    low_high, low_low, low_high_at, low_low_at, low_high_ord, low_low_ord = _price_extrema(
    ...
    _ = (low_high, low_high_at, low_high_ord)
    ...
    for name in SOURCE_FILTERS:
        ...
        row[f'{name}__coverage_complete'] = bool(np.all(view[f'{name}__coverage_complete']))
        ...
        if path is not None:
            ...
            if row[f'{name}__coverage_complete']:
...
def _label_row(identity, instrument_id, contract_key, cut_ns, latency, horizon_kind, horizon_minutes,
               start, end, acquired_end, calendar_state, trades, stages):
    import numpy as np

    known = None if end is None else end + latency
    ...
    if start is None or end is None:
        ...
        complete = not left_censor and not right_censor and (horizon_kind != 'remaining_session' or remaining_applicable)
    row = {
        'root': identity['root'],
        'source_path': identity['source_path'],
        'source_metadata_sha256': identity['source_metadata_sha256'],
        ...
        'censor_reason': None if complete else ','.join(reasons) if reasons else 'incomplete_label',
        'complete': False,
        'no_new_trade': True,
    ...
    if start >= acquired_end or start >= observed_end:
        ...
        if row['censor_reason'] is None:
            row['censor_reason'] = 'right_future_endpoint'
        return row
    if trades is None:
    ...
    signed = _signed_path(events, trades['source_order'], trades['size'], trades['side'], all_mask, left, right)
    row['signed_open'] = signed['open']
    row['signed_high'] = signed['high']
    row['signed_low'] = signed['low']
    row['signed_close'] = signed['close']
    row['signed_high_at_ns'] = signed['high_at_ns']
    row['signed_low_at_ns'] = signed['low_at_ns']
    row['signed_high_source_order'] = signed['high_source_order']
    row['signed_low_source_order'] = signed['low_source_order']
    for name in SOURCE_FILTERS:
...
def _prepare_instrument_trades(bundle):
    ...
    prepared['print_prefix'] = _prefix_count(np.ones(len(size), dtype=bool))
...
def _contract_at_cut(arrays, idx, cut_ns):
    ...
    if pos < 0 or int(ends[pos]) > cut_ns:
        if pos >= 0 and int(starts[pos]) < cut_ns <= int(ends[pos]):
