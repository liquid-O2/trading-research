...
_IMPLEMENTATION_FILES = (
    ...
    ('data/clock_uncertainty.py', 'trading_research.data.clock_uncertainty'),
...
_verified_implementation_compatibility = None


def configure_verified_compatibility(*, previous, current):
    """Admit one frozen transition; exact replay value/source checks still apply."""
    global _verified_implementation_compatibility
    if current != implementation_identity():
        raise IntegrityError('quote replay compatibility target is not the running implementation')
    previous = (previous,) if isinstance(previous, dict) else tuple(previous)
    if not previous:
        raise IntegrityError('explicit previous replay implementations required')
    for item in previous:
        if (not isinstance(item, dict) or set(item) != set(current)
                or {k: v for k, v in item.items() if k != 'files'} != {k: v for k, v in current.items() if k != 'files'}
                or set(item['files']) not in (set(current['files']), set(current['files']) - {'data/clock_uncertainty.py'})):
            raise IntegrityError('quote replay compatibility changed runtime or file membership')
    _verified_implementation_compatibility = (copy.deepcopy(previous), copy.deepcopy(current))


def _require_implementation(recorded):
    current = implementation_identity()
    if (_verified_implementation_compatibility is not None
            and current == _verified_implementation_compatibility[1]
            and recorded in _verified_implementation_compatibility[0]):
        return
...
def _stream_kwargs(descriptor):
    ...
    kwargs = dict(data_root=Path(descriptor['data_root']), dataset=descriptor['dataset'],
    ...
    if 'source_clock_policy' in descriptor:
        kwargs['source_clock_policy'] = named_source_clock_policy(descriptor['source_clock_policy'])
    return kwargs
...
class QuoteReplaySeries:
    def __init__(self, outputs, name, *, data_root, source_index_reference, root, start_ns, end_ns,
                 ...
                 source_clock_policy=SOURCE_CLOCK_STRICT):
    ...
    def _descriptor_payload(self, source_manifest):
        ...
        payload = {'version': VERSION, 'encoding': ENCODING, 'series': self.name,
    ...
    def finish(self, source_manifest):
        ...
        charged_write = False
        try:
            if named_source_clock_policy(source_manifest.get('source_clock_policy')) != self.source_clock_policy:
                raise IntegrityError('quote replay source clock policy differs from its producer')
