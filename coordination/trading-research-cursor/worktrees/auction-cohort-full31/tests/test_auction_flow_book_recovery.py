...
class AuctionFlowBookRecoveryTests(unittest.TestCase):
    ...
    def test_sparse_python_episode_transitions_on_long_clean_and_repeated_flags(self):
        ...
        with patch("numpy.searchsorted", wraps=np.searchsorted) as searches:
            projected, report, _, value = project(rows, end=3000, maximum_rows=10_000)
        self.assertEqual(searches.call_count, 3)
    ...
    def test_empty_adjacent_interval_has_its_own_row_count(self):
        _, _, carry, _ = project([quote_row(10, source_order=0)], end=40)
        _, report, _, _ = project([], start=40, end=100, continuation=carry)
        self.assertTrue(report["empty_observed_window"])
        self.assertEqual(report["current_interval_rows"], 0)
        self.assertEqual(report["rows"], 1)

    def test_carry_nested_values_are_rejected_before_encoding(self):
        _, _, carry, _ = project([quote_row(10, source_order=0)], end=40)
        malformed = copy.deepcopy(carry)
        malformed["last_source_address"]["known_at"] = [1, 2, 3]
        for value in ({**carry, "rows": [1, 2, 3]}, malformed):
            with patch("trading_research.research.auction_flow_book_recovery.canonical_json",
                       side_effect=AssertionError("malformed carry reached encoding")):
                with self.assertRaises(IntegrityError):
                    kernel(start=40, continuation=value)
        # One unchanged trusted bit cannot have an odd mask Hamming distance.
        with self.assertRaises(IntegrityError):
            kernel(start=40, continuation=redigest(carry, changed_book_valid_rows=1))
