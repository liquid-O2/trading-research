...
def source_cost_projection(units, protocol, schedule, *, excluded_storage_reserve_bytes_per_source_year=512 * 1024**2):
    ...
    if replay_quotes:
        batch_items, metadata_items = [], []
        for u in units:
            cpu = replay_validation_cpu(u)
            batches = u['source_manifest']['physical_reader_batches']
            windows = len(u['source_manifest']['sources'])
            metadata_cpu = u['event_storage']['quotes']['quote_replay_metadata_cpu_seconds']
            if not 0 <= metadata_cpu <= cpu + 1e-5:
                raise IntegrityError('replay metadata CPU must be a disjoint part of full replay CPU')
            # A zero-batch replay is entirely fixed work. Nonempty replays also
            # measure this term, so unseen quiet windows are not forecast free.
            fixed_cpu = metadata_cpu if batches else cpu
            batch_items.append((u, max(0.0, cpu - fixed_cpu), batches))
            metadata_items.append((u, fixed_cpu, windows))
        coefficient('quote_replay_validation', batch_items, 'reader_batches_upper',
            'per-batch portion of one required exact source/quote replay')
        coefficient('quote_replay_metadata_and_dispatch', metadata_items, 'source_windows',
            'fixed replay metadata, implementation/source identity and dispatch, including quiet windows')
