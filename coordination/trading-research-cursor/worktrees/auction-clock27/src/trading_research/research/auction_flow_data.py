...
class AuctionFlowStream:
    ...
    def _validated_last_original_record(self, record, last, *, origin, start, highwater):
        ...
        if any(type(record.get(name)) is not int for name in
               ('t', 'source_row', 'source_order', 'instrument_id', 'flags', 'size', 'bid_sz', 'ask_sz')):
            raise IntegrityError('source continuation lost its source-clock policy or replay carry')
        if any(type(record.get(name)) is not str or not record[name] for name in ('action', 'side', 'source_key')):
            raise IntegrityError('source continuation lost its source-clock policy or replay carry')
        if any(type(record.get(name)) not in (int, float) or type(record.get(name)) is bool
               for name in ('price', 'bid_px', 'ask_px')):
            raise IntegrityError('source continuation lost its source-clock policy or replay carry')
        return dict(record)
    ...
    def carry(self):
        ...
        if self.source_clock_policy != SOURCE_CLOCK_STRICT:
            ...
            result['last_original_record'] = None if self._last_original_record is None else dict(self._last_original_record)
    ...
    def _project(self, original, *, source, physical_rows):
        ...
        last_original = {name: raw[name][-1].as_py() for name in RAW_FIELDS}
        last_original['source_row'] = raw['source_row'][-1].as_py()
        last_original['source_order'] = before + len(raw) - 1
        last_original['source_key'] = key
        for name in ('t', 'source_row', 'source_order', 'instrument_id', 'flags', 'size', 'bid_sz', 'ask_sz'):
            last_original[name] = int(last_original[name])
        self._last_original_record = last_original
        self.last_source_event = {'event_ns': last_original['t'], 'source_order': last_original['source_order'],
                                 'source_row': last_original['source_row'], 'source_key': key}
        quote_parts = []
        for q in quotes:
        ...
        replay_orders = self.projector.last_replay_orders
        if replay_orders:
