...
def compare_complete_payloads(current, previous, *, kind):
    ...
    # Cache-loaded payloads use the already-validated exact measurement domain.
    # Avoid rebuilding their entire plain JSON trees through generic dataclass
    # conversion; retain the identical complete canonical bytes and digests.
    current_sha = hashlib.sha256(exact_measurement_json(current_payload)).hexdigest()
    previous_sha = hashlib.sha256(exact_measurement_json(previous_payload)).hexdigest()
