...
CHECK_EXTENSION = ROOT / 'validation/AUCTION_FLOW_SOURCE_CHECK_EXTENSION_V17.json'
...
def production_mode_limits(protocol, mode, extension=None):
    ...
    from trading_research.research.auction_flow_production import (
        declared_allocation, resolve_declared_document, validate_production_limits)
    ...
    declared, _ = declared_allocation(extension, mode)
    declared = resolve_declared_document(declared, load_reference=load_reference)
    return validate_production_limits(declared, protocol, mode=mode)
...
def execute_source_units_parallel(windows, *, protocol, index, coordinates, outputs, data_root, calendar,
    ...
    try:
        ...
        if packet['configuration']['mode'] in ('throughput', 'source-clock', *EXTRACT_MODES) and evidence:
...
def _source_file_failure_receipt(*, outputs, packet, child, compiled, error):
    if outputs is None or packet is None or child is None:
        return
    try:
        outputs.json('source-file-receipt.json', {
        ...
        }, kind='auction_flow_source_file_receipt')
    except Exception:
        pass
...
def source_file(registered_packet_path, child_packet_path):
    ...
    try:
        ...
        outputs.json('source-file-receipt.json', result, kind='auction_flow_source_file_receipt')
...
def extract_check_source(authenticated, packet_path):
    import unittest
    from trading_research.operations.artifacts import canonical_json, publish_new
    from trading_research.research.auction_flow_production import (
    ...
    report.update(family=protocol['family'], mode='extract-check', success=True,
        tests=tests, production_plan_validated=True,
        ...
        runtime_versions=authenticated['versions'], derived_output_bytes=outputs.written,
        scope='Bounded extract-check of the production controller, file grouping and reservations. No full-source scan or family statistics.')
    publish_new(Path(packet['worker_report']), canonical_json(report) + b'\n')
    return 0
...
def extract_source(authenticated, packet_path, worker_started):
    ...
    report.update(family=protocol['family'], mode='extract',
        success=summary['success'], complete_source_extraction=summary['complete_source_extraction'],
...
def worker(packet_path):
    ...
    try:
        ...
        if packet['configuration']['mode'] == 'extract-check':
            return extract_check_source(authenticated, packet_path)
        ...
        if packet['configuration']['mode'] == 'clock-audit':
            return audit_clock_records(authenticated, packet_path)
...
def parent(mode='check'):
    ...
    if mode in EXTRACT_MODES:
        from trading_research.research.auction_flow_production import declared_allocation
        limits_decl, plan_decl = declared_allocation(extension, mode)
