...
CORE_PARALLEL_POLICY = 'eight_core_partition_consumers_v1'
CORE_WORKER_COUNT = 8
POPULATION_MANIFEST_MAXIMUM_BYTES = 40 * 1024 ** 2
ZONE = 'America/New_York'
FUTURES_ROLL_HOUR = 18
NS = 1_000_000_000
MAXIMUM_JSON_BYTES = 512 * 1024 ** 2
BATCH_HINT = 65536
# Operational fields never enter the scientific partition identity.
OPERATIONAL_CONTRACT_FIELDS = (
    'phase', 'pilot_source_paths', 'accepted_partitions',
...
def scientific_contract_definition(contract):
    """Scientific fields only; operational parallel/reuse metadata is excluded."""
    if not isinstance(contract, dict):
        raise IntegrityError('frozen auction_flow_core_statistics_contract_v1 required')
    return {name: value for name, value in contract.items() if name not in OPERATIONAL_CONTRACT_FIELDS}


def encode_core_partition_key(key):
    if not isinstance(key, (list, tuple)) or len(key) != 3:
        raise IntegrityError('core partition key must be (collection, root, economic_year)')
    collection, root, year = key
    if type(collection) is not str or not collection or type(root) is not str or not root:
        raise IntegrityError('core partition key collection and root must be nonempty strings')
    if type(year) is bool or type(year) is not int:
        raise IntegrityError('core partition economic year must be an exact integer')
    return [collection, root, year]


def decode_core_partition_key(key):
    return tuple(encode_core_partition_key(key))


def core_partition_source_refs(members):
    return [unit.get('receipt', {'source_path': unit['source_path'],
        'start': unit['source_window_start_ns'], 'rows': unit['atomic_rows']}) for unit in members]


def core_partition_identity(contract, *, collection, root, year, source_refs):
    from trading_research.operations.artifacts import digest
    return digest({'definition': scientific_contract_definition(contract),
                   'collection': collection, 'root': root, 'economic_year': year,
                   'source_refs': source_refs})


def core_partition_plan(units, *, collections=None, selected_paths=None):
    """Pure metadata: map units onto (collection, root, economic_year) partitions.

    A source window that crosses an 18:00 economic-year boundary is a member of
    each year it touches. Row-level year filtering later keeps those days from
    being counted twice in a global join.
    """
    ...
    if collections and any(unit['source_path'] not in collections for unit in units):
    ...
    selected = [unit for unit in units if selected_paths is None or unit['source_path'] in selected_paths]
    if selected_paths is not None and set(selected_paths) != {unit['source_path'] for unit in selected}:
    ...
    for key in sorted(partitions):
        members[key] = sorted(partitions[key], key=lambda unit: (unit['source_window_start_ns'], unit['source_path']))
    return {
        'keys': list(members),
        'members': members,
        'overlap': overlap,
...
def select_core_shard_keys(keys, *, ordinal, worker_count=CORE_WORKER_COUNT):
    """Assign sorted partition keys to one of eight workers by index modulo 8."""
    if type(worker_count) is not int or worker_count != CORE_WORKER_COUNT:
        raise IntegrityError('core shard selection is frozen to eight workers')
    if type(ordinal) is not int or not 0 <= ordinal < worker_count:
        raise IntegrityError('core shard ordinal is outside the frozen worker range')
    ordered = [decode_core_partition_key(key) for key in keys]
    if len(set(ordered)) != len(ordered):
        raise IntegrityError('core partition key selection contains duplicates')
    if ordered != sorted(ordered):
        ordered = sorted(ordered)
    return [key for index, key in enumerate(ordered) if index % worker_count == ordinal]


def project_core_parallel_resources(*, annual_measurements, small_measurements, remaining_rows,
                                    remaining_partition_rows, declared_partition_count,
                                    workers=CORE_WORKER_COUNT, parent_reserve_seconds=120):
    """Conservative remaining-work envelope. Reused partitions are not recharged."""
    if type(remaining_rows) is bool or type(remaining_rows) is not int or remaining_rows < 0:
        raise IntegrityError('remaining core rows must be a nonnegative integer')
    if type(declared_partition_count) is bool or type(declared_partition_count) is not int or declared_partition_count < 0:
        raise IntegrityError('declared core partition count must be a nonnegative integer')
    if not annual_measurements:
        raise IntegrityError('core full projection requires a complete large measurement partition')
    if any(item['observations'] < 100000 for item in annual_measurements):
        raise IntegrityError('annual core measurement must contain at least 100000 rows')
    per_row_cpu = max(item['cpu_seconds'] / item['observations'] for item in annual_measurements)
    per_row_wall = max(item['wall_seconds'] / item['observations'] for item in annual_measurements)
    per_row_output = max(item['output_bytes'] / item['observations'] for item in annual_measurements)
    small_cpu = max((item['cpu_seconds'] for item in small_measurements), default=10.)
    small_wall = max((item['wall_seconds'] for item in small_measurements), default=10.)
    cpu = 1.5 * (per_row_cpu * remaining_rows + declared_partition_count * small_cpu)
    walls = []
    for rows in remaining_partition_rows:
        if type(rows) is bool or type(rows) is not int or rows < 0:
            raise IntegrityError('remaining partition row count must be a nonnegative integer')
        walls.append(1.5 * (per_row_wall * rows + small_wall))
    parallel_wall = ((sum(walls) / workers) + (max(walls) if walls else 0.0) + parent_reserve_seconds)
    return {
        'cpu_seconds': cpu,
        'parallel_wall_seconds': parallel_wall,
        'output_bytes': 1.5 * per_row_output * remaining_rows + 512 * 1024 ** 2,
        'remaining_rows': remaining_rows,
        'declared_partition_count': declared_partition_count,
        'remaining_partition_count': len(remaining_partition_rows),
        'parent_reserve_seconds': parent_reserve_seconds,
        'workers': workers,
    }


def _compute_core_partition(*, key, members, identity, source_refs, estimands, calendar,
                            outputs, ordinal, np, pa):
    collection, root, year = key
...
def run_core_statistics(*, population, contract, outputs, load_reference,
                        partition_keys=None, finalize=True):
    ...
    if phase == 'pilot' and (not isinstance(selected_paths, list) or not selected_paths):
        raise IntegrityError('pilot needs explicit physical source-path selection')
    plan = core_partition_plan(all_units, collections=contract.get('source_collections', {}),
                               selected_paths=selected_paths)
    units, partitions, overlap = plan['selected_units'], plan['members'], plan['overlap']
    no_instrument, intended_rows = plan['no_instrument'], plan['intended_rows']
    if partition_keys is None:
        selected_keys = list(plan['keys'])
    else:
        if not isinstance(partition_keys, list):
            raise IntegrityError('partition_keys must be a list of partition keys')
        selected_keys = [decode_core_partition_key(key) for key in partition_keys]
        known = set(plan['keys'])
        if any(key not in known for key in selected_keys):
            raise IntegrityError('partition_keys are not members of the authenticated population')
        if len(set(selected_keys)) != len(selected_keys):
            raise IntegrityError('partition_keys contain a duplicate')
    if finalize and selected_keys != list(plan['keys']):
        raise IntegrityError('finalize requires every authenticated partition')
    accepted = {}
    ...
    for ordinal, key in enumerate(selected_keys):
        ...
        identity = core_partition_identity(contract, collection=collection, root=root,
                                           year=year, source_refs=source_refs)
        if identity in accepted:
            ...
            ref, part, measurement = _compute_core_partition(
                key=key, members=members, identity=identity, source_refs=source_refs,
                estimands=estimands, calendar=calendar, outputs=outputs, ordinal=ordinal, np=np, pa=pa)
            recomputed += 1
        ...
        print(f'core statistics {ordinal + 1}/{len(selected_keys)} {key} observations={part["completeness"]["observations"]}', flush=True)
    if not finalize:
        identities = []
        for key in selected_keys:
            identities.append(core_partition_identity(
                contract, collection=key[0], root=key[1], year=key[2],
                source_refs=core_partition_source_refs(partitions[key])))
        return {
            'kind': KIND, 'version': VERSION, 'phase': phase, 'passed': True, 'finalize': False,
            'partition_keys': [encode_core_partition_key(key) for key in selected_keys],
            'processed_observations': total['observations'],
            'reused_partitions': reused, 'new_partitions': recomputed,
...
__all__ = [
    ...
    'KIND',
    'OPERATIONAL_CONTRACT_FIELDS',
    'POPULATION_MANIFEST_MAXIMUM_BYTES',
    'REQUIRED_CASH_CALENDAR_REFERENCE',
    'VERSION',
