...
def measured_unit(unit, *, protocol, index, coordinates, outputs, data_root, storage_encoding='structural', anchor_calendar=None):
...
def run_preflight(*, protocol, root, outputs, load_reference, check_extension):
    ...
    for unit in windows:
        result, reference = measured_unit(unit, protocol=protocol, index=index, coordinates=coordinates[unit['root']],
            outputs=outputs, data_root=root.parent / 'data', storage_encoding='structural', anchor_calendar=calendar)
