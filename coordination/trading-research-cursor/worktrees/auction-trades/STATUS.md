...
## Auction/flow source check 4 (2026-09-08)

The separate [auction/flow source check 4](/workspace/trading-research/reports/auction-flow-source-check4-review.md) passed 245 tests, all nine retained-value comparisons and three complete split-window comparisons. All four attempts retain 768.174990 cumulative CPU seconds. The corrected source projection is 311,099 CPU seconds and 801 GB, above existing limits; full extraction has not started. Lossless storage and complete anchor workloads are the next implementation work. See the [active family table](/workspace/planning/trading-research/STATUS.md) for research deliverables.

## Current Jumbo resource check (2026-09-08)

[Check 16](/workspace/trading-research/reports/jumbo-check16-complete-resource-review.md) passed 442 tests and completed the workload projection. All sixteen attempts and 1,473.051801 CPU seconds remain counted. The projection is 8,670 CPU seconds for fit, 5,355 for confirmation, 591 MB confirmation output and 3.81 GB peak memory. The [concrete continuation amendment](/workspace/trading-research/reports/jumbo-complete-workload-continuation-proposal-v2.md) requests bounded resources sufficient for that workload and a correction reserve; approval is pending. No further Jumbo execution is authorized. The separate [auction/flow source check 3](reports/auction-flow-source-check3-review.md) passed 235 tests and nine source-window units, retaining both earlier failures (343.772452 cumulative CPU seconds). Exact source/native/storage evidence is reusable within its dependency scope; full extraction still needs partition carry and corrected component costs. No family completion is claimed.
