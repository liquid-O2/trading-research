...
from references.jumbo_extraction_v1.date_statistics import observed_date_statistics
...
def metric_intervals(metrics, dates, settings):
    ...
    for at in range(0, len(names), 1024):
        ...
        result.update({name: _compact(value) for name, value in batch["metrics"].items()})
    return result
...
def _descriptive(values, levels):
    ...
    return {"count": len(values), "mean": sum(values) / len(values), "minimum": min(values),
            "maximum": max(values), "quantiles": {str(q): quantile(values, q) for q in levels}}
...
def special_statistics(rows, dates, *, plan):
    ...
    for clock, observations in sorted(groups.items()):
        ...
        if clock == "PIN078_daily_not_combined":
            from references.jumbo_extraction_v1.ohlc_mechanisms import daily_hit_statistics
        ...
        if metrics:
            record["metrics"] = metric_intervals(metrics, tuple(day for day, _ in observations), plan["statistics"])
