...
def worker(packet_path):
    ...
    try:
        ...
        if packet["mode"] == "check":
            import unittest
            selected = ["tests.test_ohlc_ranges", "tests.test_ohlc_admission",
                        ...
                        "tests.test_jumbo_fitting", "tests.test_jumbo_verification", "tests.test_trial_budget_amendment",
                        "tests.test_research.LabelTests", "tests.test_research.TrialTests", "tests.test_v01_v02"]
...
def parent(mode, parent_report=None, phase=None, resource_authorization=None):
    ...
    required_phase = "extract" if phase == "fit" else "fit" if mode == "confirm" else None
    if required_phase and predecessor.get("phase") != required_phase:
    ...
    packet = {"mode": mode, "trial_id": trial, "attempt_id": attempt,
              ...
              "predecessor": parent_identity, "worker_report": str(destination / "worker.json"), "supervisor_pid": os.getpid(),
              "limits": phase_limits}
    ...
    try:
        ...
        joined = worker_value is not None and all(worker_value.get(k) == packet[k] for k in
                  ("trial_id", "attempt_id", "protocol_sha256", "code_snapshot", "tools_snapshot", "mode", "phase", "analysis_plan", "execution_plan", "model_plan", "verification_contracts", "resource_amendment"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resource-authorization", help="Explicit retained attempt-count-only approval; all phase limits stay fixed")
