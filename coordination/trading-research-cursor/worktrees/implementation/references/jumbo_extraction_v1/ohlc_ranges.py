...
class OhlcWindow:
    ...
    def geometry(self) -> RangeArithmetic:
        ...
        return RangeArithmetic(min(s.low[a:b]), max(s.high[a:b]), s.open[a], s.close[b - 1])
    ...
    def summary(self):
        common = {"window_version": self.version, "source_version": self.series.source_version,
                  ...
                  "exclusion_reasons": list(self.reasons)}
        if not self.complete:
            return common
        g, s, a, b = self.geometry(), self.series, self.left, self.right
        return {**common, "available_at_ns": max(self.end, max(s.known[a:b])),
                "open_ticks": int(g.open), "high_ticks": int(g.high),
                "low_ticks": int(g.low), "close_ticks": int(g.close),
                "width_ticks": int(g.width), "volume": sum(s.volume[a:b]),
...
def range_path(window: OhlcWindow, *, low: int, high: int, strict=True) -> dict:
    ...
    maximum, minimum = max(s.high[a:b]), min(s.low[a:b])
    ...
    return {**base, "status": "ambiguous" if side == "ambiguous" else "observed", "path": path,
            ...
            "maturity_at_ns": max(s.known[a:b]),
...
def level_outcome(window: OhlcWindow, *, level: Fraction, side: int) -> dict:
    ...
    s, a, b = window.series, window.left, window.right
    contact = next((i for i in range(a, b) if s.low[i] <= level <= s.high[i]), None)
    gap = any(min(s.close[i - 1], s.open[i]) < level < max(s.close[i - 1], s.open[i])
              and not s.low[i] <= level <= s.high[i] for i in range(a + 1, b))
    if contact is None:
        return {**base, "status": "gap_through_without_observed_contact" if gap else "no_contact",
                "gap_through": gap}
    definite = next((i for i in range(contact, b)
                     if level in (s.open[i], s.high[i], s.low[i], s.close[i])), None)
    ...
    latest = definite if definite is not None else next(
        i for i in range(b - 1, contact - 1, -1) if s.low[i] <= level <= s.high[i])
