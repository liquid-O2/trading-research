    ...
    for group in sorted(universes):
        ...
        if probability_matrix_array is not None:
            for class_index in range(probability_matrix_array.shape[1]):
                ...
                bin_indices = np.full(n, -1, dtype=np.int64)
                bin_indices[finite] = np.minimum((p_column[finite] * 10.0).astype(np.int64), 9)
                for bin_index in range(10):
                    selected = bin_indices == bin_index
                    p_values = np.full(len(intended_tuple), np.nan, dtype=np.float64)
                    low_values = np.full(len(intended_tuple), np.nan, dtype=np.float64)
                    high_values = np.full(len(intended_tuple), np.nan, dtype=np.float64)
                    counts = np.zeros(len(intended_tuple), dtype=np.int64)
                    for day, row_indices in date_cells.items():
                        date_index = day_index.get(day)
                        if date_index is None:
                            raise EvaluationInputError("observed date lies outside its intended group universe")
                        chosen = selected[row_indices]
                        count = int(np.count_nonzero(chosen))
                        if count:
                            selected_rows = row_indices[chosen]
                            p_values[date_index] = float(np.mean(p_column[selected_rows]))
                            low_values[date_index] = float(np.mean(low_column[selected_rows]))
                            high_values[date_index] = float(np.mean(high_column[selected_rows]))
                            counts[date_index] = count
                    names = (
                        f"__calibration__class_{class_index}__bin_{bin_index}__predicted_mean",
                        f"__calibration__class_{class_index}__bin_{bin_index}__outcome_lower",
                        f"__calibration__class_{class_index}__bin_{bin_index}__outcome_upper",
                    )
                    date_values_batch = np.vstack((date_values_batch, p_values, low_values, high_values))
                    date_events_batch = np.vstack((date_events_batch, counts, counts, counts))
                    batch_names += names
