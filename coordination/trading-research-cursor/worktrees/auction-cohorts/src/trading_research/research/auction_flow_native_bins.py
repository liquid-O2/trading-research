...
class NativeEventBins:
    ...
    def quote_exposure(self, *, starts, ends, bid, ask, bid_size, ask_size):
        ...
        extra_bin, extra_duration, extra_index = [], [], []
        for i in np.flatnonzero(~same):
            for number in range(int(first[i]), int(last[i]) + 1):
                a, b = self.start + number * self.width, self.start + (number + 1) * self.width
                extra_bin.append(number)
                extra_duration.append(int(min(ends[i], b) - max(starts[i], a)))
                extra_index.append(i)
        bins = np.r_[first[same], np.asarray(extra_bin, dtype=np.int64)]
        duration = np.r_[(ends - starts)[same], np.asarray(extra_duration, dtype=np.int64)]
        source = np.r_[index, np.asarray(extra_index, dtype=np.int64)]
