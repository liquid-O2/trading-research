...
from references.jumbo_extraction_v1.ohlc_ranges import MINUTE, OhlcWindow, level_outcome
...
def _known(window):
    return max(window.end, max(window.series.known[window.left:window.right]))
...
def _contact(window, level, contract):
    ...
    compatible = [i for i in range(a, b) if s.low[i] <= level <= s.high[i]]
    definite = [i for i in compatible if level in (s.open[i], s.high[i], s.low[i], s.close[i])]
    return {**base, "status": "complete", "maturity_at_ns": _known(window),
            "compatible_reach": bool(compatible), "definite_print": bool(definite),
            "compatible_bar_count": len(compatible), "definite_print_bar_count": len(definite),
            "origin_open_equals_target": s.open[a] == level,
            "exact_integer_tick_print_possible": level.denominator == 1,
            "first_compatible_interval_ns": None if not compatible else [s.start[compatible[0]], s.end[compatible[0]]],
            "first_definite_interval_ns": None if not definite else [s.start[definite[0]], s.end[definite[0]]],
            "contact_evidence": level_outcome(window, level=level, side=1)}
...
def _pending_events(s, i, side, eq, invalidation, *, first=False,
                    break_at_open=False, both_first=False):
    upper = side == "upper"
    target = s.low[i] <= eq if upper else s.high[i] >= eq
    invalid = s.high[i] >= invalidation if upper else s.low[i] <= invalidation
    open_target = s.open[i] <= eq if upper else s.open[i] >= eq
    open_invalid = s.open[i] >= invalidation if upper else s.open[i] <= invalidation
    if open_invalid:
        return {"invalidation"}
    ...
    if first:
        ...
        close_target = s.close[i] <= eq if upper else s.close[i] >= eq
...
def _magic_branch(window, first, side, low, high, q):
    ...
    width, eq = high - low, Fraction(high + low, 2)
    inv = high + q * width if side == "upper" else low - q * width
    opening_break = s.open[first] > high if side == "upper" else s.open[first] < low
    both = s.high[first] > high and s.low[first] < low
    states = _pending_events(s, first, side, eq, inv, first=True,
    ...
    for i in range(first + 1, end):
        if not pending:
            break
        states = _pending_events(s, i, side, eq, inv)
    ...
    compatible = any(s.low[i] <= eq <= s.high[i] for i in range(first, end))
    definite = any(eq in (s.open[i], s.high[i], s.low[i], s.close[i]) for i in range(first + 1, end))
    definite = definite or s.close[first] == eq or opening_break and eq in (
...
def _magic_source_comparator(window, first, low, high, q):
    ...
    eq, width = Fraction(high + low, 2), high - low
    inv = high + q * width if side == "upper" else low - q * width
    for i in range(first + 1, window.right):
        invalid = s.high[i] >= inv if side == "upper" else s.low[i] <= inv
        target = s.low[i] <= eq if side == "upper" else s.high[i] >= eq
