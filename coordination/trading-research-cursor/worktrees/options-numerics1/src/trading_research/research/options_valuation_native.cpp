...
void auto_domain(double S0, double K, double T, double sigma, double r,
    ...
    double span = 8.0 * st + std::fabs(r) * T + 2.0;
    if (span < 2.0) span = 2.0;
    double ref = dmax(S0, K);
    if (ref < 1e-12) ref = 1.0;
    *shi = ref * std::exp(span) + 2.0 * sumD + ref;
    *slo = ref * std::exp(-span);
    const double lo_cap = 0.25 * dmin(S0, K);
    if (*slo > lo_cap) *slo = lo_cap;
    if (*slo < 1e-12) *slo = 1e-12;
    const double hi_need = 4.0 * dmax(S0, K) + 2.0 * sumD;
    if (*shi < hi_need) *shi = hi_need;
    if (!(*slo < S0 && S0 < *shi && *slo < K && K < *shi)) {
        *slo = dmin(*slo, 0.25 * dmin(S0, K));
        if (*slo < 1e-12) *slo = 1e-12;
        *shi = dmax(*shi, hi_need);
    }
}
...
struct StepScratch {
    std::vector<double> A, B, C, rhs, Ac, Bc, Cc, rhsc, H;
    std::vector<char> ex;
    explicit StepScratch(int n)
        : A(n), B(n), C(n), rhs(n), Ac(n), Bc(n), Cc(n), rhsc(n), H(n), ex(n, 0) {}
};
...
double step_residual(int n, const double* A, const double* B, const double* C,
    ...
    double e = 0.0;
    for (int i = 0; i < n; ++i) {
        if (V[i] + 1e-14 < H[i]) e = dmax(e, H[i] - V[i]);
        if (i == 0 || i == n - 1) continue;
        if (V[i] > H[i] + 1e-14) {
            const double r = A[i] * V[i - 1] + B[i] * V[i] + C[i] * V[i + 1] - rhs[i];
            e = dmax(e, std::fabs(r));
        }
    }
    return e;
...
int american_cn_step(int n, double* V, const double* S, double dx, double dt,
    ...
    const double a = nu * invdx2 - mu * inv2dx;
    const double b = -2.0 * nu * invdx2 - r;
    const double c = nu * invdx2 + mu * inv2dx;
    const double blo = bc_low(right, K, r, tau_new);
    const double bhi = bc_high(right, S[n - 1], K, r, tau_new, pv_new);
    std::vector<double> Vold(V, V + n);
    for (int i = 0; i < n; ++i) {
    ...
    for (int it = 0; it < POLICY_ITERS; ++it) {
        ...
        for (int i = 1; i < n - 1; ++i) {
            ...
            } else if (sc.ex[i] && V[i] > sc.H[i] + 1e-13 * scale) {
                sc.ex[i] = 0;
                changed = true;
            }
        }
...
SolveOut solve_one(double S0, double K, double T, double sigma, double r, int right,
    ...
    for (int j = 0; j < n_div; ++j) {
        if (!std::isfinite(div_t[j]) || !std::isfinite(div_a[j]) || div_a[j] < 0.0) return o;
    }
    if (!(T > 0.0)) {
    ...
    if (!(sigma > 0.0)) {
        std::vector<double> Vold(static_cast<size_t>(ns));
        for (int k = ntimes - 2; k >= 0; --k) {
            const double dt = times[k + 1] - times[k];
            if (!(dt > 0.0)) continue;
            const double growth = std::exp(r * dt);
            const double df = std::exp(-r * dt);
            const double tau_src = T - times[k + 1];
            const double blo = bc_low(right, K, r, tau_src);
            const double bhi = bc_high(right, S[ns - 1], K, r, tau_src,
                                       pv_divs(times[k + 1], r, div_t, div_a, n_div));
            Vold = V;
            for (int i = 0; i < ns; ++i) {
                const double cont = df * interp_S(S.data(), Vold.data(), ns, S[i] * growth, blo, bhi);
                V[i] = dmax(intrinsic(right, S[i], K), cont);
            }
            const double D = cash_at(times[k], T, div_t, div_a, n_div);
            if (D > 0.0) {
                const double tau = T - times[k];
                apply_dividend(V.data(), S.data(), ns, D, right, K, bc_low(right, K, r, tau));
            }
        }
        o.price = interp_S(S.data(), V.data(), ns, S0,
                           bc_low(right, K, r, T),
                           bc_high(right, S[ns - 1], K, r, T, pv_divs(0.0, r, div_t, div_a, n_div)));
        o.residual = 0.0;
        o.iterations = 0;
        o.status = ST_SIGMA0;
        return o;
    ...
    for (int k = ntimes - 2; k >= 0; --k) {
        ...
        const double pv = pv_divs(times[k], r, div_t, div_a, n_div);
        auto run = [&](double h, double theta) {
            double res = 0.0;
            const int u = american_cn_step(ns, V.data(), S.data(), dx, h, theta, sigma, r,
                                           right, K, tau_new, pv, sc, &res);
            last_res = res;
            it_acc += std::abs(u);
            if (u < 0) failed = true;
        };
...
SolveOut solve_refined(double S0, double K, double T, double sigma, double r, int right,
                       ...
                       int n_space, int n_time, double domain_lo, double domain_hi,
                       double* price_error) {
    const int ns0 = n_space > 0 ? n_space : 128;
    const int nt0 = n_time > 0 ? n_time : ns0;
    SolveOut a = solve_one(S0, K, T, sigma, r, right, div_t, div_a, n_div, ns0, nt0,
                           domain_lo, domain_hi);
    *price_error = nan_v();
    if ((a.status & STATUS_MASK) == ST_INVALID_INPUT || (a.status & STATUS_MASK) == ST_DOMAIN_ERROR) {
        return a;
    }
    if ((a.status & STATUS_MASK) == ST_ZERO_T) {
        *price_error = 0.0;
        return a;
    }
    const int ns1 = ns0 * 2;
    if (ns1 > MAX_SPACE) {
        *price_error = nan_v();
        a.status = with_flag(a.status, FLAG_GRID_ERROR);
        return a;
    }
    int nt1 = nt0 * 2;
    if (nt1 > MAX_TIME) nt1 = MAX_TIME;
    SolveOut b = solve_one(S0, K, T, sigma, r, right, div_t, div_a, n_div, ns1, nt1,
                           domain_lo, domain_hi);
    if ((b.status & STATUS_MASK) == ST_INVALID_INPUT || (b.status & STATUS_MASK) == ST_DOMAIN_ERROR) {
        *price_error = nan_v();
        a.status = with_flag(a.status, FLAG_GRID_ERROR);
        return a;
    }
    SolveOut best = b;
    *price_error = std::fabs(b.price - a.price);
    if (ns0 == 128 && ns1 * 2 <= MAX_SPACE) {
        const int ns2 = 512;
        int nt2 = n_time > 0 ? nt0 * 4 : 512;
        if (nt2 > MAX_TIME) nt2 = MAX_TIME;
        SolveOut c = solve_one(S0, K, T, sigma, r, right, div_t, div_a, n_div, ns2, nt2,
                               domain_lo, domain_hi);
        if ((c.status & STATUS_MASK) != ST_INVALID_INPUT && (c.status & STATUS_MASK) != ST_DOMAIN_ERROR) {
            *price_error = std::fabs(c.price - b.price);
            best = c;
        }
    }
    if (std::isfinite(*price_error) && *price_error > grid_tol(S0)) {
        if ((best.status & STATUS_MASK) == ST_CONVERGED || (best.status & STATUS_MASK) == ST_SIGMA0) {
            best.status = ST_GRID_INCONSISTENT | (best.status & ~STATUS_MASK);
        }
        best.status = with_flag(best.status, FLAG_GRID_ERROR);
    }
    best.iterations += a.iterations;
    return best;
}
...
int check_batch(int64_t n_rows, int64_t n_div, int32_t n_space, int32_t n_time,
    ...
    for (int64_t i = 0; i < n_rows; ++i) {
        if (div_off[i] < 0 || div_off[i + 1] < div_off[i] || div_off[i + 1] > n_div) return 2;
    }
    return 0;
}
...
int price_batch_impl(
    ...
    for (int64_t i = 0; i < n_rows; ++i) {
        try {
            ...
            double err = nan_v();
            const SolveOut o = solve_refined(
                S[i], K[i], T[i], sigma[i], r[i], static_cast<int>(right[i]),
...
SolveOut iv_one(double target, double S0, double K, double T, double r, int right,
    ...
    *hi_out = SIGMA_HI;
    if (!std::isfinite(target) || target < 0.0) {
        o.status = ST_FAILED_BRACKET;
        return o;
    }
    const int ns = n_space > 0 ? n_space : 128;
    const int nt = n_time > 0 ? n_time : ns;
    const SolveOut p0 = solve_one(S0, K, T, 0.0, r, right, div_t, div_a, n_div, ns, nt,
                                  domain_lo, domain_hi);
    if ((p0.status & STATUS_MASK) == ST_INVALID_INPUT || (p0.status & STATUS_MASK) == ST_DOMAIN_ERROR) {
        return p0;
    }
    if ((p0.status & STATUS_MASK) == ST_ZERO_T) {
        o.status = ST_ZERO_T;
        o.residual = target - p0.price;
        return o;
    }
    const double intr = intrinsic(right, S0, K);
    const double put_cap = K * dmax(1.0, std::exp(-r * T));
    const double call_cap = S0;
    const double cap = right > 0 ? call_cap : put_cap;
    if (target + 1e-14 < intr) {
        o.status = ST_FAILED_BRACKET;
        o.residual = target - intr;
        return o;
    }
    const SolveOut pHi = solve_one(S0, K, T, SIGMA_HI, r, right, div_t, div_a, n_div, ns, nt,
                                   domain_lo, domain_hi);
    int it = p0.iterations + pHi.iterations;
    const double tol = grid_tol(S0);
    if (!std::isfinite(p0.price) || !std::isfinite(pHi.price)) {
        o.status = ST_NONCONVERGED;
        o.iterations = it;
        return o;
    }
    if (target > cap + 1e-12) {
        o.status = ST_FAILED_BRACKET;
        o.residual = target - cap;
        o.iterations = it;
        return o;
    }
    const double span = pHi.price - p0.price;
    if (target <= p0.price + tol) {
        o.price = 0.0;
        o.residual = p0.price - target;
        o.iterations = it;
        o.status = ST_ENDPOINT_LOWER;
        if (std::fabs(span) <= 2.0 * tol) o.status = with_flag(o.status, FLAG_WEAK_VEGA);
        return o;
    }
    if (target >= pHi.price - tol) {
        o.price = SIGMA_HI;
        o.residual = pHi.price - target;
        o.iterations = it;
        o.status = (target > pHi.price + tol) ? ST_FAILED_BRACKET : ST_ENDPOINT_UPPER;
        if (std::fabs(span) <= 2.0 * tol) o.status = with_flag(o.status, FLAG_WEAK_VEGA);
        return o;
    }
    if (std::fabs(span) <= 2.0 * tol) {
        o.status = with_flag(ST_FAILED_BRACKET, FLAG_WEAK_VEGA);
        o.residual = target - p0.price;
        o.iterations = it;
        return o;
    }
    double lo = 0.0, hi = SIGMA_HI;
    double plo = p0.price, phi = pHi.price;
    SolveOut last = p0;
    bool conv = false;
    for (int k = 0; k < IV_ITERS; ++k) {
        const double mid = 0.5 * (lo + hi);
        last = solve_one(S0, K, T, mid, r, right, div_t, div_a, n_div, ns, nt,
                         domain_lo, domain_hi);
        it += last.iterations;
        if (!std::isfinite(last.price)) {
            o.status = ST_NONCONVERGED;
            o.iterations = it;
            return o;
        }
        const double res = last.price - target;
        if (std::fabs(res) <= dmax(1e-6, 1e-8 * dmax(1.0, target))) {
            o.price = mid;
            o.residual = res;
            o.iterations = k + 1;
            o.status = (last.status & STATUS_MASK) == ST_NONCONVERGED ? ST_NONCONVERGED : ST_CONVERGED;
            *lo_out = lo;
            *hi_out = hi;
            conv = true;
            break;
        }
        if (last.price > target) {
            hi = mid;
            phi = last.price;
        } else {
            lo = mid;
    ...
    if (!conv) {
        o.status = ST_NONCONVERGED;
        o.price = 0.5 * (lo + hi);
        last = solve_one(S0, K, T, o.price, r, right, div_t, div_a, n_div, ns, nt,
                         domain_lo, domain_hi);
        o.residual = last.price - target;
        o.iterations = IV_ITERS;
    }
    if (std::fabs(span) <= 4.0 * tol) o.status = with_flag(o.status, FLAG_WEAK_VEGA);
    return o;
}
