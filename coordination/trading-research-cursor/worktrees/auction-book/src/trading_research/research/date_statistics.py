...
def _moving_weights(np, date_count: int, *, seed: int, block_length: int, replicates: int):
    ...
    for row in range(replicates):
        ...
        # ``selected`` is already the complete truncated resample index.  A
        # bincount produces the same integer occurrence vector as ``add.at``
        # while avoiding one indexed ufunc update for every selected date.
        weights[row, :] = np.bincount(selected, minlength=date_count)
...
def _percentile(values: Sequence[float], level: float) -> float | None:
    ...
    if isinstance(values, tuple):
        return float(quantile(values, level))
    return float(quantile(tuple(float(value) for value in values), level))


def _percentile_pair(values: Sequence[float], lower_level: float, upper_level: float):
    """Calculate both tails from one sorted replicate population."""
    if not values:
        return None, None
    ...
    return pick(lower_level), pick(upper_level)
...
def _metric_result(np, metric: dict[str, Any], labels: tuple[str, ...], weights,
                   ...
                   retain_replicate_estimates: bool = True):
    ...
    if retain_replicate_estimates:
        ...
        # Report batches discard ``replicate_estimates`` immediately after
        # each metric is summarized.  Keep only the values needed for the two
        # empirical quantiles and avoid allocating one 1000-element Python
        # list per metric.
        replicate_values = None
        valid_values = tuple(float(value) for value in replicate_array[valid_replicates]
                             if math.isfinite(float(value)))
    alpha = (1 - float(confidence)) / 2
    lower, upper = _percentile_pair(valid_values, alpha, 1 - alpha)
    ...
    if retain_replicate_estimates:
        bootstrap["replicate_estimates"] = replicate_values
    return {
...
def _ratio_result(np, name: str, numerator_name: str, denominator_name: str,
    ...
    if retain_replicate_estimates:
        ...
        serial = None
        valid_values = tuple(float(value) for value in values[valid]
                             if math.isfinite(float(value)))
    alpha = (1 - float(confidence)) / 2
    reasons = []
    ...
    if retain_replicate_estimates:
        bootstrap["replicate_estimates"] = serial
    return {
...
def observed_date_statistics(
    ...
    *,
    ...
    _retain_replicate_estimates: bool = True,
) -> dict[str, Any]:
    ...
    if type(_retain_replicate_estimates) is not bool:
        raise ContractError("internal replicate-estimate retention flag must be boolean")
    if _bootstrap_state is None:
        ...
        try:
            state_np, weights, starts_by_rep = _bootstrap_state
        except (TypeError, ValueError) as exc:
            raise ContractError("invalid internal bootstrap state") from exc
        if state_np is not np:
            raise ContractError("internal bootstrap state uses a different numerical provider")
        if weights.shape != (replicates if len(dates) >= block_length else 0, len(dates)):
            raise ContractError("internal bootstrap state has an incompatible date universe")
        if len(starts_by_rep) != replicates:
            raise ContractError("internal bootstrap state has an incompatible replicate count")
    ...
    for index, name in enumerate(names):
        result, replicate_array, valid_mask = _metric_result(
            ...
            retain_replicate_estimates=_retain_replicate_estimates,
        )
