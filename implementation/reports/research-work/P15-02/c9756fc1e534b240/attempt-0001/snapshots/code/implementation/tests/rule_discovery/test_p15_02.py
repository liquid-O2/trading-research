...
from trading_research.research.rule_discovery.baseline import (
    ...
    recovered_date_job_invocations,
    replay_date,
    scan_baseline,
)
...
def _arrays_from_trades(
    ...
    *,
    ...
    if not events:
        return empty_session_arrays(start_ns=start_ns or 0, end_ns=end_ns or 0, instrument_id=instrument_id)
    start = min(item["event_ns"] for item in events) if start_ns is None else start_ns
    end = (max(item["event_ns"] for item in events) + 1) if end_ns is None else end_ns

    def decimal_column(key: str) -> pa.Array:
        values = [None if item.get(key) is None else Decimal(str(item[key])) for item in events]
        return pa.array(values, type=pa.decimal128(15, 4))

    payload = {
        "t": pa.array([int(item["event_ns"]) for item in events], type=pa.int64()),
        "action": [str(item.get("action") or "") for item in events],
        "side": [str(item.get("side") or "") for item in events],
        "price": decimal_column("price"),
        "size": [int(item.get("executed_size") or item.get("size") or 0) for item in events],
        "bid": decimal_column("bid"),
        "ask": decimal_column("ask"),
        "bid_size": [item.get("bid_size") for item in events],
        "ask_size": [item.get("ask_size") for item in events],
        "flags": [int(item.get("flags") or 0) for item in events],
        "ts_recv": [int(item.get("known_at") or item["event_ns"]) for item in events],
    }
    sequences = [item.get("exchange_sequence") for item in events]
    if any(item is not None for item in sequences):
        payload["exchange_sequence"] = pa.array([-1 if item is None else int(item) for item in sequences], type=pa.int64())
    table = pa.table(payload)
    offsets = np.array([int(item.get("source_row") if item.get("source_row") is not None else i) for i, item in enumerate(events)], dtype=np.int64)
    source = str(events[0].get("source_file") or "synth.parquet")
    return decode_arrow_table(
        table,
        start_ns=start,
        end_ns=end,
...
def test_a05_partial_gap_and_ambiguous_batch_stay_explicit():
    ...
    arrays = _arrays_from_trades([missing, present], start_ns=0, end_ns=40, instrument_id="test")
    ...
    batches = list(NativeMarketView(_arrays_from_trades([first, second], start_ns=20, end_ns=21, instrument_id="t")).executions(20, 21))
...
def test_vectorized_bars_match_python_reference():
    ...
    arrays = _arrays_from_trades(events, start_ns=0, end_ns=3_000_000_000, instrument_id="t")
...
def test_s09_permutation_preserves_ambiguity():
    ...
    left = list(NativeMarketView(_arrays_from_trades([a, b], start_ns=5, end_ns=6, instrument_id="t")).executions(5, 6))
    right = list(NativeMarketView(_arrays_from_trades([b, a], start_ns=5, end_ns=6, instrument_id="t")).executions(5, 6))
...
def test_s11_complete_empty_is_zero_and_gap_is_unknown():
    empty = empty_session_arrays(start_ns=0, end_ns=60_000_000_000, instrument_id="t")
    prefix = build_prefix_sums(empty)
    price, _, volume = vwap_from_prefix(prefix, 0, 60_000_000_000)
    assert price is None and volume == 0
    gapped = _arrays_from_trades([_trade(90_000_000_000, "100.00", 1, "B")], start_ns=0, end_ns=120_000_000_000, instrument_id="t")
...
def test_s15_duplicate_and_shuffled_keys():
    ...
    arrays = _arrays_from_trades(rows, start_ns=10, end_ns=12, instrument_id="t")
    assert list(arrays.row_id) == ["synth.parquet:1", "synth.parquet:2"]
    shuffled = _arrays_from_trades(list(reversed(rows)), start_ns=10, end_ns=12, instrument_id="t")
...
def test_p2_zero_length_coverage_is_unknown_never_complete():
    events = [_trade(10_000_000_000, "100.00", 1, "B")]
    view = NativeMarketView(_arrays_from_trades(events))
    receipt = view.coverage(10_000_000_000, 10_000_000_000)
    assert receipt.status == Coverage.MISSING
    assert receipt.status != Coverage.COMPLETE
    assert receipt.observed_intervals == ()


def test_p3_population_completeness_uses_earliest_formation_clock():
    from datetime import timedelta
    from trading_research.research.method_pack.clocks import et_ns

    day = date(2020, 1, 2)
    account_start = et_ns(day - timedelta(days=1), 18, 0)
    asia = earliest_population_start_ns(day, ["asia"], account_start_ns=account_start)
    cash = earliest_population_start_ns(day, ["cash_open"], account_start_ns=account_start)
    assert asia < cash
    rth_trade = [_trade(et_ns(day, 10, 0), "100.00", 1, "B")]
    view = NativeMarketView(_arrays_from_trades(rth_trade, start_ns=account_start, end_ns=et_ns(day, 16, 0), instrument_id="t"))
    overnight = population_coverage(view, day=day, required_groups=["asia", "cash_open"])
    rth_only = view.coverage(et_ns(day, 9, 30), et_ns(day, 16, 0))
    assert overnight.status != Coverage.COMPLETE
    assert overnight.start_ns < rth_only.start_ns


def test_b9_monthly_seam_contiguous_when_empty():
    left_end = 1_000
    right_start = 1_050
    plan = {
        "owned_spans": [
            {"path": "/tmp/2021-05.parquet", "start_ns": 0, "end_ns": left_end},
            {"path": "/tmp/2021-06.parquet", "start_ns": right_start, "end_ns": 2_000},
        ],
        "unowned_intervals": [{"start_ns": left_end, "end_ns": right_start}],
    }
    unowned, rule = stitch_monthly_seams(plan)
    assert rule == SEAM_OWNERSHIP
    assert unowned == []
    leftover = stitch_monthly_seams(
        {
            "owned_spans": plan["owned_spans"],
            "unowned_intervals": [{"start_ns": 3_000, "end_ns": 4_000}],
        }
    )
    assert leftover[0] == [{"start_ns": 3_000, "end_ns": 4_000}]


def test_b5_production_decode_does_not_iterate_python_rows():
    source = Path("/workspace/implementation/src/trading_research/research/rule_discovery/native.py").read_text()
    assert "def decode_rows" not in source
    assert "for index, row in enumerate(rows)" not in source
    empty = empty_session_arrays(start_ns=0, end_ns=1, instrument_id="t")
    assert empty.t_ns.size == 0
    assert empty.source_sha256 == ()


def test_replay_recovers_run_101_date_job_invocations():
    registry, manifest = load_phase1_registry()
    branches = list(manifest["branches"])
    one = recovered_date_job_invocations("2020-01-02", compare_root=PHASE1_RUN, branches=branches)
    assert len(one) == 1
    assert len(one[0]) == 57
    march = recovered_date_job_invocations("2020-03-02", compare_root=PHASE1_RUN, branches=branches)
    assert len(march) == 2
    assert march[1][0]["method_id"] == "JETBUNDLE-STATES"
    april = recovered_date_job_invocations("2020-04-01", compare_root=PHASE1_RUN, branches=branches)
    assert len(april) == 2
    assert sum(len(cluster) for cluster in april) == 57
