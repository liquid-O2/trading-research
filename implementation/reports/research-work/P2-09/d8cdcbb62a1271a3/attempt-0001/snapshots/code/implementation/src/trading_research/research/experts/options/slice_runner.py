...
def run_p2_10_slice(run_root: Path, dates: list[str]) -> dict[str, Any]:
    ...
    throughput = throughput_document(
        ...
        note="P2-10 QQQ/SPY boards at 10:00 ET, one core, 20 dates",
    )
    ...
    build_atlas(boards, run_root, coverage=coverage, slice_dates=dates)
    return {"boards": len(boards), "jobs": len(jobs), "throughput": throughput}
...
def run_p2_03_slice(run_root: Path, dates: list[str]) -> dict[str, Any]:
    ...
    throughput = throughput_document(
        ...
        dates,
        rss=[int(j.get("peak_rss_bytes") or 0) for j in jobs],
        note="P2-03 NQ 1-minute GK/RV/heads at 10:00 ET, one core, 20 dates",
    )
