...
def load_quote_arrays(root: str, day: date, *, kind: str = "quote_dte14") -> QuoteArrays | None:
    ...
    if spec.databento_prefix:
        return load_futures_quote_arrays(root, day)
...
def load_contract_arrays(root: str, day: date, *, available_at_ns: int | None = None) -> ContractArrays | None:
    spec = root_spec(root)
    if spec.databento_prefix:
        return load_futures_contract_arrays(root, day, available_at_ns=available_at_ns)
...
def load_oi_arrays(root: str, effective: date, *, extra_sessions: int = 0) -> OIArrays | None:
    spec = root_spec(root)
    if spec.databento_prefix:
        return load_futures_oi_arrays(root, effective, extra_sessions=extra_sessions)
...
def _align_definition_fields(ids: np.ndarray, defs: dict[str, np.ndarray] | None) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    n = ids.size
    strike = np.full(n, -1, dtype=np.int64)
    right = np.zeros(n, dtype=np.int8)
    expiry = np.zeros(n, dtype=np.int64)
    osi = np.array([str(int(i)) for i in ids.tolist()], dtype=object)
    if defs is None or defs["instrument_id"].size == 0:
        return strike, right, expiry, osi
    order = np.argsort(defs["instrument_id"], kind="mergesort")
    keys = defs["instrument_id"][order]
    idx = np.searchsorted(keys, ids)
    in_range = idx < keys.size
    hit = np.zeros(n, dtype=np.bool_)
    clipped = np.minimum(idx, max(keys.size - 1, 0))
    hit[in_range] = keys[clipped[in_range]] == ids[in_range]
    src = order[clipped]
    strike[hit] = defs["strike_millis"][src[hit]]
    right[hit] = defs["right"][src[hit]]
    expiry[hit] = defs["expiry_ns"][src[hit]]
    osi[hit] = defs["raw_symbol"][src[hit]]
    return strike, right, expiry, osi


def _et_day_mask(ts_ns: np.ndarray, day: date) -> np.ndarray:
    start = et_ns(day, 0, 0)
    end = et_ns(day + timedelta(days=1), 0, 0)
    return (ts_ns >= start) & (ts_ns < end)


_DEF_ASOF_CACHE: dict[tuple[str, int], dict[str, np.ndarray] | None] = {}


def _last_definitions_asof(root: str, asof_ns: int) -> dict[str, np.ndarray] | None:
    from trading_research.research.experts.options.databento_decode import definitions_table

    key = (root, int(asof_ns))
    if key in _DEF_ASOF_CACHE:
        return _DEF_ASOF_CACHE[key]
    table = definitions_table(root)
    if table.num_rows == 0:
        _DEF_ASOF_CACHE[key] = None
        return None
    ts = np.asarray(table.column("ts_event").to_numpy(), dtype=np.int64)
    keep = ts <= asof_ns
    if not np.any(keep):
        _DEF_ASOF_CACHE[key] = None
        return None
    ids = np.asarray(table.column("instrument_id").to_numpy(), dtype=np.int64)[keep]
    ts_k = ts[keep]
    action = np.asarray(table.column("security_update_action").to_numpy(zero_copy_only=False), dtype=object)[keep]
    order = np.lexsort((ts_k, ids))
    ids_s = ids[order]
    change = np.empty(ids_s.size, dtype=np.bool_)
    change[0] = True
    change[1:] = ids_s[1:] != ids_s[:-1]
    starts = np.flatnonzero(change)
    ends = np.append(starts[1:], ids_s.size) - 1
    last = order[ends]
    live = np.array([str(action[i]) != "D" for i in last], dtype=np.bool_)
    last = last[live]
    if last.size == 0:
        _DEF_ASOF_CACHE[key] = None
        return None
    idx = np.flatnonzero(keep)[last]
    result = {
        "instrument_id": np.asarray(table.column("instrument_id").to_numpy(), dtype=np.int64)[idx],
        "ts_event": ts[idx],
        "raw_symbol": np.asarray(table.column("raw_symbol").to_numpy(zero_copy_only=False), dtype=object)[idx],
        "underlying": np.asarray(table.column("underlying").to_numpy(zero_copy_only=False), dtype=object)[idx],
        "right": np.asarray(table.column("right").to_numpy(), dtype=np.int8)[idx],
        "strike_millis": np.asarray(table.column("strike_millis").to_numpy(), dtype=np.int64)[idx],
        "expiry_ns": np.asarray(table.column("expiry_ns").to_numpy(), dtype=np.int64)[idx],
    }
    _DEF_ASOF_CACHE[key] = result
    return result


def load_futures_contract_arrays(root: str, day: date, *, available_at_ns: int | None = None) -> ContractArrays | None:
    from trading_research.research.experts.options.databento_decode import decoded_path, ensure_decoded

    ensure_decoded(root)
    if available_at_ns is None:
        available_at_ns = et_ns(day, 17, 0)
    defs = _last_definitions_asof(root, available_at_ns)
    if defs is None:
        return None
    live = defs["expiry_ns"] > available_at_ns
    if not np.any(live):
        return None
    osi = defs["raw_symbol"][live]
    return ContractArrays(
        strike_millis=defs["strike_millis"][live],
        right=defs["right"][live],
        expiry_ns=defs["expiry_ns"][live],
        osi=osi,
        osi_code=defs["instrument_id"][live],
        request_day=day.isoformat(),
        source_path=str(decoded_path(root, "definitions")),
        source_sha256=digest_path(decoded_path(root, "definitions")),
        root=root,
        available_at_ns=int(available_at_ns),
    )


def load_futures_oi_arrays(root: str, effective: date, *, extra_sessions: int = 0) -> OIArrays | None:
    from trading_research.research.experts.options.databento_decode import decoded_path, ensure_decoded, oi_table
    ...
    if table.num_rows == 0:
        return None
    ts = np.asarray(table.column("ts_event").to_numpy(), dtype=np.int64)
    mask = _et_day_mask(ts, effective)
    if not np.any(mask):
        return None
    ids = np.asarray(table.column("instrument_id").to_numpy(), dtype=np.int64)[mask]
    oi = np.asarray(table.column("open_interest").to_numpy(), dtype=np.int64)[mask]
    ts_k = ts[mask]
    order = np.lexsort((ts_k, ids))
    ids_s = ids[order]
    change = np.empty(ids_s.size, dtype=np.bool_)
    change[0] = True
    change[1:] = ids_s[1:] != ids_s[:-1]
    ends = np.append(np.flatnonzero(change)[1:], ids_s.size) - 1
    last = order[ends]
    defs = _last_definitions_asof(root, et_ns(effective, 17, 0))
    strike, right, expiry, osi = _align_definition_fields(ids[last], defs)
    available = assumed_oi_available_ns(effective, extra_sessions=0)
    delayed = assumed_oi_available_ns(effective, extra_sessions=1)
    path = decoded_path(root, "open_interest")
    return OIArrays(
        ...
        oi=oi[last],
        osi=osi,
        osi_code=ids[last].astype(np.int64),
        vendor_ts_ns=ts_k[last],
        effective_session=effective.isoformat(),
        published_at_ns=available,
        available_at_ns=available if extra_sessions == 0 else delayed,
        publication_policy="assumed_next_regular_session_12et",
...
def load_futures_quote_arrays(root: str, day: date) -> QuoteArrays | None:
    from trading_research.research.experts.options.databento_decode import decoded_path, ensure_decoded, quotes_table
    ...
    ids = np.asarray(table.column("instrument_id").to_numpy(), dtype=np.int64)[mask]
    t_ns = ts[mask]
    close = np.asarray(table.column("close").to_numpy(), dtype=np.float64)[mask]
    defs = _last_definitions_asof(root, et_ns(day, 17, 0))
    n = t_ns.size
    strike, right, expiry, osi = _align_definition_fields(ids, defs)
    cents = _cents_from_float(close)
    path = decoded_path(root, "quotes_ohlcv1m")
    return QuoteArrays(
        t_ns=t_ns,
        available_at_ns=t_ns + MINUTE_NS,
        strike_millis=strike,
        right=right,
        expiry_ns=expiry,
        bid_cents=cents,
        ask_cents=cents,
        bid_sz=np.ones(n, dtype=np.int64),
        ask_sz=np.ones(n, dtype=np.int64),
        osi=osi,
        osi_code=ids.astype(np.int64),
        source_row=np.arange(n, dtype=np.int64),
        source_path=str(path),
        source_sha256=digest_path(path),
        root=root,
...
def coverage_row(root: str, day: date) -> dict[str, Any]:
    ...
    if spec.databento_prefix:
        from trading_research.research.experts.options.databento_decode import ensure_decoded

        ensure_decoded(root)
        oi_arr = load_oi_arrays(root, day)
        quotes = load_quote_arrays(root, day)
        contracts = load_contract_arrays(root, day)
        has_oi = oi_arr is not None and int(oi_arr.oi.size) > 0
        has_q = quotes is not None and int(quotes.t_ns.size) > 0
        has_c = contracts is not None and int(contracts.osi.size) > 0
        full_n = 0 if contracts is None else int(contracts.osi.size)
        scoped_n = 0 if quotes is None else int(quotes.t_ns.size)
        if has_c and has_oi:
            disposition = "complete_observed_scope" if has_q else "definitions_and_oi_quotes_sparse"
            reason = "ok" if has_q else "ohlcv_1m_last_as_mid_sparse"
        elif has_c or has_oi:
            disposition = "partial"
            reason = "scoped_or_missing"
        else:
            disposition = "missing"
            reason = "no_decoded_rows_for_day"
        return {
            "root": root,
            "day": day.isoformat(),
            ...
            "reason": reason,
            "oi_path": None if oi_arr is None else oi_arr.source_path,
            "quote_path": None if quotes is None else quotes.source_path,
            "contracts_path": None if contracts is None else contracts.source_path,
            "quote_kind": "ohlcv_1m_last_as_mid",
        }
    if spec.underlying_kind == "cash_index" and not spec.native_intraday_spot:
...
def search_futures_option_inputs(root: str) -> dict[str, Any]:
    from trading_research.research.experts.options.databento_decode import (
        coverage_by_year,
        decode_log_path,
        decoder_identity,
        ensure_decoded,
    ...
    result = {
        ...
        "owned": inv["owned"],
        "quote_kind": "ohlcv_1m_last_as_mid",
        "decode_log": str(decode_log_path()),
    }
    if not ident["import_ok"]:
        result["disposition"] = "decoder_import_failed"
        result["failed_files"] = []
        result["n_failed"] = 0
        return result
    if decode_log_path().is_file():
        log = ensure_decoded(root)
        failed = [row for row in log.get("files") or [] if not row.get("ok")]
        result.update(
            {
                "disposition": "decoded",
                "outputs": log.get("outputs") or {},
                "failed_files": failed,
                "n_failed": len(failed),
                "coverage_by_year": coverage_by_year(root),
            }
        )
        return result
    result["disposition"] = "decoder_available"
    result["failed_files"] = []
    result["n_failed"] = 0
    return result
