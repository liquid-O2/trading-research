...
def test_s08_future_quotes_do_not_enter_earlier_snapshot():
    ...
    if early.osi.size and late.osi.size:
...
def test_s25_stale_spot_and_unknown_futures_definitions():
    ...
    nq = root_spec("NQ")
    assert nq.definition_unparsed is True
    assert nq.exercise == "unknown_unparsed_definition"
...
def test_s31_proxies_stay_labelled():
    ...
    assert search_futures_option_inputs("ES")["decoder"] == "unavailable_databento_python_package"
