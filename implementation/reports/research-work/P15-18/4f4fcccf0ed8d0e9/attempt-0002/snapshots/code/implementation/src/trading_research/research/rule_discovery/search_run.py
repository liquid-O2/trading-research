    ...
    *,
    ...
    meta = {
        ...
        "multiplicity": {
            ...
            "smallest_attainable_p": 1.0 / (BOOTSTRAP_DRAWS + 1),
            "declared_family_size": len(resolved),
            "attainable_p_floor": attainable_holm_floor(len(resolved)),
            "floor_limit": 0.025,
            "floor_admits_decision": floor_admits_decision(len(resolved)),
        },
...
def _outer_from_paired(
    ...
    *,
    ...
    for paired in per_fold:
        ...
        candidate_values.extend(float(p["candidate"]) for p in paired)
        baseline_values.extend(float(p["baseline"]) for p in paired)
        stress_diffs.extend(
    ...
    candidate_mean = float(np.mean(candidate_values)) if candidate_values else 0.0
    ...
    return {
        ...
        "baseline_resolved_opportunities": baseline_entries,
        "candidate_entries": entries,
        "baseline_entries": baseline_entries,
        "candidate_mean_net_points": candidate_mean,
        "baseline_mean_net_points": baseline_mean,
...
def trial_records(
    ...
    *,
    ...
    ``fold_metrics`` carries each fold's own outer-block evidence so a fold row
    does not repeat only the pooled decision metric."""
        ...
        for item in resolved:
            ...
            rows.append(
                refinement.make_trial_record(
                    ...
                    loss=None,
                    support={} if decided_row is None else support_block(decided_row["promotion"]),
                    test_metrics={} if decided_row is None else {
                        ...
                        "attainable_p_floor": decided_row["promotion"].get("attainable_p_floor"),
                        "family_size": decided_row["promotion"].get("family_size"),
                        "ci_low": decided_row["promotion"]["ci_low"],
                        "ci_high": decided_row["promotion"]["ci_high"],
                        "fold": (fold_metrics.get(item.candidate_id) or {}).get(year),
                    },
                    reason=reason,
...
def _multiplicity_block(decided: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """The Holm family of this decision stage and the floor it can attain.

    EVALUATION.md (amended 2026-09-17): the smallest attainable adjusted p-value
    is m/(B+1); a stage whose floor exceeds .025 cannot promote anything, so the
    floor is published beside the adjusted p-values instead of being implied.
    """
    family_size = len(decided)
    floor = attainable_holm_floor(family_size)
    return {
        ...
        "floor_limit": 0.025,
        "floor_admits_decision": floor_admits_decision(family_size),
        "holm_alpha": 0.05,
        "inconclusive_multiplicity": sorted(
            row["candidate_id"]
            for row in decided
...
def fold_metrics_from_series(
    item: search.ResolvedCandidate,
    series: "CandidateSeries | None",
    folds: Sequence[Mapping[str, Any]],
) -> dict[int, dict[str, Any]]:
    """That fold's own outer-block evidence, per outer fold.

    The promotion metric is pooled over the outer blocks by design (EVALUATION.md
    requires three supported blocks and a positive difference in 60% of them, over
    one paired series), so a per-fold ledger row would otherwise repeat the pooled
    numbers. These are the fold's own numbers, written beside the pooled ones.
    """
    out: dict[int, dict[str, Any]] = {}
    for fold in folds:
        year = int(fold["test_year"] if "test_year" in fold else fold["outer_fold"])
        paired = series.select(fold["test"]) if series is not None else []
        if not paired:
            out[year] = {
                "mean_diff": None,
                "eligible_test_days": 0,
                "candidate_entries": 0,
                "baseline_entries": 0,
                "resolved_opportunities": 0,
            }
            continue
        out[year] = {
            "mean_diff": float(np.mean([p["diff"] for p in paired])),
            "eligible_test_days": len(paired),
            "candidate_entries": int(sum(p["candidate_fills"] for p in paired)),
            "baseline_entries": int(sum(p["baseline_fills"] for p in paired)),
            "resolved_opportunities": int(sum(p["candidate_fills"] for p in paired)),
        }
    return out


def support_block(promotion: Mapping[str, Any]) -> dict[str, Any]:
    """The support evidence a ledger row must carry to be auditable: the counts on
    both sides of the pair, the gate outcome for each side, and the half/nominal/
    twice sensitivity (EVALUATION.md)."""
    return {
        "resolved_opportunities": promotion.get("resolved_opportunities"),
        "baseline_resolved_opportunities": promotion.get("baseline_resolved_opportunities"),
        "eligible_test_days": promotion.get("eligible_test_days"),
        "supported_outer_blocks": promotion.get("supported_outer_blocks"),
        "entry_ratio_to_baseline": promotion.get("entry_ratio_to_baseline"),
        "pass": promotion.get("support_pass"),
        "pass_candidate": promotion.get("support_pass_candidate"),
        "pass_baseline": promotion.get("support_pass_baseline"),
        "candidate_mean_net_points": promotion.get("candidate_mean_net_points"),
        "baseline_mean_net_points": promotion.get("baseline_mean_net_points"),
        "sensitivity": promotion.get("support_sensitivity") or {},
        "baseline_sensitivity": promotion.get("baseline_support_sensitivity") or {},
    }
...
def family_report(
    ...
    *,
    ...
    lines = [
        ...
        "## Multiplicity",
        "",
        f"Holm family of this decision stage: {len(decided)} candidates, {BOOTSTRAP_DRAWS} bootstrap "
        f"draws, smallest attainable p {1.0 / (BOOTSTRAP_DRAWS + 1):.6f}, attainable Holm floor "
        f"m/(B+1) = {attainable_holm_floor(len(decided)):.6f} "
        f"({'admits a decision at .025' if floor_admits_decision(len(decided)) else 'ABOVE .025: no candidate could pass whatever the data say'}).",
        "",
        _table(
            ...
            [
                [
                    row["candidate_id"],
                    f"{row['promotion']['p_raw']:.6f}" if row["promotion"].get("p_raw") is not None else "",
                    f"{row['promotion']['p_holm']:.6f}",
                    f"{row['promotion'].get('attainable_p_floor', 0.0):.6f}",
                    row["promotion"]["disposition"],
                ]
                for row in sorted(rows, key=lambda row: row["candidate_id"])
            ],
        ...
        _table(
            ...
            [
                [
                    row["candidate_id"],
                    row["promotion"].get("resolved_opportunities"),
                    row["promotion"].get("baseline_resolved_opportunities"),
                    row["promotion"].get("eligible_test_days"),
                    row["promotion"].get("supported_outer_blocks"),
                    "pass" if row["promotion"].get("support_pass_candidate") else "fail",
                    "pass" if row["promotion"].get("support_pass_baseline") else "fail",
                    ""
                    if row["promotion"].get("entry_ratio_to_baseline") is None
                    else f"{row['promotion']['entry_ratio_to_baseline']:.2f}",
                ]
                for row in sorted(rows, key=lambda row: row["candidate_id"])
            ],
        ),
...
def evaluate_run(
    *,
    ...
    fold_metrics = {
        item.candidate_id: fold_metrics_from_series(item, series.get(item.candidate_id), folds)
        for item in resolved
        if item.supported
    }
    ...
    results = {
        ...
        "uncertainty": freeze["uncertainty"],
        "multiplicity": _multiplicity_block(decided),
        "promotion_gates": freeze["promotion_gates"],
        "folds": [
