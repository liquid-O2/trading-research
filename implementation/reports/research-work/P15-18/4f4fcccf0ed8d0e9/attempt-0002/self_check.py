"""P15-18 self-check (attempt-0002: the amended evaluation).
...
# attempt-0002 is an evaluation-only attempt: the run shards it reads are the bound
# run root named on the command line, never a directory this attempt did not declare.
RUN = Path(sys.argv[4]) if len(sys.argv) > 4 else R
