...
| JJ-M-contacts1 ({"jumbo.MAX_CONTACTS_PER_LEVEL": 1}) | memory | 0.0 | none; identical to B0.3 on every session | the override changes no executed trade: the first contact is already the one the executed list takes, so the constant is inert on this pool |
