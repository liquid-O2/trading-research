...
Rescan candidates judged so far (`phase15/rescan-v1/RESULTS.md`): the Judas window ending 10:00 or 10:30 instead of 10:15 is indistinguishable from B0.3 (-0.6 and +0.1 points a session, no fold significant); the sourced major-level objective rule scores -4.8 a session, worse on every fold, because on the whole pool the farther targets are reached less often (win rate 0.259 to 0.250). His 200-point rewards therefore come with his choice of the level, not from aiming farther on every admitted opportunity; the grading layer, not the objective rule, is where that lives. With the five new labelled sessions (29 positives) the grading fit gives Jumbo 0.21 at two a day, the ticket in the top five on 7 of 14 days; Green Bird 0.00, median rank 18 of 38.

## 13. Generated structure for the discretionary families (2026-09-17 late night)

The user's rule: every family must be source-accurate as a baseline before it is improved, and nothing in a family's sources may be missed. Phase 1 recreated the tickets on the levels the authors drew; this section replays them on structure the framework generates itself, because the population scan (Phase 1.5) and the grading rows (Phase 2) can only use generated structure. Evidence: `phase15/generated-structure/`; the fidelity guard after these changes: 71 tickets, none lost, none gained (`scratchpad/guard-cycle`).

### 13.1 The shared profile structure (`method_pack/profile_nodes.py`)

Nodes by topographic prominence (an HVN's height above the higher of the troughs on each side, however far; 20% of the profile's peak), ledges at half a shelf's peak next to an LVN, the P-zone keep rule (FIND p.8), Saint's five shapes (RTVP pp.7-11) and the composite with its value area (VP2 p.6). One rule for the feature layer and for every scanner's candidate. Unit tests on constructed profiles (`test_profile_nodes.py`, 9). The fixed eight-bin prominence window of the first object layer missed a trough wider than two points; the LVN between two overnight distributions can be tens of points wide (MAMT p.16).

### 13.2 Member (K10) on the ES tape, through the adapter's own generation

The adapter's reasons are look-left reactions (five-minute pivots with a four-tick reaction) paired within two ticks with a minor HVN of the higher-timeframe profile, or a KG1 level. With the HTF profile read as the prior single day, the ES tape of 2026-08-03 has no node at either of his pairs: the prior day (07-31) traded 7,427-7,541, below both. The composite of the prior ten sessions (07-20 to 07-31; 07-22 last traded 7,563.5) carries minor nodes at 7,558-7,561 and 7,543.25-7,547, and through the adapter's own path (ten prior ES sessions supplied as `b02_prior_sessions`) the generated pairs land on both drawn pairs before he traded them: shorts 7,559.75-7,560.5 on the 7,560 node, known 22:40-05:40; longs 7,543.5-7,547.5 on the 7,543.25-7,547 nodes, known 18:30-04:30. `member.HTF_SESSIONS = 10`; the current overnight profile alone also carries nodes at both pairs (`MEMBER_HTF_PROFILE_VARIANTS.log`).

### 13.3 Saint on a generated balance and generated levels

`saint.composite_balance`: the 70% value area of the composite of the prior five full sessions, each edge snapped to the nearest composite ledge within 40 points, gives 29,576.5-29,950 for the 2026-08-11 session against his drawn 29,600-29,960 (his WIC weekly balance 29,560-29,960). The levels inside it are the composite's nodes and ledges plus the last session's 68% VAH/POC/VAL of the RTH and of the whole session, merged within two points: five of his six drawn intraday levels sit within three points of one of them, his 29,740 is the prior RTH VAL 29,738.5, his 29,840 the prior session VAH 29,840.5 (`SAINT_COMPOSITE_FIT.log`). The balance's own profile (the composite's volume inside it) is what the F04 shape read classifies: the whole composite reads as trending by construction, since the balance is its value area, and blocked every episode at the context stage.

The scanner walked one break per level (the first close through) and its first touch; on 2026-08-10 that was the failed 18:21 break of 29,738.5, so his 18:31 break and 19:47 retest were never examined. It now walks every break-and-retest cycle of every level with the rule the ticket replay was accepted on, extracted into `source_adapters/break_retest.py` and shared with `replay_sires.py` (still 20 of 20 strict): a break after a close inside, a 24-point departure, the retest within 14 points from the broken side, a close back through by more than a point ends the cycle, three retests at most, one episode per level and retest bar. The scanner takes one retest per cycle (TRAP: "one retest"; RR-22); the fill is the retest's extreme and the stop a tick through the broken level (his stop box 29,729-29,736.75 under the 29,740 line; the balance-edge stop was 221 points). On his drawn 29,740 the 18:30 break has exactly one retest, 19:47 at 29,726.75: his ticket. On the fixture-free scan the generated line is 29,738.5 (the prior RTH VAL), 1.5 points under his, and the same cycle counts the 19:20 pullback high 29,725.0 as its retest (14 points from the generated line, 15 from his), so the generated-structure entry is 19:21 short 29,725.0: 4.25 points and 30 minutes from his fill, the same cycle and the earlier pullback. That is recorded as it is; the retest band against a small line offset is a population fit question, not a constant to move for one ticket. The session holds about thirty all-stage passes across both sides of the continuation branch before the one-retest rule: the generated level set is the candidate list, his one trade among them is the grading layer's, as for Jumbo and Green Bird.

### 13.4 Jumbo's P-zone node snap, measured

`JJ-pzone-node-snap` (v3 queue) keeps a generated P-zone only where an edge sits on a node or ledge of the profile built from 18:00 to the zone's anchor. His two printed zones (2026-01-02, 01-09) survive it; the generated zone of 2026-05-20 (his 09:46 long at 29,170) is dropped at tolerances 5, 10 and 15 points. Recall 44 of 45; the fold result decides whether one dated ticket is the price of the rule.

### 13.5 Two lessons for the machinery

The seven files digested into the window-cache identity (`event_cache.transform_identity`) must never be edited for a convenience: a value-fraction keyword added to `event_time.py` re-keyed every cached window, a scan began rebuilding them (six stray entries of about 6 MB each under the wrong key before the edit was reverted), and the guarded tests could no longer read the cache. The guard now prints a failed replay's exit code and error output; a replay failure under a load of 18 on 17.85 cores was transient and the manual re-run matched the pin.

Addendum to 13.5 (21:50): the cost was larger than six cache entries. `transform_identity` digests the files on every cache lookup, so the two populations running on the phase15 tree at that minute (the executed-list export `pop-b03-exec`, 996 of 1,742 sessions; the `JJ-PM-windows` rescan, 619 sessions) began missing the cache at once, and since the population runner installs the write guard every remaining session failed with `CacheWriteBlocked` within a minute and both runs reported complete. The partial PM-windows fold read (1,123 sessions) is discarded; both runs are being completed on exactly the failed dates (`--dates`) and their session lines merged with the untouched ones. The queue on the fidelity-jj-gb tree was not affected (JJ-M-contacts1 finished 1,742 of 1,742).

Second addendum to 13.5 (22:05): the v2 queue ran through three candidates inside those minutes, `JJ-projection-lines`, `JJ-big-print` and `JJ-band-limit` (1,741, 1,739 and 1,719 failures), each in about a minute; their outputs are set aside as `ruined-*` and the three are re-run in full by the queue once the two `--dates` completions finish. Every population that ran on the phase15 tree between 21:31 and 21:36 is checked for `failures` in its POPULATION.json before its rows are read.

Third addendum to 13.5 (22:35), a defect in the override plumbing: `JJ-M-contacts1` and `JJ-M-contacts2` returned exactly the baseline on every one of 1,742 sessions. The cause is not the market: `jumbo.MAX_CONTACTS_PER_LEVEL` was bound as a default argument (`max_contacts: int = MAX_CONTACTS_PER_LEVEL`), evaluated once when the module loads, so `--scanner-overrides` changed the module attribute and never the value the scanner used. Nine such bindings existed across the adapters (contacts and cycles per level, the ladder spacing, the selection cap, the lookback count, the level merge, three replay tolerances); every one now reads its constant at call time, `tests/rule_discovery/test_overrides_reach_scanners.py` rejects any parameter default bound to a module constant, and the two contacts candidates and the two Green Bird cycle candidates (not yet run) are measured on the corrected scanners. Every earlier v1/v2 result moved the executed list (non-zero differences), so none of them was inert. A candidate that returns exactly zero on every session is now treated as a plumbing failure until proven otherwise.

### 13.6 Sires grading on generated boxes: the first dataset is not a candidate list

`build_grading_dataset_sires.py` (written before the Sires population runner) enumerates every fill mode of every generated box on the nine dated sessions: 248,018 rows, 27,558 candidates a day, 1,358 rows labelled positive by proximity to his 19 tickets. A logistic grading fit on the object-layer features (leave-one-day-out, threshold at two candidates a day) ranks his tickets at a median 8,265th and recalls none (`scratchpad/grading-sires-fit`). That is a construction result, not a grading one: the population runner's candidate list (one admitted fill per box contact, `run_sires_population.py`, section 12.3) is the list his choice is graded against, as for Jumbo and Green Bird. The Sires grading rows are rebuilt from that list with the object-layer features before any fit is read.

## 14. Exits on the rebuilt entries (P15-19 on the B0.3 executed list, 2026-09-17 night)

Every session's B0.3 executed trades (`run_jj_gb_population.py --export-executed`, 1,742 sessions completed on their failed dates after the cache incident) were frozen as entries and managed by the exits module under its five policies (`run_exits_population.py`, 1,721 sessions; 2020-06-30 fails on a NaN quote). E0 holds the entry's own stop and objective with a 60-minute expiry, E1 a 30-minute expiry, E2 a 120-minute expiry, E3 and E4 move the stop on R (undefined R counts as open). Judged like the selection candidates (`evaluate_variants.py --baseline E0`; `phase15/exits-b03/`):

| family | E0 (points a session) | E1 | E2 | E3 | E4 |
| --- | ---: | ---: | ---: | ---: | ---: |
| JJ-TBR (1,719 sessions) | 50.6 | -6.4, worse on every fold | +5.4, positive on every fold, Holm-promoted on 2025 (p 0.009) and 2026 (p 0.048) | +0.8, mixed | -6.6, worse on every fold |
| GB-FAIL (1,721) | 44.7 | 0.0, mixed | +0.7, mixed | -6.0, worse | -17.6, worse on every fold |

Reading. Jumbo's fades need more than an hour to reach their objective: the 30-minute expiry loses on every fold, the 120-minute expiry gains on every fold and is promoted on two. The R-managed policies (E3, E4) lose for both families, Green Bird's most of all: moving the stop to breakeven or trailing it takes his trades out before his rewards. The expiries are study conventions, not the authors' rules (Jumbo holds to the objective or the session; Green Bird targets the opposing liquidity), so E0 is not the source baseline: the selection study's executed list, resolved at stop or objective without expiry, is (section 12.1, 67.7 a session for Jumbo on its own accounting). E2 is the nearest tested policy to that rule, and its promotion is read as "do not cut his trades at an hour", not as a management upgrade. The source-faithful policy was then added as E5 (the entry's stop and objective held to the source deadline or the account day's close, no expiry, no stop movement; `exits.POLICIES`, exits tests 31 passed) and the study re-run (`VARIANTS_with_E5.md`, six policies in the Holm family):

| family | E0 | E5 (hold to the objective) |
| --- | ---: | ---: |
| JJ-TBR | 50.6 | 60.3, +9.7 a session, positive on every fold, Holm-promoted on 2025 (p 0.0075; 0.15 to 0.61 on the others) |
| GB-FAIL | 44.7 | 44.6, -0.1, mixed |

For Jumbo the author's own management is the best of the six policies and every expiry costs him; the 120-minute E2 was a step toward it. For Green Bird holding to the objective is neutral against the hour: his fades resolve inside it or not at all on the drawn targets, which is the section 12.2 finding again (the far rewards come with his level choice). The R-based partials of DELTA pp.4-5 (Sires) belong to the Sires family's own study, not to these two.

Fourth addendum to 13.5 (23:35): the contacts candidates were inert for a second reason. After the default-argument fix, `JJ-M-contacts1` re-ran to exactly the baseline on every session again; both call sites of `level_contacts` passed a literal `max_contacts=2`, so `MAX_CONTACTS_PER_LEVEL` (3) was dead and the scan's cap was two tests of a line. The call sites now read the constant and the constant is set to the two they carried, which leaves the baseline unchanged by construction; a one-session check (2026-01-09) confirms the override now reaches the scan: 27 Jumbo candidates at one contact, 40 at three (judas_reversal episodes 71 against 95), the executed list unchanged on that day; contacts1 with a new contacts3 run on the corrected scanner (v1c). The AST guard cannot see a call-site literal; the operating rule stands: a candidate that returns exactly the baseline on every session is a plumbing failure until a one-session check shows the override changing the scan.

### 12.4 addendum (2026-09-18 00:30): the grading refit on the current object layer

Dataset v6 (`build_grading_dataset_v2.py` on the merged object layer: topographic-prominence nodes and ledges, the shape reads, the composite and naked POCs, the weekly delta-print band; 1,069 rows, 30 positives, 43 dated days including the seven added on 2026-09-17), logistic fit leave-one-day-out at two candidates a day (`grading/GRADING_FIT_v6_per_day_2.json`): Jumbo recall 0.286 against chance 0.113 (ticket in the top five on 9 of 14 days, median rank 4.5 of 17.7 candidates); Green Bird 0.0 against chance 0.054 (median rank 16 of 37). The Jumbo number sits inside the 0.15-0.31 band of the earlier versions and is not a change with 14 positives; Green Bird's choice is still not carried by any of the object features at this density. The lever remains labelled data (the chart posts without images) and, for Green Bird, the transcription of his own profile set from the chart pages.

### 13.7 Green Bird's charts carry no profile (2026-09-18 00:40)

The audit's note that a profile and delta bands sit beside his boxes on every chart was checked against the chart images themselves (rendered from GB pp.44-60 with PyMuPDF, read at native resolution: 2026-04-28, 2026-05, 2026-07-29 overnight, the daily Fibonacci chart, 2026-08-27). None carries a volume profile or a delta band. His chart set is the shaded session boxes, the drawn extremes and lines (session highs and lows, True Day Open, PDH, the London low), the daily Fibonacci bands of the April-July swing with the Golden Pocket at 0.5-0.618, buy-limit ladders below the entry and the bracket. The note is withdrawn in the ledger; the profile features stay cross-strategy candidates for his family (section 12.4: they do not carry his choice), and the one open check is whether the adapter's golden pocket is anchored on the same daily swing as his chart.

### 13.6 addendum (2026-09-18 00:55): the Sires fit on the population's candidate list

The dated sessions were re-scanned with the population runner's own admission (`run_sires_population.py --export-candidates`, after porting the stop rule for stop-less fills that the filed population had carried uncommitted): 3,400 admitted fills a session on about 170 alive boxes. The grading rows (`build_grading_dataset_sires_v2.py`) keep every fill within ten points of a ticket on its bars (271 rows across 19 tickets) and a seeded ten-percent sample of the rest (3,021 rows, nine sessions), augmented with the object layer; the sampling weight and the session's candidate count are bookkeeping columns the fit now excludes (the first fit read the weight as a feature and leaked the label). Logistic fit, leave-one-day-out, at two candidates a day of the sampled list (about twenty of the real one) and at 0.2 (about two): recall 0.0 at both densities against chance 0.006 and 0.001, the ticket in the top five on one day of nine, median rank 55 of 336 sampled (about 550 of 3,400 admitted). The weights that carry most (`grading/GRADING_FIT_sires_v2_*.json`): minutes from the open, the developing value low toward the trade, a trending prior-day shape, range position, the developing POC distance, the 30-minute delta, the prior day's value, a naked POC ahead. Reading: on this candidate list the object features do not carry his choice; the list itself is the problem, as section 12.3 already said of the boxes (13-34 generated a session against his 2-6). The first grading cut for Sires is which boxes, not which fills at a box: the box-level features (construction, absorption, memory, the delta print at the box) fitted on the drawn-box labels are the next fit, and the fill-level list is graded only within the boxes that survive it.

### 13.6 second addendum (2026-09-18 01:35): the box-level fit

`build_grading_dataset_sires_boxes.py`: one row per alive generated box on each of the nine dated sessions (1,768 boxes, about 196 a session), labelled by overlap with a drawn band of box scale (fifteen points or less: his aggression, refill, failure, squeeze and PM boxes; the wide balance and prior-day bands are context and are not labels), with the box's construction, memory and location and the object layer at its midpoint when it can first be traded. A first fit had read the label's own name as a feature and was discarded. The corrected logistic fit, leave-one-day-out at four boxes a day (`grading/GRADING_FIT_sires_boxes_per_day_4.json`): recall 0.053 against chance 0.020, his box in the top five on one session of nine, median rank 70 of 196; on 2026-07-08 and 2026-08-04 his box ranks third and sixth. The features that carry most are the object layer's overnight objects: the distance to the overnight delta print (nearer, more likely his box), to the overnight LVN, ledge and HVN, whether the RTH open sits inside the ETH value, and the box's width and carried-in flag. That is DELTA pp.7-9 read back from the data (the delta print at a low-volume node of the dealing range's own profile), at two and a half times chance and still far from his two to six boxes a session. The next lever is the label: his drawn boxes on more sessions (the ten records hold 19 tickets) and the delta print as the box's own attribute rather than a distance.

## 15. A candidate with a signal on every fold, and what it actually changed (JJ-R-coincide8, 2026-09-18)

Of the seventeen rescan and selection candidates read on the year folds, one carries a signal on every fold and keeps every dated ticket: `JJ-R-coincide8`, `jumbo.LEVEL_COINCIDENCE` raised from five to eight points, gains 7.0 points a session over B0.3 (67.7 to 74.6), is positive on all five test years (+0.9 to +10.4), Holm-promoted on 2024 (p 0.010) and at 0.29-0.50 on the other fit-tested years (`phase15/rescan-v1/VARIANTS_JJ_after_R-coincide8.md`); the full ticket replay with the override reproduces exactly the pinned set (45 of 49 inside-tape entries, none lost, none gained; `scratchpad/replay-coincide8`). The three-point version lost 5.8.

Before the reading was written down the constant was checked in the scanner, and it is not one rule. `LEVEL_COINCIDENCE` is the tolerance of the coincident-level read and of the SessionStat edge hit, and it was also the resting stop distance beyond the swept extreme for the extension band's limit fills and beyond the band for the give-back fill. The candidate therefore widened stops from five to eight points at the same time as it widened the coincidence band, and coincide3 tightened both. A wider stop that loses fewer round trips is the simplest account of +7.0, and the source question it raises is his stop rule, not his confluence rule. The stop distance is now its own constant (`STOP_BEYOND_EXTREME`, five points by default, so the baseline is unchanged), and the two effects run separately (v1e: the coincidence tolerance alone at eight; the stop distance alone at three, eight and twelve). The candidate stays recorded as "signal, attribution pending"; nothing is promoted until the split reads land and the stop distances are checked against his printed stops in the record.
