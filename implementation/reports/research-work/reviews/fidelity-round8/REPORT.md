...
## 15. The first faithful upgrade: a coincident level within eight points (JJ-R-coincide8, 2026-09-18 01:50)

Of the seventeen rescan and selection candidates read on the year folds, one carries a signal on every fold and keeps every dated ticket. `JJ-R-coincide8` admits a Jumbo entry only where a second drawn level sits within eight points of the line (the confluence his sources describe and the S1 ledger lists). On the executed list it gains 7.0 points a session over B0.3 (67.7 to 74.6), is positive on all five test years (+0.9 to +10.4), Holm-promoted on 2024 (p 0.010) and at 0.29-0.50 on 2022, 2023 and 2025 (`phase15/rescan-v1/VARIANTS_JJ_after_R-coincide8.md`). The full ticket replay with the override reproduces exactly the pinned set (45 of 49 inside-tape entries, none lost, none gained; `scratchpad/replay-coincide8`): every dated Jumbo entry already sits at a confluence, so the rule removes lone-line fades he did not take and nothing he did. The three-point version lost 5.8 a session, so the band is the rule's substance: the grid 5, 12 and 16 points runs next (v1d), then the combination with the hold-to-objective management of section 14, whose gain is on the same trades. Until the grid and the combination are read, coincide8 is recorded as promotable, not promoted.
