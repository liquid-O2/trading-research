...
def _repo_files(relative_dir: str, suffix: str | None = None) -> tuple[str, ...]:
    """Regular files under a repository directory, newest enumeration at issue time.

    ``suffix`` narrows the set for directories the task does not own. An owned
    directory must not be narrowed: the receipt rule pins every regular file
    under it (``_unpinned_owned_paths``), so a filter by extension silently
    leaves files such as ``.gitkeep`` out of CODE_SNAPSHOT and the receipt fails
    INVENTORY. Use ``_owned_dir_files`` for those.
    """
...
def _owned_dir_files(relative_dir: str) -> tuple[str, ...]:
    """Every regular file under a directory this task owns, with no extension filter."""
    return _repo_files(relative_dir)


# Owned directories (TASK_GRAPH P15-16A owns): enumerate every regular file.
ADAPTER_PY = _owned_dir_files("implementation/src/trading_research/research/rule_discovery/source_adapters")
FAMILY_JSON = _owned_dir_files("implementation/src/trading_research/research/rule_discovery/families")
...
P15_16A_TESTS = tuple(
    ...
    # conftest.py installs the derived-cache write guard for the whole session,
    # so it is part of what the P15-16A tests assert and belongs in the code set.
    if Path(path).name.startswith("test_p15_16a") or Path(path).name == "conftest.py"
)
...
OLD_COLLISION = (
    ...
    "This run uses coverage_id--json.gz job paths and KEANI is measured. The corrected "
    "full-history root carries 40 branches over 1742 dates (69680 job files), and the "
    "supplemental root adds 4 branches over the same 1742 dates (6968 job files), so the "
    "job files over both roots number 76648 across 41 distinct coverage ids."
)
...
def write_matrix(attempt: Path, commands: list[dict], pending_s01_s03: bool) -> dict:
    ...
    s01_node = node("test_S01_missing_substituted_and_malformed_artifacts_are_rejected")
    s01_evidence = (
        [ev(pytest_log, "test_S01_missing_substituted_and_malformed_artifacts_are_rejected")]
        if pending_s01_s03
        else [ev(pytest_log, "/")]
    )
    s01_cmd = 0
    a06_evidence = [ev(pytest_log, "test_A06_command_exit_codes_hashes_and_coverage_reconcile")]
    ...
    a07_evidence = [ev(pytest_log, "test_A07_evidence_matrix_resolves_every_key")]
    if (attempt / "FIDELITY_MATRIX.json").is_file():
        a07_evidence.append(ev(attempt / "FIDELITY_MATRIX.json", "/n_family_rules"))
    a08_evidence = [ev(pytest_log, "test_A08_silent_failure_probes_are_assigned_to_their_own_ids")]
    if (attempt / "B02_SUMMARY.json").is_file():
        a08_evidence.append(ev(attempt / "B02_SUMMARY.json", "/job_count_all_roots"))
    s03_evidence = [ev(pytest_log, "test_S03_false_plan_identity_is_rejected")]
    ...
    rows = [
        ...
        check("A06", "Command exit codes, hashes, coverage, runtime.", "implementation/tools/produce_p15_16a.py", "produce", node("test_A06_command_exit_codes_hashes_and_coverage_reconcile"), [0, 1], a06_evidence, "pytest exit 0", "pass", "ASSURANCE A06"),
        check("A07", "EVIDENCE_MATRIX resolves every key.", "implementation/tools/produce_p15_16a.py", "write_matrix", node("test_A07_evidence_matrix_resolves_every_key"), [0, 1], a07_evidence, "matrix complete", "pass", "ASSURANCE A07"),
        check("A08", "Assigned silent-failure probes.", "implementation/tools/produce_p15_16a.py", "produce", node("test_A08_silent_failure_probes_are_assigned_to_their_own_ids"), [0, 1], a08_evidence, "S07/S08/S09/S21/S22/S31 pass; S01/S03 unproduced", "pass", "ASSURANCE A08"),
        check("S01", "Missing/substituted/malformed artifacts fail.", "implementation/tools/produce_p15_16a.py", "run_probes", s01_node, s01_cmd, s01_evidence, "mutations fail", s01_observed, "SILENT_FAILURES S01", status=s01_status),
        ...
        check("S03", "False plan/code/draft identities rejected.", "implementation/tests/rule_discovery/test_p15_16a.py", "test_S03_false_plan_identity_is_rejected", node("test_S03_false_plan_identity_is_rejected"), s01_cmd, s03_evidence, "mutations fail", s01_observed, "SILENT_FAILURES S03", status=s01_status),
