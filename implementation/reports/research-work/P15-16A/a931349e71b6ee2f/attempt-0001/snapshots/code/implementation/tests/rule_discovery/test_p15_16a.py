...
def test_S01_missing_substituted_and_malformed_artifacts_are_rejected(tmp_path):
    """A receipt whose artifacts are removed, substituted or made unparseable is
    rejected, each mutation with its own failure code.

    This is the S01 rule the probe harness applies to an issued attempt. The
    fixture receipt is bound the same way (manifest entries pin bytes), so the
    mutation classes are exercised without re-issuing an attempt.
    """
    from trading_research.research.contracts.identity import ASSURANCE_VERSION, artifact_entry
    ...
    def bound(name: str):
        root = tmp_path / name
        receipt = write_bound_task(root, "P15-00")
        return root, receipt

    root, receipt = bound("control")
    control = invoke("task", "--receipt", str(receipt), "--graph", str(graph), "--receipts-root", str(root))
    assert control.returncode == 0, control.stdout

    # missing: a pinned artifact is deleted.
    root, receipt = bound("missing")
    (receipt.parent / "WORK_LOG.md").unlink()
    proc = invoke("task", "--receipt", str(receipt), "--graph", str(graph), "--receipts-root", str(root))
    assert proc.returncode == 2, proc.stdout
    assert FailureCode.ARTIFACT_MISSING in failure_codes(proc)

    # substituted: another file's bytes stand in for a pinned artifact.
    root, receipt = bound("substituted")
    target = receipt.parent / "REPORT.md"
    target.write_bytes((receipt.parent / "WORK_LOG.md").read_bytes())
    proc = invoke("task", "--receipt", str(receipt), "--graph", str(graph), "--receipts-root", str(root))
    assert proc.returncode == 2, proc.stdout
    assert FailureCode.ARTIFACT_HASH in failure_codes(proc)

    # malformed: a pinned JSON artifact stops parsing and its entry is re-pinned,
    # so only the content is wrong.
    root, receipt = bound("malformed")
    broken = receipt.parent / "EVIDENCE_MATRIX.json"
    broken.write_text("not-json\n")
    payload = json.loads(receipt.read_text())
    payload["artifact_manifest"] = [
        artifact_entry(broken, schema="research-evidence-matrix-v2")
        if Path(entry["path"]).name == broken.name
        else entry
    ...
    proc = invoke("task", "--receipt", str(receipt), "--graph", str(graph), "--receipts-root", str(root))
    assert proc.returncode == 2, proc.stdout
    assert FailureCode.JSON_PARSE in failure_codes(proc)
...
# --- A06, A07, A08 and S01: the receipt's own evidence invariants ------------
#
# The producer runs this suite before it writes the attempt it is issuing, so
# none of these can read that attempt. They exercise the producing code itself:
# the command record ``run_cmd`` writes, the matrix ``write_matrix`` writes
# judged by the receipt contract's own evidence-matrix check, and the B0.2
# coverage the receipt declares recomputed from the job files on disk. Each one
# carries the failure it has to catch.

NODE_FILE = Path("/workspace/implementation/tests/rule_discovery/test_p15_16a.py")
_PRODUCE_MODULE = None


def _produce_module():
    """The receipt producer, loaded by path (``tools`` is not an import package)."""
    global _PRODUCE_MODULE
    if _PRODUCE_MODULE is None:
        import importlib.util

        spec = importlib.util.spec_from_file_location(
            "produce_p15_16a", "/workspace/implementation/tools/produce_p15_16a.py"
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        _PRODUCE_MODULE = module
    return _PRODUCE_MODULE


def _stand_in_attempt(tmp_path: Path, *, pending_s01_s03: bool) -> tuple[Path, list[dict], dict]:
    """Run the producer's ``write_matrix`` over a stand-in attempt directory.

    The stand-in holds only the files the matrix digests. Its ``pytest.log``
    lists every test node this module defines, so an evidence selector that
    names a test which does not exist fails to resolve here exactly as it would
    on a real attempt.
    """
    from trading_research.research.contracts.receipts import _test_node_names

    module = _produce_module()
    attempt = tmp_path / "attempt-0001"
    attempt.mkdir(parents=True, exist_ok=True)
    names = sorted(_test_node_names(NODE_FILE) or ())
    assert names
    (attempt / "pytest.log").write_text(
        "".join(f"tests/rule_discovery/test_p15_16a.py::{name} PASSED\n" for name in names)
    )
    (attempt / "FIDELITY_MATRIX.json").write_text(
        json.dumps({"n_family_rules": 1, "n_findings_implemented": 1}, indent=2) + "\n"
    )
    (attempt / "LEDGER_CORRECTIONS.json").write_text(
        json.dumps({"corrections": [{"id": "L018"}]}, indent=2) + "\n"
    )
    commands = [
        {"argv": [".venv/bin/python", "-m", "pytest", "tests/rule_discovery", "tests/contracts"]},
        {"argv": [".venv/bin/python", "run_adapter_populations.py", "--baseline", "B0.2"]},
        {"argv": [".venv/bin/python", "tools/replay_author_examples.py"]},
    ]
    document = module.write_matrix(attempt, commands, pending_s01_s03=pending_s01_s03)
    return attempt, commands, document
...
def _p15_16a_spec():
    ...
    assert graph is not None and not failures, failures
    spec = graph.require("P15-16A")
    assert spec is not None
    return spec


def test_code_snapshot_pins_every_file_under_an_owned_directory():
    """The code set the producer pins covers every regular file the task owns.

    ``_unpinned_owned_paths`` walks an owned directory and reports any regular
    file the receipt does not pin, so enumerating owned directories by
    extension leaves files such as ``families/.gitkeep`` unpinned and the
    receipt fails INVENTORY.
    """
    from trading_research.research.contracts.receipts import DEFAULT_ROOT, _unpinned_owned_paths

    module = _produce_module()
    spec = _p15_16a_spec()
    pinned = {path: "" for path in module.CODE_PATHS}
    assert _unpinned_owned_paths(spec.owns, pinned, DEFAULT_ROOT) == []
    owned_dirs = [item for item in spec.owns if str(item).endswith("/")]
    assert owned_dirs
    # negative control: the extension filter that produced the rejection.
    by_extension = {path: "" for path in pinned if Path(path).suffix in {".py", ".json", ".md", ".tsv"}}
    missing = _unpinned_owned_paths(spec.owns, by_extension, DEFAULT_ROOT)
    assert missing == ["implementation/src/trading_research/research/rule_discovery/families/.gitkeep"], missing


def test_A06_command_exit_codes_hashes_and_coverage_reconcile():
    """A command record carries the real exit code and a log digest that matches
    the bytes on disk, and the declared B0.2 coverage reconciles with the jobs."""
    import hashlib
    import tempfile
    ...
    module = _produce_module()
    with tempfile.TemporaryDirectory(prefix="p15-16a-cmd-") as raw:
        scratch = Path(raw)
        log = scratch / "ok.log"
        record = module.run_cmd(
            [sys.executable, "-c", "print('alpha')"], scratch, log
        )
        assert record["exit_code"] == 0
        assert log.read_text() == "alpha\n"
        assert record["log_sha256"] == hashlib.sha256(log.read_bytes()).hexdigest()
        assert record["seconds"] > 0
        # negative control: a failing command is recorded as failing, and the
        # stderr that explains it reaches the log the receipt hashes.
        bad = scratch / "bad.log"
        record = module.run_cmd(
        ...
        assert "boom" in bad.read_text()
        assert record["log_sha256"] == hashlib.sha256(bad.read_bytes()).hexdigest()

    main = _complete_b02_run()
    if main is None:
        pytest.skip("no complete full-history B0.2 run root")
    _rows, supplemental = _merged_b02_branch_rows()
    total = 0
    for root in [main, *supplemental]:
        complete = json.loads((root / "RUN_COMPLETE.json").read_text())
        manifest = json.loads((root / "MANIFEST.json").read_text())
        expected = len(manifest["branches"]) * len(manifest["dates"])
        on_disk = sum(1 for path in (root / "jobs").rglob("*.json.gz") if path.is_file())
        assert complete["job_count"] == expected == on_disk, (
            root.name,
            complete["job_count"],
            expected,
            on_disk,
        )
        assert not complete.get("failed_dates"), (root.name, complete.get("failed_dates"))
        assert complete.get("n_branches") == len(manifest["branches"]), root.name
        total += on_disk
    issued = main / "B02_SUMMARY.json"
    if issued.is_file():
        # the coverage number the receipt copies from the summary, against the disk
        assert json.loads(issued.read_text())["job_count_all_roots"] == total


def test_A07_evidence_matrix_resolves_every_key(tmp_path):
    """Every row the producer writes satisfies the receipt contract's own
    evidence-matrix check, and the row shape the verifier rejected still fails."""
    from trading_research.research.contracts.receipts import (
        FailureCode,
        _check_evidence_matrix,
        required_matrix_ids,
    )

    spec = _p15_16a_spec()
    attempt, commands, document = _stand_in_attempt(tmp_path, pending_s01_s03=True)
    matrix_path = attempt / "EVIDENCE_MATRIX.json"
    receipt = {
        "task_id": "P15-16A",
        "disposition": "implemented_verified",
        "acceptance_checks": {key: True for key in spec.required_acceptance_keys},
        "command_results": commands,
        "artifact_manifest": [{"name": "EVIDENCE_MATRIX.json", "path": str(matrix_path)}],
    }
    failures: list = []
    _check_evidence_matrix(receipt, spec, attempt / "TASK_RECEIPT.json", failures)
    assert [(item.code, item.detail) for item in failures] == []
    assert [row["id"] for row in document["checks"]] == list(required_matrix_ids(spec))
    for row in document["checks"]:
        if row["status"] not in {"pass", "accepted-limit"}:
            continue
        assert row["command_indices"], row["id"]
        assert row["test_nodeids"], row["id"]
        for nodeid in row["test_nodeids"]:
            assert nodeid.partition("::")[2].startswith(f"test_{row['id']}_"), (row["id"], nodeid)
    # negative control: the shape the verifier rejected on 8228d638a16d71bd --
    # a passing row supported only by command indices that are not audit runs.
    payload = json.loads(matrix_path.read_text())
    a06 = next(row for row in payload["checks"] if row["id"] == "A06")
    a06["test_nodeids"] = []
    matrix_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
    regressed: list = []
    _check_evidence_matrix(receipt, spec, attempt / "TASK_RECEIPT.json", regressed)
    assert [(item.code, item.detail) for item in regressed] == [
        (FailureCode.INVENTORY, "passing check has no test_nodeids or audit command")
    ]


def test_A08_silent_failure_probes_are_assigned_to_their_own_ids(tmp_path):
    """Every assurance case assigned to P15-16A has a row bound to a test named
    for that case, and only the probes that were produced are reported as pass."""
    from trading_research.research.contracts.receipts import DEFAULT_ROOT, _test_node_names

    spec = _p15_16a_spec()
    _attempt, _commands, document = _stand_in_attempt(tmp_path, pending_s01_s03=True)
    rows = {row["id"]: row for row in document["checks"]}
    assert set(spec.assurance_cases) <= set(rows)
    for case in spec.assurance_cases:
        row = rows[case]
        assert row["test_nodeids"], case
        for nodeid in row["test_nodeids"]:
            file_part, _, name = nodeid.partition("::")
            assert name.startswith(f"test_{case}_"), (case, nodeid)
            assert name in (_test_node_names(DEFAULT_ROOT / file_part) or set()), (case, nodeid)
    # SILENT_FAILURES assigns nine cases; the probes stage that produces S01 and
    # S03 has not run, so those two stay accepted-limit and the rest are pass.
    produced = {case for case in spec.assurance_cases if rows[case]["status"] == "pass"}
    assert produced == {"S02", "S07", "S08", "S09", "S21", "S22", "S31"}
    assert rows["S01"]["status"] == "accepted-limit"
    assert rows["S03"]["status"] == "accepted-limit"
    assert rows["S01"]["observed"] == "probes stage has not run"
    # negative control: the same writer reports pass only once the probes ran.
    _a, _c, produced_doc = _stand_in_attempt(tmp_path / "produced", pending_s01_s03=False)
    produced_rows = {row["id"]: row for row in produced_doc["checks"]}
    assert produced_rows["S01"]["status"] == "pass"
    assert produced_rows["S03"]["status"] == "pass"
    assert produced_rows["S01"]["observed"] == "mutations fail"
