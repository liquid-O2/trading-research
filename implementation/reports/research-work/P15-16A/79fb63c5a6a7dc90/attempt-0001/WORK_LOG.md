...
- Receipt rewrite: PLAN_SNAPSHOT adds SOURCE_ADDITIONS and astra REVIEW; CODE_SNAPSHOT carries owned directory entries; pytest -v so evidence selectors resolve; RULES annotation dropped; S01/S03 are accepted-limit until --probes (`unsupported` fails ACCEPTANCE while disposition is implemented_verified); B02_SUMMARY.md and AUTHOR_EXAMPLE_REPLAY.md in the manifest.
