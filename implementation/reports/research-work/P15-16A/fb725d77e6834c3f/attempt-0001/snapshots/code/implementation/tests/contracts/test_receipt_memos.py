"""Scoped verification memos and fresh artifact-byte validation."""
...
def test_file_digest_rechecks_bytes_per_access(monkeypatch, tmp_path: Path):
...
def test_predecessor_cache_key_includes_receipt_path(monkeypatch, tmp_path: Path):
    ...
    keys = [key for key in state._predecessor_results if key[0] == str(predecessor)]
    assert keys
