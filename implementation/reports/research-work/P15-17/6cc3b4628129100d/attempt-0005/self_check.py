"""P15-17 stage B self-check (attempt-0005: the amended evaluation).
...
# attempt-0005 is an evaluation-only attempt: its results are written here and the
# run shards it reads are the bound run root (attempt-0003), named explicitly so
# nothing is read from a directory this attempt did not declare.
...
job = json.loads(gzip.open(RUN / "jobs" / day / (cid.replace(":", "--") + ".json.gz")).read())
...
# the frozen bank plus the executed profile supplement, which is one Holm family
_registered = int(results["candidates"])
print(f"  ledger rows {len(ledger)} = {_registered} registered candidates "
      f"({len(manifest['candidates'])} frozen + {_registered - len(manifest['candidates'])} supplement)"
      f" x {len(folds)} folds  -> {len(ledger) == _registered * len(folds)}")
