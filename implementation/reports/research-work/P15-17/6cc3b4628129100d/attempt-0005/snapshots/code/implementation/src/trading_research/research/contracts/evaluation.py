...
BOOTSTRAP_DRAWS = 20000  # amended 2026-09-17: m/(B+1) must stay <= .025 for the largest family
BLOCK_LENGTH = 5
PRIMARY_SCORE = "mean_daily_net_points_one_mini_benchmark"
FIT_END = date(2021, 12, 31)
EXCLUSION_LABEL_AFTER_FIT = "label_known_after_fit"


def build_evaluation_splits(
    declared_dates: Sequence[str],
    labeled_rows: Sequence[Mapping[str, Any]] | None = None,
    *,
    fit_cutoff_ns: int | None = None,
) -> dict[str, Any]:
    """Split builder that split_manifest and evaluation_protocol feed.

    Future labels are purged here so a removed leak guard breaks this producer,
    not only a test fixture.
    """
    manifest = split_manifest(declared_dates)
    protocol = evaluation_protocol()
    cutoff = 0 if fit_cutoff_ns is None else int(fit_cutoff_ns)
    purged = purge_future_labels(list(labeled_rows or ()), fit_cutoff_ns=cutoff)
    manifest["evaluation_protocol"] = {
        "primary_score": protocol["primary_score"],
        "registered_before_candidate_search": protocol["registered_before_candidate_search"],
    }
    manifest["label_purge"] = purged
    return manifest
...
def purge_future_labels(rows: Sequence[Mapping[str, Any]], *, fit_cutoff_ns: int) -> dict[str, list[dict[str, Any]]]:
    kept: list[dict[str, Any]] = []
    excluded: list[dict[str, Any]] = []
    for row in rows:
        known = int(row["label_known_at_ns"])
        payload = dict(row)
        if known > int(fit_cutoff_ns):
            payload["exclusion_code"] = EXCLUSION_LABEL_AFTER_FIT
...
def attainable_holm_floor(family_size: int, *, draws: int = BOOTSTRAP_DRAWS) -> float:
    """Smallest Holm-adjusted p-value a family of ``family_size`` can reach.

    A centered bootstrap p-value cannot fall below 1/(B+1), so the smallest adjusted
    p-value of the largest hypothesis in a family of m is m/(B+1) (EVALUATION.md,
    amended 2026-09-17). A decision stage whose floor exceeds .025 cannot promote any
    candidate whatever the data say, which is why every result reports it.
    """
    if family_size <= 0 or draws <= 0:
        return 0.0
    return family_size / (draws + 1)


def floor_admits_decision(family_size: int, *, draws: int = BOOTSTRAP_DRAWS, limit: float = 0.025) -> bool:
    """Whether the family's attainable floor leaves room for a decision at all."""
    return attainable_holm_floor(family_size, draws=draws) <= limit
