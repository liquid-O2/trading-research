...
# --------------------------------------------------------------------------
# EVALUATION.md, amended 2026-09-17: the attainable multiplicity floor, the
# two-sided support gate and the inconclusive_multiplicity disposition
# --------------------------------------------------------------------------


def _family_trials(target_p: float, others: int, *, target_id: str = "c-main") -> list[dict]:
    """A Holm family of `others` + 1 trials where only the target can be rejected."""
    trials = [{"trial_id": "t-main", "candidate_id": target_id, "p_raw": target_p, "synthetic": True}]
    trials += [
        {"trial_id": f"t-{index}", "candidate_id": f"c-{index}", "p_raw": 1.0, "synthetic": True}
        for index in range(others)
    ]
    return trials


def test_the_attainable_holm_floor_is_the_family_size_over_draws_plus_one():
    """A centered bootstrap p cannot fall below 1/(B+1), so the smallest adjusted
    p-value in a family of m is m/(B+1). The declared families must leave room."""
    assert attainable_holm_floor(164, draws=2000) == pytest.approx(164 / 2001)
    assert attainable_holm_floor(164, draws=20000) == pytest.approx(164 / 20001)
    # the breadth family (164) and the refinement family (237) under the amended
    # 20,000 draws are both under the .025 the contract requires
    assert floor_admits_decision(164) is True
    assert floor_admits_decision(237) is True
    # the superseded 2,000 draws could not decide either family: 164/2001 = .082
    assert floor_admits_decision(164, draws=2000) is False
    assert floor_admits_decision(237, draws=2000) is False
    # and a family large enough still fails at 20,000 draws, so the rule bites
    assert floor_admits_decision(600, draws=20000) is False


def test_a_candidate_that_fails_only_holm_is_inconclusive_multiplicity():
    """Every other gate passed and the multiplicity step did not: that is not
    evidence against the candidate (EVALUATION.md, amended 2026-09-17)."""
    candidate = _promo_candidate()
    # p at the 2,000-draw floor in a family of 164: .0005 > .05/164 = .000305
    blocked = evaluate_promotion(candidate, _family_trials(1 / 2001, 163))
    assert blocked["promoted"] is False
    assert blocked["disposition"] == "inconclusive_multiplicity"
    assert blocked["reason"] == "holm"
    assert blocked["replace_baseline_allowed"] is False
    assert blocked["support_pass"] is True and blocked["holm_reject"] is False

    # positive control: the same family at 20,000 draws clears the same threshold
    promoted = evaluate_promotion(candidate, _family_trials(1 / 20001, 163))
    assert promoted["promoted"] is True
    assert promoted["disposition"] == "promoted"

    # negative control: a candidate that also fails another gate is rejected on
    # that gate, not relabelled inconclusive
    reversal = evaluate_promotion(
        _promo_candidate(cost_stress_sign_reversal=True), _family_trials(1 / 2001, 163)
    )
    assert reversal["disposition"] == "rejected_by_evidence"
    assert reversal["reason"] == "cost_stress"


def test_the_promotion_row_reports_the_floor_beside_the_adjusted_p():
    result = evaluate_promotion(_promo_candidate(), _family_trials(1 / 20001, 163))
    assert result["family_size"] == 164
    assert result["attainable_p_floor"] == pytest.approx(164 / 20001)
    assert result["attainable_floor_admits_decision"] is True
    assert result["p_holm"] >= result["attainable_p_floor"]


def test_a_pair_whose_baseline_is_below_the_support_gate_is_inconclusive():
    """The gate applies to both sides: a candidate that fires 500 times against a
    baseline of 19 entries is not an upgrade of a supported rule."""
    thin = _promo_candidate(
        resolved_opportunities=10325,
        baseline_resolved_opportunities=19,
        candidate_entries=10325,
        baseline_entries=19,
        candidate_mean_net_points=26.0,
        baseline_mean_net_points=0.2,
    )
    verdict = evaluate_promotion(thin, _family_trials(1 / 20001, 163))
    assert verdict["promoted"] is False
    assert verdict["disposition"] == "inconclusive_support"
    assert verdict["reason"] == "baseline_support"
    assert verdict["support_pass_candidate"] is True
    assert verdict["support_pass_baseline"] is False
    # the candidate's own frequency, the ratio and its unpaired result travel with it
    assert verdict["resolved_opportunities"] == 10325
    assert verdict["baseline_resolved_opportunities"] == 19
    assert verdict["entry_ratio_to_baseline"] == pytest.approx(10325 / 19)
    assert verdict["candidate_mean_net_points"] == 26.0
    assert verdict["baseline_mean_net_points"] == 0.2

    # negative control: the same candidate against a supported baseline passes
    supported = evaluate_promotion(
        {**thin, "baseline_resolved_opportunities": 120, "baseline_entries": 120},
        _family_trials(1 / 20001, 163),
    )
    assert supported["support_pass_baseline"] is True
    assert supported["disposition"] == "promoted"


def test_the_baseline_side_falls_back_to_the_baseline_entries_when_not_counted():
    """A row from an older producer carries no baseline_resolved_opportunities;
    the gate then reads the baseline entry count rather than passing silently."""
    legacy = _promo_candidate(baseline_entries=19)
    legacy.pop("baseline_resolved_opportunities", None)
    verdict = evaluate_promotion(legacy, _family_trials(1 / 20001, 163))
    assert verdict["baseline_resolved_opportunities"] == 19
    assert verdict["support_pass_baseline"] is False
    assert verdict["disposition"] == "inconclusive_support"


def _combo_fold(year: int) -> dict:
    days = [f"{year}-01-{index:02d}" for index in range(1, 11)]
    return {
        "test_year": year,
        "outer_fold": year,
        "fit": [f"{year - 2}-01-01"],
        "tune": [f"{year - 1}-07-01"],
        "calibrate": [f"{year - 1}-10-01"],
        "test": days,
    }


def test_a_promoted_combination_carries_its_own_mean_interval_and_raw_p():
    """A combined candidate is a trial in its own right: its ledger row must show
    the mean, interval and raw p of its own paired series, never an ingredient's."""
    from trading_research.research.rule_discovery.refinement import _combination_trial_rows

    folds = [_combo_fold(2022)]
    cid = "GB-FAIL:COMBINED:a+b"
    rows = [
        {
            "candidate_id": cid,
            "family": "GB-FAIL",
            ...
            "status": "attempted",
            "reason": None,
            "ingredients": ["GB-FAIL:previous_hour:T4", "GB-FAIL:previous_hour:F3"],
            "parent_trial_ids": ["GB-FAIL:previous_hour:T4", "GB-FAIL:previous_hour:F3"],
            "inner_improvement_a": 1.0,
            "inner_improvement_b": 2.0,
            "inner_improvement_vs_b02": 3.0,
    ...
    decided = {
        cid: {
            "candidate_id": cid,
            "mean_diff": 3.25,
            "candidate_entries": 420,
            "baseline_entries": 400,
            "eligible_test_days": 120,
            "failure_attribution": [],
            "promotion": {
                ...
                "ci_low": 1.5,
                "ci_high": 5.0,
                "attainable_p_floor": 237 / 20001,
                "family_size": 237,
                "support_pass": True,
                "support_pass_candidate": True,
                "support_pass_baseline": True,
                "resolved_opportunities": 420,
                "baseline_resolved_opportunities": 400,
                "eligible_test_days": 120,
                "supported_outer_blocks": 5,
                "entry_ratio_to_baseline": 1.05,
                "support_sensitivity": support_sensitivity(420, 120, 5),
                "baseline_support_sensitivity": support_sensitivity(400, 120, 5),
            },
        }
    }
    fold_metrics = {cid: {2022: {"mean_diff": 2.5, "eligible_test_days": 24, "candidate_entries": 84,
                                 "baseline_entries": 80, "resolved_opportunities": 84}}}
    identity = {"code_sha256": "c", "bank_sha256": "b", "freeze_sha256": "f", "run_root": "/tmp/x"}
    record = _combination_trial_rows(
        rows, decided, folds=folds, identity=identity, fold_metrics=fold_metrics
    )[0]
    assert record["disposition"] == "promoted"
    metrics = record["test_metrics"]
    assert metrics["mean_diff"] == 3.25
    assert metrics["p_raw"] == 0.0004
    assert (metrics["ci_low"], metrics["ci_high"]) == (1.5, 5.0)
    assert metrics["attainable_p_floor"] == pytest.approx(237 / 20001)
    # the fold row carries this fold's own outer-block number, not only the pooled one
    assert metrics["scope"] == "pooled_outer_blocks"
    assert metrics["fold"]["mean_diff"] == 2.5
    # and the support gate can be audited from the row alone
    assert record["support"]["resolved_opportunities"] == 420
    assert record["support"]["baseline_resolved_opportunities"] == 400
    assert record["support"]["pass_candidate"] is True
    assert record["candidate_population_counts"]["candidate_entries"] == 420

    # negative control: a combination with no decision of its own carries no
    # metrics at all rather than borrowing an ingredient's
    undecided = _combination_trial_rows(
        rows, {}, folds=folds, identity=identity, fold_metrics=fold_metrics
    )[0]
    assert undecided["test_metrics"] == {}
    assert undecided["support"] == {}
    assert undecided["disposition"] == "inconclusive_support"
