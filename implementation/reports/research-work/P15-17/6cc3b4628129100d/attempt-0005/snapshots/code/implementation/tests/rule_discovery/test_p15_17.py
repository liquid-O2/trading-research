...
from trading_research.research.contracts import evaluation as _evaluation
...
def test_stage_b_holm_and_bootstrap_are_the_frozen_calls():
    ...
    is the frozen moving block: seed 15022026, 20,000 draws (amended 2026-09-17
    so the attainable Holm floor stays under .025), block length 5."""
    ...
    assert contracts_evaluation.BOOTSTRAP_DRAWS == 20000  # amended 2026-09-17
    ...
    assert set(seen["kwargs"]) == {(5, 20000, 15022026), (1, 20000, 15022026), (10, 20000, 15022026)}
...
def test_every_gate_is_load_bearing():
    ...
    # the support gate reads both sides of the pair (EVALUATION.md, amended
    # 2026-09-17), so the baseline side is lifted over the gate as well
    row["baseline_resolved_opportunities"] = 400
    ...
    mutations = {
        ...
        "baseline_support": {"baseline_resolved_opportunities": 1},
...
SLICE_ROOT = next(
    (
        candidate
        for candidate in (
            _ATTEMPTS / "attempt-0005/native-slice",
...
# --------------------------------------------------------------------------
# EVALUATION.md, amended 2026-09-17: the attainable floor, the two-sided support
# counts and each fold's own outer-block evidence in the ledger
# --------------------------------------------------------------------------


def test_the_outer_row_counts_both_sides_of_the_pair():
    """The support gate applies to the baseline as well, so the row carries the
    baseline's own resolved count and both unpaired means, not just the diff."""
    table = _synthetic_table()
    item = _synthetic_bank()[0]
    row = _sr.outer_row(item, table[item.candidate_id], _SYNTHETIC_FOLDS)
    assert row["baseline_resolved_opportunities"] == row["baseline_entries"]
    # the unpaired means are the two sides of the same paired days
    assert row["candidate_mean_net_points"] - row["baseline_mean_net_points"] == pytest.approx(
        row["mean_diff"]
    )
    # and they are real numbers from the table, not zeros
    assert row["candidate_mean_net_points"] != 0.0


def test_the_ledger_row_carries_the_support_counts_and_its_own_fold_metric():
    """A fold row that repeats only the pooled decision metric cannot be audited:
    it must name this fold's own outer-block evidence and the support counts."""
    resolved = _synthetic_bank()
    table = _synthetic_table()
    # make the two outer folds genuinely different for S1, so a row that repeated
    # the pooled number would be visibly wrong
    for day in _FOLD_B["test"]:
        row = table["SYN:syn_branch:S1"][day]
        row["candidate_net_points"] = str(_D(row["candidate_net_points"]) + _D("3.0"))
    rows = [_sr.outer_row(item, table[item.candidate_id], _SYNTHETIC_FOLDS) for item in resolved]
    decided = _sr.decide(rows)
    selections = [_sr.select_for_fold(resolved, table, fold) for fold in _SYNTHETIC_FOLDS]
    fold_metrics = {
        item.candidate_id: {
            int(fold["test_year"]): {
                "mean_diff": float(
                    _np.mean([p["diff"] for p in _sr.paired_days(table[item.candidate_id], fold["test"])])
                ),
                "eligible_test_days": len(_sr.paired_days(table[item.candidate_id], fold["test"])),
                "candidate_entries": sum(
                    p["candidate_fills"] for p in _sr.paired_days(table[item.candidate_id], fold["test"])
                ),
                "baseline_entries": sum(
                    p["baseline_fills"] for p in _sr.paired_days(table[item.candidate_id], fold["test"])
                ),
    ...
    s1 = {int(row["outer_fold"]): row for row in records if row["candidate_id"] == "SYN:syn_branch:S1"}
    assert set(s1) == {2022, 2023}
    # the pooled metric is the same in both rows, by design; the fold metric is not
    assert s1[2022]["test_metrics"]["scope"] == "pooled_outer_blocks"
    assert s1[2022]["test_metrics"]["mean_diff"] == s1[2023]["test_metrics"]["mean_diff"]
    assert s1[2022]["test_metrics"]["fold"]["mean_diff"] != s1[2023]["test_metrics"]["fold"]["mean_diff"]
    assert s1[2023]["test_metrics"]["fold"]["mean_diff"] == pytest.approx(
        fold_metrics["SYN:syn_branch:S1"][2023]["mean_diff"]
    )
    # the support gate is auditable from the row alone
    support = s1[2022]["support"]
    assert support["resolved_opportunities"] is not None
    assert support["baseline_resolved_opportunities"] is not None
    assert support["eligible_test_days"] == decided[0]["eligible_test_days"]
    assert support["pass"] in (True, False)
    assert set(support["sensitivity"]) == {"half", "nominal", "twice"}
    # the attainable floor travels with the adjusted p-value
    assert s1[2022]["test_metrics"]["family_size"] == len(decided)
    assert s1[2022]["test_metrics"]["attainable_p_floor"] == pytest.approx(
        _evaluation.attainable_holm_floor(len(decided))
    )

    # negative control: without fold metrics the row says so rather than
    # silently repeating the pooled number as if it were the fold's
    plain = _sr.trial_records(
        resolved,
        decided,
        selections,
        _SYNTHETIC_FOLDS,
    ...
    assert all(row["test_metrics"].get("fold") is None for row in plain if row["test_metrics"])


def test_the_published_multiplicity_block_names_the_floor_and_the_family():
    """A gate that cannot be passed by construction must be visible in the
    results document, not inferred by a reader."""
    resolved = _synthetic_bank()
    table = _synthetic_table()
    rows = [_sr.outer_row(item, table[item.candidate_id], _SYNTHETIC_FOLDS) for item in resolved]
    decided = _sr.decide(rows)
    block = _sr._multiplicity_block(decided)
    assert block["family_size"] == len(decided)
    assert block["bootstrap_draws"] == _evaluation.BOOTSTRAP_DRAWS
    assert block["attainable_p_floor"] == pytest.approx(len(decided) / (_evaluation.BOOTSTRAP_DRAWS + 1))
    assert block["floor_admits_decision"] is True
    assert block["inconclusive_multiplicity"] == [
        row["candidate_id"] for row in decided
        if row["promotion"]["disposition"] == "inconclusive_multiplicity"
    ]
    # negative control: the same family under the superseded 2,000 draws could
    # not have decided anything
    assert _evaluation.floor_admits_decision(164, draws=2000) is False


def test_the_family_report_publishes_the_floor_and_both_support_sides():
    resolved = _synthetic_bank()
    table = _synthetic_table()
    rows = [_sr.outer_row(item, table[item.candidate_id], _SYNTHETIC_FOLDS) for item in resolved]
    decided = _sr.decide(rows)
    retention = _sr.retention_set(
        resolved, decided, [_sr.select_for_fold(resolved, table, fold) for fold in _SYNTHETIC_FOLDS]
    )
    body = _sr.family_report(
        "SYN",
        decided,
        retention,
        [_sr.select_for_fold(resolved, table, fold) for fold in _SYNTHETIC_FOLDS],
        run_root="/tmp/x",
        report_path="/tmp/x/SYN.md",
    )
    assert "## Multiplicity" in body
    assert f"{_evaluation.BOOTSTRAP_DRAWS} bootstrap" in body
    assert "attainable Holm floor" in body
    assert "## Support on both sides of the pair" in body
    assert "baseline gate" in body
