...
AMENDMENT_ROOT = {
    "id": "research-assurance-2026-09-14-v2",
    "date": "2026-09-14",
    "reason": "chain-root fixture",
}


def _amendments_path(root: Path, entries: list[dict]) -> Path:
    path = root / "AMENDMENTS.json"
    path.write_text(json.dumps({"schema": "research-plan-amendments-v1", "amendments": entries}, indent=2) + "\n")
    return path


def _new_shape_entry(
    *,
    path: str,
    before: str,
    after: str,
    previous: str | None,
    ident: str = "research-plan-2026-09-14-v3",
    date: str = "2026-09-14",
) -> dict:
    return {
        "id": ident,
        ...
        "changed_files": [{"path": path, "sha256_before": before, "sha256_after": after}],
    }


def _stale_plan(receipt: Path, payload: bytes = b"historical-plan-bytes\n") -> tuple[str, str, str]:
    plan_path = receipt.parent / "PLAN_SNAPSHOT.json"
    plan = json.loads(plan_path.read_text())
    rel = next(iter(plan["files"]))
    copy = receipt.parent / plan["snapshot_paths"][rel]
    copy.write_bytes(payload)
    old = file_digest(copy)
    plan["files"][rel] = old
    plan_path.write_text(json.dumps(plan, indent=2) + "\n")
    _refresh_named(receipt, "PLAN_SNAPSHOT.json")
    _rebind_draft(receipt, plan_sha256=digest(plan["files"]))
    live = file_digest(Path("/workspace") / rel)
    return rel, old, live


def _stale_code(receipt: Path, payload: bytes = b"historical-code-bytes\n") -> tuple[str, str, str]:
    code_path = receipt.parent / "CODE_SNAPSHOT.json"
    code = json.loads(code_path.read_text())
    rel = next(iter(code["files"]))
    copy = receipt.parent / code["snapshot_paths"][rel]
    copy.write_bytes(payload)
    old = file_digest(copy)
    code["files"][rel] = old
    code_path.write_text(json.dumps(code, indent=2) + "\n")
...
def _bound_graph(tmp_path: Path) -> Path:
    return write_graph(
        tmp_path / "graph.json",
        bound_graph_tasks(),
        required_task_artifacts=[
...
def test_supersession_valid_amendment_chain_accepted(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    receipt = write_bound_task(tmp_path, "P15-00")
    rel, old, live = _stale_plan(receipt)
    assert old != live
    amendments = _amendments_path(
        tmp_path,
        [AMENDMENT_ROOT, _new_shape_entry(path=rel, before=old, after=live, previous=digest(AMENDMENT_ROOT))],
    )
    proc = invoke(
        "task",
        "--receipt",
...
def test_supersession_missing_link_rejected(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    receipt = write_bound_task(tmp_path, "P15-00")
    rel, old, live = _stale_plan(receipt)
    mid = digest({"missing-link": rel})
    other = digest({"other-link": rel})
    first = _new_shape_entry(path=rel, before=old, after=mid, previous=digest(AMENDMENT_ROOT))
    second = _new_shape_entry(
        path=rel,
        before=other,
        after=live,
        previous=digest(first),
        ident="research-assurance-2026-09-14-v3",
    )
    amendments = _amendments_path(tmp_path, [AMENDMENT_ROOT, first, second])
    proc = invoke(
        "task",
        "--receipt",
        str(receipt),
...
def test_supersession_unrecorded_live_edit_rejected(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    receipt = write_bound_task(tmp_path, "P15-00")
    rel, old, live = _stale_plan(receipt)
    elsewhere = digest({"unrecorded": live})
    amendments = _amendments_path(
        tmp_path,
        [AMENDMENT_ROOT, _new_shape_entry(path=rel, before=old, after=elsewhere, previous=digest(AMENDMENT_ROOT))],
    )
    proc = invoke(
        "task",
        "--receipt",
...
def test_supersession_wrong_previous_entry_sha256_rejected(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    ...
    amendments = _amendments_path(
        tmp_path,
        [AMENDMENT_ROOT, _new_shape_entry(path=rel, before=old, after=live, previous="0" * 64)],
    )
    proc = invoke(
        "task",
        "--receipt",
...
def test_supersession_code_verified_successor_accepted(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    ...
    child_code = json.loads((child.parent / "CODE_SNAPSHOT.json").read_text())
    assert child_code["files"][rel] == live
    proc = invoke("task", "--receipt", str(parent), "--graph", str(graph), "--receipts-root", str(tmp_path))
    assert proc.returncode == 0, proc.stdout


def test_supersession_unverified_successor_rejected(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    parent = write_bound_task(tmp_path, "P15-00")
    rel, old, live = _stale_code(parent)
    child = write_bound_task(
        tmp_path,
        "P15-01",
        predecessors={"P15-00": file_digest(parent)},
        acceptance={**acceptance_true(), "A01": False},
    )
    child_code = json.loads((child.parent / "CODE_SNAPSHOT.json").read_text())
    assert child_code["files"][rel] == live
    proc = invoke("task", "--receipt", str(parent), "--graph", str(graph), "--receipts-root", str(tmp_path))
...
def test_supersession_assurance_version_mismatch_rejected(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    receipt = write_bound_task(tmp_path, "P15-00")
    payload = json.loads(receipt.read_text())
    payload["assurance_version"] = "research-assurance-2026-09-14-v0"
    receipt.write_text(json.dumps(payload, sort_keys=True, indent=2) + "\n")
    proc = invoke("task", "--receipt", str(receipt), "--graph", str(graph), "--receipts-root", str(tmp_path))
    assert proc.returncode == 2, proc.stdout
    assert FailureCode.IDENTITY in failure_codes(proc)
    assert FailureCode.SCHEMA not in failure_codes(proc) or "assurance_version is not current" in proc.stdout


def _mid_chain_entries(rel: str, declared: str, live: str) -> tuple[dict, dict, dict]:
    first_before = digest({"h0": rel})
    first_after = digest({"h1": rel})
    first = _new_shape_entry(path=rel, before=first_before, after=first_after, previous=digest(AMENDMENT_ROOT), ident="link-1")
    second = _new_shape_entry(
        path=rel,
        before=first_after,
        after=declared,
    ...
    third = _new_shape_entry(
        path=rel,
        before=declared,
        after=live,
        previous=digest(second),
        ident="link-3",
    )
    return first, second, third


def test_supersession_mid_chain_declared_hash_accepted(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    receipt = write_bound_task(tmp_path, "P15-00")
    rel, declared, live = _stale_plan(receipt)
    first, second, third = _mid_chain_entries(rel, declared, live)
    amendments = _amendments_path(tmp_path, [AMENDMENT_ROOT, first, second, third])
    proc = invoke(
        "task",
        "--receipt",
        str(receipt),
...
def test_supersession_mid_chain_without_later_entry_rejected(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    receipt = write_bound_task(tmp_path, "P15-00")
    rel, declared, live = _stale_plan(receipt)
    first, second, _third = _mid_chain_entries(rel, declared, live)
    amendments = _amendments_path(tmp_path, [AMENDMENT_ROOT, first, second])
    proc = invoke(
        "task",
        "--receipt",
...
def test_supersession_mid_chain_wrong_later_previous_rejected(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout
    ...
    first, second, third = _mid_chain_entries(rel, declared, live)
    third = dict(third)
    third["previous_entry_sha256"] = "0" * 64
    amendments = _amendments_path(tmp_path, [AMENDMENT_ROOT, first, second, third])
    proc = invoke(
        "task",
        "--receipt",
...
def test_gate_review_mid_chain_graph_hash_accepted(tmp_path: Path) -> None:
    graph, first, child = _write_pair(tmp_path)
    subphase = _write_subphase(tmp_path, graph, first, child)
    review = _write_review(tmp_path, subphase=subphase, graph=graph, first=first, child=child)
    control = invoke(
    ...
    assert control.returncode == 0, control.stdout
    live = file_digest(graph)
    declared = digest({"mid-graph": str(graph)})
    link1, link2, link3 = _mid_chain_entries(CANONICAL_GRAPH_REL, declared, live)
    amendments = _amendments_path(tmp_path, [AMENDMENT_ROOT, link1, link2, link3])
    payload = json.loads(review.read_text())
    payload["graph"]["sha256"] = declared
    results_path = Path(payload["independent_suite"]["results_path"])
    results = json.loads(results_path.read_text())
    results["input_hashes"][str(graph)] = declared
    results_path.write_text(json.dumps(results, sort_keys=True, indent=2) + "\n")
    payload["independent_suite"]["results_sha256"] = file_digest(results_path)
...
def test_gate_review_mid_chain_graph_hash_without_chain_rejected(tmp_path: Path) -> None:
    graph, first, child = _write_pair(tmp_path)
    subphase = _write_subphase(tmp_path, graph, first, child)
    review = _write_review(tmp_path, subphase=subphase, graph=graph, first=first, child=child)
    control = invoke(
    ...
    live = file_digest(graph)
    declared = digest({"mid-graph": str(graph)})
    link1, link2, _link3 = _mid_chain_entries(CANONICAL_GRAPH_REL, declared, live)
    amendments = _amendments_path(tmp_path, [AMENDMENT_ROOT, link1, link2])
    payload = json.loads(review.read_text())
    payload["graph"]["sha256"] = declared
    results_path = Path(payload["independent_suite"]["results_path"])
    results = json.loads(results_path.read_text())
...
def test_supersession_predating_entry_skipped_unless_successor(tmp_path: Path) -> None:
    graph = _bound_graph(tmp_path)
    control = write_bound_task(tmp_path / "control", "P15-00")
    good = invoke("task", "--receipt", str(control), "--graph", str(graph), "--receipts-root", str(tmp_path / "control"))
    assert good.returncode == 0, good.stdout

    skipped = write_bound_task(tmp_path / "skipped", "P15-00")
    rel, declared, live = _stale_plan(skipped)
    earlier = digest({"predating-before": rel})
    predating = _new_shape_entry(
        path=rel,
        before=earlier,
        after=digest({"predating-after": rel}),
        previous=digest(AMENDMENT_ROOT),
        ident="predating-unrelated",
        date="2026-09-13",
    )
    successor = _new_shape_entry(
        path=rel,
        before=declared,
        after=live,
        previous=digest(predating),
        ident="on-time-successor",
        date="2026-09-14",
    )
    skip_path = _amendments_path(tmp_path / "skipped", [AMENDMENT_ROOT, predating, successor])
    skip_proc = invoke(
        "task",
        "--receipt",
        str(skipped),
    ...
    assert skip_proc.returncode == 0, skip_proc.stdout

    rejected = write_bound_task(tmp_path / "rejected", "P15-00")
    rel2, declared2, live2 = _stale_plan(rejected)
    predating_successor = _new_shape_entry(
        path=rel2,
        before=declared2,
        after=live2,
        previous=digest(AMENDMENT_ROOT),
        ident="predating-successor",
        date="2026-09-13",
    )
    reject_path = _amendments_path(tmp_path / "rejected", [AMENDMENT_ROOT, predating_successor])
    reject_proc = invoke(
        "task",
        "--receipt",
        str(rejected),
    ...
    assert reject_proc.returncode == 2, reject_proc.stdout
    assert FailureCode.IDENTITY in failure_codes(reject_proc)
