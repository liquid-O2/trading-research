...
ACCEPTED_MATRIX_STATUSES = frozenset({"pass", "accepted-limit", "unsupported"})
...
def _check_identities(receipt: Mapping[str, Any], receipt_path: Path, graph: TaskGraph, failures: list[CheckFailure]) -> None:
    ...
    if not isinstance(files, dict):
        ...
        _check_declared_source_files(files, failures, plan_entry["path"], kind="plan")
    ...
    if not isinstance(code_files, dict):
        ...
        _check_declared_source_files(code_files, failures, code_entry["path"], kind="code")
...
def _bind_review_file(raw: Any, failures: list[CheckFailure], loc: str, *, label: str) -> Path | None:
    ...
    if not _hex_digest(digest_value):
        ...
        return path
    if file_digest(path) != digest_value:
...
def verify_gate_review(review_path: Path, candidate_path: Path, candidate_hash: str, graph: TaskGraph) -> tuple[CheckFailure, ...]:
    ...
    graph_file = _bind_review_file(graph_binding, failures, str(review_path), label="graph")
    ...
    elif isinstance(graph_binding, dict) and _hex_digest(graph_binding.get("sha256")) and graph_binding.get("sha256") != file_digest(Path(graph.path)):
    ...
    required_input_values = {candidate_hash, graph_digest}
    required_input_keys = {str(candidate_path), str(Path(graph.path).resolve()), str(Path(graph.path))}
    if not required_input_values <= set(inputs.values()):
        _append(failures, FailureCode.GATE_REVIEW, results_path, "independent results do not bind this candidate receipt")
    if graph_digest not in inputs.values():
        _append(failures, FailureCode.GATE_REVIEW, results_path, "independent results do not bind the current graph")
    bound_keys = set(inputs)
    if not any(key in bound_keys or Path(key).name == Path(candidate_path).name for key in bound_keys) or str(candidate_path) not in bound_keys:
        if str(candidate_path) not in bound_keys:
            _append(failures, FailureCode.GATE_REVIEW, results_path, "independent results keys do not name the candidate receipt")
    if str(Path(graph.path)) not in bound_keys and str(Path(graph.path).resolve()) not in bound_keys:
        _append(failures, FailureCode.GATE_REVIEW, results_path, "independent results keys do not name the current graph")
    for task_id, ref in task_refs.items():
        if isinstance(ref, dict) and isinstance(ref.get("path"), str) and str(ref["path"]) not in bound_keys:
            if _hex_digest(ref.get("sha256")) and ref.get("sha256") in inputs.values():
                continue
            _append(
                failures,
                FailureCode.GATE_REVIEW,
                results_path,
                f"independent results do not bind task receipt {task_id}",
            )
    return tuple(failures)
