...
def test_s17_future_labels_do_not_enter_fit():
    ...
    split = purge_future_labels(rows, fit_cutoff_ns=cutoff)
    assert [row["row_id"] for row in split["kept"]] == ["ok"]
    assert len(split["excluded"]) == 1
    assert split["excluded"][0]["row_id"] == "leak"
    assert split["excluded"][0]["exclusion_code"] == EXCLUSION_LABEL_AFTER_FIT
