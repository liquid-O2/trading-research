...
def test_a03_zero_entry_day_stays_in_denominator():
    days = [
        {"account_day": "2020-01-02", "net": Decimal("0"), "complete": True, "zero": True},
        {"account_day": "2020-01-03", "net": Decimal("2"), "complete": True, "zero": False},
        {"account_day": "2020-01-06", "net": None, "complete": False, "zero": False},
    ]
    complete = [row for row in days if row["complete"]]
    mean = sum((row["net"] or Decimal(0)) for row in complete) / len(complete)
    assert mean == Decimal("1")
    missing = [row for row in days if not row["complete"]]
    assert missing[0]["net"] is None
    assert missing[0]["account_day"] not in {row["account_day"] for row in complete if row["net"] == 0}
...
def test_s17_future_labels_do_not_enter_fit():
    protocol = evaluation_protocol()
    assert protocol["primary_score"] == "mean_daily_net_points_one_mini_benchmark"
    assert protocol["registered_before_candidate_search"] is True
    adjusted = holm([0.001, 0.04, 0.2])
