...
def slice_p15_05(day: str) -> dict[str, Any]:
    ...
    f1 = f1_trailing_minutes(rows, issue_ns=issue)
    volumes = [int(row["volume"]) for row in rows[-20:]]
    median = median_int(volumes) if volumes else 0
    f2 = f2_volume_completed(rows, issue_ns=issue, median_volume=median)
    f3 = f3_balance(rows, issue_ns=issue)
    prefix = view.prefix
    left = int(np.searchsorted(prefix.t_ns, view.start_ns, side="left"))
    right = int(np.searchsorted(prefix.t_ns, issue, side="left"))
    profile = build_profile(
        prefix.ticks[left:right],
        prefix.size[left:right],
        bandwidth=0,
        window="developing_intraday",
        as_of_ns=issue,
    )
    formed = [item for item in (freeze_formation(f1, cutoff_ns=issue, asset_id=view.asset_id), freeze_formation(f2, cutoff_ns=issue, asset_id=view.asset_id), freeze_formation(f3, cutoff_ns=issue, asset_id=view.asset_id)) if item is not None]
    return {
        "date": day,
        "jobs": 1,
        "matches": 1,
        ...
        "f2_available": bool(f2.get("available")),
        "f2_overshoot": f2.get("overshoot"),
        "f3_available": bool(f3.get("available")),
