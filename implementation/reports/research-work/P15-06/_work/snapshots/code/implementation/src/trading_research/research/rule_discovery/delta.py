...
def slice_p15_06(day: str) -> dict[str, Any]:
    ...
    from trading_research.research.rule_discovery.native import build_market_view, cvd_from_prefix, install_write_guard

    install_write_guard()
    view = build_market_view(day, full_account_day=True)
    issue = min(int(et_ns(date.fromisoformat(day), 9, 30)), view.end_ns)
    signed, volume, unknown = cvd_from_prefix(view.prefix, view.start_ns, issue)
    batch = batch_delta([volume - unknown if volume >= unknown else 0, unknown], [1 if signed >= 0 else -1, 0]) if volume else batch_delta([], [])
    c1 = c1_normalized(signed, volume, unknown)
    features = emit_features({"delta": signed, "volume": volume, "unknown": unknown, "c0": signed}, available_at_ns=issue)
    unresolved = markout([], horizon_s=30, snapshot_ns=issue)
    return {
        "date": day,
        "jobs": 1,
        "matches": 1,
        ...
        "volume": volume,
        "unknown": unknown,
        "c1_available": c1["available"],
        "feature_names": [item.name for item in features],
        "unresolved_cohorts": unresolved["unresolved"],
