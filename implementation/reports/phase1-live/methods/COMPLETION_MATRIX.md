...
Independent lifecycle repair regressions: pass (six preserved cases required).
...
| O150 | Observed order lifecycle | partial | pass | resolved | complete | 4 | 1 | 7 | derived | Reconcile the full order and position state machine across placement, amendment, fills, cancel/expiry/reopen, brackets, exits, and source policy. |
| O151 | Refill-study fill assumption | partial | pass | resolved | complete | 8 | 2 | 5 | derived | Separate modeled touch-fill eligibility from actual fill reports and leave queue priority unverified without order-level evidence. |
...
| O166 | Conditioned next-state transition | partial | pass | resolved | complete | 8 | 2 | 7 | derived | Validate an actual adjacent dated state transition and causal conditioning evidence; aggregate counts alone never certify it. |
...
| C04 | partial | C04-F1:pass, C04:pytest:test_source_snapshot_after_use_is_rejected_without_rewriting_inputs:pass, C04:pytest:test_explicit_unknown_fill_clock_does_not_become_admitted_quantity:pass, C04:pytest:test_real_future_state_parent_cannot_certify_transition_at_earlier_snapshot:pass | resolved | complete | Propagate latest consumed member, parent and window availability. Reject later observations and source-parent laundering. Preserve false versus unknown and named coverage/source holes. |
