...
def test_markout_volume_weighted_unequal_quantities():
    result = markout(_unequal_qty_trades(), horizon_s=30, snapshot_ns=30_000_000_000)
    assert result["buy"]["mean"] == pytest.approx(0.8181818181818182)
    assert result["buy"]["mean"] != 0.0
    assert result["buy"]["volume"] == 11
    assert result["unresolved"] == 1
    assert result["buy"]["median"] is not None
    assert result["buy"]["positive_fraction"] == pytest.approx(0.5)
    unweighted = (1.0 + (-1.0)) / 2
    assert result["buy"]["mean"] != pytest.approx(unweighted)


def test_python_markout_is_independent_and_matches():
    import inspect

    source = inspect.getsource(python_markout)
    assert python_markout is not markout
    assert "return markout(" not in source
    trades = _unequal_qty_trades()
    vector = markout(trades, horizon_s=30, snapshot_ns=30_000_000_000)
    reference = python_markout(trades, horizon_s=30, snapshot_ns=30_000_000_000)
    assert reference["buy"]["mean"] == vector["buy"]["mean"]
    assert reference["buy"]["median"] == vector["buy"]["median"]
    assert reference["buy"]["positive_fraction"] == vector["buy"]["positive_fraction"]
    assert reference["buy"]["volume"] == vector["buy"]["volume"]
    assert reference["unresolved"] == vector["unresolved"]
    assert reference["sell"] == vector["sell"]


def test_memory_and_refilling_use_volume_weighted_mean():
    result = markout(_unequal_qty_trades(), horizon_s=30, snapshot_ns=30_000_000_000)
    row = resolved_memory_row(result, side=1)
    assert row["mean"] == pytest.approx(0.8181818181818182)
    memory = memory_features(
        touch_count=1,
        formation_ns=0,
        last_contact_ns=5,
        ...
        pre_touch_departure=True,
        signed_volume_at_band=3,
        prior_resolved=[row, {"available": False, "mean": 9.0}],
    )
    assert memory["prior_favorable_count"] == 1
    assert memory["prior_resolved_count"] == 2
