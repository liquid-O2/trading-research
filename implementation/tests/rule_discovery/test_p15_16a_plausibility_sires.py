...
def _scan_family(family: str, branches: tuple[str, ...], dates: tuple[str, ...]) -> dict:
    ...
    install_write_guard()
    for day in dates:
        if date.fromisoformat(day) > TAPE_END:
            continue
        try:
...
def test_p15_16a_plausibility_sires_and_refill():
    ...
    (REPAIR / "PLAUSIBILITY_SIRES.json").write_text(json.dumps({"family": "SIRES", "dates": list(SLICE), "branches": payload["SIRES"]}, indent=2) + "\n")
    (REPAIR / "PLAUSIBILITY_REFILL-STUDY.json").write_text(json.dumps({"family": "REFILL-STUDY", "dates": list(SLICE), "branches": payload["REFILL-STUDY"]}, indent=2) + "\n")
    header = "| family | branch | sessions | episodes | pass/fail/unknown | pass_rate | eps_bound | rate_bound | in/out |"
    (REPAIR / "PLAUSIBILITY_SIRES.md").write_text(
        "# SIRES plausibility\n\n"
        + header
        + "\n|---|---|---|---|---|---|---|---|---|\n"
    ...
    (REPAIR / "PLAUSIBILITY_REFILL-STUDY.md").write_text(
...
def test_mutated_stop_four_stage_bound_fails_the_gate():
    ...
    payload_path = REPAIR / "PLAUSIBILITY_SIRES.json"
