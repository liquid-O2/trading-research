...
from .report import counts, headline_row, implementation_identity, markdown_twin, sha256_file, utc_now, write_json, write_text
...
def cmd_method_pass(args):
    ...
    try:
        ...
        if args.episodes:
            supplied, parsed, manifest_hash = read_manifest(args.episodes, method_id)
...
def _run(args, data_root, report_root, supplied, parsed, manifest_hash):
    ...
    if parsed:
        ...
        for candidate in inputs.values():
            scored = score_episode(candidate, objects, assertions, evidence)
    ...
    summaries = [_summarize([], p, 'historical_discovery', 'raw_derived', requested_years) for p in predicates]
    ...
    branches = {branch: {**counts([]), 'predicate': primary, 'cohort': 'historical_discovery',
                         'evidence_mode': 'raw_derived', 'status': 'source_hole',
    ...
    case_branches = {branch: {**counts([]), 'predicate': 'case_description', 'cohort': 'historical_discovery',
                              'evidence_mode': 'raw_derived', 'status': 'source_hole', 'candidate_discovery': 'hole',
    ...
    status = 'implementation_fail' if any(quality[k] for k in QUALITY_ZERO) else 'source_hole'
    ...
    for name, rows in [('candidates.jsonl', candidates), ('objects.jsonl', object_rows), ('holes.jsonl', holes),
                       ...
                       ('reference-outcomes.jsonl', outcome_rows)]:
    ...
    doc = {'identity': identity,
           ...
           'cohort': {**inclusion, 'manifest_hash': inclusion_hash,
                      ...
                      'historical_n': 0, 'fixture_count': len(fixture_rows),
           ...
           'years': requested_years, 'status': status, 'quality': quality,
    ...
    headlines = [headline_row(method_id, s['predicate'], s, doc['status'], str(md_path)) for s in summaries]
