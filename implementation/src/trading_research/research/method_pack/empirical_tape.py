...
def load_tape_session(data_root: Path | str, dataset: str, start: int, end: int,
    ...
    known_at = end
    file_metadata = {}
    file_digests = {identity.path: identity.sha256 for identity in stream.file_identities}
    for row in stream:
        ...
        if source not in file_metadata:
            ...
            file_metadata[source] = (relative, file_digests[relative])
        relative, digest = file_metadata[source]
