    ...
    *,
    ...
    for rule in fixed["rules"]:
        ...
        for policy in POLICIES:
            ...
            block_years = [int(year) for year, _ in sorted(per_block.items())]
            rows.append(
                {
                    ...
                    # E0 and the policy manage the same frozen entries on the same
                    # common complete days, so both sides of the pair resolve the
                    # same count: the two-sided support gate (EVALUATION.md, amended
                    # 2026-09-17) is symmetric here by construction, not by assertion
                    "baseline_resolved_opportunities": resolved,
    ...
    for row in decided:
        ledger.append(
            _refinement.make_trial_record(
                ...
                score=row["mean_diff"],
                support=search_run.support_block(row["promotion"]),
                test_metrics={
                    "scope": "pooled_outer_blocks",
                    "mean_diff": row["mean_diff"],
                    "p_raw": row["promotion"]["p_raw"],
                    "p_holm": row["promotion"]["p_holm"],
                    "attainable_p_floor": row["promotion"].get("attainable_p_floor"),
                    "family_size": row["promotion"].get("family_size"),
                    "ci_low": row["promotion"]["ci_low"],
                    "ci_high": row["promotion"]["ci_high"],
                    "fold": {
                        str(year): {"mean_diff": value}
                        for year, value in zip(row.get("block_years") or [], row["block_improvements"])
                    },
    ...
    results = {
        ...
        "multiplicity": search_run._multiplicity_block(decided),
    ...
    search_run._write_json(out / "EXIT_COMPARISON.json", results)
    # the task card names EXIT_COMPARISON.json; EXIT_RESULTS.json is the same
    # document under the name the exit study reports it by
