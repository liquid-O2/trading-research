...
def assemble_b02_summary() -> dict | None:
    summary_path = B02_RUN / "SUMMARY.json"
    ...
    distinct_jobs = _count_job_files(B02_RUN)
    branches = []
...
def render_family_report(attempt: Path, fidelity: dict, summary: dict | None) -> str:
...
def check(id: str, requirement: str, path: str, symbol: str, nodeid, command, evidence: list, expected: str, observed: str, oracle: str, status="pass") -> dict:
    ...
    payload = {
        ...
        "test_nodeids": nodes,
        "command_indices": [command] if command is not None else [],
        "evidence": evidence,
        "expected": expected,
        "observed": observed,
        "oracle": oracle,
...
def write_matrix(attempt: Path, commands: list[dict], pending_s01_s03: bool) -> dict:
    ...
    rows = [
        ...
        check("A05", "Populations plausible vs published statistics or explained by funnel.", "implementation/src/trading_research/research/rule_discovery/source_adapters/jumbo.py", "compute_published_statistics", node("test_A01_jumbo_rr01_extension_band_is_1_33_1_66"), 0, [ev(attempt / "FAMILY_REPORT.md", "/") if (attempt / "FAMILY_REPORT.md").is_file() else ev(pytest_log, "test_A01")], "funnel or published stats", "pass", "task card A05"),
        check("A06", "Command exit codes, hashes, coverage, runtime.", "implementation/tools/produce_p15_16a.py", "produce", node("test_A01_jumbo_rr01_extension_band_is_1_33_1_66"), 0, a06_evidence, "pytest exit 0", "pass", "ASSURANCE A06"),
        check("A07", "EVIDENCE_MATRIX resolves every key.", "implementation/tools/produce_p15_16a.py", "write_matrix", node("test_A01_fidelity_binds_fixtures_and_records_partials"), 0, a07_evidence, "matrix complete", "pass", "ASSURANCE A07"),
        check("A08", "Assigned silent-failure probes.", "implementation/tests/rule_discovery/test_p15_16a.py", "test_S07_native_replay", node("test_S07_native_replay"), 0, [ev(pytest_log, "test_S07_native_replay")], "S07/S08/S09/S21/S22/S31 pass; S01/S03 unproduced", "pass", "ASSURANCE A08"),
        ...
        check("S03", "False plan/code/draft identities rejected.", "implementation/tools/produce_p15_16a.py", "run_probes", s01_node, s01_cmd, s01_evidence, "mutations fail", s01_observed, "SILENT_FAILURES S03", status=s01_status),
...
def produce(pending_s01_s03: bool) -> Path:
    ...
    if b02_summary is not None:
        ...
        shutil.copy2(attempt / "B02_SUMMARY.json", B02_RUN / "B02_SUMMARY.json")
    ...
    (B02_RUN / "FAMILY_REPORT.md").write_text(family_md)
    run_log = (B02_RUN / "WORK_LOG.md").read_text() if (B02_RUN / "WORK_LOG.md").is_file() else ""
    (attempt / "WORK_LOG.md").write_text(run_log if run_log.strip() else "# P15-16A work log\n\nSee DECISIONS.tsv.\n")
    run_dec = (B02_RUN / "DECISIONS.tsv").read_text() if (B02_RUN / "DECISIONS.tsv").is_file() else ""
    ...
    for extra in ("AUTHOR_EXAMPLE_REPLAY.json", "B02_SUMMARY.json", "RUN_COMPLETE.json", "SUMMARY.json"):
        path = attempt / extra
        if path.is_file():
