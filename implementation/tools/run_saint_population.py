#!/usr/bin/env python3
"""Saint's mechanics over the whole tape on GENERATED structure: the composite
balance and the generated intraday lines of ``saint.py`` (no drawn fixture),
every branch of the family, the passing episodes as the candidate pool, and the
same one-position executed list the other families use. Until 2026-09-18 Saint
had one dated ticket and no population, so no fold read and no upgrade could
be judged for him.

    run_saint_population.py --out <dir> [--workers 4] [--dates a,b] [--limit N]
        [--scanner-overrides JSON] [--export-executed] [--export-candidates]

The session clock is the whole account day (his one ticket is an Asia-session
retest, 2026-08-10 evening): the pool is not restricted to the cash session.
The executed-list policy is the study's convention, stated in ``POLICY``; his
sources print no daily trade count.
"""
...
def scan_one(day: str, overrides: dict | None, export_executed: bool, export_candidates: bool) -> dict:
    from trading_research.research.rule_discovery.native import install_write_guard
    from trading_research.research.rule_discovery.source_adapters import saint
    ...
    document = saint.scan_b02(market, {"family": FAMILY})
    ...
    for ep in episodes:
        ...
        level = geometry.get("break_level") or geometry.get("entry")
        # the line's identity for the one-opportunity key: its kind and price
        values.setdefault("reference_kind", "intraday_level")
        ...
        pool.append({**ep, "values": values, "candidate_id": f"saint|{branch}|{ep.get('side')}|{level}|{ep.get('decision_at')}"})
...
def main(argv=None) -> int:
    ...
    with (args.out / "rows.jsonl").open("w") as sink:
        with ProcessPoolExecutor(max_workers=max(1, args.workers)) as pool:
            futures = {pool.submit(scan_one, day, overrides, args.export_executed, args.export_candidates): day for day in dates}
    ...
    summary = {
        "schema": "saint-population-b03-v1",
