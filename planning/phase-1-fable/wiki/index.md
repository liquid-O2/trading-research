...
| `[XF p.N]` | `sources/documents/jumbo/xfcmg2.pdf` page N (48 pp, 66 posts, 2026 tweets) | 2 |
...
| `[ABS p.N]` etc. | `sources/documents/discretionary/<file>.pdf` page N (short codes in `REVIEW_LEDGER.md`) | 2 |
...
| `[BRIEF]` | the user's Phase 1 planning brief (2026-09-09), locked decisions | 1 |
...
- [data-coverage](data-coverage.md) — what exists, frozen slice F and long slice L, caveats.
...
- [tbr-6-9-range](tbr-6-9-range.md) — the published 06:00–09:00 box, its internals, its published stats.
- [range-path-class](range-path-class.md) — high-only / low-only / both / neither, break order, range-size table, day-type labels.
...
- [clock-grid-and-bars](clock-grid-and-bars.md) — fixed window grid (5–9 family, London, Asia, Green Bird clocks, 9:40–9:50 vs first 20 min), volume-elapsed and dollar bars.
- [ev-range-expected-move](ev-range-expected-move.md) — AM expected-move envelope, own page, own estimator grid.
- [sessionstat-9-12-envelope](sessionstat-9-12-envelope.md) — SessionStat+ 09:00–12:00 average/median H/L, comparison row.
...
- [session-fail-boxes](session-fail-boxes.md) — sweep + fail-back inside across named boxes; GB-NYAM, GB-10-11, GB-hour, GB-Asia, GB-London, TDO, NWOG, 9:30 sweep, golden pocket, A+.

**Flow and value objects (Phase 1 objects with upgrades)**
- [value-and-profiles](value-and-profiles.md) — RTH-scoped VP, value areas, delta profile, key zones.
- [absorption-and-big-trades](absorption-and-big-trades.md) — absorption definitions, BigTrades 100 NY / 75 London, refill, MBP-1 limits.
- [cvd-variants](cvd-variants.md) — trade-level, OHLC-level, participant-separated (size proxies), gamma CVD (named weights).
- [smt-divergence](smt-divergence.md) — trade-level on NQ, OHLC-level across NQ/ES/YM/RTY, multi-scale, hypothesis not snapshot.
- [vol-estimators](vol-estimators.md) — GK, YZ, RV/HAR, IV/skew/VX as features, not a product.
- [options-nodes](options-nodes.md) — OI/gamma strike nodes as levels; hidden book and unpublished engines marked not measurable.

**Sources**
- [sources-pine-archive](sources-pine-archive.md) — the 84-file Pine archive: what each file contributes, hardcoded tables to recompute, reference-only files.

## Phase boundaries

Phase 1 labels sessions and measures objects. Phase 2 predicts (context). Phase 3 learns location (reversal / P-zone model).
Skylit Heatseeker / Flowseeker / Atlas: Phase 2–3, skipped here `[README sources/documents/README.md L27–30]`.
Columns Phase 1 must emit for later phases: one page, in `../SPEC.md` §7. Nothing more is designed here.
