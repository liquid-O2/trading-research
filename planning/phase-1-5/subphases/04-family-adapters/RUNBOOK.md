# Phase 1.5 / 04-family-adapters — coordinator runbook
...
| --- | --- | --- |
| P15-09 — Jumbo range branches | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-09.md) |
| P15-10 — Green Bird failure branches | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-10.md) |
| P15-11 — Green Bird VWAP and scalp configurations | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-11.md) |
| P15-12 — Sires thesis and flow branches | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-12.md) |
| P15-13 — Saint auction alignment branches | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-13.md) |
| P15-14 — Member independent-reasons branches | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-14.md) |
| P15-15 — Keani ordered opening-value branch | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-15.md) |
| P15-16 — Preserve research-process and risk observations | P15-08 | [Task](/workspace/planning/phase-1-5/tasks/P15-16.md) |
...
This runbook includes only cases assigned to: P15-09, P15-10, P15-11, P15-12, P15-13, P15-14, P15-15, P15-16.
...
- `/workspace/implementation/src/trading_research/research/rule_discovery/source_adapters/jumbo.py`
- `/workspace/implementation/src/trading_research/research/rule_discovery/families/jumbo.json`
- `/workspace/implementation/tests/rule_discovery/test_p15_09.py`
...
2. Preserve all eight branches and their distinct source clocks, context, confirmation and deadlines. P-zone recipe remains unchanged; source opening outbound is not silently turned into retest.
3. Use formation/sequence/timing variants only in the bank applicability rows; enumerate new native contacts when geometry changes.
4. Keep range topology, consumed objective, selected direction and prior purge evidence separately measurable.
...
Acceptance amendment: `research-assurance-2026-09-14-v2`. Assigned cases: **S01, S02, S03, S07, S08, S09, S21, S22, S31**. Use the exact probes, expected results and evidence in SILENT_FAILURES.md. Reuse current bound shared evidence where applicable; do not replace the task’s own sensitive/native check with a test count. Preserve prior attempts and produce the common task artifacts listed in TASK_GRAPH.json as well as the task-specific outputs below.

### Acceptance checklist

- [ ] A01: Unchanged outbound respects09:40 expiry and one-source-opening identity.
- [ ] A02: An alternate formation can create a new contact absent from baseline setup records.
- [ ] A03: A source-context false or unknown state is retained when changing only confirmation.
...
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_09.py -q
```
...
```bash
/workspace/implementation/.venv/bin/python /workspace/implementation/tools/run_rule_discovery.py slice --run-root RUN_ROOT --manifest FROZEN_MANIFEST --dates FROZEN_DATES --task P15-09
```
...
```text
/poteto-mode new task. Implement this feature: P15-09 — Jumbo range branches.
Read /workspace/planning/phase-1-5/tasks/P15-09.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
...
- `/workspace/implementation/src/trading_research/research/rule_discovery/source_adapters/green_failure.py`
- `/workspace/implementation/src/trading_research/research/rule_discovery/families/green_failure.json`
- `/workspace/implementation/tests/rule_discovery/test_p15_10.py`
...
1. Implement the family adapter against the named existing scanner and source wiki predicate. Empty delta must delegate to and reproduce the accepted baseline.
2. Preserve eight branch identities, previous-hour/day/week/month distinctions, cash-open and Asia source cases.
3. Retain the printed sweep-entry exception as its own baseline route; changes to reclaim/refinement are labelled and keep other prerequisites.
4. Scan own candidate populations and preserve the supplemental refinement scope.
5. Trace one actual output through its serialized schema, feature/stage parents and native receipt. Then run the verification below and write immutable evidence. A formula unit test alone does not complete a native-data or fitted-expert task.
...
```bash
cd /workspace/implementation
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_10.py -q
```

Then run the frozen engineering slice with the actual manifest and date list produced by the prerequisite (substitute the three ALL_CAPS paths/values; do not invent dates from outcomes):

```bash
/workspace/implementation/.venv/bin/python /workspace/implementation/tools/run_rule_discovery.py slice --run-root RUN_ROOT --manifest FROZEN_MANIFEST --dates FROZEN_DATES --task P15-10
```
...
```text
/poteto-mode new task. Implement this feature: P15-10 — Green Bird failure branches.
Read /workspace/planning/phase-1-5/tasks/P15-10.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
Use the coordinator's actual working root, code identity, predecessor receipt paths/hashes, frozen manifest, dates and run root. Resolve missing values from those artifacts; report unresolved fields to the coordinator instead of inventing them.
Use the parent Grok model for every pstack role (inherit-parent means omit model). Use the installed poteto-agent wrapper when supported. No nested delegation; the coordinator owns fresh review and shared-file integration. Respect its single-writer checkout assignment.
Follow the exact formulas, clocks, schemas, allowed paths, finite budgets and A01, A02, A03, A04, A05, A06, A07, A08 checks. Name the data shape, implement one vertical behavior, run sensitive tests and the declared native slice/full run, and inspect actual output.
Done means all required artifacts exist and verify_research_release.py task exits 0 on the actual TASK_RECEIPT.json, with truthful coverage and disposition. Return real artifact paths, command results, decision-trail path and limitations. Keep runtime todos in the run's WORK_LOG.md and record playbook skips there.
...
- `/workspace/implementation/src/trading_research/research/rule_discovery/source_adapters/green_vwap_scalp.py`
- `/workspace/implementation/src/trading_research/research/rule_discovery/families/green_vwap_scalp.json`
- `/workspace/implementation/tests/rule_discovery/test_p15_11.py`
...
1. Implement the family adapter against the named existing scanner and source wiki predicate. Empty delta must delegate to and reproduce the accepted baseline.
2. Create separate configurations and output namespaces for source-long VWAP and the two directional scalp cases.
3. Preserve VWAP prerequisite closes above London/Asia highs and the source anchor; custom account-day bands use a custom sibling ID.
4. Keep the incomplete proprietary scalp admission issue visible while evaluating the explicit reconstruction and registered variants.
5. Trace one actual output through its serialized schema, feature/stage parents and native receipt. Then run the verification below and write immutable evidence. A formula unit test alone does not complete a native-data or fitted-expert task.
...
```bash
cd /workspace/implementation
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_11.py -q
```

Then run the frozen engineering slice with the actual manifest and date list produced by the prerequisite (substitute the three ALL_CAPS paths/values; do not invent dates from outcomes):

```bash
/workspace/implementation/.venv/bin/python /workspace/implementation/tools/run_rule_discovery.py slice --run-root RUN_ROOT --manifest FROZEN_MANIFEST --dates FROZEN_DATES --task P15-11
```
...
```text
/poteto-mode new task. Implement this feature: P15-11 — Green Bird VWAP and scalp configurations.
Read /workspace/planning/phase-1-5/tasks/P15-11.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
Use the coordinator's actual working root, code identity, predecessor receipt paths/hashes, frozen manifest, dates and run root. Resolve missing values from those artifacts; report unresolved fields to the coordinator instead of inventing them.
Use the parent Grok model for every pstack role (inherit-parent means omit model). Use the installed poteto-agent wrapper when supported. No nested delegation; the coordinator owns fresh review and shared-file integration. Respect its single-writer checkout assignment.
Follow the exact formulas, clocks, schemas, allowed paths, finite budgets and A01, A02, A03, A04, A05, A06, A07, A08 checks. Name the data shape, implement one vertical behavior, run sensitive tests and the declared native slice/full run, and inspect actual output.
Done means all required artifacts exist and verify_research_release.py task exits 0 on the actual TASK_RECEIPT.json, with truthful coverage and disposition. Return real artifact paths, command results, decision-trail path and limitations. Keep runtime todos in the run's WORK_LOG.md and record playbook skips there.
...
Acceptance amendment: `research-assurance-2026-09-14-v2`. Assigned cases: **S01, S02, S03, S07, S08, S09, S21, S22, S23, S31**. Use the exact probes, expected results and evidence in SILENT_FAILURES.md. Reuse current bound shared evidence where applicable; do not replace the task’s own sensitive/native check with a test count. Preserve prior attempts and produce the common task artifacts listed in TASK_GRAPH.json as well as the task-specific outputs below.
...
```text
/poteto-mode new task. Implement this feature: P15-12 — Sires thesis and flow branches.
Read /workspace/planning/phase-1-5/tasks/P15-12.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
Use the coordinator's actual working root, code identity, predecessor receipt paths/hashes, frozen manifest, dates and run root. Resolve missing values from those artifacts; report unresolved fields to the coordinator instead of inventing them.
Use the parent Grok model for every pstack role (inherit-parent means omit model). Use the installed poteto-agent wrapper when supported. No nested delegation; the coordinator owns fresh review and shared-file integration. Respect its single-writer checkout assignment.
Follow the exact formulas, clocks, schemas, allowed paths, finite budgets and A01, A02, A03, A04, A05, A06, A07, A08 checks. Name the data shape, implement one vertical behavior, run sensitive tests and the declared native slice/full run, and inspect actual output.
Done means all required artifacts exist and verify_research_release.py task exits 0 on the actual TASK_RECEIPT.json, with truthful coverage and disposition. Return real artifact paths, command results, decision-trail path and limitations. Keep runtime todos in the run's WORK_LOG.md and record playbook skips there.
Apply /workspace/planning/research-program/ASSURANCE.md and the card's assigned silent-failure cases: S01, S02, S03, S07, S08, S09, S21, S22, S23, S31. Produce EVIDENCE_MATRIX.json linking every required check to real code, executed commands, hashes and inspected output selectors. Show sensitive accepted/rejected controls; preserve missing/ambiguous data and all declared jobs. A pass flag or your own verifier alone cannot prove completion. Return the evidence for the coordinator's separate GATE_REVIEW.json; do not edit fixed independent checks to obtain a pass.
Preserve accepted Phase 1 evidence, raw sources, missing/ambiguous inputs and unsuccessful trials. Perform the local task only; no external publication, shipping workflow or later phase. Do not stop at a plan or a passing toy test.
```
...
- `/workspace/implementation/src/trading_research/research/rule_discovery/source_adapters/saint.py`
- `/workspace/implementation/src/trading_research/research/rule_discovery/families/saint.json`
- `/workspace/implementation/tests/rule_discovery/test_p15_13.py`
...
1. Implement the family adapter against the named existing scanner and source wiki predicate. Empty delta must delegate to and reproduce the accepted baseline.
2. Preserve four source routes, HTF balance, arrival/control, independent LTF agreement and retest.
3. Apply F/P/S/M variants at their declared stage; snapshot HTF and LTF at independent available clocks.
4. Retain POC traversal and trapped-buyer/failed-auction routes as distinct outcomes.
5. Trace one actual output through its serialized schema, feature/stage parents and native receipt. Then run the verification below and write immutable evidence. A formula unit test alone does not complete a native-data or fitted-expert task.
...
- [ ] A01: A later HTF value area cannot explain an earlier retest.
- [ ] A02: LTF agreement without HTF source context does not qualify the unchanged branch.
- [ ] A03: POC/profile alternatives preserve raw volume and construction lineage.
- [ ] A04: Changed geometry/sequence enumerates its own population, not a filter of already-qualified baseline setups.
- [ ] A05: Run native parity, future-perturbation, long/short where applicable, no-setup and input-unknown checks; keep source/proxy labels honest.
- [ ] A06: Evidence includes command exit codes, artifact hashes, coverage/unknown counts, runtime, actual output inspection and the task’s declared limitations. All predecessor receipts verify.
...
```bash
cd /workspace/implementation
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_13.py -q
```

Then run the frozen engineering slice with the actual manifest and date list produced by the prerequisite (substitute the three ALL_CAPS paths/values; do not invent dates from outcomes):

```bash
/workspace/implementation/.venv/bin/python /workspace/implementation/tools/run_rule_discovery.py slice --run-root RUN_ROOT --manifest FROZEN_MANIFEST --dates FROZEN_DATES --task P15-13
```
...
```text
/poteto-mode new task. Implement this feature: P15-13 — Saint auction alignment branches.
Read /workspace/planning/phase-1-5/tasks/P15-13.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
Use the coordinator's actual working root, code identity, predecessor receipt paths/hashes, frozen manifest, dates and run root. Resolve missing values from those artifacts; report unresolved fields to the coordinator instead of inventing them.
Use the parent Grok model for every pstack role (inherit-parent means omit model). Use the installed poteto-agent wrapper when supported. No nested delegation; the coordinator owns fresh review and shared-file integration. Respect its single-writer checkout assignment.
Follow the exact formulas, clocks, schemas, allowed paths, finite budgets and A01, A02, A03, A04, A05, A06, A07, A08 checks. Name the data shape, implement one vertical behavior, run sensitive tests and the declared native slice/full run, and inspect actual output.
Done means all required artifacts exist and verify_research_release.py task exits 0 on the actual TASK_RECEIPT.json, with truthful coverage and disposition. Return real artifact paths, command results, decision-trail path and limitations. Keep runtime todos in the run's WORK_LOG.md and record playbook skips there.
...
- `/workspace/implementation/src/trading_research/research/rule_discovery/source_adapters/member.py`
- `/workspace/implementation/src/trading_research/research/rule_discovery/families/member.json`
- `/workspace/implementation/tests/rule_discovery/test_p15_14.py`
...
1. Implement the family adapter against the named existing scanner and source wiki predicate. Empty delta must delegate to and reproduce the accepted baseline.
2. Preserve prior reaction and independently formed minor HVN as two separately evidenced reasons.
3. Implement only the declared profile/response/memory alternatives and track each distinct contact.
4. Prevent same-price labels or the reaction-period profile from manufacturing independent evidence.
5. Trace one actual output through its serialized schema, feature/stage parents and native receipt. Then run the verification below and write immutable evidence. A formula unit test alone does not complete a native-data or fitted-expert task.
...
- [ ] A01: Using the reaction period to form the purported independent HVN fails.
- [ ] A02: Two records with the same parent formation do not automatically count as two reasons.
- [ ] A03: A new retest requires the explicit rearm/departure evidence.
- [ ] A04: Changed geometry/sequence enumerates its own population, not a filter of already-qualified baseline setups.
- [ ] A05: Run native parity, future-perturbation, long/short where applicable, no-setup and input-unknown checks; keep source/proxy labels honest.
- [ ] A06: Evidence includes command exit codes, artifact hashes, coverage/unknown counts, runtime, actual output inspection and the task’s declared limitations. All predecessor receipts verify.
...
```bash
cd /workspace/implementation
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_14.py -q
```

Then run the frozen engineering slice with the actual manifest and date list produced by the prerequisite (substitute the three ALL_CAPS paths/values; do not invent dates from outcomes):

```bash
/workspace/implementation/.venv/bin/python /workspace/implementation/tools/run_rule_discovery.py slice --run-root RUN_ROOT --manifest FROZEN_MANIFEST --dates FROZEN_DATES --task P15-14
```
...
```text
/poteto-mode new task. Implement this feature: P15-14 — Member independent-reasons branches.
Read /workspace/planning/phase-1-5/tasks/P15-14.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
Use the coordinator's actual working root, code identity, predecessor receipt paths/hashes, frozen manifest, dates and run root. Resolve missing values from those artifacts; report unresolved fields to the coordinator instead of inventing them.
Use the parent Grok model for every pstack role (inherit-parent means omit model). Use the installed poteto-agent wrapper when supported. No nested delegation; the coordinator owns fresh review and shared-file integration. Respect its single-writer checkout assignment.
Follow the exact formulas, clocks, schemas, allowed paths, finite budgets and A01, A02, A03, A04, A05, A06, A07, A08 checks. Name the data shape, implement one vertical behavior, run sensitive tests and the declared native slice/full run, and inspect actual output.
Done means all required artifacts exist and verify_research_release.py task exits 0 on the actual TASK_RECEIPT.json, with truthful coverage and disposition. Return real artifact paths, command results, decision-trail path and limitations. Keep runtime todos in the run's WORK_LOG.md and record playbook skips there.
...
```text
/poteto-mode new task. Implement this feature: P15-15 — Keani ordered opening-value branch.
Read /workspace/planning/phase-1-5/tasks/P15-15.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
Use the coordinator's actual working root, code identity, predecessor receipt paths/hashes, frozen manifest, dates and run root. Resolve missing values from those artifacts; report unresolved fields to the coordinator instead of inventing them.
Use the parent Grok model for every pstack role (inherit-parent means omit model). Use the installed poteto-agent wrapper when supported. No nested delegation; the coordinator owns fresh review and shared-file integration. Respect its single-writer checkout assignment.
Follow the exact formulas, clocks, schemas, allowed paths, finite budgets and A01, A02, A03, A04, A05, A06, A07, A08 checks. Name the data shape, implement one vertical behavior, run sensitive tests and the declared native slice/full run, and inspect actual output.
Done means all required artifacts exist and verify_research_release.py task exits 0 on the actual TASK_RECEIPT.json, with truthful coverage and disposition. Return real artifact paths, command results, decision-trail path and limitations. Keep runtime todos in the run's WORK_LOG.md and record playbook skips there.
...
Adapt Refilling touch/memory observations, jetbundle state/transition evidence, Stoic macro/collection and risk records without manufacturing market entries.
...
- [SEARCH_CONTRACT.md](/workspace/planning/phase-1-5/SEARCH_CONTRACT.md)
- [DATA_CONTRACTS.md](/workspace/planning/research-program/DATA_CONTRACTS.md)
- [method-refill-effect.md](/workspace/wiki/method-refill-effect.md)
- [method-jetbundle-auction-states.md](/workspace/wiki/method-jetbundle-auction-states.md)
- [method-stoic-data-engine.md](/workspace/wiki/method-stoic-data-engine.md)
- [method-stoic-asymmetric-compounding.md](/workspace/wiki/method-stoic-asymmetric-compounding.md)
...
- `/workspace/implementation/src/trading_research/research/rule_discovery/source_adapters/processes.py`
- `/workspace/implementation/src/trading_research/research/rule_discovery/families/processes.json`
- `/workspace/implementation/tests/rule_discovery/test_p15_16.py`

A worker does not edit another task’s shared files. Request an explicit integration patch from the subphase coordinator for runner registration, imports or additive shared-schema changes; the coordinator records it and reruns the affected contract tests. Keep independently fitted artifacts separate even when calculators are shared.

### Implementation steps

1. Adapt Refilling touch/memory observations, jetbundle state/transition evidence, Stoic macro/collection and risk records without manufacturing market entries.
2. Evaluate M1/M2 memory descriptors where native formation/contact evidence exists. Keep source-state and operational-state labels distinct.
3. Produce support/availability dispositions for actual selected orders, private account states and unpublished state thresholds.
...
- [ ] A01: Context/research/personal units have no entry win-rate denominator.
- [ ] A02: Complete no-arrival and unknown coverage differ; conditional reaction is missing for no-arrival.
- [ ] A03: B/A/D/E/W state labels are not AMT day types.
- [ ] A04: Native BBO cannot certify ten-level cancellation behavior.
- [ ] A05: Actual-order/private-account absences remain explicit without preventing independent market-feature implementation.
- [ ] A06: Evidence includes command exit codes, artifact hashes, coverage/unknown counts, runtime, actual output inspection and the task’s declared limitations. All predecessor receipts verify.
...
```bash
cd /workspace/implementation
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_16.py -q
```

Then run the frozen engineering slice with the actual manifest and date list produced by the prerequisite (substitute the three ALL_CAPS paths/values; do not invent dates from outcomes):

```bash
/workspace/implementation/.venv/bin/python /workspace/implementation/tools/run_rule_discovery.py slice --run-root RUN_ROOT --manifest FROZEN_MANIFEST --dates FROZEN_DATES --task P15-16
```
...
```text
/poteto-mode new task. Implement this feature: P15-16 — Preserve research-process and risk observations.
Read /workspace/planning/phase-1-5/tasks/P15-16.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
Use the coordinator's actual working root, code identity, predecessor receipt paths/hashes, frozen manifest, dates and run root. Resolve missing values from those artifacts; report unresolved fields to the coordinator instead of inventing them.
Use the parent Grok model for every pstack role (inherit-parent means omit model). Use the installed poteto-agent wrapper when supported. No nested delegation; the coordinator owns fresh review and shared-file integration. Respect its single-writer checkout assignment.
Follow the exact formulas, clocks, schemas, allowed paths, finite budgets and A01, A02, A03, A04, A05, A06, A07, A08 checks. Name the data shape, implement one vertical behavior, run sensitive tests and the declared native slice/full run, and inspect actual output.
Done means all required artifacts exist and verify_research_release.py task exits 0 on the actual TASK_RECEIPT.json, with truthful coverage and disposition. Return real artifact paths, command results, decision-trail path and limitations. Keep runtime todos in the run's WORK_LOG.md and record playbook skips there.
