# Phase 1.5 / 02-source-reconstruction — coordinator runbook
...
Source content SHA256: `080a66927fce6161986de481374a911c69aeb51ebce584be0011ffeb6d7cec90`.

Previous gate: **01-native-and-outcomes**. External task dependencies: P15-00, P15-02. Read and verify their actual receipts before implementation.
...
| --- | --- | --- |
| P15-04 — Close the numerical source-reconstruction ledger | P15-00, P15-02 | [Task](/workspace/planning/phase-1-5/tasks/P15-04.md) |
...
This runbook includes only cases assigned to: P15-04.
...
Canonical source: [P15-04.md](/workspace/.worktrees/docs-amend/planning/phase-1-5/tasks/P15-04.md).

## P15-04 — Close the numerical source-reconstruction ledger

Status: **planned; implementation not started by this planning task**.

Subphase: `02-source-reconstruction`. Dependencies: P15-00, P15-02.

### Goal and boundary

Map all still-inferred or unavailable numerical operands in the accepted coverage registry to source/wiki evidence and existing code. Prioritize EV, P-zones, KG1/gamma, auction and macro, without dropping other registry gaps.
...
Consume verified predecessor receipts: **P15-00, P15-02**. Their hashes and actual artifact paths go in this task receipt. Required schema details are in DATA_CONTRACTS; source adapters also read their named method predicates.

Allowed implementation/test paths:

- `/workspace/implementation/src/trading_research/research/rule_discovery/reconstruction.py`
- `/workspace/planning/phase-1-5/SOURCE_RECONSTRUCTION_LEDGER.json`
- `/workspace/implementation/tests/rule_discovery/test_p15_04.py`
...
1. Map all still-inferred or unavailable numerical operands in the accepted coverage registry to source/wiki evidence and existing code. Prioritize EV, P-zones, KG1/gamma, auction and macro, without dropping other registry gaps.
2. Implement only printed recoverable arithmetic and explicitly named conventional EV scaling. Preserve existing inferred baselines; state source-exact versus research-model identity.
3. Execute at most two source-construction checks per recoverable formula. For unpublished constants return the exact nonidentifiability disposition and dependency impact; stop the matching effort.
...
Acceptance amendment: `research-assurance-2026-09-14-v2`. Assigned cases: **S01, S02, S03, S21, S31**. Use the exact probes, expected results and evidence in SILENT_FAILURES.md. Reuse current bound shared evidence where applicable; do not replace the task’s own sensitive/native check with a test count. Preserve prior attempts and produce the common task artifacts listed in TASK_GRAPH.json as well as the task-specific outputs below.

### Acceptance checklist

- [ ] A01: Every input-limited numerical operand has a source pointer, implementation state, clock and disposition.
- [ ] A02: EV point/variance/annualization units remain distinct; the 100,.2,.25 fixture gives10.
- [ ] A03: No P-zone/forward-volatility optimizer is added in Phase 1.5.
- [ ] A04: A figure without dated inputs cannot produce a fabricated source-exact pass.
- [ ] A05: Updated source ledger links to immutable evidence without editing sources.
...
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_04.py -q
```
...
- `SOURCE_RECONSTRUCTION_LEDGER.json`
- `SOURCE_CHECKS.json`
- `DEPENDENCY_LIMITS.json`
...
```text
/poteto-mode new task. Implement this feature: P15-04 — Close the numerical source-reconstruction ledger.
Read /workspace/planning/phase-1-5/tasks/P15-04.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
