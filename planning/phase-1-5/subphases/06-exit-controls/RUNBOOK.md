# Phase 1.5 / 06-exit-controls — coordinator runbook
...
Source content SHA256: `1daa9eec8e9034e5133883ec974404740d648792340fbcce7001e72aedb05d4c`.

Previous gate: **05-finite-search**. External task dependencies: P15-18. Read and verify their actual receipts before implementation.
...
| --- | --- | --- |
| P15-19 — Compare fixed-entry exit policies | P15-18 | [Task](/workspace/planning/phase-1-5/tasks/P15-19.md) |
...
This runbook includes only cases assigned to: P15-19.
...
Canonical source: [P15-19.md](/workspace/planning/phase-1-5/tasks/P15-19.md).

## P15-19 — Compare fixed-entry exit policies

Status: **planned; implementation not started by this planning task**.

Subphase: `06-exit-controls`. Dependencies: P15-18.

### Goal and boundary

Freeze selected entry records and initial structural risk. Implement exactly E0–E4 with source deadlines/account-day flatten.
...
Consume verified predecessor receipts: **P15-18**. Their hashes and actual artifact paths go in this task receipt. Required schema details are in DATA_CONTRACTS; source adapters also read their named method predicates.

Allowed implementation/test paths:

- `/workspace/implementation/src/trading_research/research/rule_discovery/exits.py`
- `/workspace/implementation/tests/rule_discovery/test_p15_19.py`
...
1. Freeze selected entry records and initial structural risk. Implement exactly E0–E4 with source deadlines/account-day flatten.
2. Compare paired entry outcomes and serialized daily paths, separating occupancy-induced entry changes from unchanged-entry comparisons.
3. Select only inside tuning, retain all outer results and cost stresses, and leave final learned management out of scope.
...
Acceptance amendment: `research-assurance-2026-09-14-v2`. Assigned cases: **S01, S02, S03, S07, S11, S12, S14, S24, S30, S32**. Use the exact probes, expected results and evidence in SILENT_FAILURES.md. Reuse current bound shared evidence where applicable; do not replace the task’s own sensitive/native check with a test count. Preserve prior attempts and produce the common task artifacts listed in TASK_GRAPH.json as well as the task-specific outputs below.

### Acceptance checklist

- [ ] A01: E3/E4 updates use known favorable batches and never loosen stops.
- [ ] A02: A future maximum cannot determine a trailing exit.
- [ ] A03: One-mini policy has no fractional partials.
- [ ] A04: Undefined initial R is an explicit unsupported E3/E4 record.
- [ ] A05: Entry IDs match across the primary unchanged-entry comparison; differing occupancy populations are separate.
...
.venv/bin/python -m pytest /workspace/implementation/tests/rule_discovery/test_p15_19.py -q
```
...
```bash
/workspace/implementation/.venv/bin/python /workspace/implementation/tools/run_rule_discovery.py slice --run-root RUN_ROOT --manifest FROZEN_MANIFEST --dates FROZEN_DATES --task P15-19
```
...
```text
/poteto-mode new task. Run this finite registered research experiment: P15-19 — Compare fixed-entry exit policies.
Read /workspace/planning/phase-1-5/tasks/P15-19.md and /workspace/planning/research-program/PSTACK_EXECUTION.md. Use the installed pstack router and the card's required contracts; this is execution of an existing specification.
