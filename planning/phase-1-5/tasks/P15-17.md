...
1. Freeze the expanded bank, split/exposure manifests and run resource profile. Execute all registered applicable candidates on each chronological fold, retaining errors and zero days. The 2026-09-16 profile supplement (P3, P4 in SEARCH_CONTRACT.md) is declared in `CANDIDATE_BANK_SUPPLEMENT.json`, hashed in RUN_META before it runs, executed as its own attempt on the same roots, folds and hold-out exclusion, and evaluated with the 160 in one Holm family.
...
- [ ] A10: The profile supplement (P3 value area .68, P4 value area .40) was declared and hashed before any of its results existed; it ran on the same roots, folds and hold-out exclusion as the 160; its trials enter the same ledger, dispositions and Holm family; a supplement candidate equal to a branch's source fraction is recorded as a duplicate, not run.
...
- `FAMILY_REPORTS/`
- `CANDIDATE_BANK_SUPPLEMENT.json`
- `RESULT_CARD.json` (the result card of DELIVERABLES.md: question, headline numbers, target met or not, verdict, lever, limits)
- `TASK_RECEIPT.json` and `REPORT.md`, including acceptance keys A01–A10 and exact predecessor/artifact hashes.
