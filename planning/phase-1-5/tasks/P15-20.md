...
- [ ] A17: `BREAKDOWN.json` and `BREAKDOWN_REPORT.md` render from artifacts with no hand-typed number: for every branch, for B0.2 and the fold-selected rule (and the all-history recommendation when it differs), the section 3 and 5 metrics by outer fold (calendar year) and by every dimension of `REGIME_DIMENSIONS.json` (session bucket, day of week, realized-volatility tercile, overnight-range tercile, prior-close VIX bucket, inferred gamma sign, prior day type, macro release day), one-way cuts only, each cell with opportunity count, days, mean daily net points, the contract's bootstrap interval and a low-support mark under 30 opportunities; unknown buckets are counted, not dropped; the report is descriptive and no selection, threshold or date reads it.
...
- `TASK_RECEIPT.json` and `REPORT.md`, including acceptance keys A01–A17 and exact predecessor/artifact hashes.
