# 09: Judas and both single-break day classes

**What to build:** Label every eligible session with faithful Jumbo class evidence, retain known-then inputs and compare the named experimental labelers.

**Blocked by:** [06: Range-clock and bar variants with Jumbo’s width table](06-range-clock-and-bar-variants-with-jumbo-s-width-table.md), [07: Cross-session geometry and clean-edge tables](07-cross-session-geometry-and-clean-edge-tables.md), [08: Continuous multiscale SMT on 3–4 markets](08-continuous-multiscale-smt-on-3-4-markets.md)

**Definition blockers:** Q02 in [QUESTIONS](../QUESTIONS.md)
...
```sh
phase1 measure --slice 09 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/09/`. Each printed row must point to its actual generated report.
