# 17: Extreme checkpoints, candle curves and AM-to-PM paths

**What to build:** Count causal checkpoint survival, extreme-time bins and each sourced quarter/AM-PM curve or continuation on the same period paths.
...
**Definition blockers:** Q01, Q02 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S17](../PRD.md). **Wiki:** [extreme time checkpoints](../wiki/extreme-time-checkpoints.md), [candle curves](../wiki/candle-curves.md). **Contract:** [SPEC](../SPEC.md).
...
A path can satisfy both curve and continuation and must not be forced into one label. The first session bar can hold HOD; the14–17 source slot has three hours.
...
```sh
phase1 measure --slice 17 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/17/`. Each printed row must point to its actual generated report.
