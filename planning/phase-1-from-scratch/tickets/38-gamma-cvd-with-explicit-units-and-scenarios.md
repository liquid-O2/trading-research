# 38: Gamma CVD with explicit units and scenarios

**What to build:** Compute the resolved gamma-CVD construction and named weighting/reset variants, then compare its path statistics with other CVD families.

**Blocked by:** [03: A disclosed options wall with honest chain coverage](03-a-disclosed-options-wall-with-honest-chain-coverage.md), [37: Options flow revisions and delayed OI checks](37-options-flow-revisions-and-delayed-oi-checks.md)

**Definition blockers:** Q20, Q22 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S38](../PRD.md). **Wiki:** [cvd constructions](../wiki/cvd-constructions.md). **Contract:** [SPEC](../SPEC.md).

## Owned scope

Own gamma CVD and its resolved weighting/reset variants. Consume options flow from ticket37; do not recreate ordinary or participant-separated CVD.
...
An independently calculated small option-trade example reconciles contract multipliers and gamma weights. Changing the declared sign scenario changes only the scenario-specific output.
...
```sh
phase1 measure --slice 38 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/38/`. Each printed row must point to its actual generated report.
