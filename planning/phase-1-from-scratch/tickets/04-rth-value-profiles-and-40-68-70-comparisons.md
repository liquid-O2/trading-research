# 04: RTH value profiles and 40/68/70% comparisons

**What to build:** Compare actual RTH traded-volume POC/value areas with source synthetic profiles, then count edge and POC paths on real sessions.

**Blocked by:** [01: Published 06–09 range to a real path report](01-published-06-09-range-to-a-real-path-report.md)

**Definition blockers:** Q01, Q08 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S04](../PRD.md). **Wiki:** [value profiles](../wiki/value-profiles.md). **Contract:** [SPEC](../SPEC.md).
...
An independently summed price-volume ladder must preserve total volume and expose a tied POC. Each area fraction must use its declared expansion rule; developing values cannot replace prior snapshots.
...
```sh
phase1 measure --slice 04 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/04/`. Each printed row must point to its actual generated report.
