# 46: Range-position probability and close maps

**What to build:** Recompute all source map cells and compare causal prior-range, pooled/shrunk and fixed-grid/bar variants without deploying a selector.

**Blocked by:** [01: Published 06–09 range to a real path report](01-published-06-09-range-to-a-real-path-report.md), [07: Cross-session geometry and clean-edge tables](07-cross-session-geometry-and-clean-edge-tables.md), [16: ORB and IB formation-to-break reports](16-orb-and-ib-formation-to-break-reports.md)

**Definition blockers:** Q01 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S46](../PRD.md). **Wiki:** [range probability maps](../wiki/range-probability-maps.md). **Contract:** [SPEC](../SPEC.md).
...
Above/below/inside sum to all eligible intervals; resolved-only direction divides only by above+below. Hidden low-probability cells and unresolved closes cannot disappear.
...
```sh
phase1 measure --slice 46 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/46/`. Each printed row must point to its actual generated report.
