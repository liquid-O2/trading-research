# 20: Local profile nodes, shelves and ledges

**What to build:** Compare sourced profile geometry with prominence, two-sided LVN, gradient, age and density variants through real node reactions.

**Blocked by:** [04: RTH value profiles and 40/68/70% comparisons](04-rth-value-profiles-and-40-68-70-comparisons.md)
...
**PRD story:** [S20](../PRD.md). **Wiki:** [nodes shelves ledges](../wiki/nodes-shelves-ledges.md). **Contract:** [SPEC](../SPEC.md).
...
A thin tail without a supported balance beyond it must differ from a two-sided LVN. Once a naked POC is touched its history remains, with naked status retired.
...
```sh
phase1 measure --slice 20 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/20/`. Each printed row must point to its actual generated report.
