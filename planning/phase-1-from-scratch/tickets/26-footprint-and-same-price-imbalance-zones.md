# 26: Footprint and same-price imbalance zones

**What to build:** Compare diagonal3–4× stacks, same-price imbalance and normalized/bar variants through zone retests and effort/result outcomes.

**Blocked by:** [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md)

**Definition blockers:** Q01, Q13 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S26](../PRD.md). **Wiki:** [footprint imbalances](../wiki/footprint-imbalances.md). **Contract:** [SPEC](../SPEC.md).
...
A hand-calculated diagonal ladder and same-price ladder yield their distinct ratios. A zero denominator follows the resolved rule; the highlighted source drawing cannot override arithmetic.
...
```sh
phase1 measure --slice 26 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/26/`. Each printed row must point to its actual generated report.
