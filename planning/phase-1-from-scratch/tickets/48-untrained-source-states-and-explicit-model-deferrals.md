# 48: Untrained source states and explicit model deferrals

**What to build:** Report raw rule/composite state frequencies and path separation, with learned HMM and performance-selector branches explicitly deferred.

**Blocked by:** [40: Flow clusters with density-aware reception statistics](40-flow-clusters-with-density-aware-reception-statistics.md), [47: Raw oscillator, momentum and price-band comparisons](47-raw-oscillator-momentum-and-price-band-comparisons.md)
...
**PRD story:** [S48](../PRD.md). **Wiki:** [adaptive indicator states](../wiki/adaptive-indicator-states.md). **Contract:** [SPEC](../SPEC.md).
...
An unreachable source grade has zero observed states rather than fabricated examples. A request for deferred HMM training must remain outside the Phase 1 measure command.
...
```sh
phase1 measure --slice 48 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/48/`. Each printed row must point to its actual generated report.
