...
**What to build:** Compare exact source anomaly/cluster objects with true-trade, decay, causal-threshold and density variants on QQQ and NQ histories.
...
**PRD story:** [S40](../PRD.md). **Wiki:** [flow cluster levels](../wiki/flow-cluster-levels.md). **Contract:** [SPEC](../SPEC.md).
...
A cluster revision is unavailable before its latest required member. An older weak cluster removed by a display cap still appears in outcomes; matched-density comparison cannot count duplicate zones twice.
...
```sh
phase1 measure --slice 40 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/40/`. Each printed row must point to its actual generated report.
