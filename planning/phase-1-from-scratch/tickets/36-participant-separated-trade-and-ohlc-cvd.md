# 36: Participant-separated trade and OHLC CVD

**What to build:** Add the two resolved participant-proxy constructions to the same CVD reconciliation and price-path report.

**Blocked by:** [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md)

**Definition blockers:** Q20 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S36](../PRD.md). **Wiki:** [cvd constructions](../wiki/cvd-constructions.md). **Contract:** [SPEC](../SPEC.md).

## Owned scope

Own participant-separated trade-CVD and participant-separated OHLC-CVD after Q20. Reuse the plain pair from ticket02. Gamma CVD belongs to ticket38.
...
Bucket increments reconcile to the unpartitioned construction when the definition partitions all volume. Unknown classification stays visible; no bucket is labeled observed institutional ownership.
...
```sh
phase1 measure --slice 36 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/36/`. Each printed row must point to its actual generated report.
