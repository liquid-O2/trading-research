...
**What to build:** Compare trade and HLC3 VWAP, full-session/RTH/fixed/dynamic anchors and SD/MAD/RMS/quantile bands through causal contacts and band walking.

**Blocked by:** [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md), [05: GK, Yang–Zhang, RV and lagged HAR inputs](05-gk-yang-zhang-rv-and-lagged-har-inputs.md)

**Definition blockers:** Q01, Q03, Q11 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S23](../PRD.md). **Wiki:** [vwap deviation bands](../wiki/vwap-deviation-bands.md). **Contract:** [SPEC](../SPEC.md).
...
An independently weighted mean differs from median/POC on an asymmetric sample. A new-high reset and fixed06anchor must diverge when the high updates; centered SD and uncentered RMS remain distinct.
...
```sh
phase1 measure --slice 23 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/23/`. Each printed row must point to its actual generated report.
