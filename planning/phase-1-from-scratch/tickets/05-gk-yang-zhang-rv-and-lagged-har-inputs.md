# 05: GK, Yang–Zhang, RV and lagged HAR inputs

**What to build:** Compare realized-volatility constructions and their ability to normalize range and rail outcomes without fitting a forecast.

**Blocked by:** [01: Published 06–09 range to a real path report](01-published-06-09-range-to-a-real-path-report.md)

**Definition blockers:** Q03 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S05](../PRD.md). **Wiki:** [realized volatility](../wiki/realized-volatility.md). **Contract:** [SPEC](../SPEC.md).
...
A constant-price, zero-gap series has zero GK/YZ/RV. A single known log-return has RV equal to its square. HAR inputs exclude the outcome session. Across multiple sessions, one isolated overnight gap with flat intraday bars leaves GK at zero but produces nonzero Yang–Zhang variance.
...
```sh
phase1 measure --slice 05 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/05/`. Each printed row must point to its actual generated report.
