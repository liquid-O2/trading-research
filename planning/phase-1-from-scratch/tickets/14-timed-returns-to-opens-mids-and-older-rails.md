# 14: Timed returns to opens, mids and older rails

**What to build:** Recompute each sourced trigger/return window and compare trade-ordered contact, departure/rearm and target-fraction variants.
...
**PRD story:** [S14](../PRD.md). **Wiki:** [timed retracements](../wiki/timed-retracements.md). **Contract:** [SPEC](../SPEC.md).
...
A target touch before a rail break cannot count as a post-break return. A zero-distance target is separately identified; a no-return event remains in the trigger denominator.
...
```sh
phase1 measure --slice 14 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/14/`. Each printed row must point to its actual generated report.
