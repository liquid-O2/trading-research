# 03: A disclosed options wall with honest chain coverage

**What to build:** Produce a disclosed scenario-based call/put wall and its next-path report, alongside actual contract/quote/OI coverage and missing-strike sensitivity.

**Blocked by:** [01: Published 06–09 range to a real path report](01-published-06-09-range-to-a-real-path-report.md)

**Definition blockers:** Q22 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S03](../PRD.md). **Wiki:** [options chain availability](../wiki/options-chain-availability.md), [options exposure nodes](../wiki/options-exposure-nodes.md). **Contract:** [SPEC](../SPEC.md).

## Owned scope

Own acquired options coverage and the disclosed scenario-based walls/flip/max-pain constructions. The screenshot’s undisclosed node formula and lifecycle/confluence variants belong to ticket44. Do not guess them here.
...
A tiny independently calculated two-strike chain must select the largest qualifying call above and put below spot under the declared scenario. Empty partitions and unreleased OI cannot contribute.
...
```sh
phase1 measure --slice 03 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/03/`. Each printed row must point to its actual generated report.
