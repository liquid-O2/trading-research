# 16: ORB and IB formation-to-break reports

**What to build:** Compare sourced ORB/IB clocks, formation order, close-half, size and projections with causal formation-close and trade-order variants.
...
**PRD story:** [S16](../PRD.md). **Wiki:** [opening range and ib](../wiki/opening-range-and-ib.md). **Contract:** [SPEC](../SPEC.md).
...
The first outside-bar close cannot replace the final formation close. Both-high-low in one minute remains order-unknown unless trades resolve it. Weekly hit-bar shares are not weekday probabilities.
...
```sh
phase1 measure --slice 16 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/16/`. Each printed row must point to its actual generated report.
