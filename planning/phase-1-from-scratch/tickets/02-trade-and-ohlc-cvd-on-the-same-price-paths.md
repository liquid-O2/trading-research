# 02: Trade and OHLC CVD on the same price paths

**What to build:** Compare trade-CVD and the exact sourced OHLC-CVD increments, resets, unknown-side mass and subsequent price paths.
...
**PRD story:** [S02](../PRD.md). **Wiki:** [cvd constructions](../wiki/cvd-constructions.md). **Contract:** [SPEC](../SPEC.md).

## Owned scope

Own trade-CVD and OHLC-CVD only. Ticket36 owns the two participant-separated branches; ticket38 owns gamma CVD; ticket39 compares divergence across all five. Their future work does not block this basic pair.
...
Signed buys3 and sells2 produce delta+1. A doji OHLC bar contributes0 in the source branch. Unknown trade side stays unknown; it must not become a sell.
...
```sh
phase1 measure --slice 02 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/02/`. Each printed row must point to its actual generated report.
