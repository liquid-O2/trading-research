# 27: Best-book renewal and withdrawal response

**What to build:** Measure execution-conditioned best-price renewal, persistence, disappearance and response across time/event/volume windows.

**Blocked by:** [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md)

**Definition blockers:** Q01, Q15 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S27](../PRD.md). **Wiki:** [bbo liquidity proxies](../wiki/bbo-liquidity-proxies.md). **Contract:** [SPEC](../SPEC.md).
...
A best-price move is distinct from same-price replenishment. A size disappearance without a print is a proxy observation, not proven spoofing; missing depth is not zero.
...
```sh
phase1 measure --slice 27 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/27/`. Each printed row must point to its actual generated report.
