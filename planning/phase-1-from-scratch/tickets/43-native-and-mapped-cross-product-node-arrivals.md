# 43: Native and mapped cross-product node arrivals

**What to build:** Compare native ETF/index/futures references with causal ratio/basis mappings, uncertainty and NQ arrival/path disagreement.
...
**PRD story:** [S43](../PRD.md). **Wiki:** [cross market price mapping](../wiki/cross-market-price-mapping.md). **Contract:** [SPEC](../SPEC.md).
...
A daily cash mark cannot become minute truth. Applying a mapping known after arrival must fail availability verification; native and mapped levels retain separate IDs.
...
```sh
phase1 measure --slice 43 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/43/`. Each printed row must point to its actual generated report.
