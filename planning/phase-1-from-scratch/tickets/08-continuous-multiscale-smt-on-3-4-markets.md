# 08: Continuous multiscale SMT on 3–4 markets

**What to build:** Compare the exact Pine matcher with continuous, native-unit and lag-aware SMT, including NQ trade triggers and 3–4-asset OHLC scales throughout the session.
...
**Definition blockers:** Q21 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S08](../PRD.md). **Wiki:** [smt extreme nonconfirmation](../wiki/smt-extreme-nonconfirmation.md). **Contract:** [SPEC](../SPEC.md).
...
A sister pivot confirmed after the trigger is unavailable then. Rescaling ES/NQ tick units must change only the normalized variant as specified; invalidated events remain counted.
...
```sh
phase1 measure --slice 08 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/08/`. Each printed row must point to its actual generated report.
