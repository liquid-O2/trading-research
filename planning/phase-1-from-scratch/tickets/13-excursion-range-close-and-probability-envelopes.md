# 13: Excursion, range, close and probability envelopes

**What to build:** Compare every sourced distribution/anchor convention with prior-only causal quantile and scale variants, including source code disagreements and complete tails.
...
**PRD story:** [S13](../PRD.md). **Wiki:** [distribution envelopes](../wiki/distribution-envelopes.md). **Contract:** [SPEC](../SPEC.md).
...
A sample where median(up)+median(down) differs from median(width) keeps those outputs distinct. Outside histogram tails and dojis remain in n; post-tag returns require time order.
...
```sh
phase1 measure --slice 13 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/13/`. Each printed row must point to its actual generated report.
