# 07: Cross-session geometry and clean-edge tables

**What to build:** Compare source containment, partial overlap, body-quartile and open-versus-prior-value relations with causal calendar and bar variants.

**Blocked by:** [01: Published 06–09 range to a real path report](01-published-06-09-range-to-a-real-path-report.md), [04: RTH value profiles and 40/68/70% comparisons](04-rth-value-profiles-and-40-68-70-comparisons.md)
...
**PRD story:** [S07](../PRD.md). **Wiki:** [session geometry](../wiki/session-geometry.md). **Contract:** [SPEC](../SPEC.md).
...
A containment pair with equal boundaries must follow each source’s equality rule. Reordering storage rows cannot change date-joined results; later highs cannot change the recorded open.
...
```sh
phase1 measure --slice 07 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/07/`. Each printed row must point to its actual generated report.
