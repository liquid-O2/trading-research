# 06: Range-clock and bar variants with Jumbo’s width table

**What to build:** Compare the published clocks with fixed-grid/time/trade/volume/dollar variants and recompute every row of Jumbo’s five-bin width/path table.
...
**Definition blockers:** Q01, Q02, Q03 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S06](../PRD.md). **Wiki:** [time based ranges](../wiki/time-based-ranges.md), [range break paths](../wiki/range-break-paths.md). **Contract:** [SPEC](../SPEC.md).
...
```sh
phase1 measure --slice 06 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/06/`. Each printed row must point to its actual generated report.
