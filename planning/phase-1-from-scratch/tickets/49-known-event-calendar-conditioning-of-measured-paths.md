# 49: Known event-calendar conditioning of measured paths

**What to build:** Compare source calendar/volatility hypotheses on acquired release-known schedules and already measured path/zone outcomes.

**Blocked by:** [05: GK, Yang–Zhang, RV and lagged HAR inputs](05-gk-yang-zhang-rv-and-lagged-har-inputs.md), [09: Judas and both single-break day classes](09-judas-and-both-single-break-day-classes.md), [41: IV, skew, volatility indices and daily VX](41-iv-skew-volatility-indices-and-daily-vx.md)

**Definition blockers:** Q22 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S49](../PRD.md). **Wiki:** [event calendar conditioning](../wiki/event-calendar-conditioning.md). **Contract:** [SPEC](../SPEC.md).
...
A revised release value cannot appear in an earlier snapshot. A missing event timestamp stays missing; no source weekday preference becomes a trade filter.
...
```sh
phase1 measure --slice 49 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/49/`. Each printed row must point to its actual generated report.
