# 34: First FVGs and body-gap fill paths

**What to build:** Compare source wick/body/first-per-clock gaps with timestamp, independent-ID, size and bar variants through partial/full fill and failure.

**Blocked by:** [01: Published 06–09 range to a real path report](01-published-06-09-range-to-a-real-path-report.md), [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md)

**Definition blockers:** Q01 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S34](../PRD.md). **Wiki:** [fvg body gap structures](../wiki/fvg-body-gap-structures.md). **Contract:** [SPEC](../SPEC.md).
...
Two zones sharing one edge still have distinct identities when their other edges differ. A body gap can exist with traded volume; near-edge touch is not far-edge fill.
...
```sh
phase1 measure --slice 34 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/34/`. Each printed row must point to its actual generated report.
