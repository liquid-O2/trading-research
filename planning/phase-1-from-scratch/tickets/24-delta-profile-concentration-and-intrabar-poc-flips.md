# 24: Delta-profile concentration and intrabar POC flips

**What to build:** Compare signed delta-profile anchors/bins and developing candle POC paths with source peak/control definitions and normalized variants.

**Blocked by:** [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md), [04: RTH value profiles and 40/68/70% comparisons](04-rth-value-profiles-and-40-68-70-comparisons.md)

**Definition blockers:** Q01, Q12 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S24](../PRD.md). **Wiki:** [delta profiles](../wiki/delta-profiles.md). **Contract:** [SPEC](../SPEC.md).
...
The highest-price nonzero delta row can differ from largest absolute delta. A final candle POC cannot disclose an earlier flip; independently summed signed rows reconcile to trade delta.
...
```sh
phase1 measure --slice 24 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/24/`. Each printed row must point to its actual generated report.
