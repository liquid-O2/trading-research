# 35: Candle patterns, round rails and floor pivots

**What to build:** Compare sourced numerical candle/round/pivot objects with scale, equality, color and matched-density variants on their respective future path tables.

**Blocked by:** [01: Published 06–09 range to a real path report](01-published-06-09-range-to-a-real-path-report.md), [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md), [05: GK, Yang–Zhang, RV and lagged HAR inputs](05-gk-yang-zhang-rv-and-lagged-har-inputs.md)

**Definition blockers:** Q01 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S35](../PRD.md). **Wiki:** [candle patterns round levels](../wiki/candle-patterns-round-levels.md), [floor pivots](../wiki/floor-pivots.md). **Contract:** [SPEC](../SPEC.md).
...
Inside-candle color and mother-candle color may differ; each variant uses the correct one. Floor-pivot values match independent arithmetic and never become volume value or Jumbo P-zones.
...
```sh
phase1 measure --slice 35 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/35/`. Each printed row must point to its actual generated report.
