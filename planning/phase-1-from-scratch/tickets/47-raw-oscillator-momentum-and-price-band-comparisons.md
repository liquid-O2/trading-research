# 47: Raw oscillator, momentum and price-band comparisons

**What to build:** Compare every sourced raw feature/band/marker with causal normalization, robust-scale, component and bar variants on common paths.

**Blocked by:** [02: Trade and OHLC CVD on the same price paths](02-trade-and-ohlc-cvd-on-the-same-price-paths.md), [05: GK, Yang–Zhang, RV and lagged HAR inputs](05-gk-yang-zhang-rv-and-lagged-har-inputs.md), [23: VWAP anchors and deviation response tables](23-vwap-anchors-and-deviation-response-tables.md)

**Definition blockers:** Q01 in [QUESTIONS](../QUESTIONS.md)

**Status:** planned — no implementation or worker authorized in this session.

**PRD story:** [S47](../PRD.md). **Wiki:** [momentum oscillator features](../wiki/momentum-oscillator-features.md), [kernel trend bands](../wiki/kernel-trend-bands.md). **Contract:** [SPEC](../SPEC.md).
...
Backdated oscillator pivots become available only after confirmation. Uncentered RMS and centered SD differ on a shifted sample; signal-bar preconfirmation extrema cannot count as later success.
...
```sh
phase1 measure --slice 47 --verify
```

This is a proposed interface for later execution, not an existing or executed command. Planned report root: `reports/phase-1/47/`. Each printed row must point to its actual generated report.
