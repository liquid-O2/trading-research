...
> Preservation note (orchestrating session, 2026-09-15): the review runner deleted its throwaway root before copying anything but REVIEW.md, so the twelve dossiers below were recovered verbatim from the run log (astra.log, printed at the end of the run); FIGURE_MANIFEST.json, DUPLICATE_FIGURES.json, COVERAGE_CHECK.json, SOURCE_INVENTORY.json, the *_notes.json files, FINAL_VERIFICATION.json and the figure crops were not preserved. The runner now copies every top-level output.
...
- Read every source image page and enlarged chart/ticket/annotation regions. FIGURE_MANIFEST.json (file not preserved) records PDF-point or raster-pixel crop coordinates, magnification, source/page and reading. Numerical readings use detailed crops, not page thumbnails. Repeated images are identified in DUPLICATE_FIGURES.json (file not preserved):62 pixel-identical images in the secondary Jumbo PDFs map to inspected archive figures; duplicate page text was read separately.
...
- COVERAGE_CHECK.json (file not preserved) verifies all711 page overviews and records source SHA-256 hashes. Detailed readings are in jj_notes.json (file not preserved), jr_notes.json (file not preserved), gb_notes.json (file not preserved), sires_notes.json (file not preserved) and other_notes.json (file not preserved). SOURCE_INVENTORY.json (file not preserved) records page dimensions and embedded-image bounds.
...
- No native replay, profitability backtest, new historical census or repository checker was run. Existing counts are cited receipts only. FINAL_VERIFICATION.json (file not preserved) checks731 exact wiki quotation fragments, all six sections and both tables in each dossier, the220-line limit, cited code-file existence, page/crop coverage, source hashes and unchanged tracked repository state. It does not validate a strategy edge.
...
All12 dossiers are below220 lines. Full report text is also concatenated in ALL_DOSSIERS.md (file not preserved), in dossier order followed by this review. The delivery print uses that exact text; the individual files remain the reviewable per-strategy artifacts.
