...
3. **A process framework layered on a process framework.** `PSTACK_EXECUTION.md` (101 lines), the generated subphase RUNBOOKs that embed it again, `PROMPTS.md` (170 lines), the "Copyable worker prompt" in every card, the Grok-only role tables and the poteto router with 23 playbooks. The rewritten `AGENTS.md` says these are replaced. Today an agent picking up P15-17 is told to read ten documents (about 1,500 lines) before its 117-line card, and half of those pages restate each other.
