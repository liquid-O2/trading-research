...
How to run a subphase, one owner per subphase, worker budgets, performance defaults and the review discipline are in [HOW_TO_RUN.md](HOW_TO_RUN.md); operating roles are in [AGENT_OPERATIONS.md](AGENT_OPERATIONS.md). This document keeps the execution contract that is not stated there: dispositions and receipt shapes, verification commands, run naming and manifests, native slices and resource budgets, review and the two family tables.
...
Profile one date and four dates before a census. Persist one canonical native transform per input hash/contract/date/transform version. Worker count is budgeted from the cgroup quota and the measured peak RSS per worker with concurrent processes counted, as HOW_TO_RUN.md and PERFORMANCE.md require; never a fixed number. Never exceed 16 without a new resource receipt. Deterministic results must match at 1 and 4 workers. Abort and record `resource_limit` when projected free disk after the run is below 20% or below twice the next shard size. Resource aborts are implementation work remaining, not evidence against a research hypothesis.
