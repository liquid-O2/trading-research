...
1. `flow.cvd.trade` from MBP-1 trade events (cross-checked against `cov.nq.trades`), `flow.cvd.ohlc`, `flow.cvd.part.trade` (≥100 / 20–99 / <20), `flow.cvd.part.ohlc`, `flow.cvd.gamma` (declared weight table from ticket 06 nodes; until then the row prints `deferred`).
