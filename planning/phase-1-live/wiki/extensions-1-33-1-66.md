...
## Faithful object
`env.ext.133.from-edge` and `env.ext.166.from-edge` (**beyond-edge**): level = H + k·(H−L) above, L − k·(H−L) below, k ∈ {1.33, 1.66}, box = `range.6-9.published`. Width is always `W69` (or the London box height when the London row is the source), never a P-zone. Sibling rows `env.ext.100` (measured move) and `env.ext.050` (the −0.5 projection, shared with the 6–9 page).
