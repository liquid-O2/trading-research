...
## Faithful object
`lvl.pclose` = last print of the prior 16:00 RTH close (settlement as a named row); `lvl.popen` = prior 09:30 first print; `lvl.pib.high` / `low` = prior 09:30–10:30 H / L; `lvl.halfgap` = (prior close + today's 09:30 open) / 2, `known_at` 09:30. Opening-location class = today's 09:30 open vs prior close, prior H / L, prior IB, prior VA.
