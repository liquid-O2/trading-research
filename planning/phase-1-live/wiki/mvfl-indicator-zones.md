...
## Upgrades
- Aggressor delta from MBP-1 in place of the OHLC proxy `[CEX L16]`; k 2 / 6; thresholds; session NY-only.

## Outcomes
- Grid at zone edges (break / rejection as the script labels them); HOD / LOD capture vs random levels of matched density (the density caution `[CEX L16]`); `bias.mvfl` agreement with `path_class`; faithful disagreements = zone sets that differ between proxy and aggressor delta.
